// Abuse / cost controls for AI autocomplete, backed by D1 (see migrations
// 0003 and 0004).
//
...
/**
 * Pre-flight check for one AI inline-completion request: the per-user
 * completion counters (migration 0004) against `limits`, then the global
 * daily token ceiling, the single backstop on total spend.
 */
...
/** Record one completion's usage (per-user completion counters + the global
 *  token total). Best-effort: run via ctx.waitUntil so it never blocks the
 *  response. */
