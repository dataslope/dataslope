// OpenAI-compatible provider adapter. OpenRouter and OpenAI implement the same
// `/chat/completions` API, so one fetch drives whichever a tier points at.
 ...
 * (app/api/ai/complete), where the whole answer is a few lines, so streaming
 * would just add latency and parsing for nothing. The low temperature keeps
 * suggestions deterministic enough to feel like completion rather than
 * brainstorming.
 */
