...
export const caption = `Ten covariates, two arms, and each bar is how far apart the arms are on that variable in standardised units. Under self-selection ${OVER} of the ten exceed the usual 0.1 threshold for "balanced", and the gaps are not random: people who chose the treatment differ systematically, and every one of those differences is a candidate explanation for whatever the study finds. Under randomization ${OVER_RAND} of the ten do. They are not zero, and expecting zero is a common misreading, because a coin flip on a finite sample does not deliver perfect balance. What randomization guarantees is that the imbalance is *random*, with a known distribution, which is what lets a p-value mean anything. The important pair is at the bottom, marked unmeasured. Matching, regression and propensity scores can only balance variables you have. Randomization balances the ones you have and the ones you have never heard of, because the coin does not know what it is balancing, and that is the whole reason a randomized trial licenses a causal claim while an observational study does not. It is not a matter of degree: no amount of careful adjustment reaches ${UNMEASURED.map((d) => d.key.toLowerCase()).join(" or ")} if they were never recorded. Two honest limits. Randomization says nothing about whether the sample represents anybody, which is a separate question. And attrition undoes it: if people drop out for reasons related to the treatment, the groups that remain are self-selected again.`;
