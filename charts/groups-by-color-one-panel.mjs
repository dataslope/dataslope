/**
 * Four groups mapped to color in one frame: the state faceting is the answer
 * to, drawn before the answer.
 *
 * Color is a perfectly good channel for a grouping variable right up to the
...
export const title =
  "One scatter of engine size against highway fuel economy for 136 cars from four manufacturers, told apart only by color. All four clouds occupy the same band and their four fitted lines leave the same corner, so no single manufacturer's trend can be read on its own.";

export const caption =
  "Highway fuel economy against engine size for 136 cars from four invented manufacturers, with the manufacturer mapped to color. That works while the groups sit in different parts of the chart and stops the moment they overlap: each of these four gives up a different amount of economy per extra litre, and picking any one of them out means ignoring the other three at the same time.";
...
export function render() {
  return plot({
    ...
    marks: [
      ...
      // the colors up is part of what it is demonstrating.
      Plot.dot(LEGEND, {
        x: "x",
        y: Y_DOMAIN[1] - 1.2,
        fill: (d) => MAKER_COLOR[d.key],
