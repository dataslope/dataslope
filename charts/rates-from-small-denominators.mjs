...
export const caption = `Sort any rate descending and the top of the table fills with whatever had the fewest observations, and this is arithmetic rather than bad luck. The standard error of a proportion is the square root of p times one minus p over n, so a region with four visitors can only land on 0, 25, 50, 75 or 100 per cent, and two of those five are above every large region's rate before anything real has happened. Here ${LEADERS} of the top four rest on fewer than ${SMALL} observations. So do ${LAGGARDS} of the bottom three, which is the tell: small groups do not gather at the top, they gather at *both ends*, because they are the only ones with the variance to reach either. Every region with a real sample sits between ${BIG_RANGE[0].toFixed(0)}% and ${BIG_RANGE[1].toFixed(0)}%. This is the mechanism behind a famous and expensive error: studies found the best-performing American schools were disproportionately small, and money went into breaking up large ones, while the fact that the *worst*-performing schools were also disproportionately small did not travel as far. Three fixes, in order of effort: print n and let the reader discount; set a minimum n and say what it was; or shrink each rate towards the overall rate in proportion to how little data it rests on, which is what a hierarchical model does and is the principled version of what your eye is already attempting.`;
