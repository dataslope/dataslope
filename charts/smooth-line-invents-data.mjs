...
export const caption = `Curve smoothing is offered next to line width and color, as though it were the same kind of setting. It is not. Joining points with a curve instead of with segments changes what the chart says about the values *between* the points, and no spline says nothing. The specific harm is overshoot: a cubic spline has to match slopes at every knot, and getting through a sharp peak while doing that means swinging past it. Here the curve reaches ${Math.round(CURVE_HI)} against a highest measurement of ${HI}, and dips to ${Math.round(CURVE_LO)} against a lowest of ${LO}, so it invents ${OVER} units above and ${UNDER} below anything anybody recorded. Read a value off the curve between two dots and you get a number that was never possible. Where the axis has a hard floor this stops being subtle: smooth a count of errors that touches zero, or a percentage that touches 100, and the curve will draw negative errors and 104 per cent. Straight segments are not neutral either, since they claim linear interpolation, which is also a claim. They are just the weakest claim that still connects the dots: a segment never leaves the range of the two values it joins, and every reader knows how much to discount it. If the process really is smooth and you want to say so, fit a model and draw it as its own line or band, so the chart shows two things and admits which is which.`;
