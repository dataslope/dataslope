...
export const caption = `False-positive rate against the variance ratio between two groups with identical true means, ${N_SMALL} observations in one and ${N_BIG} in the other. Student's reaches ${(WORST.student * 100).toFixed(0)}%, ${(WORST.student / ALPHA).toFixed(0)} times the advertised ${ALPHA * 100}%; Welch's stays between ${(WELCH_RANGE[0] * 100).toFixed(1)}% and ${(WELCH_RANGE[1] * 100).toFixed(1)}%.`;
