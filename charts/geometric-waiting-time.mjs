...
export const caption = `Attempts until the first success at a ${FOCUS.p * 100}% chance each time. The mean is ${FOCUS.mean}, and that is not a typical experience of anything here: the commonest outcome is a single attempt, the median is ${FOCUS.median}, and ${(TAIL_10 * 100).toFixed(0)}% of the time you need more than ten.`;
