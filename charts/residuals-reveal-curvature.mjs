 ...
 * dropped line and colored by its sign.
 *
 ...
 * the line it was fitted with, and the segment takes one of two colors by
 * whether the point sits above or below. The colors then arrive in blocks:
 * below at the left, above through the middle, below again at the right. A
 * correct model leaves those two colors interleaved at random, so blocks are
...
export const title =
  "Advertising spend against sales, with a straight line fitted through a relationship that flattens out. Each point is joined to the line by a short vertical segment, colored by whether the point sits above or below it. The colors arrive in blocks: below the line at both ends of the axis and above it through the middle.";
...
const X_MIN = 4;
const X_MAX = 120;
/** Spread evenly rather than drawn uniformly at random, so the blocks of color
