...
export const caption = `One series against row order and against real dates. There is a ${MISSING}-day outage in it, and against row order the levels either side become neighbours, so a drop of about ${Math.abs(STEP)} units reads as a gentle slide.`;
