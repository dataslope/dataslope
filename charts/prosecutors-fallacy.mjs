...
export const caption = `A forensic test with a false-positive rate of one in ten thousand supports a true sentence: the chance of this match if the defendant is innocent is one in ten thousand. The fallacy is reading it backwards, as the chance the defendant is innocent *given* the match, and those are different conditionals that Bayes's rule exists to separate. The arithmetic is drawn on the right. In a pool of a million people with one guilty party, that false-positive rate produces about ${FALSE_MATCHES} innocent matches alongside the one real one, so given a match and nothing else the chance of guilt is 1 in ${MATCHES}, which is ${(P_GUILTY_GIVEN_MATCH * 100).toFixed(1)}%. The evidence is genuinely strong, since it moved the field from one in a million to one in ${MATCHES}, and it is nowhere near proof. What decides which it is is the prior, which is the part the courtroom version drops. Run the same test on a suspect already placed at the scene, so the pool is ${NARROW_POOL} rather than a million, and the chance of guilt given a match is ${(P_NARROW * 100).toFixed(1)}%. Same test, same false-positive rate, opposite conclusion, entirely because of how the suspect came to be tested. This has convicted people, and the identical error runs through medical screening, fraud detection and every alerting system with a low base rate.`;
