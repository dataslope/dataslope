...
export const caption = `The folk rule is "t-test for normal data, Mann-Whitney otherwise", which is true and stops exactly where the useful part begins: how much does the wrong choice cost, and in which direction? Each panel shifts a population against itself by the same amount and runs both tests ${REPS.toLocaleString("en-GB")} times. On normal data the t-test is the optimal test and wins, by ${COST_NORMAL} percentage points: that is the premium for insuring against a shape you did not have, and it is small. On heavy-tailed data a handful of extreme values inflate the sample standard deviation, the t statistic's denominator swells, and its power falls to ${pct(HEAVY.t)}% while Mann-Whitney holds ${pct(HEAVY.u)}%, a gap of ${COST_HEAVY} points. Ranks are why: to Mann-Whitney a value of 400 counts for exactly as much as one of 40, because it is the largest and that is all it is. On the skewed population the gap is wider still, ${COST_SKEW} points, because most of that distribution is packed into a narrow range where a shift swaps a great many pairs of values around while barely moving a mean that the long tail dominates. One caveat belongs with that result: Mann-Whitney is not a test of medians unless the two distributions have the same shape. What it tests is whether a random value from one group tends to exceed a random value from the other, which is a good question and a different one. The asymmetry is the argument. Picking ranks when the data was normal costs a few points; picking the t-test when the data was heavy-tailed costs most of the power you had.`;
