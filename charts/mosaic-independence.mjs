...
export const caption = `A mosaic draws a contingency table as area: columns as wide as their marginal counts, each split vertically by the conditional distribution inside it. That gives independence a shape you can check without arithmetic. If the two variables are independent, the probability of the outcome is the same whichever group you are in, so every column splits at the same height and the divider runs straight across, which is the left panel. Note that the columns are very different widths, because ${Math.round((GROUPS[0].n / TOTAL) * 100)}% of the traffic is desktop, and the split is identical anyway, because group size has nothing to do with outcome. If they are dependent the divider steps. The right panel has the same margins and the same overall rate (${(CHECK * 100).toFixed(0)}%), so the columns are exactly as wide, and the only difference is where each one splits: ${Math.round(DEPENDENT[0].p * 100)}% against ${Math.round(DEPENDENT[1].p * 100)}%, a gap of ${SPREAD} points. The reason to prefer this over a stacked bar is that it shows both margins at once. A 100 per cent stacked bar normalises every column to full height, so it shows the conditional distribution and hides how many observations are behind each; a grouped bar shows the counts and makes the conditional distribution something you compute. The mosaic shows the joint distribution, which is what a contingency table is, and the flat divider is a chi-squared test you can run with your eye.`;
