...
export const caption = `Two predictors, one outcome, and the correlation between the predictors swept from zero to 0.99. The two curves say opposite things and both are true. The coefficient's confidence interval blows up: ${HIGH.width.toFixed(1)} times as wide at the far end, and already ${MID.width.toFixed(1)} times as wide at 0.9. The reason is that the data no longer contains the comparison the coefficient is defined by. A coefficient in a multiple regression means "the effect of moving this predictor while holding the other fixed", and when the two move together there are almost no observations where one moved and the other did not. The model is being asked a question the data barely answers and says so, honestly, in the standard error. The predictions do not care at all: out-of-sample error is flat across the whole sweep, because if two predictors carry the same information it makes no difference to a fitted value which of them gets the credit. So the rule is about the question rather than the model. To predict, collinearity is not a problem and dropping a variable to fix a VIF is superstition that costs information. To interpret a particular coefficient, collinearity is exactly the problem, and no amount of data cleaning fixes it: you need variation where one predictor moves and the other does not, which is a fact about the experiment rather than the fit. The tell most people miss is that with collinear predictors, coefficients can flip sign between two samples from the same population while accuracy is unchanged in both.`;
