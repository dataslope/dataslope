...
export const caption = `These six categories are unordered: there is no sense in which Billing comes before Data, they are names. The left panel shades them on a light-to-dark ramp, which is what a library hands you when it is given a continuous color scale and what somebody picks when the qualitative palette looks untidy. The effect is not subtle, because the eye reads light-to-dark as low-to-high, that being what lightness is for, so the chart asserts a ranking that is not in the data, in a channel readers trust without checking. And the ranking it asserts is the alphabetical order the groupby returned: ${MISLED} of the ${N} categories sit in a different place on the ramp from where their size would put them, and the darkest bar, ${DARKEST.key}, is only ${RANK_OF_DARKEST}${RANK_OF_DARKEST === 1 ? "st" : RANK_OF_DARKEST === 2 ? "nd" : RANK_OF_DARKEST === 3 ? "rd" : "th"} by volume. The right panel uses hues of roughly equal lightness, deliberately out of sequence, so nothing about the color suggests a rank and the bar lengths are left to carry the one that is real. The rule is a short table. Nominal data takes a qualitative palette. Ordinal data, small to large or strongly disagree to strongly agree, takes a sequential one, and that is the case where the lightness ramp is doing real work. Data with a meaningful middle takes a diverging palette. Picking the family is the decision; picking which pretty colors go in it is not.`;
