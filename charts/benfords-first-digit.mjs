...
export const caption = `Leading-digit frequencies in a real ledger and a fabricated one, against Benford's log10(1 + 1/d). The fabricated ledger contains no unusual single row, and its chi-squared against Benford is ${CHI[1].chi.toFixed(0)} where the real one scores ${CHI[0].chi.toFixed(1)}, on a critical value of ${CRIT}.`;
