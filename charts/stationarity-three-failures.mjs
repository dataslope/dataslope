...
export const caption = `"Stationary" is a claim about three things staying put: the mean, the variance, and the way each value relates to the one before it. A series can break any one of them on its own, and the Dickey-Fuller test that everybody reaches for is looking at exactly one of them, the mean. The first panel is a random walk, where today is yesterday plus noise and there is no level to return to. That is the failure the test was built for: p ${fmtP(CASES[0].p)}, no rejection, difference it once and move on. The second panel has a mean that never moves and a spread four times wider after the halfway mark. There is no unit root to find, so the test rejects at p ${fmtP(CASES[1].p)} and hands back the word "stationary" for a series the eye rules out at a glance. That is the blind spot: any interval, error bar or noise model fitted on the first half is wrong about the second, and the test had nothing to say about it. The third panel is a clean seasonal cycle around a fixed level, and it returns p ${fmtP(CASES[2].p)}, the same verdict as the random walk. The same verdict, a different disease, and a different cure: first differencing is the answer to panel one and not to panel three, which wants seasonal differencing or explicit seasonal terms. The p-values here are simulated rather than looked up. ${NULL_STATS.length.toLocaleString("en-GB")} random walks of this length were generated and the same statistic computed on each, so the number under each panel is the share of pure random walks that looked at least this mean-reverting. None of this makes the test broken; it makes it narrow. Plot the series first, decide from the picture which of the three things is moving, and use the test to confirm the one thing it examines.`;
