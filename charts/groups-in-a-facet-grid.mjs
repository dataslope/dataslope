/**
 * The same four groups as `groups-by-color-one-panel`, one per panel.
 *
 * Nothing has been added and nothing removed: the points, the colors, the
 * fitted lines and both scales are identical. The only change is that the
 * grouping variable has stopped being a color and become a *position* — which
...
export const caption =
  "The same 136 cars, the same four invented manufacturers, the same colors and the same axes. The only change is that the manufacturer is now a position, which panel, rather than a hue. Because every panel is scaled identically, a difference between panels is a difference in the data: Northwind gives up 4.8 mpg per extra litre and Brightside only 1.0. That was in the chart above as well and could not be read there.";
