...
export const caption = `A residual plot answers one question: is there structure left over? The left panel says no. The residuals form a horizontal band of roughly constant width around zero, and the spread at the top of the fitted range is ${EVEN_RATIO} times the spread at the bottom, which is to say the same. The right panel fans. The residuals still average zero at every fitted value, so the coefficients are unbiased and the line is in the right place; what has failed is constant variance, and everything computed from it fails too. Standard errors, confidence intervals, p-values and prediction intervals all come from one pooled estimate of the residual variance, and there is no single residual variance here: the spread at the right of the range is ${FAN_RATIO} times the spread at the left. So the intervals are too wide at the low end and too narrow at the high end, and the second half of that is the dangerous half, because it is where the model is most confident and least entitled to be. Three responses, in order of how much they change. Robust standard errors keep the fit and recompute the uncertainty without assuming constant variance, which is cheapest and usually enough. Transforming the response, normally to logs, is right when the spread grows with the level, which is this case and is extremely common for money, counts and durations. Modelling the variance directly, with weighted least squares or a generalised linear model, is the thorough option. What makes doing nothing tempting is that the R-squared is unaffected and the fit looks healthy.`;
