...
export const caption = `Least squares fits the conditional *mean*. Quantile regression fits a conditional quantile, and running it at several at once produces this fan, which says something one line cannot: how the spread changes across the range rather than only the level. Here the fan opens. At one year of experience the gap between the 10th and 90th percentile is about £${Math.round(BAND_LEFT)}k; at ${X1} years it is about £${Math.round(BAND_RIGHT)}k, ${FAN} times wider. A mean line reports one slope and hides that, and so does a residual standard deviation, because there is no single residual standard deviation here to report. Two things follow. The median line and the mean line answer different questions and separate as soon as the conditional distribution is skewed, which it usually is for money, durations and counts: at ${X1} years the mean sits £${MEAN_GAP}k above the median, pulled up by the same few large values every year. "Typical" nearly always means the median and nearly always gets estimated with the mean. And the quantile lines are what a service-level question actually needs. "How long does a request take" is a mean; "how long does the slowest one in twenty take" is the 95th percentile, and only one of those goes in an SLA. Fitting the mean and adding two standard deviations is a normal-tailed approximation to the second question, and on skewed data it is wrong in the direction that hurts.`;
