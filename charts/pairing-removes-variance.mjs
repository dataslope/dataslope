...
export const caption = `The left panel is what an independent-samples test sees: two piles of measurements that overlap so completely nobody would claim a difference from them, and its t statistic is ${UNPAIRED_T.toFixed(2)}. The right panel is the same numbers subtracted subject by subject, and its t is ${PAIRED_T.toFixed(1)}. Nothing statistical has been smuggled in. The two piles were never independent, and treating them as though they were throws away the fact that the same person is in both. Look at what varies on the left: subjects differ enormously, some people's resting values sitting twenty units above others', and that between-subject spread buries the treatment effect. But a subject's own before and after move together, because a high person stays high, so within one subject the treatment is the only thing left changing. Subtracting cancels each personal level and the between-subject variance goes with it, leaving the effect and a little measurement noise: ${positive} of the ${N} differences are positive. The rule is to pair whenever the design allows, because a paired design buys the same power for a fraction of the sample. The warning that comes with it is that the pairing has to be real. Pairing rows that merely arrived in the same order gives you the paired test's smaller degrees of freedom and none of its variance reduction, which is worse than not pairing.`;
