...
export const caption = `Noise in y costs precision and biases nothing: the points spread, the standard errors grow, the slope stays put. Noise in *x* does something else. It biases the slope towards zero, systematically, and more data does not help. The mechanism is one line: least squares divides the covariance of x and y by the variance of x, and adding independent noise to x leaves the covariance alone (the noise is unrelated to y) while inflating the denominator. Constant numerator over growing denominator is a shrinking slope, and the shrinkage factor is the reliability ratio, the share of x's variance that is real. Here a true slope of ${TRUE_SLOPE} falls to ${WORST.slope.toFixed(2)} once the measurement error matches ${WORST.k} times the spread of the true values, and it is already halved by ${HALVED.k} times. The dangerous part is what the diagnostics do. R-squared falls, which you would notice, and the residuals get *more* evenly spread, because the added noise is homoscedastic by construction, so the residual plot looks better rather than worse. Nothing in the output says "your x is measured badly". This shows up wherever a predictor is a proxy: self-reported intake, a survey score, a single blood-pressure reading, a noisy sensor. The effect of the true quantity is understated, sometimes by half, and "this barely matters" can be a fact about the instrument. Fixes exist and all of them need extra information: repeated measurements to estimate the reliability, an instrumental variable, or an errors-in-variables model. More rows is not one of them.`;
