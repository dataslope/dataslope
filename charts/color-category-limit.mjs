/**
 * Where color stops working as a categorical channel.
 *
 * The bullet immediately above this in the lesson says "more than about seven
 * categorical colors and the eye starts confusing similar shades", which is the
 * right rule and exactly the sort of claim a reader accepts without believing.
 *
 * The obvious way to draw it, three scatters colored at three granularities,
 ...
 * the palette and not of the marks it colors.
 *
 * So: three rows of swatches, one row per category count. The point lands on the
 * third. A palette has a fixed number of distinguishable colors (seven here),
 * and no plotting library refuses when you ask for more than that. It cycles.
 * Every swatch past the seventh is a color already spoken for, and nothing on
 ...
 * "these colors are hard to tell apart", which sounds like a matter of taste,
 * but "these two are the same color and nobody was told", which is a defect.
 *
 * It belongs in a section about color vision because it is the same conclusion
...
/** How many distinguishable colors the palette actually has. Past this a
 ...
 *  row already has a collision, which a label reading "still one color each"
  ...
  COUNTS.map((n) => {
    ...
    return [
      ...
      k === 0
        ? "one color each"
        : k === 1
          ? "one color already repeats"
          : `${k} of these repeat a color already used`,
    ];
  }),
);

export const caption = `Three rows of swatches, one per category count, and the failure is in the third. A palette has a fixed number of distinguishable colors, ${PALETTE} in this one, and no plotting library refuses when you ask for more: it starts again from the beginning. So at ${COUNTS[COUNTS.length - 1]} categories, ${OVERFLOW} groups carry a color that already belongs to another group, and nothing in the chart or the legend says so. That is the real failure mode. Not "these colors are hard to tell apart", which sounds like taste, but "these two are identical and nobody was told", which is a defect. If a categorical variable has more levels than your palette has colors, the fix is fewer levels: collect the tail into "other", facet, or use a channel that is not color.`;
...
export function render() {
  return plot({
    ...
    marks: [
      ...
      // has to say and the color alone cannot say it.
      Plot.dot(ROWS.filter((d) => d.repeated), {
        x: "x",
        y: () => 0.5,
        fy: "panel",
