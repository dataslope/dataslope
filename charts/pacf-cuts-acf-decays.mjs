...
export const caption = `The series is an AR(2): every value is built from the two before it and nothing else. Ask the two plots about lag 6 and they answer differently, and the difference is why both exist. The ACF asks how related two values six steps apart are, and the answer is ${ACF[6].toFixed(2)}, even though the process contains no lag-6 term whatsoever. What it contains is a chain: today leans on yesterday, yesterday leaned on the day before, and correlation propagates down the line while decaying. You can see the arithmetic at lag 2, where the total correlation is ${ACF[2].toFixed(2)} and the path through lag 1 alone accounts for ${INDIRECT.toFixed(2)} of it, leaving ${DIRECT.toFixed(2)} that is genuinely direct. The PACF asks the stricter question: once every shorter lag has had its say, is there anything left? For this process the honest answer beyond lag ${ORDER} is no, so the PACF drops into the noise band and stays there, and the lag where it cuts is the order of the process read straight off the plot. That gives the rule people memorise: PACF cutting off at lag p with a decaying ACF means AR(p); an ACF that cuts off at lag q with a decaying PACF means MA(q), the mirror image, because a moving-average process has a finite memory of shocks; and both decaying means both parts are present and neither order is readable by eye. The shaded bands are the ±1.96/√n envelope, the size a bar reaches on noise alone, so on a plot of ${LAGS} lags one bar poking out is arithmetic rather than a discovery.`;
