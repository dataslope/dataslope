...
export const caption = `Neurath's isotype system is one icon per fixed unit, repeated, and it has a genuine advantage over a bar: the reader can *count*, which is exact in a way that reading a length off an axis is not. Ten against six is unarguable and needs no axis. The advantage lasts exactly as long as the values are multiples of the unit, and real values are not. At one icon per ${UNIT} people, ${GROUPS[0].n} is six and two fifths of an icon, and there are three ways out, all of which change the comparison. Round, and the small groups take the damage: ${WORST.a.n} and ${WORST.b.n} draw the same number of icons here, so a ${WORST.ratio.toFixed(1)}-fold difference is shown as a tie. Clip the last icon to a partial shape and you are comparing areas again, which is what the counting was meant to replace, and readers reliably read a half-width icon as somewhere between a third and a half. Or change the unit until everything divides, and the smallest group needs so many icons that the largest will not fit. Neurath knew all of this and designed around it: the unit was chosen so the values landed on it. That is the honest way to use the form, and it means the data has to suit the chart rather than the other way about, which is unusual enough to say out loud. The bars are the control. No unit problem, because a length is continuous, and no exact counting either.`;
