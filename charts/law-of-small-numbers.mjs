...
export const caption = `Every one of these ${N_COUNTIES.toLocaleString()} counties was generated from exactly the same true rate of ${(TRUE_RATE * 100).toFixed(0)}%. There is no real variation in the picture at all; the only thing that differs between counties is how many people live in one. The shape that comes out is a funnel, which is the square root of p times one minus p over n, drawn. Now do what a real analysis does and take the top twenty. Their median population is ${MED_TOP.toLocaleString()} against ${MED_ALL.toLocaleString()} for the whole set, and the bottom twenty are just as small, and both facts follow from the funnel rather than from anything about the places. A map of the highest-rate counties is a map of the smallest ones. This is not hypothetical. The best-known case is the American study of school size and performance, which found the highest-scoring schools were disproportionately small and launched a programme of breaking up large ones; the lowest-scoring schools were also disproportionately small, which was in the same data and did not travel. The fix that works is to plot this funnel: rate against sample size with the expected variation drawn as bounds. A point outside the bounds is interesting. A point at the top of a ranking is not, until you know how big it is.`;
