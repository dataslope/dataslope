...
export const caption = `One series and a shuffle of it. They share a mean of ${mean(ORIGINAL).toFixed(1)}, a standard deviation of ${sd(ORIGINAL).toFixed(1)}, the same minimum, maximum, median and histogram; the autocorrelation starts at ${ACF_A[0].r.toFixed(2)} and the shuffle sits inside the noise band at every lag.`;
