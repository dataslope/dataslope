...
export const caption = `The standard error of a proportion against the rate, which peaks at exactly one half. At n = ${REF_N.toLocaleString()} the standard error at 50% is ${ABS_RATIO} times the one at 2%, and in relative terms the ranking reverses: ±${seAt(RARE, REF_N).toFixed(2)} points is ${REL_RARE}% of a 2% rate, while ±${seAt(HALF, REF_N).toFixed(2)} is ${REL_HALF}% of a half.`;
