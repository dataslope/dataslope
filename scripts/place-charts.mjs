 ...
 * place in an MDX file guaranteed not to be inside a block: headings do not
 * appear inside components or fences, so the line before the next heading is
...
for (const [file, rows] of Object.entries(PLACEMENTS)) {
  ...
  for (const { slug, after, lead } of rows) {
    ...
    const at = lines.findIndex((l) => headingLevel(l) > 0 && anchor.test(l));
    if (at === -1) {
    ...
    const block = lead ? ["", ...lead.split("\n"), "", `<Chart slug="${slug}" />`] : ["", `<Chart slug="${slug}" />`];
