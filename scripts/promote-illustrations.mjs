 ...
 * WebP and drops them into `assets/images/` where `build-images.mjs` takes over.
 *
 * Why WebP is the committed source format:
 *
 ...
 * too (10x). Committing PNG sources put ~58 MB in git for twenty illustrations,
 * which projects to ~2.9 GB per thousand; as WebP the same set is ~5 MB and
 * ~266 MB. `build-images.mjs` already accepts `.webp` sources, so nothing
 * downstream changes.
 *
 * The one tradeoff: build-images re-encodes the source to its own WebP
 * (quality 80) for serving, so a WebP source is encoded twice. That is why the
 * source is written at a deliberately high quality (`--quality`, default 92) —
 * the second pass then has plenty of headroom and the result is
 * indistinguishable on flat isometric artwork.
 *
 * Deliberately kept separate from build-images.mjs, which is a deterministic,
 * content-hashed, committed-output build step that must stay a true no-op when
 * nothing changed. Promotion is a network-touching, human-triggered action; it
 * would poison those properties if it lived in the same script.
 *
 * Usage:
 ...
 *   --quality <n>     WebP quality for the committed source (default: 92)
...
async function main() {
  ...
  console.log(
    `Promoting ${stems.length} image(s) from ${source.describe} ` +
      `→ assets/images (webp q${opts.quality})${opts.dryRun ? " [dry-run]" : ""}\n`,
  );

  if (!opts.dryRun) mkdirSync(ASSETS_DIR, { recursive: true });
  let promoted = 0;
  let before = 0;
  let after = 0;
  for (const stem of stems) {
    ...
    before += raw.length;
    after += webp.length;
    const out = join(ASSETS_DIR, `${stem}.webp`);
