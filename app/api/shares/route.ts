/**
 * Playground share links. POST mints an immutable snapshot share for members
 * (storage quota + share cap) or guests (smaller cap, ~30-day expiry, per-IP
 * + global daily budgets via salted-hash counters — no raw IPs stored). GET
 * lists the signed-in member's live shares. Multipart body identical to
