...
export default function EmailPreviewPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      ...
      <p className="mt-2 text-sm text-[var(--ds-gray-500,#6b7280)]">
        The transactional auth emails from{" "}
        <code className="rounded bg-[var(--ds-gray-100,#f0f1f2)] px-1 py-0.5 text-[0.8em]">
          lib/auth/email.ts
        </code>
        ...
        {previews.map((p) => (
          <section key={p.key}>
            <div className="mb-3">
              ...
              <p className="mt-1 text-sm text-[var(--ds-gray-500,#6b7280)]">
                <span className="font-medium">Subject:</span> {p.subject}
              </p>
            </div>
            <PreviewFrame title={`${p.name} email preview`} html={p.html} />
            <details className="mt-2">
              <summary className="cursor-pointer text-sm text-[var(--ds-gray-500,#6b7280)]">
                Plain-text version
              </summary>
              <pre className="mt-2 overflow-x-auto whitespace-pre-wrap rounded-lg border border-[var(--ds-gray-200,#d0d7de)] bg-[var(--ds-gray-50,#f6f8fa)] p-3 text-xs text-[#121212] dark:text-white">
                {p.text}
              </pre>
            </details>
          </section>
