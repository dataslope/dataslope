 ...
 *
 * Lists the home page, the `/learn` course index, and every prerendered
 * lesson resolved from the Fumadocs `source` loader (the same page set behind
 * `generateStaticParams`). This is the single biggest indexing win for ~800
 ...
 * against `SITE_URL`.
 *
 * `/playground` and the raw-Markdown mirrors are intentionally omitted to stay
 * consistent with `app/robots.ts`, which disallows them.
 */
