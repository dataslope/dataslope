...
export default function GitChallengeCard({
  ...
  return (
    <div className={solved ? "gitcard solved" : "gitcard"}>
      <div className="gitcard-head">
        <span className="gitcard-badge">Git challenge</span>
        <h3 className="gitcard-title">{title}</h3>
        <span className="gitblock-head-sep" />
        <span className={solved ? "gitcard-progress solved" : "gitcard-progress"}>
          {solved ? (
            <>
      ...
      <div className="gitcard-body">
        <div className="gitcard-left">
          <div className="gitcard-instructions">{renderInstructions(instructions)}</div>

          <div className="gitcard-terminal">
            <GitTerminal
          ...
          <div className="gitcard-actions">
            ...
            {hint && (
              <button type="button" className="gitblock-btn" onClick={() => setShowHint((v) => !v)}>
          ...
          {showHint && hint && <p className="gitcard-hint">{hint}</p>}
          {showSolution && solution && (
            <pre className="gitcard-solution">{solution.trim()}</pre>
          )}
          {checked && !solved && (
            <p className="gitcard-verdict">
              Not there yet. The unticked objectives above say what is still missing.
            </p>
          )}
        </div>

        <div className="gitcard-right">
