...
function newDuckDbStructureId(): string {
  return `duckc_${++_duckDbStructureIdCounter}`;
}
...
const DUCKDB_TYPE_GROUPS = [
  ...
  { label: "Floating point", types: ["FLOAT", "DOUBLE", "DECIMAL(10,2)"] },
  {
    label: "Text",
  ...
  {
    label: "Date / time",
    types: ["DATE", "TIME", "TIMESTAMP", "TIMESTAMP WITH TIME ZONE", "INTERVAL"],
  },
  { label: "Collections", types: ["INTEGER[]", "VARCHAR[]", "STRUCT(name VARCHAR, value INTEGER)", "MAP(VARCHAR, INTEGER)"] },
  { label: "Special", types: ["ENUM('a', 'b')", "BIT"] },
] as const;

const DUCKDB_TYPE_OPTIONS: readonly string[] = DUCKDB_TYPE_GROUPS.flatMap(
  (group) => group.types,
);
const DUCKDB_IDENTITY_TYPES = new Set<string>();
// Accepts common DuckDB type text, including multi-word types,
...
function DuckDbPlaygroundInner() {
    ...
    (
      ...
      void engine.updateRows(tableName, updates).then((count) => {
        ...
        if (refetchSql) {
          ...
          // end of the result set (DuckDB may reorder rows after UPDATE,
  ...
  // ─── Import handlers ─────────────────────────────────────────────────
  const openNativeFileQueryTab = useCallback(
    async (file: File, flavor: ImportFlavor) => {
      const engine = engineRef.current;
      if (!engine) return;
      const registeredName = await engine.registerFileBuffer(
        file.name,
      ...
      openTabAndRun(`Query ${file.name}`, `SELECT *\nFROM ${reader}\nLIMIT 100;`);
      showToast(`Registered "${file.name}" for native DuckDB file queries.`);
    },
    [openTabAndRun, showToast],
  );

  const handleCsvFile = useCallback(
    (file: File) => {
      void openNativeFileQueryTab(file, "csv").catch((err: unknown) => {
        showToast(
          `Could not register CSV for direct query: ${err instanceof Error ? err.message : String(err)}`,
          "warn",
        );
      });
    ...
    [showToast, tables, openNativeFileQueryTab],
  );

  const handleJsonFile = useCallback(
    (file: File) => {
      void openNativeFileQueryTab(file, "json").catch((err: unknown) => {
        showToast(
          `Could not register JSON for direct query: ${err instanceof Error ? err.message : String(err)}`,
          "warn",
        );
      });
      const reader = new FileReader();
  ...
  const handleParquetFile = useCallback(
    async (file: File) => {
      try {
        await openNativeFileQueryTab(file, "parquet");
  ...
  return (
    <div className="duckdb-root">
      ...
      <div className="duckdb-app">
        <header className="duckdb-header">
          <div className="logo">
            ...
            >
              ...
              <Select.Portal>
                <Select.Positioner
                  className="duckdb-lang-switcher-positioner"
                  sideOffset={6}
                  alignItemWithTrigger={false}
                >
                  <Select.Popup className="bui-select-popup duckdb-lang-switcher-popup">
                      ...
                      <>
                        ...
                        {generatedCols.length > 0 && (
                          <div className="sql-modify-gen-section">
                            ...
                            <p className="sql-modify-duckdb-note">
        ...
        <div className="sql-shell duckdb-shell" ref={shellRef}>
          <aside className="sql-sidebar" aria-label="Database explorer">
            ...
            <div className="sql-tree">
              ...
              {/* DuckDB does not support triggers, so this schema section is intentionally omitted. */}
            </div>
            ...
            >
              {result?.sourceTable ? (
                <div className="sql-import-warning" style={{ margin: "8px 10px 0" }}>
                  DuckDB has no implicit rowid; inline cell editing is disabled.
                  Use an UPDATE statement with primary-key predicates instead.
                </div>
              ) : null}
              <ResultView
                result={result}
