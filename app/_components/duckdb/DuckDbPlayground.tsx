...
/** Can this column type carry `GENERATED BY DEFAULT AS IDENTITY`? Only an
 *  eligibility test — it says nothing about whether the column *is* an
 *  identity column. It used to be read as "this column is identity", which
 *  ticked the Identity box on every INTEGER/BIGINT column in the table,
 *  could not be unticked (the flag came from the type, not from state), and
 *  made every save emit an identity clause DuckDB rejects with
 *  "Constraint not implemented!". DuckDB reports no identity metadata over
 *  its catalog, so the only correct default is off. */
function canBeDuckDbIdentity(type: string): boolean {
...
function DuckDbStructureColumnRow({
  ...
  const fkTargetColumns = col.fkTable
    ...
    : [];
  const identityAllowed = canBeDuckDbIdentity(col.type);
...
function DuckDbPlaygroundInner() {
  ...
  /** Re-run the active tab's query, keeping its page and page size. A data
   *  tab otherwise showed whatever rows it was opened with: an INSERT from a
   *  query tab left it reading "Rows 1-2 of 2" beside a four-row table, and
   *  nothing in the tab offered a way to reload it. */
  ...
  const exportResultSet = useCallback(
    async (
      ...
      // Column types drive per-format serialization (temporal, LIST/STRUCT/
      // MAP, DECIMAL); the source table (or the tab's own title) gives the
      // SQL export a real target instead of the placeholder `result_set`.
      const opts = {
                        ...
                        <div className="sql-modify-columns">
                          <div className="sql-modify-cols-header">
                            ...
                            <span className="sql-modify-cols-count">
                              {/* The editor always renders one blank row for
                                  adding a column; counting it told the user a
                                  5-column table had 6. */}
                              {
                                regularCols.filter((c) => c.name.trim() !== "")
                                  .length
                              }
                            </span>
                          </div>
