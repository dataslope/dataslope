"use client";
// DuckDB Playground — mirrors PostgresPlayground with DuckDB-specific
// adaptations: StandardSQL dialect, no triggers, DuckDB type groups, and
// the @duckdb/duckdb-wasm async engine.
...
const STORAGE_PREFIX = "pg_duckdb_";
...
// DuckDB-specific type groups (triggers are not supported)
const PG_TYPE_GROUPS = [
  {
    label: "Integers",
    types: ["TINYINT", "SMALLINT", "INTEGER", "BIGINT", "HUGEINT", "UBIGINT"],
  },
  {
    label: "Floating point",
    types: ["FLOAT", "DOUBLE", "DECIMAL", "DECIMAL(18, 3)"],
  },
  { label: "Text", types: ["VARCHAR", "TEXT", "VARCHAR(255)"] },
  ...
  { label: "Lists", types: ["INTEGER[]", "VARCHAR[]", "DOUBLE[]"] },
  { label: "Binary", types: ["BLOB", "BIT"] },
] as const;

const PG_TYPE_OPTIONS: readonly string[] = PG_TYPE_GROUPS.flatMap(
  (group) => group.types,
);
const PG_SERIAL_TYPES = new Set<string>();  // DuckDB does not use serial types
...
function PgStructureColumnRow({
  ...
  const fkTargetColumns = col.fkTable
    ...
    : [];
  const serialType = false;  // DuckDB has no serial types
...
const RUNTIME_INFO: RuntimeInfo = {
  ...
  notes:
    "WASM build of DuckDB that runs entirely in your browser. Each sample database is rebuilt in memory on every page load. Note: triggers are not supported.",
};
...
function DuckDbPlaygroundInner() {
  ...
  const refreshSchema = useCallback(async () => {
    ...
    const [nextTables, nextViews, nextIndexes] =
      await Promise.all([
        engine.listTables(),
        engine.listViews(),
        engine.listIndexes(),
    ...
    (
      ...
      void engine.updateRows(tableName, updates).then((count) => {
        ...
        if (refetchSql) {
          ...
          // end of the result set (re-fetch with PK ordering after UPDATE)
  ...
  const openEntityStructure = useCallback(
    async (name: string) => {
      ...
      const displaySql = `SELECT column_name AS name, column_type AS type, is_nullable, column_default AS default FROM duckdb_columns() WHERE schema_name = 'main' AND table_name = '${name.replace(/'/g, "''")}' ORDER BY column_index;`;
      const tab: QueryTab = {
        id: newTabId(),
      ...
      const paramSql = `SELECT column_name AS name, column_type AS type, is_nullable, column_default AS default FROM duckdb_columns() WHERE schema_name = 'main' AND table_name = $1 ORDER BY column_index`;
      try {
              ...
              >
                {indexes.map((name) => (
                  <SchemaLeafItem
                    ...
                    onCopy={copyEntityName}
                    onViewDDL={(n) => void viewDDL(n)}
                    onDrop={(n, k) => requestDropEntity(n, k as "table" | "view" | "index")}
                  />
                ))}
              </SchemaSection>
              {/* DuckDB does not support triggers — section omitted */}
            </div>
