...
`app/magicui.css` intentionally includes Tailwind's Preflight reset and uses
...
new directory, add an `@source` line for it in `app/magicui.css`.**
...
1. **shadcn CLI** (one-off, works in any session):
   ...
   `cn` → `@/lib/utils`, css → `app/magicui.css`).
...
After adding a component, check `app/magicui.css`: components that ship custom
