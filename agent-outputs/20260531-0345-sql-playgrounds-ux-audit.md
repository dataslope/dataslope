...
>
> - **2026-06-01 — Column statistics (this pass).** Added a **"Column statistics"** item to the result-grid column menu: a dialog with non-null / null %, distinct, numeric **min/max/mean/median/sum** and text **length** stats, plus a **most-frequent values** list — and a **"Copy column values"** action. Lives entirely in the shared `ResultView` (so all three engines get it); pure helper `utils/columnStats.ts` (**+20** unit tests, 341→361); live-verified on SQLite/PostgreSQL/DuckDB (`e2e/sql-column-stats.spec.ts`). Closes the §9 "column quick stats" gap.
