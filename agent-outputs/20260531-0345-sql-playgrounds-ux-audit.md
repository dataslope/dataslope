...
>   - **Run statement at cursor (§9 IDE feature).** In a multi-statement tab, **Ctrl/⌘+Enter** with no selection now runs just the statement under the cursor (the universal IDE behavior) instead of the whole tab; a single-statement tab is unchanged, and Ctrl/⌘+Shift+Enter still forces run-all. Pure `splitSqlStatements` / `statementAtCursor` in `utils/sqlAnalysis.ts` (string/identifier/comment/dollar-quote aware; **+9** unit tests; **369→378** total) drive a one-line change to the shared `editorSetup.ts` keymap. Live-verified `e2e/sql-run-statement-at-cursor.spec.ts`.
...
- ✅ **Run-selection / run-statement-at-cursor** — both ship. Run-selection is in the editor toolbar; in a multi-statement tab, **Ctrl/⌘+Enter** (no selection) now runs just the statement under the cursor (single-statement tabs unchanged; Ctrl/⌘+Shift+Enter still runs all). `utils/sqlAnalysis.ts` `statementAtCursor`, shared `editorSetup.ts` keymap.
...
16. **Missing IDE features (§9):** ✅ **in-grid filter/search** on all three engines for any in-memory result (footer "Filter rows…" field, `column:term` scoping, `utils/resultFilter.ts`; engine-paged results push the filter down to SQL — subquery-wrap + `LIKE`/`ILIKE` `WHERE`, re-paged so infinite scroll is preserved). ✅ **column quick-stats** — "Column statistics" dialog (null %, distinct, numeric min/max/mean/median/sum, text length, most-frequent) + "Copy column values", shared `ResultView`, `utils/columnStats.ts`, live-verified (`e2e/sql-column-stats.spec.ts`). ✅ **run-selection** already ships (the editor's Run Selection / Run All toolbar). ✅ **run-statement-at-cursor** — in a multi-statement tab, Ctrl/⌘+Enter (no selection) runs just the statement under the cursor (`statementAtCursor` in `utils/sqlAnalysis.ts` + the shared `editorSetup.ts` keymap; single-statement tabs and the Run buttons are unchanged; Ctrl/⌘+Shift+Enter forces run-all). Live-verified `e2e/sql-run-statement-at-cursor.spec.ts`. ✅ **saved queries** — star a History entry → persisted "Saved queries" section (`useSavedQueries` + shared `QueryHistoryPane`, `e2e/sql-saved-queries.spec.ts`). **Remaining:** query cancellation + elapsed timer; EXPLAIN visualization.
