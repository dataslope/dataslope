...
>   - **Run statement at cursor (§9 IDE feature) — keyboard *and* a discoverable button.** In a multi-statement tab, **Ctrl/⌘+Enter** with no selection runs just the statement under the cursor (the universal IDE behavior); a single-statement tab is unchanged, and Ctrl/⌘+Shift+Enter forces run-all. To make it discoverable, the toolbar Run control becomes a **split button** that never duplicates its own primary action: for a multi-statement tab the primary is **Run All** and the dropdown offers **"Run statement at cursor · Ctrl+Enter"**; with a selection the primary is **Run Selection** and the dropdown offers **"Run All · Ctrl+Shift+Enter"** (so the relevant key is taught in-app, per-platform — `Ctrl` on Windows/Linux, `⌘` on macOS). Single-statement tabs keep the plain **Run** button. Pure `splitSqlStatements` / `statementAtCursor` in `utils/sqlAnalysis.ts` (string/identifier/comment/dollar-quote aware; **+9** unit tests; **369→378** total) drive the shared `editorSetup.ts` keymap and a `hasMultipleStatements` memo wired into all three playgrounds + the shared `SqlEditorToolbar`. Live-verified keyboard + button + dropdown + no-duplicate + single-statement fallback (`e2e/sql-run-statement-at-cursor.spec.ts`). `fix-run-statement-menu.png`.
...
- ✅ **Run-selection / run-statement-at-cursor** — both ship. The toolbar Run control is a split button whose dropdown holds only the *alternative* action: multi-statement → primary **Run All** + dropdown **"Run statement at cursor · Ctrl+Enter"**; selection → primary **Run Selection** + dropdown **"Run All · Ctrl+Shift+Enter"**; single statement → plain **Run**. In a multi-statement tab **Ctrl/⌘+Enter** (no selection) runs just the statement under the cursor. `utils/sqlAnalysis.ts` `statementAtCursor`, shared `editorSetup.ts` keymap + `SqlEditorToolbar`.
