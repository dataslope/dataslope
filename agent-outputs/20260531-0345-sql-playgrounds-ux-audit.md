...
> **✅ Update — debounced filter input + loading overlay; fixed dropped keystrokes (2026-06-01).** Typing in the result filter lagged and **dropped characters** once a result exceeded one page. Two causes: (1) the input was fully controlled by the result-set filter state, so every keystroke ran the heavy client filter / re-render synchronously, **and** a server-filter reload restored the filter from a snapshot — clobbering characters typed during the async reload; (2) only the engine-paged path was debounced, so the in-memory path re-filtered on every keystroke. **Fix:** a self-contained `FilterInput` owns its displayed text locally (keystrokes are never dropped by a re-render/reload) and **debounces the *applied* filter (300 ms)** for *both* the client and engine-paged paths (clearing via button/Escape is immediate; a `committedRef` re-syncs the field only on external changes, never mid-type). A **loading overlay** (spinner + "Filtering…") covers the result table while a filter is pending / re-querying; it honors a **minimum on-screen time** (`FILTER_OVERLAY_MIN_MS = 400 ms`) and **fades out via CSS** so a fast filter doesn't blink (mirrors the run overlay's `MIN_ANIMATION_MS`), with `pointer-events: none` + `prefers-reduced-motion`. The redundant second server-path debounce was removed (the input is the single debounce). **No separate worker** was needed — engine-paged queries already run off-thread in the engine worker, and in-memory filtering is bounded and now fires once after typing settles. **Re-verified with Playwright** on SQLite/PostgreSQL/DuckDB (`e2e/sql-result-filter.spec.ts`) + `e2e/sql-edit-refetch.spec.ts`; `tsc`/ESLint clean; 341 unit tests green. (This change also merges latest `main` into the branch.)
>
> **✅ Update — inline-edit refetch dropped the query's `LIMIT` (2026-06-01, twelfth pass).** Reported on Postgres: running `SELECT * FROM users LIMIT 10`, editing a cell inline, and hitting **Update** refreshed the grid to **20 rows** (the whole table) — the `LIMIT 10` was lost. Root cause: a query with its own `LIMIT` is *materialized* (not engine-paged), so it had no `lazyBaseSql`; the post-edit refetch then fell back to a bare `SELECT * FROM <table>` (no `LIMIT`). Fix: the result now records the exact `querySql` it ran, and `commitEdits` re-runs **that** query after an edit (preserving the `LIMIT`/shape) for materialized results — client-side sort is still preserved across the reload. Shared `commitEdits` (`ResultView`) + a new `querySql` field on `QueryRunResult` set by all three engines' `runSqlForTab`; **zero** change to the edit/update logic itself. **Verified live with Playwright on all three engines** (new `e2e/sql-edit-refetch.spec.ts`): `SELECT * FROM <users|customers> LIMIT 3` → edit the name cell → Update → grid stays at **3 rows** (was jumping to the full 20/10). `tsc` clean; unit suite green.
>
> **✅ Update — engine-paged filter via SQL pushdown (DBeaver model), preserving infinite scroll (2026-06-01, eleventh pass).** Closed the usability gap the tenth pass left behind: on an **engine-paged** result (a table with more rows than "Rows per page" — e.g. the `transactions` table's **260 rows** at the default **50/page**), the filter was rendered **disabled**, so the user simply *couldn't type in it*. It now filters — **without breaking infinite scroll**.
> - **[fix] The filter is pushed down to SQL.** Per how DBeaver/DataGrip do it (researched + linked in the PR), an engine-paged result's filter **wraps the original query as a subquery and adds a native `WHERE`** — `SELECT * FROM (<base>) AS _filter WHERE (CAST(col AS TEXT) LIKE/ILIKE '%term%' ESCAPE '\\' OR …)` — and re-pages through the **existing lazy/infinite path**. So the match covers the **whole** result (not just the loaded window) **and only one page is loaded** — the result keeps streaming/paging instead of materialising every row. `LIKE` for SQLite (ASCII case-insensitive), `ILIKE` for Postgres/DuckDB; the `column:term` scoping carries over; the term is escaped for the string literal **and** LIKE wildcards (`%`, `_`), so it's injection-safe (`item_500` matches the literal underscore).
> - **[hybrid] Small in-memory results still filter client-side.** A materialized result, or a lazy result that fit one page / is fully loaded, keeps the tenth pass's **exact displayed-text** client filter (no re-query). Only genuinely engine-paged (or already server-filtered) results push down. The readout is **"Rows 1–N of M · filtered from K"** either way; clearing restores the full result.
> - **[fix] Re-page no longer blocked by schema introspection.** A re-page (filter / sort / page) doesn't change the schema, yet Postgres/DuckDB held the run-lock through a ~1–2 s `refreshSchema()`, which could drop a quick follow-up filter. `refreshSchema()` after a run is now **fire-and-forget** (the sidebar still updates; it just no longer blocks the lock), so iterative filtering is responsive.
> - **Implementation.** New pure, unit-tested helper `buildResultFilterWhere` (+ `dialectFromEngineLabel`) in `utils/resultFilter.ts`. `ResultView` composes the filtered+sorted lazy SQL (`composeLazyQuery`), debounces the re-query (`triggerServerFilter`/`scheduleServerFilter`), tracks which sets are server-filtered (to skip client-filtering already-filtered rows and surface the pre-filter total), and pages load-more off `result.lazySql` so infinite scroll streams the *filtered* set. The "load all rows" dialog from the first cut of this pass is **removed** (superseded). **Zero** runtime/adapter changes; the only playground edit is the fire-and-forget refresh.
> - **Verification (live, all three engines).** Driven **live with Playwright** (`e2e/sql-result-filter.spec.ts`): build a 600-row table → `SELECT *` (engine-paged, 50/page) → the filter is an **enabled input** (no button, no disabled state) → type **`even`** → **"Rows 1–50 of 300 · filtered from 600"** (300 matched across all 600, only 50 loaded → paging preserved, no "odd" rows) → **`item_500`** → **"of 1 · filtered from 600"** (wildcard-escape correct) → clear → **of 600**. Plus a SQLite client-side check on a `LIMIT 20` result. **+13** unit tests for the SQL builder (321 → 341). `tsc` **0 errors**; ESLint **0 errors**. Screenshots in `audit-results/filter-pushdown/`.
>
> **✅ Update — Phase 4 in-grid filter / "find in results" (2026-06-01, tenth pass).** Shipped the top remaining **Result grid** gap from §9 ("❌ In-grid filter/search across a result set") — the next priority after the now-done type-aware cell viewers.
> - **[done] In-grid filter ("find in results").** A compact **Filter rows…** field now sits in the result footer (left of the row count), **on all three engines**. Typing narrows the displayed/paginated rows to those whose **visible text** matches (case-insensitive substring across **all** columns — matched against exactly what `formatCellValue` renders, so `null`, `[10, 20, 30]`, `BLOB (4 bytes)` etc. are searchable). A leading **`column:term`** prefix (when `column` names a real result column) scopes the match to that one column; otherwise the whole string is the term (so a `12:30` time or `note: x` with no `note` column is never silently swallowed). The row readout becomes **"Rows 1–N of M · filtered from K"** (or **"No matching rows (of K)"**), and **Esc** or the **✕** clears it. Pagination + sorting operate on the filtered view; the filter resets on every new query/result and is remembered per result-set tab. **Export is deliberately untouched** — it always operates on the *full* result (the filter is a view-only find aid), so its row/page counts stay on the unfiltered total.
> - **[fix] Now visible in Postgres & DuckDB (not just SQLite).** First cut gated the field to non-lazy results — but **all three** engines route a bare `SELECT … ` (no `LIMIT`) through the engine-paged *lazy* path, so the field looked absent in PG/DuckDB (and for most SQLite queries too). Corrected to offer the filter whenever **the whole result is in memory** — every materialized result, *and* a lazy result that fit one page (rows ≤ the page size, default 50) or an "All" result that's fully loaded. New pure helper `canClientFilterResult` makes that call; the lazy branch of `computeSetRenderData` now filters its rows too (its loaded offset is 0, so each row's index is its absolute position — edits/selection/delete stay correct). For a genuinely **engine-paged** result (only a window loaded), the field was originally **shown but disabled with a tooltip**. *(Superseded by the eleventh pass above — engine-paged results now push the filter down to SQL and re-page, so the field is a normal **enabled** input that filters the whole result while preserving infinite scroll, never a dead/disabled field.)*
> - **Implementation.** Entirely in the shared result grid (`components/ResultView.tsx`) + a new **pure, unit-tested** helper `utils/resultFilter.ts` (`parseResultFilter` / `rowMatchesResultFilter` / `filterResultRowIndices` / `canClientFilterResult`). The filter narrows the in-memory rows in `computeSetRenderData` *before* sort/paginate, keying each row by its **original index** so cell edits, selection and delete stay correct. **Works identically on all three engines** because it operates on already-materialized rows — zero engine/runtime/adapter changes, zero changes to the three playground bodies.
> - **Scope / follow-up.** Covers every result that fits in memory (the common case — most learning queries return well under the page size, and raising "Rows per page" / choosing All loads more so they become filterable). **True SQL-level filtering of large engine-paged results** (an `ILIKE`/`LIKE` `WHERE` *pushdown* re-paged through the engine, so you can filter without loading every row) is the dedicated follow-up (see §12 Phase 4) — it's per-engine SQL best landed with a live engine to test against.
> - **Verification.** New `__tests__/resultFilter.test.ts` (**+26**, full truth table: column-scoping, case-insensitivity, NULL/array/blob/boolean display matching, original-index preservation, empty-query passthrough, and the `canClientFilterResult` in-memory/lazy/paged decision). `tsc --noEmit` **0 errors**; ESLint **0 errors** (8 pre-existing warnings unchanged — verified against the stashed baseline); unit suite **333 passed** (307 → 333). ⚠️ As with the other CDN-blocked passes (the WASM CDN returns `403 host_not_allowed` this session, so the engines can't boot here), live Playwright verification of the rendered grid wasn't possible — the change is confined to the shared `ResultView` plus the pure, exhaustively-unit-tested `resultFilter` helper to keep that risk contained. `components/ResultView.tsx`, `utils/resultFilter.ts`, `sqlPlayground.css`.
>
> **✅ Update — Phase 4 visual polish (2026-06-01, ninth pass).** Contained, engine-free-verifiable §12-Phase-4 polish:
> - **[done] UX-19 — green dialog subtitle.** `.sql-modify-drawer-subtitle` ("Create a new table" / "Add a row" / etc.) used `color: var(--accent1)` — the teal/green accent, which reads like a success status. Changed to the muted `var(--text-dim)` used by every other secondary caption in the file. *Verified live:* injected a probe element with that class into the running app and read the resolved color — `rgb(131,135,142)` = `--text-dim` (`#83878e`), no longer the `--accent1` accent. `sqlPlayground.css`.
> - **[done] UX-15 (partial) — identity header mid-word wrap.** The Postgres structure tables (Add Table + Edit Structure) hard-broke the column header as `Identity/<br/>serial`, splitting the token across two lines. Replaced with a clean single-line **"Identity"** header + a `title` tooltip ("Identity / serial (auto-incrementing) column"), which also aligns Postgres with DuckDB's existing "Identity" label. SQLite keeps "Auto-increment" (a distinct concept — `AUTOINCREMENT` — in a different form-field context), so the cross-engine wording is now intentional rather than drifting. `tsc`/ESLint **0 errors**. (The PG Add/Edit-Structure dialog is engine-gated — it needs a booted DB — so it couldn't be screenshotted in this CDN-blocked session; the change is static JSX.) `PostgresPlayground.tsx`.
>
> **✅ Update — Request 3 + latest `main` merge (2026-06-01, eighth pass).**
> - **[done] Merged latest `main` (#432 — "CORS proxy Vercel previews; R download/print fixes; CSV-from-URL examples").** Clean merge, no conflicts (it touches the language-runtime + Cloudflare-CORS-proxy files, not the SQL playgrounds). Full re-verify after merging: `tsc --noEmit` **0 errors**; unit suite **303 passed** (the merge brings in #432's `corsProxy.test.ts` + R/CSV e2e, hence 290 → 303); ESLint **0 errors** on the changed source files.
> - **[done] Request 3 — mobile: each query tab remembers its own bottom pane.** Previously the mobile bottom-pane choice (Schema / Editor / Results) was a **single global state**, so leaving tab A on Results and activating a never-run tab B stranded you on B's **empty** Results. Now each query tab remembers the last pane it was on (keyed by the tab's `data-tab-id`); activating a tab **restores that tab's pane**, with one guard — a tab never lands on an empty Results, falling back to the **Editor** ("no results ⇒ show the editor") — and a brand-new tab defaults to the Editor. Implemented **entirely in the shared `SqlPlaygroundShell`** via the existing DOM-observation approach (the `MutationObserver` that already tracks `hasResults` now also watches the query tab's `.active` class toggle and reads `data-tab-id`; the bottom-tab buttons + Run/open-table affordances record the choice). **Zero changes to the three playground bodies.** New pure, unit-tested helper `sql/utils/mobilePane.ts` (`paneForActivatedTab`) with `mobilePane.test.ts` (**+4**, full truth table incl. the empty-Results→Editor guard). **Verification:** `tsc`/ESLint **0 errors**; unit suite **307 passed**; mobile e2e spec **5/5 green** (`playground-mobile.spec.ts`, run live against the dev server). The WASM CDN is **blocked in this session** (the engines don't boot here), so — like the rest of this spec by design — the e2e stays **engine-free**: the new Request-3 case exercises the per-tab restore *wiring* (tab creation/activation + the `data-tab-id`/`.active` observer) and asserts a tab switch never strands the user on the gated, empty Results pane, while the Editor↔Results *decision* is covered by the unit test. This pass also **fixed 3 pre-existing per-engine tests** in that spec that clicked the Results bottom tab: the sixth pass's "Results disabled until there's output" change had since **gated** it, so those clicks hung whenever the engine wasn't booted; they now assert the gated state on a fresh tab (deterministic regardless of CDN). `SqlPlaygroundShell.tsx`, `sql/utils/mobilePane.ts`, `e2e/playground-mobile.spec.ts`.
>
...
16. **Missing IDE features (§9):** ✅ **in-grid filter/search done (tenth pass)** on all three engines for any in-memory result (footer "Filter rows…" field, `column:term` scoping, `utils/resultFilter.ts`; engine-paged results push the filter down to SQL — subquery-wrap + `LIKE`/`ILIKE` `WHERE`, re-paged so infinite scroll is preserved, eleventh pass); **remaining:** query cancellation + elapsed timer; run-selection; saved snippets; column quick-stats; EXPLAIN visualization.
