...
| **UX-04 ✅ Partial** | Inline edit = single-line `<input type=text>` for every type; modal = plain `<textarea>`; neither is type-aware | 🔴 | All | JSON-aware modal **+** inline **boolean toggle** (tri-state checkbox; was `0`/`1`) done in `ResultView.tsx`. Remaining inline editors (date/enum/blob/array) → Phase 2b |
...
6. **UX-04 inline type-aware editors** — **✅ boolean toggle DONE (2026-05-31)**: boolean columns render as a tri-state checkbox (true/false/NULL) instead of `0`/`1`, with engine-correct commit (PG boolean param, DuckDB `TRUE`/`FALSE` literal, SQLite coerced to `0`/`1` — verified it stores `typeof=integer`, not text). Verified round-trip 6/6 on all engines. **Remaining:** date/time picker (UX-09), enum dropdown, array editor, BYTEA/BLOB hex viewer + upload (UX-21) — driven by `set.columnTypes`; the modal already covers JSON.
