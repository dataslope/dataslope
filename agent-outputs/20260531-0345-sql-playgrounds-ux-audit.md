...
| **UX-10 ✅ Partial** | Commit affordance ("Update N cell…") sits far bottom-right; ~~no per-row/column discard~~, no post-commit undo | 🟡 | All | Per-cell discard **✕** + **Ctrl/⌘+Enter** commit added (post-commit undo still open). `components/ResultView.tsx` |
...
- 🟡 **UX-10 — Commit/undo ergonomics. ✅ Partial.** Pending edits flush via an "Update N cell…" button in the bottom-right footer (`03b-pg-pending-commit-bar.png`). **Fixed:** each pending cell now shows a **per-cell discard "✕"** (revert just that cell, on hover/focus), and **Ctrl/⌘+Enter commits** the pending edits (scoped so it never collides with the editor's run shortcut; Esc still discards all). **Remaining:** a one-step post-commit undo.
...
9. **UX-10 Commit/undo — ✅ Partial.** Per-cell discard ("✕" on a pending cell) and **Ctrl/⌘+Enter** keyboard commit are done (Esc already discarded all); live-verified on all three engines (`e2e/sql-edit-ergonomics.spec.ts`). **Remaining:** a one-step post-commit undo.
