...
- ✅ **In-grid filter/search** across a result set — **done (tenth pass)** for materialized query results: a footer "Filter rows…" field, case-insensitive substring across all columns (matching the displayed text), `column:term` scoping, "filtered from N" readout, Esc/✕ to clear. Pure helper `utils/resultFilter.ts`. *Remaining:* engine-paged "lazy" results (sidebar-opened tables / infinite scroll) need a SQL `WHERE` pushdown — follow-up.
...
16. **Missing IDE features (§9):** ✅ **in-grid filter/search done (tenth pass)** for materialized results (footer "Filter rows…" field, `column:term` scoping, view-only — `utils/resultFilter.ts`); **remaining:** extend the filter to engine-paged "lazy" results via a SQL `WHERE`/`ILIKE` pushdown, query cancellation + elapsed timer; run-selection; saved snippets; column quick-stats; EXPLAIN visualization.
