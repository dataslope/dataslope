...
>
> - **2026-06-03 — Create Index/View (UX-11), type-aware cell editors (UX-04: enum + array), and Explain/query-plan (§9) (this PR).**
>   - **Create Index / Create View from the schema tree (UX-11).** The **INDEXES** and **VIEWS** sidebar sections gained a **"+"** (and an empty-state "Create …" button) that open compact, **shared** dialogs — so SQLite/PostgreSQL/DuckDB get them at once with no per-engine UI. **Create Index**: pick a table, check columns (a per-column **order badge** shows the index column order), toggle **Unique** / **IF NOT EXISTS**, with an auto-suggested name and a **live SQL preview**. **Create View**: a name + a **SELECT** body (seeded from the editor when it already holds a query) + a dialect-aware **"Replace if it already exists"** (PG/DuckDB → `CREATE OR REPLACE VIEW`; SQLite, which has no such form → `DROP VIEW IF EXISTS` + `CREATE VIEW`). On submit the generated DDL runs through the engine and the sidebar refreshes, with a success/failure toast — mirroring the Add Table flow (no new tab, no editor disruption). Pure builders `buildCreateIndexSql` / `buildCreateViewSql` / `suggestIndexName` live in `utils/ddl.ts` (**+22** unit tests). Also fixed `SchemaSection`'s label singularizer ("Indexes" → "index", not "indexe") and added an a/an article, now that INDEXES has an add affordance. Live-verified on **all three** engines (`e2e/sql-create-object.spec.ts`). `fix-create-index-dialog.png`, `fix-create-view-dialog.png`.
>   - **Inline enum dropdown editor (UX-04).** An **enum-typed** column now opens a `<select>` of its declared labels on double-click instead of a free-text input; the committed value is the chosen label — the same string the text editor would produce — so the engine casts it to the enum type on write (no new write-path risk). The labels ride along on `listColumns`: **Postgres** via a `pg_enum` lookup (keyed by `udt_name`), **DuckDB** by parsing `duckdb_columns()`'s `ENUM('a','b')` type string (new pure `parseDuckDbEnumValues`). They flow through `TableColumnInfo.enumValues` → `ColumnKeyHints.enums` (built by the shared `enumHintsFromColumns`) into the shared `ResultView`. SQLite has no native enum type, so it's unaffected. **+10** unit tests; live-verified edit→commit→persist on PG + DuckDB (`e2e/sql-enum-editor.spec.ts`). `fix-enum-dropdown.png`.
>   - **Inline array / LIST editor (UX-04 — completes it).** An array column (Postgres `integer[]`, DuckDB Arrow `list<…>`) renders as a JSON string and is **edited as JSON**; on commit the parsed JS array is written back as a real array. **Postgres** binds the JS array via pglite; **DuckDB-Wasm can't bind a JS array** as a LIST parameter (verified — it errors), so it's inlined as a **safely-escaped list literal** (new pure `toDuckDbListLiteral`; every other value in the row still binds, per UX-17). Invalid/partial JSON is kept as raw text so a clear engine error surfaces on commit rather than silently writing garbage. New pure `arrayEditorText` / `parseArrayEditValue`; `classifyCellEditor` now routes `[]` / `list<…>` to the array editor. **+12** unit tests (**425→437** total across this PR); live-verified edit→commit→round-trip on PG + DuckDB (`e2e/sql-array-editor.spec.ts`). **This closes UX-04.** `fix-array-editor.png`.
>   - **Explain / query-plan (§9 IDE feature).** A new **"Explain"** toolbar button (next to Run) shows the execution plan for the **selection / statement at the cursor / whole query** in a read-only modal with a Copy action. Per-engine via the pure `buildExplainSql` (SQLite `EXPLAIN QUERY PLAN`; PG/DuckDB plain `EXPLAIN` — *plans* without executing, so it's safe even for DML), and a pure `formatExplainResult` renders the three engines' differing shapes (PG's line-per-row plan, SQLite's `(id,parent,…,detail)`, DuckDB's ASCII tree) uniformly. Shared `ExplainPlanDialog` + `SqlEditorToolbar` (one new `onExplain`), with a shared `activeSqlForEditor` helper picking the target statement. **+8** unit tests (**437→445**); live-verified on **all three** engines (`e2e/sql-explain.spec.ts`). `fix-explain-plan.png`.
...
>
> **Still open / deferred** (see §12): UX-08 multi-statement error attribution, UX-11/UX-16 trigger & CHECK-constraint UI + live DDL preview in the Table dialog (create-index/view now shipped), UX-24 card-per-column structure editor, and UX-Q1 SSR tab-id hydration (its own PR). **UX-04 is now fully closed** (JSON, boolean, date/time, enum and array editors all ship).
...
| **UX-04 ✅ Fixed** | Inline edit was a single-line `<input type=text>` for every type; neither inline nor modal was type-aware | 🔴 | All | Type-aware editors now: JSON modal, **boolean toggle**, **date/time pickers** (UX-09), **enum dropdown**, **array (JSON) editor**, BLOB viewer (UX-21). `ResultView.tsx`/`cellEditing.ts` |
...
- 🔴 **UX-04 — No type-aware editors.** Every type shares one single-line text input (`ResultView.tsx:1803-1830`); the only "advanced" path is an 8-row plain `<textarea>` (`:2200`). No JSON editor/validator, date picker, boolean toggle, enum dropdown, array editor, or blob viewer. **✅ Done (UX-04 closed):** JSON modal, boolean toggle, **date/time pickers** (UX-09), a **BLOB viewer** (UX-21), an **enum dropdown** (PG `pg_enum` / DuckDB `ENUM(...)` labels → a `<select>`) and an **array (JSON) editor** (PG binds the JS array via pglite; DuckDB inlines a safely-escaped list literal since it can't bind one) are all done.
...
| Inline edit type-awareness | ✅ bool/date/time/JSON/array | ✅ bool/date/time/JSON/enum/array | ✅ bool/date/time/JSON/enum/array | UX-04 ✅ (SQLite has no enum type) |
...
- ✅ **Type-aware cell viewers/editors** (UX-04, closed) — JSON modal, date/time picker, boolean toggle, **enum dropdown** (PG/DuckDB), **array (JSON) editor** and blob hex viewer all ship.
...
- ✅ **EXPLAIN / query-plan** — an **"Explain"** toolbar button shows the execution plan for the active statement in a read-only modal (SQLite `EXPLAIN QUERY PLAN`; PG/DuckDB plain `EXPLAIN`). Pure `buildExplainSql` / `formatExplainResult` (`utils/explain.ts`); shared `ExplainPlanDialog`; live-verified `e2e/sql-explain.spec.ts`. (A richer graphical tree is a possible future enhancement.)
...
6. **UX-04 inline type-aware editors** — **✅ boolean toggle DONE (2026-05-31)**: boolean columns render as a tri-state checkbox (true/false/NULL) instead of `0`/`1`, with engine-correct commit (PG boolean param, DuckDB `TRUE`/`FALSE` literal, SQLite coerced to `0`/`1` — verified it stores `typeof=integer`, not text). Verified round-trip 6/6 on all engines. **✅ date/time pickers DONE (2026-05-31, 2nd pass):** `date`/`timestamp(tz)`/`time` columns open a native `<input type="date|datetime-local|time">` on double-click; `classifyCellEditor` (in the new pure `utils/cellEditing.ts`) maps each engine's `columnTypes` string, and `from/toDateEditorValue` convert without JS-`Date` timezone math, preserving the original value's separator/fraction/zone so the commit round-trips like the text editor (falls back to text for non-string temporals). 28 unit tests; live engine boot was not possible this pass (CDN denied — see top note). **✅ enum dropdown DONE (2026-06-03):** an enum column opens a `<select>` of its declared labels (Postgres `pg_enum` lookup keyed by `udt_name`; DuckDB parses the `ENUM('a','b')` type from `duckdb_columns()` via the pure `parseDuckDbEnumValues`), threaded through `TableColumnInfo.enumValues` → `ColumnKeyHints.enums` (built by `enumHintsFromColumns`) into the shared `ResultView`. The commit is the chosen label string — the same value the text editor produced — so the engine casts it to the enum type (no write-path change). SQLite has no native enum, so it's unaffected. **+10** unit tests; live-verified on PG + DuckDB (`e2e/sql-enum-editor.spec.ts`). **✅ array editor DONE (2026-06-03) — closes UX-04:** an array column (`integer[]` / Arrow `list<…>`) is edited as a JSON array (`arrayEditorText` / `parseArrayEditValue`; `classifyCellEditor` routes `[]`/`list<…>` here). On commit the parsed JS array is written back — Postgres binds it via pglite; DuckDB-Wasm **can't** bind a JS array as a LIST param (verified live — it errors), so it's inlined as a safely-escaped list literal (`toDuckDbListLiteral`), with the rest of the row still bound. **+12** unit tests; live-verified round-trip on PG + DuckDB (`e2e/sql-array-editor.spec.ts`).
...
| `fix-enum-dropdown.png` | Inline enum cell editor — a `<select>` of the column's declared labels (UX-04) |
| `fix-array-editor.png` | Inline array / LIST cell editor — edit the value as a JSON array (UX-04) |
| `fix-explain-plan.png` | "Explain" query-plan modal — the execution plan for the active statement (§9) |
