...
- ✅ **In-grid filter/search** across a result set — **done (tenth pass)**, on all three engines, for any result held in memory: a footer "Filter rows…" field, case-insensitive substring across all columns (matching the displayed text), `column:term` scoping, "filtered from N" readout, Esc/✕ to clear. Pure helper `utils/resultFilter.ts`. Engine-paged results show the field **disabled with a tooltip** (increase Rows per page / All to enable). *Remaining:* true SQL `WHERE` pushdown so very large engine-paged results filter without loading every row — follow-up.
...
16. **Missing IDE features (§9):** ✅ **in-grid filter/search done (tenth pass)** on all three engines for any in-memory result (footer "Filter rows…" field, `column:term` scoping, view-only — `utils/resultFilter.ts`; engine-paged results show it disabled w/ a tooltip); **remaining:** a SQL `WHERE`/`ILIKE` *pushdown* so very large engine-paged results filter without loading every row; query cancellation + elapsed timer; run-selection; saved snippets; column quick-stats; EXPLAIN visualization.
