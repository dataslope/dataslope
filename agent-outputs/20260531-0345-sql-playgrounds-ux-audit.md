...
>
> - **2026-06-02 — Post-commit undo, DuckDB parameter binding, run-statement-at-cursor (this PR).**
...
>   - **Run statement at cursor (§9 IDE feature) — keyboard *and* a discoverable button.** In a multi-statement tab, **Ctrl/⌘+Enter** with no selection runs just the statement under the cursor (the universal IDE behavior); a single-statement tab is unchanged, and Ctrl/⌘+Shift+Enter forces run-all. To make it discoverable (and keep the button consistent with the shortcut), the toolbar Run control becomes a **"Run statement" split button** for multi-statement tabs, whose dropdown lists **"Run statement at cursor · Ctrl+Enter"** and **"Run All · Ctrl+Shift+Enter"** (so the keys are taught in-app, per-platform — `Ctrl` on Windows/Linux, `⌘` on macOS). Single-statement tabs keep the plain **Run** button. Pure `splitSqlStatements` / `statementAtCursor` in `utils/sqlAnalysis.ts` (string/identifier/comment/dollar-quote aware; **+9** unit tests; **369→378** total) drive the shared `editorSetup.ts` keymap and a `hasMultipleStatements` memo wired into all three playgrounds + the shared `SqlEditorToolbar`. Live-verified keyboard + button + dropdown + single-statement fallback (`e2e/sql-run-statement-at-cursor.spec.ts`). `fix-run-statement-menu.png`.
...
>
> **Still open / deferred** (see §12): UX-08 multi-statement error attribution, UX-11/UX-16 index/trigger/CHECK UI + live DDL preview, UX-24 card-per-column structure editor, UX-Q1 SSR tab-id hydration (its own PR), enum/array inline editors, and query cancellation.
...
- ✅ **Run-selection / run-statement-at-cursor** — both ship. Run-selection is in the editor toolbar; in a multi-statement tab, **Ctrl/⌘+Enter** (no selection) runs just the statement under the cursor, and the toolbar shows a discoverable **"Run statement" split button** (dropdown: "Run statement at cursor · Ctrl+Enter" / "Run All · Ctrl+Shift+Enter"). Single-statement tabs keep the plain Run button. `utils/sqlAnalysis.ts` `statementAtCursor`, shared `editorSetup.ts` keymap + `SqlEditorToolbar`.
...
16. **Missing IDE features (§9):** ✅ **in-grid filter/search** on all three engines for any in-memory result (footer "Filter rows…" field, `column:term` scoping, `utils/resultFilter.ts`; engine-paged results push the filter down to SQL — subquery-wrap + `LIKE`/`ILIKE` `WHERE`, re-paged so infinite scroll is preserved). ✅ **column quick-stats** — "Column statistics" dialog (null %, distinct, numeric min/max/mean/median/sum, text length, most-frequent) + "Copy column values", shared `ResultView`, `utils/columnStats.ts`, live-verified (`e2e/sql-column-stats.spec.ts`). ✅ **run-selection** already ships (the editor's Run Selection / Run All toolbar). ✅ **run-statement-at-cursor** — in a multi-statement tab, Ctrl/⌘+Enter (no selection) runs just the statement under the cursor, surfaced via a discoverable **"Run statement" split button** (dropdown shows both actions + their shortcuts) so it isn't keyboard-only; single-statement tabs keep the plain Run button; Ctrl/⌘+Shift+Enter forces run-all. `statementAtCursor`/`splitSqlStatements` (`utils/sqlAnalysis.ts`) + shared `editorSetup.ts` keymap + `SqlEditorToolbar`, wired through `hasMultipleStatements` in all three playgrounds. Live-verified `e2e/sql-run-statement-at-cursor.spec.ts`. ✅ **saved queries** — star a History entry → persisted "Saved queries" section (`useSavedQueries` + shared `QueryHistoryPane`, `e2e/sql-saved-queries.spec.ts`). **Remaining:** query cancellation + elapsed timer; EXPLAIN visualization.
...
| `fix-run-statement-menu.png` | "Run statement" split button + dropdown (run-statement-at-cursor) |
