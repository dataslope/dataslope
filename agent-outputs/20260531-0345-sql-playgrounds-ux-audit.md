...
- 🟡 **UX-Q4 — Pre-existing failing unit test.** `__tests__/opfs.workspace.test.ts > registry > "filters out invalid entries"` fails on a clean tree (`getWorkspaceRegistry()` returns `[]` rather than retaining the single valid entry alongside an invalid one). Confirmed unrelated to the sixth-pass changes by stashing them and re-running. Either the registry validator is over-rejecting a batch when any entry is malformed, or the test's fixture drifted from the validator. Worth a dedicated fix so the suite is green.
