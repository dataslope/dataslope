...
>   - **Inline enum dropdown editor (UX-04).** An **enum-typed** column now opens a `<select>` of its declared labels on double-click instead of a free-text input; the committed value is the chosen label — the same string the text editor would produce — so the engine casts it to the enum type on write (no new write-path risk). The labels ride along on `listColumns`: **Postgres** via a `pg_enum` lookup (keyed by `udt_name`), **DuckDB** by parsing `duckdb_columns()`'s `ENUM('a','b')` type string (new pure `parseDuckDbEnumValues`). They flow through `TableColumnInfo.enumValues` → `ColumnKeyHints.enums` (built by the shared `enumHintsFromColumns`) into the shared `ResultView`. SQLite has no native enum type, so it's unaffected. **+10** unit tests (**415→425**); live-verified edit→commit→persist on PG + DuckDB (`e2e/sql-enum-editor.spec.ts`). The **array editor** is the remaining UX-04 piece. `fix-enum-dropdown.png`.
...
>
> **Still open / deferred** (see §12): UX-08 multi-statement error attribution, UX-11/UX-16 trigger & CHECK-constraint UI + live DDL preview in the Table dialog (create-index/view now shipped), UX-24 card-per-column structure editor, UX-Q1 SSR tab-id hydration (its own PR), and the UX-04 array inline editor (enum dropdown now shipped).
...
| **UX-04 ✅ Partial** | Inline edit = single-line `<input type=text>` for every type; modal = plain `<textarea>`; neither is type-aware | 🔴 | All | JSON-aware modal **+** inline **boolean toggle**, **date/time pickers** (UX-09) and **enum dropdown** done in `ResultView.tsx`/`cellEditing.ts`. Remaining: **array** editor |
...
| Inline edit type-awareness | ⚠️ bool/date/time/JSON | ⚠️ bool/date/time/JSON/**enum** | ⚠️ bool/date/time/JSON/**enum** | UX-04 (array remains; SQLite has no enum) |
...
- ⚠️ **Type-aware cell viewers/editors** (UX-04) — JSON modal, date/time picker, boolean toggle, **enum dropdown** (PG/DuckDB) and blob hex viewer ✅; **array editor** still ❌.
