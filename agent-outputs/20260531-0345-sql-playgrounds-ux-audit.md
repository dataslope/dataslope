...
14. **Playwright mobile coverage — ✅ Done.** `e2e/playground-mobile.spec.ts` (5 tests) asserts, for all three engines, no horizontal overflow + a working bottom tab bar (Schema/Editor/Results reachable) at 390 px, plus a desktop non-regression check (3-pane intact, no bottom bar) at 1280 px, plus (Request 3) a **per-query-tab pane-memory** check: a tab set to Schema, a new "+" tab defaulting to Editor, then re-activating each tab restoring *its* remembered pane. Deliberately does **not** wait for the WASM engine to boot (it hides the boot overlay and exercises the shell/CSS), so it's fast and CDN-independent.
