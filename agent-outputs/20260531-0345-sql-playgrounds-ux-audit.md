...
- 🟡 **UX-09 — Date/time editing is opaque.** `timestamptz` is shown as `2026-05-31T03:35:51.558Z` (raw UTC ISO). No picker; no indication the stored value will be normalized to UTC. **✅ Fixed:** a native date/datetime-local/time picker now opens on double-click; the commit preserves the original value's exact format (separator/fraction/zone) so it round-trips. Non-string temporal values fall back to text.
