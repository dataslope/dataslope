...
>
> **✅ Update — engine-paged filter made actionable: "load all rows to filter" (2026-06-01, eleventh pass).** Closed the usability gap the tenth pass left behind: on an **engine-paged** result (a table with more rows than "Rows per page" — e.g. the `transactions` table's **260 rows** at the default **50/page**), the filter was rendered **disabled**, so the user simply *couldn't type in it*. The field is now **actionable** instead of dead.
> - **[fix] The filter offers to load the whole result.** When only a window is loaded, the footer shows a clickable **"Filter rows…"** button (styled like the field, not a greyed-out input). Clicking it opens a small **"Filter all rows?"** confirmation — *"Only the current page is loaded (N of M rows). To search the whole result, every row will be loaded into the grid and 'Rows per page' switches to All."* — with **Cancel** / **Load all rows**. Confirming switches the result to **All** (reusing the existing lazy "All" reload, preserving the active sort), materializes every row, and **auto-focuses** the now-enabled filter input so the user can type immediately. This satisfies "find in results" semantics — typing then narrows across the **entire** result, not a misleading partial window.
> - **[guard] No reload loop for very large results.** If a result is already in **All/infinite** mode but still streaming (> the 500-row infinite-scroll chunk), the field stays **disabled with a tooltip** (*"Still loading every row — scroll to load the rest, then filter, or add a SQL WHERE clause"*) rather than offering a "load all" that can't complete in one shot. The one-click button is only shown when a single "All" load *will* materialize the whole result — which is the reported case and the vast majority of sample datasets.
> - **Implementation.** Confined to the shared result grid: `ResultView` adds a `filterLoadPrompt` state + `performLoadAllToFilter` (which mirrors choosing "All" in the pager) and a confirmation `AlertDialog`; `ResultPager` gains an `onRequestLoadAll` prop that swaps the disabled input for the button. **Zero** engine/runtime/playground changes — the load path is the engines' existing lazy reload, so it behaves identically on SQLite / Postgres / DuckDB.
> - **Verification (live, this session).** The WASM CDN is reachable this session, so this was driven **live with Playwright** (new `e2e/sql-result-filter.spec.ts`): open `transactions` (Rows 1–50 **of 260**) → the filter shows as a **button**, not a disabled field → click → **"Filter all rows?"** dialog ("50 of 260 rows") → **Load all rows** → input is enabled, focused, and the grid now holds all 260 → type **`amazon`** → readout reads **"Rows 1–18 of 18 · filtered from 260"** with matches spanning transaction_ids 1→232 (i.e. the *whole* set, not page 1) → a no-match term shows **"No matching rows (of 260)"** → **Esc** clears. 5 screenshots in `audit-results/filter-screens/`. `tsc --noEmit` **0 errors**; ESLint **no new warnings** (8 pre-existing, unchanged — verified against the stashed baseline); unit suite **333 passed**. `components/ResultView.tsx`, `sqlPlayground.css`, `e2e/sql-result-filter.spec.ts`.
>
...
- ✅ **In-grid filter/search** across a result set — **done (tenth pass)**, on all three engines, for any result held in memory: a footer "Filter rows…" field, case-insensitive substring across all columns (matching the displayed text), `column:term` scoping, "filtered from N" readout, Esc/✕ to clear. Pure helper `utils/resultFilter.ts`. Engine-paged results (more rows than "Rows per page") show the filter as an actionable **"Filter rows…" button** that opens a *"Filter all rows?"* confirmation and, on confirm, loads every row + focuses the filter (eleventh pass) — so the field is never a dead/disabled input. *Remaining:* true SQL `WHERE` pushdown so very large engine-paged results filter without loading every row — follow-up.
...
16. **Missing IDE features (§9):** ✅ **in-grid filter/search done (tenth pass)** on all three engines for any in-memory result (footer "Filter rows…" field, `column:term` scoping, view-only — `utils/resultFilter.ts`; engine-paged results offer a one-click "load all rows to filter" button + confirmation, eleventh pass); **remaining:** a SQL `WHERE`/`ILIKE` *pushdown* so very large engine-paged results filter without loading every row; query cancellation + elapsed timer; run-selection; saved snippets; column quick-stats; EXPLAIN visualization.
