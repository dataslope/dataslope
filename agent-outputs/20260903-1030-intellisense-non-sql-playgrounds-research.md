...
### 9.9 Follow-up: popup shape, ranking, settings polish

- **Ordering** (`completion/ranking.ts`). CodeMirror adds `boost` to its fuzzy score (a prefix match scores 0, a partial one -100, a non-initial one -700), so boosts within ±40 reorder equally good matches without outranking a better one. Three signals, in the spirit of VS Code's suggest widget: the source's tier (declared in the document +6, the language pack's locals +5, builtins 0, keywords -1, snippets -2, document words -4), locality (+3 for a name that already appears in the document, cached per document version), and recency (+4 decaying to +1 for the last accepted completions, recorded through CodeMirror's `pickedCompletion` annotation). Runtimes add what they know: jedi marks names from the reader's own namespace (+4), common builtins (+2) and exception classes (-2); the TypeScript service's own `sortText` tiers map to +6 (local declarations) … +1 (globals and keywords), deprecated -3; clang's keywords sit below symbols. Verified in the browser: `pri` in a C block that already calls `printf` lists `printf` first with its signature.
- **Popup shape** (`completion/popupTheme.ts`, shared with the SQL editors): twelve rows, 16–34em wide, label left and the signature or type as a quiet right-aligned detail that truncates with an ellipsis instead of widening the popup; the docstring panel is capped at 28em.
- **Settings**: the Code Suggestions row uses a lightbulb (Sparkles is Ask AI's mark), and the select's chevron is one lucide icon laid over the native control rather than two CSS gradients that rendered as separate triangles.
- A first-query race is closed: the document-word source now waits for the static list before filtering, so `printf` no longer appears twice on the first popup of a page.

### 9.10 Follow-up: the SQL editors

**Settings.** The three SQL playgrounds render the Code Suggestions row already: their `SqlSettingsPanelContent` wraps the same `SettingsPanelContent`. Verified in the browser for SQLite, PostgreSQL and DuckDB. The SQL editors and SQL lesson blocks also follow the mode live and share the popup theme; section headers (Columns, Tables, Keywords), which only SQL uses, now read as small quiet captions rather than a bordered row.

**Review of `sql/sqlCompletion.ts`** (2,450 lines, 89 tests before this pass). What was already strong: clause-aware modes (tables after FROM/JOIN, columns after WHERE/ON/SET, qualified `alias.` columns, INSERT and REFERENCES column lists, CREATE TABLE type and constraint slots, index and ALTER targets), a per-slot keyword tier system with dialect catalogs, CTE extraction, FK-derived ready-made join conditions and related-table ranking, `USING` shared columns, schema-qualified names, keyword casing that follows what is typed, and multi-statement scoping around the cursor. What was missing, and is added:

- **Function signatures and descriptions** for the ~110 catalog functions (`(expr | *)` for `COUNT`, `(field, source)` for `DATE_TRUNC`, …) as detail and info, matching the other languages' popups; `CURRENT_DATE`/`CURRENT_TIME`/`CURRENT_TIMESTAMP` insert without parentheses, which the snippet form got wrong in PostgreSQL.
- **A cast slot**: `expr::|` offers the dialect's types in PostgreSQL and DuckDB (and nothing in SQLite, which has no `::`).
- **SELECT-list awareness** in ORDER BY, GROUP BY, HAVING and QUALIFY: columns the SELECT list uses are bumped ahead of the rest of the table, and the list's output aliases (`SUM(total) AS spent`, `COUNT(o.id) order_count`) are offered where the dialect accepts them (ORDER BY everywhere; GROUP BY and HAVING in SQLite and DuckDB, not PostgreSQL), ranked first since the aggregate just computed is the likeliest sort key.
- **Locality and recency**, case-insensitively: a name already used in the document and a completion accepted a moment ago edge ahead of their section mates, capped below the join-condition section so a well-used column never pushes `c.id = o.customer_id` off the top of the ON slot.

The slot boosts (80–100) stay the primary signal; the new bumps are small (3–8) so they reorder within a section and never move one section past another. Twenty new tests cover all of the above; the suite is 109 tests. Verified in the browser on a DuckDB lesson block: `SELECT COU` shows `COUNT (expr | *)`, and `… GROUP BY category ORDER BY ` lists the alias `n` first under Columns.
