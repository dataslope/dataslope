...
| Topic-page format | Every topic page is a **question bank** with three parts: a `## Q&A` section (each question is an `###` heading with a written answer + code blocks where relevant), `## Concept checks` (MCQs), and `## Challenges` (runnable cards). Topics include SQL, pandas, statistics & communication, data modeling, pipelines, ML, experimentation, dimensional modeling, dbt, DSA, and Python fundamentals. (Duplicating course content here is intentional.) |
| Interactive challenges (all solutions validated) | **SQL** (`SqlChallengeCard`, DuckDB): aggregation, joins (LEFT JOIN + COALESCE), window-function ranking, deduplication (`ROW_NUMBER`), fact×dimension mart. **Python** (`ChallengeCard` w/ tests + reference `solutionCode`): Two Sum, Valid Anagram, Valid Parentheses, precision/recall, accuracy. **pandas**: `groupby`. **18 MCQ files**. |
