...
| **Q2** | Interview Prep section | **Strong yes — and ✅ built out on this branch** at `/interview`: a second `defineDocs` collection (one navigable tree, linked from the home header) with six role tracks and topic-grouped question banks — **~469 written Q&A (30+/page) + MCQs + runnable SQL/Python challenges**. See §2.4. Your single best **SEO** lever and a natural paid-AI funnel. |
...
| Content | `content/interview/`: a landing page + **six role tracks** — Data Analyst, Data Scientist, Data Engineer, Analytics Engineer, ML Engineer, Backend/SWE. Each role has an overview + **2–3 topic pages** (22 routes total). |
| Topic-page format | Every topic page is a **question bank** with three parts: a `## Q&A` section (each question is an `###` heading with a written answer + code blocks where relevant), `## Concept checks` (MCQs), and `## Challenges` (runnable cards). |
| Q&A depth | Each topic page carries **30–35 written Q&A** — **~469 across the 15 topic pages**. Authored in parallel by six role-scoped subagents, then validated. (Duplicating course content here is intentional.) |
| Navigation | `/interview` is **one Fumadocs tree** (role folders are not `root`s), so the sidebar exposes every role from any page — verified that a topic page links to all five other roles. A **"Interview Prep"** link sits in the home header (desktop menu + mobile drawer). |
| Interactive challenges (all solutions validated) | **SQL** (`SqlChallengeCard`, DuckDB): aggregation, joins, window ranking, deduplication, fact×dimension mart. **Python** (`ChallengeCard` w/ tests + reference `solutionCode`): Two Sum, Valid Anagram, Valid Parentheses, precision/recall, accuracy. **pandas**: `groupby`. **18 MCQ files**. |

**Verification:** `validate-mdx` ✅ (all 22 compile), MCQ linter ✅ (18 files, neutral explanations per `AGENTS.md`), `tsc --noEmit` ✅, `eslint` ✅, production `next build` ✅ (22 routes prerender; the one-tree sidebar resolves cross-role links). Interactive challenge solutions were executed locally before embedding (Python/pandas via `python3`, SQL via `duckdb`). The ~469 written Q&A are static Markdown — lower risk than the runnable cards — and were spot-checked; treat them as a strong first pass to refine editorially over time.

**Possible follow-ups:** difficulty/role tags + filtering; harder/multi-file challenges; extend the solution e2e to sweep `/interview`; `FAQPage` JSON-LD on these Q&A pages; an "Ask AI to explain this answer" hook once Q4 lands.
