...
A scheduled GitHub Action (`.github/workflows/r2-cache-cleanup.yml`) prunes it every 6 hours. The live production build is always kept — a stable site can go weeks without a deploy, and deleting its cache would 500 the home page and `/courses/*` lessons. Every other build folder is deleted once it is **either** older than 12 hours **or** outside the 6 newest, so superseded production builds and PR previews age out within hours of their last deploy and the bucket holds at most 7 builds (a few GB) instead of tens.
