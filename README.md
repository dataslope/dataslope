...
A scheduled GitHub Action (`.github/workflows/r2-cache-cleanup.yml`) prunes it every 6 hours. The live production build is always kept — a stable site can go weeks without a deploy, and deleting its cache would 500 the home page and `/courses/*` lessons. Every other build folder is deleted once it is **either** older than 24 hours **or** outside the 10 newest, so superseded production builds and PR previews age out within a day of their last deploy and the bucket holds at most 11 builds (~14 GB worst case, typically well under that) instead of growing unbounded. Note the flip side: a PR preview URL stops rendering once its cache folder is pruned — push any commit to the PR (or re-run its build) to regenerate it.
