...
The cost that decides the design. A run that reprocesses everything grows
with the table; one that reprocesses a partition does not, which is what
makes a backfill a loop over date ranges rather than one enormous job.
