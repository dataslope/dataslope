...
A related number people underestimate: a request that fans out to many
backends is as slow as its slowest call, so a one-percent tail per call
becomes a coin flip for the page.
