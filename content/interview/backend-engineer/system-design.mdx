...
The set worth memorising is really one picture: what each level of the
memory and network hierarchy costs, and how far apart they are.

<Chart slug="memory-latency-ladder" />

One more habit worth having: quote latency as a percentile, never as a
mean. The distribution is skewed enough that the average describes almost
nobody's experience.

<Chart slug="latency-percentiles" />
...
The reason headroom is the thing you are buying: queueing delay is
proportional to one over the slack, so the last few points of
utilisation cost more than all the ones before them.

<Chart slug="queue-utilization-latency" />
...
What the quip is really about is that the TTL controls both failures at
once and in opposite directions, so there is no setting at which both are
small:

<Chart slug="cache-ttl-staleness" />

Lengthening the TTL saves database trips and gives writes longer to overtake
the copy, so the two failures trade directly against each other and the best a
single number can do is the crossing point. Write-through pays at write time
to move the stale curve down. The rates here are illustrative rather than
measured.
...
Removing the single point of failure is only half of it. The other half
