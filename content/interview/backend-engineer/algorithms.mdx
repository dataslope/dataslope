...
in O(n) instead of O(n²). The same idea works for removing duplicates,
...
from scratch. It reduces many O(n²) substring problems to O(n). For
...
O(n²) when the pivot is always the smallest or largest element (e.g.,
