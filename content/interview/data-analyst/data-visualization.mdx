...
accurately, then length, then angle, then area, and colour last.
...
A related axis question with the same shape. Two series of very
different sizes on a linear axis, and then on a log one, where the
smaller is visibly the faster-growing:
...
And the choice a histogram carries that a bar chart does not: the bin
width is yours, and it changes the answer.
...
The same chart in a default palette and a colourblind-safe one, each
