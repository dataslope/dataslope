...
Five values with depth added, so each bar meets the axis twice.

<Chart slug="chartjunk-3d-bars" />

And the same five values as circles, sized correctly by area and then by
radius, which squares every ratio in the chart.
