...
<Figure
  slug="interview-machine-learning-engineer-mlops-cutout"
  alt="A model block travelling along a platform through a registry shelf, then a gate check, and into a deployment container, with a rollback rail curving back to the start"
/>
...
Batch size pulls in two directions at once, which is what makes the
choice a real one: throughput rises with it, and so does the latency any
individual request sees.

<Chart slug="batch-size-throughput" />
...
Packaging is also where serving cost is decided, and quantization is the
