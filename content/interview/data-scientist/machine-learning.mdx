...
and both ways out of it fail differently.
...
What averaging can and cannot buy: variance falls with the number of
models, onto a floor set by how correlated their errors are. Bagging
works on the variance; correlated models are why it stops working.
...
Why the model works in log-odds and reports probability: the two carry
the same information, and only one of them is unbounded enough to be
...
And where the splitting has to stop. Training accuracy climbs to a
hundred per cent and stays there, which is exactly why it cannot be the
signal you tune depth against.
...
What k does to the decision boundary: jagged enough to encircle single
points at k = 1, nearly a straight line at k = 75.
...
How the k gets picked in practice, and how much judgement is left in it:
the within-cluster sum of squares always falls, so the elbow is a bend
...
Calibration as a picture: predicted probability against observed
frequency, with the diagonal marking a model whose 0.7 means 0.7.
...
The two shapes worth recognising. A gap still open at the right-hand edge
is variance, and more data will help; two curves that met early and
flattened high is bias, and more data will not.
