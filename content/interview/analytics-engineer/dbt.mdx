...
<Figure
  slug="interview-analytics-engineer-dbt-cutout"
  alt="A chain of small model blocks on a platform connected by arrows, each block stamped with a round check disc, one block being rebuilt in place on a turntable"
/>
...
The graph is what turns "what does this change break?" into a selector.
How much it selects depends entirely on which layer the change lands in,
which is the real argument for the staging/intermediate/mart split:

<Chart slug="dbt-blast-radius" />
