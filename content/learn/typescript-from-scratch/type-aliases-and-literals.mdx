...
<CodeBlock
  adapter="typescript"
  initialCode={`// Interface with extension
interface Animal {
  name: string;
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`type PrimaryColor = "red" | "green" | "blue";
...
<CodeBlock
  adapter="typescript"
  initialCode={`const message = "Hello";  // inferred as "Hello" (literal)
...
<CodeBlock
  adapter="typescript"
  initialCode={`const STATUS_CODES = {
  OK: 200,
  NOT_FOUND: 404,
  SERVER_ERROR: 500,
} as const;
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Success = { status: "success"; data: string };
...
<ChallengeCard
  ...
  initialCode={`type TrafficLight = "red" | "yellow" | "green";
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Greeting = "hello" | "hi";
