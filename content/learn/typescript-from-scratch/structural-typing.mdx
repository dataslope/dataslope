...
<CodeBlock
  adapter="typescript"
  initialCode={`// Two unrelated interfaces with the same shape
...
<CodeBlock
  adapter="typescript"
  initialCode={`interface Loggable {
  message: string;
}
...
const event = { message: "User logged in", timestamp: Date.now(), userId: "123" };
log(event);
`}
/>
...
2. The test will pass various objects with extra properties — all should work as long as they have \`level\` and \`message\`.`}
  initialCode={`interface LogEntry {
  level: string;
  message: string;
}
