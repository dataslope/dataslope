...
<CodeBlock
  adapter="typescript"
  initialCode={`const message = "Hello, TypeScript!";
const count = 42;
const isActive = true;
...
<CodeBlock
  adapter="typescript"
  initialCode={`const literal = "hello";
...
<CodeBlock
  adapter="typescript"
  initialCode={`function add(a: number, b: number) {
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Explicit return type for a public API
...
<CodeBlock
  adapter="typescript"
  initialCode={`// All numbers → inferred as number[]
const nums = [1, 2, 3];
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Without annotation: inferred as (string | number)[]
const loose = [42];
...
<CodeBlock
  adapter="typescript"
  initialCode={`function getConfig() {
  return {
    host: "localhost",
    port: 3000,
...
<CodeBlock
  adapter="typescript"
  initialCode={`// ❌ Bad: No initializer, no annotation → inferred as any
...
3. The test will verify that the sum is computed correctly.`}
  initialCode={`function process(nums: number[]): number {
