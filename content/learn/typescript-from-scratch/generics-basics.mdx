...
<CodeBlock
  adapter="typescript"
  initialCode={`// Without generics: lose type information
...
<CodeBlock
  adapter="typescript"
  initialCode={`function wrap<T>(value: T): T[] {
  return [value];
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`function pair<A, B>(first: A, second: B): [A, B] {
  return [first, second];
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`function createArray<T>(): T[] {
  return [];
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`function map<T, U>(array: T[], fn: (item: T) => U): U[] {
  const result: U[] = [];
...
<CodeBlock
  adapter="typescript"
  initialCode={`function firstElement<T>(array: T[]): T | undefined {
  return array[0];
}
...
The tests will verify that \`map\` transforms the value and returns a new container with the correct type.`}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { Container } from "./container";
...
`,
    },
    {
      ...
      initialContent: `export class Container<T> {
...
`,
      solutionContent: `export class Container<T> {
  private value: T;
