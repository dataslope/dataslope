...
<CodeBlock
  adapter="typescript"
  initialCode={`// Define a constraint: T must have a 'length' property
...
<CodeBlock
  adapter="typescript"
  initialCode={`function getProperty<T, K extends keyof T>(obj: T, key: K): T[K] {
  return obj[key];
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Default type parameter: T defaults to string if not provided
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Without 'extends readonly': input widens to string[]
...
<CodeBlock
  adapter="typescript"
  initialCode={`function pluck<T, K extends keyof T>(array: T[], key: K): T[K][] {
...
<CodeBlock
  adapter="typescript"
  initialCode={`function pick<T, K extends keyof T>(obj: T, keys: K[]): Pick<T, K> {
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Covariant example: readonly containers
type ReadonlyBox<T> = {
  readonly value: T;
};
...
function registerHandler(handler: Handler<{ species: string; breed: string }>): void {
...
**Hint:** Use \`String(item[key])\` to convert the key value to a string for use as a record key.`}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { groupBy } from "./groupBy";
...
`,
      solutionContent: `import { groupBy } from "./groupBy";
