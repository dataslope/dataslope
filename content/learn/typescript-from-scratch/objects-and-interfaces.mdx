...
<CodeBlock
  adapter="typescript"
  initialCode={`const point: { x: number; y: number } = { x: 10, y: 20 };
...
<CodeBlock
  adapter="typescript"
  initialCode={`interface Rectangle {
  width: number;
  height: number;
  area(): number;
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`interface StringMap {
  [key: string]: string;
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`interface MixedMap {
