...
<CodeBlock
  adapter="typescript"
  initialCode={`const count: number = 42;
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Explicit annotation clarifies intent
let userId: number;
userId = 12345;
...
<CodeBlock
  adapter="typescript"
  initialCode={`// The type is literally the string "north"
...
<CodeBlock
  adapter="typescript"
  initialCode={`const big: bigint = 123456789012345678901234567890n;
...
<CodeBlock
  adapter="typescript"
  initialCode={`const sym1: symbol = Symbol("id");
...
<CodeBlock
  adapter="typescript"
  initialCode={`function area(width: number, height: number): number {
  return width * height;
}
...
<ChallengeCard
  ...
  initialCode={`function celsiusToFahrenheit(c: number): number {
  // Your code here
  return 0;
}
