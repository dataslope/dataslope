...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { Order, summarize } from "./order";
...
console.log(processPayment({ type: "credit-card", cardNumber: "4111111111111111", cvv: "123" }, 99.99));
console.log(processPayment({ type: "bank-account", accountNumber: "123456", routingNumber: "987654" }, 50.00));
console.log(processPayment({ type: "paypal", email: "alice@example.com" }, 25.00));
`}
/>
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Priority = "low" | "medium" | "high" | "urgent";
...
4. The tests will verify all three states render correctly.`}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { Post } from "./post";
...
`,
      solutionContent: `import { Post } from "./post";
...
`,
    ...
    {
      filename: "post.ts",
      initialContent: `// TODO: Define PostState as a discriminated union
...
`,
      solutionContent: `type PostState =
  | { status: "draft" }
