...
function logEvent(event: AppEvent): void {
  switch (event.type) {
    case "user-logged-in":
      console.log(\`[\${new Date(event.timestamp).toISOString()}] User \${event.userId} logged in\`);
      break;
    case "user-logged-out":
      console.log(\`User \${event.userId} logged out\`);
      break;
    case "page-viewed":
      console.log(\`Page viewed: \${event.path} (referrer: \${event.referrer || "none"})\`);
      break;
    case "error-occurred":
...
logEvent({ type: "error-occurred", message: "DB connection lost", stack: "at ..." });
`}
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { RemoteData, isLoading, isSuccess, getData } from "./remoteData";
...
`,
    },
    {
      ...
      initialContent: `// A discriminated union for async data
...
3. The test will check that all three variants render correctly.`}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { Notification } from "./notification";
...
`,
      solutionContent: `import { Notification } from "./notification";
...
`,
    },
    {
      ...
      initialContent: `// TODO: Define a discriminated union Notification with:
...
`,
      solutionContent: `export type Notification =
  | { type: "info"; message: string }
  | { type: "warning"; message: string; severity: number }
  | { type: "error"; message: string; stack: string };
`,
