...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { parseConfig } from "./parser";
...
`,
    ...
    {
      filename: "parser.ts",
      initialContent: `import { Result } from "./result";
...
**Hint**: If \`result.ok\`, apply \`fn\` to \`result.value\` and return a new success \`Result\`. Otherwise, return the error unchanged.`}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { Result, map } from "./result";
