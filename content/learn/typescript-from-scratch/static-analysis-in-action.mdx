...
<CodeBlock
  adapter="typescript"
  initialCode={`function getLength(s: string | null): number {
...
<CodeBlock
  adapter="typescript"
  initialCode={`type User = { id: number; fullName: string };
...
<CodeBlock
  adapter="typescript"
  initialCode={`function processValue(value: number): string {
  if (value > 0) {
    return "positive";
  } else if (value < 0) {
    return "negative";
