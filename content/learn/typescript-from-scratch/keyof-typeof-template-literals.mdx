...
<CodeBlock
  adapter="typescript"
  initialCode={`type Event = "click" | "focus" | "blur";
...
<CodeBlock
  adapter="typescript"
  initialCode={`type LoudGreeting = Uppercase<"hello">;
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Event = "click" | "focus";
...
`}
  initialCode={`type EventHandlerMap<T> = any;
