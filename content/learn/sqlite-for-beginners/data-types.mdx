...
INSERT INTO examples (label, value) VALUES
  ...
  ('word text', 'banana'),
  ('missing value', NULL);`}
  starterCode={`SELECT label, value, typeof(value) AS storage_class
FROM examples
ORDER BY id;`}
/>
...
INSERT INTO type_demo (as_integer, as_text, as_real, as_numeric, is_active, created_on) VALUES
  ...
  starterCode={`SELECT
  id,
  as_integer, typeof(as_integer) AS integer_type,
  as_text, typeof(as_text) AS text_type,
  as_real, typeof(as_real) AS real_type,
  as_numeric, typeof(as_numeric) AS numeric_type,
  is_active,
  created_on
FROM type_demo
ORDER BY id;`}
