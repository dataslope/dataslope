...
INSERT INTO tickets (priority) VALUES
  ...
  ('Low'),
  ('Low');`}
  initialCode={`SELECT priority, COUNT(*) AS ticket_count
FROM tickets
GROUP BY priority
...
INSERT INTO sales (product, amount) VALUES
  ...
  ('Backpack', 45.00),
  ('Pencil', 2.50);`}
  initialCode={`SELECT product, SUM(amount) AS total_sales
FROM sales
GROUP BY product
...
INSERT INTO sales (region, year, amount) VALUES
  ...
  ('East', 2026, 120.00),
  ('East', 2026, 60.00);`}
  initialCode={`SELECT region, year, SUM(amount) AS total_sales
FROM sales
...
INSERT INTO employees (name, dept, salary, is_intern) VALUES
  ...
  ('Dave', 'Sales', 80000, 0),
  ('Eve', 'Sales', 72000, 0);`}
  initialCode={`SELECT dept, ROUND(AVG(salary), 0) AS average_salary
FROM employees
WHERE is_intern = 0
GROUP BY dept
ORDER BY dept;`}
...
INSERT INTO pets (name, species) VALUES
  ...
  ('Pip', 'Bird'),
  ('Rex', 'Dog');`}
  initialCode={`SELECT species, COUNT(*) AS pet_count
FROM pets
GROUP BY species
ORDER BY species;`}
/>
