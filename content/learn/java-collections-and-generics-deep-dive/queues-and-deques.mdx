...
public class Main {
    public static void main(String[] args) {
        ...
        q.offer(1); seen.add(1);
        while (!q.isEmpty()) {
            int node = q.poll();
            order.add(node);
            for (int nb : g.get(node)) {
                if (seen.add(nb)) q.offer(nb);   // add returns true on first sight
            }
        }
...
<CodeBlock
  adapter="java"
  initialCode={`import java.util.Comparator;
...
public class Main {
    public static void main(String[] args) {
        ...
        rev.offer(5); rev.offer(1); rev.offer(9); rev.offer(3);
        while (!rev.isEmpty()) System.out.println(rev.poll());
    }
}
`}
/>
...
`,
    ...
    {
      filename: "Task.java",
      initialContent: `public record Task(String name, int deadline) {}
`,
    },
    {
