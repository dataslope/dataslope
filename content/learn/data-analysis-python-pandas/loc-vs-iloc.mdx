...
- [o] \`df.loc["a":"e"]\`
  > Yes. Label-based slicing in Pandas is inclusive of both ends — different from \`iloc\` and from regular Python slicing.
- \`range(0, 5)\`
...
- [o] The row whose **label** is 10 — i.e., the first row
  > Right. When the index contains integers, the distinction between label and position becomes very visible: \`df.iloc[10]\` would be out of bounds here.
