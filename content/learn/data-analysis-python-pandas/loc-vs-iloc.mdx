...
df = pd.DataFrame({
  ...
  initialCode={`print(">>> df.loc['Aiko':'Chen']     # 3 rows — INCLUSIVE of Chen")
...
df = pd.DataFrame({
  ...
  initialCode={`# loc: labels for rows AND columns
...
df = pd.DataFrame({
  ...
  initialCode={`# Give every Engineer a 10% raise. Use loc — never chained []:
