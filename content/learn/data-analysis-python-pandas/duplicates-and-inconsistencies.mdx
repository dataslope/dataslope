...
df = pd.DataFrame({
  ...
  initialCode={`# Drop full-row duplicates
...
- [o] Decide based on the business rules (keep first? keep latest? aggregate?) and document the choice; do not blindly drop or sum
  > Right. Duplicate resolution is a business judgement. Whatever you pick, make it visible in your code so future readers know.
- Ignore them
...
- [o] Strip whitespace → normalize case → replace known variants with a canonical form
  > Yes. That three-step recipe covers the great majority of category-label cleanup.
- Convert to numeric
