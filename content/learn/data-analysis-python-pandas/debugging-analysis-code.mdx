...
df = pd.read_csv("https://raw.githubusercontent.com/bdi475/datasets/main/HR-dataset-v14.csv")`}
  initialCode={`print("Shape:", df.shape)
print()
df.info()
`}
/>
...
- [o] The column is a string, and \`.sum()\` concatenated the values instead of adding them — check \`df.dtypes\`
  > Right. String-vs-number is the most common silent bug in Pandas.
...
- [o] Check whether the join key is duplicated on the right-hand table (\`df.duplicated(subset=...).sum()\`), and/or re-run the merge with \`validate="one_to_many"\` or similar
  > Right. Cardinality blow-ups have a clear root cause that the \`validate\` flag would have caught.
...
- [o] Bisect — check the intermediate output halfway through; whichever half is wrong, bisect again
  > Right. Logarithmic search through your pipeline beats linear search.
- Add more comments
...
- [o] Both of those, AND a mini-example forces you to truly understand the bug — and is the perfect format to ask someone for help
  > Right. "Can you reproduce it minimally?" is a question that often answers itself in the act of trying.
- It uses less memory
