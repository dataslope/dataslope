...
df = pd.DataFrame({
  ...
  initialCode={`new_df = (
    ...
    .assign(bonus_pct=lambda x: (x["bonus"] / x["salary"] * 100).round(1))
)
print(new_df)
print()
...
df["SatLabel"] = df["JobSatisfaction"].map({
    1: "low", 2: "med-low", 3: "med-high", 4: "high",
})
...
print(df[["Age","YearsAtCompany","JobSatisfaction","TenureBand","SatLabel","AtRisk"]].head(10))
print()
...
- [o] Direct assignment modifies \`df\` in place; \`.assign\` returns a *new* DataFrame and leaves the original untouched
  > Right. \`.assign\` is the immutable alternative and chains nicely in pipelines.
...
- [o] When you need to assign one of *more than two* values depending on multiple conditions
  > Yes. \`np.where\` is binary (if/else). \`np.select\` extends to many branches without nesting.
