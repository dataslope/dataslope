...
`melt` widens-to-long. `pivot` (or `pivot_table`) does the
reverse.
...
- [o] The years are sitting in column *names*, not in a column of *values* — to do anything year-aware you must first lift them out
  > Right. That column-names-are-values smell is the classic signal that you should melt.
...
- merge
- [o] melt
  > Yes. Think "melt the wide table down into a tall one".
- concat
