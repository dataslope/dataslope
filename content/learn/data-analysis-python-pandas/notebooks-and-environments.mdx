...
- [o] It records the package versions a project depends on, so a collaborator can install the same versions and reproduce the environment
  > Yes. It is the simplest form of dependency pinning. Anyone with the repo can run \`pip install -r requirements.txt\` and get a matching environment.
...
- [o] The long-running Python process that holds your variables in memory and executes the cells you run
  > Right. Every cell run mutates the kernel's state. Many notebook bugs come from running cells out of order or forgetting to re-run a dependent cell.
...
- [o] Code in git + data with a stable reference + a pinned environment (\`requirements.txt\` or lockfile)
  > Exactly. Code, data, and environment together let someone else reproduce your work months or years later.
