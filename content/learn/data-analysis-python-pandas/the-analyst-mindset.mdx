...
- [o] All the low scores came in through one channel; aggregating across channels hid the variation that mattered
  > Yes — this is a small Simpson's-paradox-flavored example. Always look at meaningful sub-groups before trusting an aggregate.
...
- [o] A pattern that holds in every sub-group can reverse direction when the sub-groups are combined
  > Right. The Berkeley admissions case is the classic example. Whenever you compute an aggregate, ask whether a known sub-grouping would change the conclusion.
...
- [o] Treating every analysis as if it will be audited — i.e., making it reproducible, traceable, and explainable
  > Exactly. When (not if) someone asks "how did you arrive at this number?", being able to re-run a script and walk through it is what protects you and your team.
...
- [o] Suspect a data or code bug, and investigate before trusting it
  > Yes. The base rate of "shocking findings" being bugs is very high. Build the reflex now.
- Ignore it
