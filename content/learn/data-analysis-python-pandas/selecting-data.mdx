...
df = pd.DataFrame({
  ...
  initialCode={`sub = df[["salary", "tenure_yrs"]]
print(sub)
...
df = pd.DataFrame({
  ...
  initialCode={`mask = df["salary"] > 100
print("Mask:")
print(mask)
print()
...
- [o] \`df["age"]\` (returns a Series)
  > Right. Single bracket → single column → Series.
...
- [o] \`loc\` slicing is *inclusive* of both endpoints, while \`iloc\` (like normal Python) is *exclusive* of the end
  > Yes. This often-surprising difference is one of the most common bugs in label-based slicing.
