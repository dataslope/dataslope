...
Chen,35,Shanghai,2023-11-20
"""`}
  initialCode={`df = pd.read_csv(io.StringIO(csv_text), parse_dates=["joined"])
print(df.dtypes)
print()
print(df)
`}
...
Carol,02134
"""`}
  initialCode={`df = pd.read_csv(io.StringIO(csv), dtype={"zip": "string"})
print(df)
print()
print("zip dtype:", df["zip"].dtype)
...
- [o] Pandas inferred the column as \`int64\` (because all values look like integers) and integers cannot have leading zeros — the \`0\` is dropped silently
  > Yes. This is the canonical example of type inference biting you. Fix it by passing \`dtype={"zip": "string"}\` to \`pd.read_csv\`.
...
- [o] All values in a column share a type, which allows tight packing and fast operations; analytical queries also tend to scan a few columns of many rows
  > Right. Column-store layouts are the standard for analytical systems (Pandas, Parquet, DuckDB, ClickHouse, BigQuery). Row-stores are better for transactional databases that touch one row at a time.
