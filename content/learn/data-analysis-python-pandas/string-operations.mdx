...
df = pd.DataFrame({
    "title": ["Senior Data Engineer", "Marketing Lead", "Data Scientist", "Sales Rep"],
})
...
parsed = logs.str.extract(r"(?P<date>\d{4}-\d{2}-\d{2}) (?P<time>\d{2}:\d{2}:\d{2}) (?P<level>\w+)\s+(?P<msg>.*)")
print(parsed)
`}
/>
...
- [o] It provides vectorized string operations on whole columns of text — clean syntax and far faster than writing a Python for-loop over each value
  > Right. \`.str.upper()\` on a million-row column is one call, not a million.
...
- [o] Propagate the NaN — the result has NaN at that position by default
  > Yes. Use \`na=False\` (or \`na=True\`) to coerce the NaN to a definite boolean — required if you want to use the result as a filter mask.
- Throw an error
