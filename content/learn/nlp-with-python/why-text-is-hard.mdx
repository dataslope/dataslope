...
- [o] Syntactic ambiguity (the grammatical structure has two valid parses).
  > Correct — the phrase "with the telescope" could attach to "saw" or to "the man".
...
- [o] A search for one form would fail to match documents using another form, reducing recall.
  > Right — "running shoes" should match a page about "shoes for runners"; treating the forms as unrelated breaks that.
...
- [o] Context — the meaning of \`good\` depends on the surrounding word \`not\`, which was discarded.
  > Exactly — \`not\` flips the sentiment, so dropping it inverts the meaning.
