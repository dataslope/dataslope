...
- **size="..."**
- [o] **x="..."**
  > Yes. **x="column_name"** tells Plotly Express to use that column for the horizontal axis. Combined with **y**, these two arguments do most of the work in any scatter, line, or bar chart.
- **facet_col="..."**
...
- [o] x = square footage, y = price, color = neighborhood, size = year built (or similar)
  > Yes. Position (x and y) is the most accurate channel — putting the two quantitative variables you most want to compare (**price** and **square footage**) on x and y gives the reader the clearest possible read of their relationship. Color carries the categorical neighborhood; size carries the secondary continuous variable.
...
- [o] A small number (~3-7) of distinct, unordered categories such as continents or product lines.
  > Right. Hue is excellent at saying "these dots belong together" for a handful of categories, and the eye distinguishes the colors instantly. For continuous or ordered data, prefer a sequential or diverging *intensity* scale instead of a categorical hue palette.
...
- [o] The chart will work, but you've spent two visual channels on a single variable — a missed opportunity that often adds clutter without adding information.
  > Yes. Redundant encoding is sometimes deliberate (e.g., for colorblind accessibility), but it's frequently a wasted channel. Each facet panel will be a single color anyway, so the color encoding adds nothing new — and you've used up a channel you could have given to a different variable.
