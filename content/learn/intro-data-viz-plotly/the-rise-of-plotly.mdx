...
- [o] Plotly Express is a high-level Python API on top of plotly.py, which is a Python wrapper around the Plotly.js JavaScript library.
  > Yes. When you call **px.bar(...)** in Python, Plotly Express builds a configuration object, plotly.py serializes it, and Plotly.js renders the interactive chart in the browser. You write Python, the browser executes JavaScript — invisible to you.
...
- [o] plotly.graph_objects — the lower-level Plotly API for full chart customization.
  > Yes. Plotly Express handles 90% of the charts you'll ever want to make, but when you need precise control — custom subplot layouts, multi-axis charts, animated transitions — you drop down to **plotly.graph_objects**. The two APIs work together seamlessly.
- JavaScript
