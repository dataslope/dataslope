...
- [o] Decide what *question* the chart will answer.
  > Yes. Without a clear question, every later choice becomes arbitrary. The question shapes the data filtering, the chart type, and the encoding decisions. "Pretty" charts that don't answer a question are decoration, not analysis.
- Pick a template.
...
- [o] Scatter plot (**px.scatter**).
  > Right. Scatter plots use the two most accurate encodings (x and y position) for the two quantitative variables. They reveal correlation, clusters, and outliers at a glance.
- Heatmap.
...
- [o] It overrides the raw column names with human-readable labels (e.g., "Total bill (USD)" instead of "total_bill") for axes, legend, and hover.
  > Yes. Real-world column names are often abbreviated or snake_cased. **labels={}** lets you keep the code referencing the column name but display readable strings to the reader. A small habit that hugely improves chart clarity.
- It hides the legend.
