...
model = LinearRegression().fit(X_train, y_train)
y_pred = model.predict(X_test)`}
  initialCode={`from sklearn.metrics import mean_absolute_error

mae = mean_absolute_error(y_test, y_pred)
print(f"MAE: {mae:.1f}")
print("Interpretation: on average the prediction is off by about",
...
model = LinearRegression().fit(X_train, y_train)
y_pred = model.predict(X_test)`}
  initialCode={`from sklearn.metrics import mean_absolute_error, mean_squared_error, root_mean_squared_error
