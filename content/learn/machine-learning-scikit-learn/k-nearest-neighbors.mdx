...
<svg
  viewBox="0 0 640 220"
  role="img"
  aria-label="A yellow query dot sits inside its translucent neighborhood circle where three green neighbors outvote two blue ones, while dots outside the circle take no part — k-nearest neighbors decides by local vote"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g style={{ color: "var(--ds-yellow-500)" }} fill="currentColor">
    <circle cx="320" cy="110" r="78" fillOpacity="0.12" />
    <circle cx="320" cy="110" r="9" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <circle cx="270" cy="80" r="8" />
    <circle cx="355" cy="60" r="8" />
    <circle cx="370" cy="150" r="8" />
    <circle cx="120" cy="60" r="8" fillOpacity="0.45" />
    <circle cx="185" cy="185" r="8" fillOpacity="0.45" />
    <circle cx="490" cy="45" r="8" fillOpacity="0.45" />
    <circle cx="560" cy="175" r="8" fillOpacity="0.45" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="280" cy="155" r="8" />
    <circle cx="345" cy="115" r="8" />
    <circle cx="85" cy="135" r="8" fillOpacity="0.45" />
    <circle cx="155" cy="35" r="8" fillOpacity="0.45" />
    <circle cx="450" cy="190" r="8" fillOpacity="0.45" />
    <circle cx="575" cy="95" r="8" fillOpacity="0.45" />
    <circle cx="225" cy="40" r="8" fillOpacity="0.45" />
  </g>
</svg>
...
Let us classify iris flowers. The API is, as always, identical to every
...
# Predict each flower's species from the vote of its 5 nearest neighbors.
...
# Wine features span very different ranges (e.g. proline near 1000 vs.
# hue near 1), so this dataset punishes unscaled KNN hard.
...
<ChallengeCard
  ...
  estimatedTime="~7 min"
  instructions={`The wine dataset has features on wildly different scales,
so unscaled KNN does poorly — and scaling fixes it. Show both.

1. Load the data with \`load_wine(return_X_y=True)\` into \`X\` and \`y\`.
2. Split into train/test with **30%** in the test set, \`random_state=0\`,
   and **stratified** on \`y\`.
3. Train a plain \`KNeighborsClassifier(n_neighbors=5)\` on the raw training
   data and store its test accuracy in \`raw_acc\`.
4. Build a pipeline with \`make_pipeline(StandardScaler(),
...
# 1. Split (30% test, random_state=0, stratified on y)
