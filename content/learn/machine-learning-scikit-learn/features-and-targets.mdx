...
houses = pd.DataFrame({
  ...
  initialCode={`# 'houses' is provided above. Take a look at the whole table.
print(houses)
print()
...
houses = pd.DataFrame({
  ...
  initialCode={`# The target: the column we want to predict.
y = houses["price"]
...
df = pd.DataFrame({
  ...
  initialCode={`# The target may sit anywhere -- here 'label' is the SECOND column.
...
patients = pd.DataFrame({
  ...
  initialCode={`# 'patients' is provided above (6 patients, 5 columns).
