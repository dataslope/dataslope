...
plt.scatter(centers[:, 0], centers[:, 1],
            c="red", s=250, marker="*", edgecolor="black", label="centers")
...
<CodeBlock
  adapter="python"
  initialCode={`from sklearn.datasets import make_blobs
from sklearn.cluster import KMeans
...
for n in [1, 10]:
    inertias = [KMeans(n_clusters=4, n_init=n, random_state=s).fit(X).inertia_
                for s in range(5)]
    print(f"n_init={n:2d}  inertia across 5 seeds: "
...
age = np.concatenate([rng.normal(25, 3, 50), rng.normal(45, 3, 50), rng.normal(65, 3, 50)])
income = np.concatenate([rng.normal(30000, 4000, 50), rng.normal(80000, 4000, 50), rng.normal(50000, 4000, 50)])
...
print(f"Avg age spread within clusters, RAW features:    {mean_age_spread(raw_labels):.1f} years")
print(f"Avg age spread within clusters, SCALED features: {mean_age_spread(scaled_labels):.1f} years")
...
X, _ = make_blobs(
  ...
  initialCode={`# 1. Standardize the features
X_scaled = None
