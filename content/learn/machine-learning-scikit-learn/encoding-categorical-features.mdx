...
<svg
  viewBox="0 0 640 210"
  role="img"
  aria-label="Three colored category pills each become a row of cells in which exactly one cell lights up with a one — one-hot encoding gives every category its own honest yes-or-no column"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <rect x="120" y="44" width="64" height="22" rx="11" />
    <rect x="280" y="42" width="68" height="26" rx="7" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <rect x="120" y="99" width="64" height="22" rx="11" />
    <rect x="364" y="97" width="68" height="26" rx="7" />
  </g>
  ...
  <g fill="currentColor" opacity="0.3">
    <circle cx="218" cy="55" r="3" />
    <circle cx="240" cy="55" r="3" />
    <circle cx="218" cy="110" r="3" />
    <circle cx="240" cy="110" r="3" />
    <circle cx="218" cy="165" r="3" />
    <circle cx="240" cy="165" r="3" />
  </g>
  <g fill="currentColor" opacity="0.08">
    <rect x="364" y="42" width="68" height="26" rx="7" />
    <rect x="448" y="42" width="68" height="26" rx="7" />
    <rect x="280" y="97" width="68" height="26" rx="7" />
    <rect x="448" y="97" width="68" height="26" rx="7" />
    <rect x="280" y="152" width="68" height="26" rx="7" />
...
<ChallengeCard
  adapter="python"
  title="One-hot encode a categorical table"
  category="preprocessing · encoding · one-hot"
  estimatedTime="~7 min"
  instructions={`A small DataFrame \`df\` with two categorical columns,
...
2. Fit-transform \`df\` and store the resulting array in \`encoded\`.
...
`,
  ...
  tests={[
    {
      id: "row_count",
      name: "One row per original row",
      description: "Encoding does not add or drop rows",
      code: `assert encoded is not None, "encoded is still None"
assert encoded.shape[0] == len(df), f"Expected {len(df)} rows, got {encoded.shape[0]}"`,
    },
    {
      id: "col_count",
      name: "Columns equal total distinct categories",
      description: "3 fruits + 3 countries = 6 one-hot columns",
      code: `n_expected = df["fruit"].nunique() + df["country"].nunique()
...
assert set(vals.tolist()).issubset({0.0, 1.0}), f"Found non-binary values: {vals}"`,
    },
    {
      ...
      name: "Each row sums to 2",
      description: "Exactly one '1' for fruit and one for country in every row",
      code: `import numpy as np
row_sums = encoded.sum(axis=1)
assert np.all(row_sums == 2), f"Each row should sum to 2 (one per feature), got sums {row_sums.tolist()}"`,
    },
    {
      id: "names",
