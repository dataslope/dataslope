...
<svg
  viewBox="0 0 640 215"
  role="img"
  aria-label="Jumbled blue dots pass through a translucent yellow prism and come out lying along a clean rising line — the same data, re-represented so the pattern becomes visible"
...
>
  ...
  <g fill="currentColor" opacity="0.3">
    <circle cx="195" cy="105" r="3" />
    <circle cx="215" cy="105" r="3" />
  </g>
  <g style={{ color: "var(--ds-yellow-500)" }} fill="currentColor">
    <polygon points="300,40 365,105 300,170 235,105" fillOpacity="0.3" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <polygon points="398,172 588,52 588,58 398,178" fillOpacity="0.35" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="400" cy="168" r="7" />
    <circle cx="438" cy="144" r="7" />
    <circle cx="476" cy="120" r="7" />
    <circle cx="514" cy="96" r="7" />
    <circle cx="552" cy="72" r="7" />
    <circle cx="586" cy="52" r="7" />
  </g>
</svg>
...
<Callout type="info" title="Why this matters more in classical ML">
...
example is an XOR-like pattern, where the label depends on *whether two
...
<ChallengeCard
  adapter="python"
  title="Engineer a ratio feature that exposes the signal"
  category="feature engineering · ratios"
  estimatedTime="~7 min"
  instructions={`A DataFrame \`df\` describes apartments with \`total_sqft\`,
\`num_rooms\`, and \`price\`. The concept that drives price here is **space per
room**, which is not any single raw column.

1. Add a new column \`sqft_per_room\` to \`df\`, equal to \`total_sqft\`
   divided by \`num_rooms\`.
2. Compute the Pearson correlation between \`sqft_per_room\` and \`price\`
   and store it in \`corr_engineered\` (use
   \`df["sqft_per_room"].corr(df["price"])\`).
...
rng = np.random.default_rng(0)
n = 80
num_rooms = rng.integers(1, 6, n)
sqft_per_room_true = rng.uniform(150, 450, n)   # the real driver of price
total_sqft = (num_rooms * sqft_per_room_true).round()
price = (sqft_per_room_true * 300 + rng.normal(0, 2000, n)).round()
...
df = pd.DataFrame({
    ...
    "price":      price,
      ...
      starterCode: `# 1. Engineer the ratio feature: space per room.
df["sqft_per_room"] = None

# 2. Correlation of the engineered feature with price.
corr_engineered = None

# 3. Correlation of raw total_sqft with price (for comparison).
corr_raw = None

print("engineered corr:", corr_engineered)
print("raw total_sqft corr:", corr_raw)
`,
      solutionCode: `df["sqft_per_room"] = df["total_sqft"] / df["num_rooms"]

corr_engineered = df["sqft_per_room"].corr(df["price"])
corr_raw = df["total_sqft"].corr(df["price"])
...
`,
  ...
  tests={[
    {
      id: "column_exists",
      name: "sqft_per_room column was added",
      description: "The engineered feature is a new column in df",
      code: `assert "sqft_per_room" in df.columns, "df is missing the 'sqft_per_room' column"`,
    },
    {
      id: "values_correct",
      name: "sqft_per_room equals total_sqft / num_rooms",
      description: "The ratio is computed correctly for every row",
      code: `import numpy as np
expected = df["total_sqft"] / df["num_rooms"]
assert np.allclose(df["sqft_per_room"].to_numpy(), expected.to_numpy()), "sqft_per_room should equal total_sqft / num_rooms"`,
    },
    {
      id: "corrs_computed",
      name: "Both correlations were computed",
      description: "corr_engineered and corr_raw are numbers",
      code: `assert corr_engineered is not None and corr_raw is not None, "Both correlations must be computed"
assert -1.0 <= corr_engineered <= 1.0, f"corr_engineered out of range: {corr_engineered}"`,
    },
    {
      id: "engineered_better",
      name: "Engineered feature correlates with price more strongly",
      description: "The ratio exposes the signal better than raw total_sqft",
      code: `assert abs(corr_engineered) > abs(corr_raw), f"Expected engineered |corr| ({abs(corr_engineered):.3f}) > raw |corr| ({abs(corr_raw):.3f})"`,
    },
  ]}
/>
...
<MultipleChoice
  markdown={`A plain linear model scores near chance on an XOR-like pattern, but jumps to high accuracy once you add the product of the two features. What does this illustrate?
