...
There is a famous story: at a county fair, hundreds of people guessed the
weight of an ox. No individual guess was exact, but the *average* of all the
guesses was within a pound of the truth — better than any single expert.
Each person's error was partly random; averaging cancelled the random part
and left the collective signal.
...
entirely in *how* you make the trees diverse.
...
decision tree against a random forest of many such trees, on the breast
cancer dataset, and compare their **test** accuracy.
...
print("Single tree   -> train:", round(tree.score(X_train, y_train), 3),
...
The lone regression tree, grown without limit, overfits and posts a weak
test R^2 — sometimes barely above zero. The forest's averaging lifts the
test R^2 substantially. Same data, same kind of base model; the only new
...
for i in order[:5]:
    print(f"  {data.feature_names[i]:<28} {forest.feature_importances_[i]:.3f}")
`}
/>
...
beats one overfit tree on held-out data.
...
2. Split into train/test with **30%** in the test set, \`random_state=0\`,
   and **stratified** on \`y\`.
3. Train a single \`DecisionTreeClassifier(random_state=0)\` (no depth limit)
   on the training set. Store its **test** accuracy in \`tree_acc\`.
4. Train a \`RandomForestClassifier(n_estimators=200, random_state=0)\` on the
