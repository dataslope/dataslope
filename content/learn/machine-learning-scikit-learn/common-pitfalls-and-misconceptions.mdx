...
<svg
  viewBox="0 0 640 210"
  role="img"
  aria-label="A wall separates solid training dots from ghosted test dots, but a dotted tunnel runs underground beneath it and a red dot has surfaced on the training side — leakage smuggles test information into training"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g fill="currentColor" opacity="0.3">
    <rect x="310" y="30" width="20" height="132" rx="8" />
  </g>
  <g fill="currentColor" opacity="0.25">
    <rect x="60" y="160" width="520" height="4" rx="2" />
  </g>
  ...
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="420" cy="75" r="8" fillOpacity="0.25" />
    <circle cx="490" cy="120" r="8" fillOpacity="0.25" />
    <circle cx="550" cy="65" r="8" fillOpacity="0.25" />
    <circle cx="460" cy="145" r="8" fillOpacity="0.25" />
  </g>
  <g style={{ color: "var(--ds-red-500)" }} fill="currentColor">
    <circle cx="380" cy="170" r="3" fillOpacity="0.45" />
    <circle cx="355" cy="181" r="3" fillOpacity="0.45" />
    <circle cx="322" cy="186" r="3" fillOpacity="0.45" />
    <circle cx="289" cy="181" r="3" fillOpacity="0.45" />
    <circle cx="264" cy="170" r="3" fillOpacity="0.45" />
    <circle cx="258" cy="138" r="9" />
  </g>
</svg>
...
people reach for it reflexively. But when 99% of cases are class 0, a model
that *always* predicts class 0 — learning nothing — scores 99% accuracy.
The number looks superb and the model is useless.
...
# 99% class 0, 1% class 1 (the rare, important class).
y = np.array([0] * 990 + [1] * 10)
X = np.zeros((1000, 1))

# A model that does NOTHING but predict the majority class.
model = DummyClassifier(strategy="most_frequent").fit(X, y)
pred = model.predict(X)

print(f"Accuracy: {accuracy_score(y, pred):.3f}   <- looks fantastic!")
print(
    f"Recall on the rare class: {recall_score(y, pred, pos_label=1):.3f}   <- it caught ZERO of them"
)
...
A 99% accuracy and a 0% catch rate on the cases that matter. **The fix:**
...
print("Five unseeded runs, five different scores:", scores)
...
# Train on x in [0, 10], target y = 2x.
rng = np.random.default_rng(0)
X_train = np.linspace(0, 10, 50).reshape(-1, 1)
y_train = 2 * X_train.ravel()

lin = LinearRegression().fit(X_train, y_train)
forest = RandomForestRegressor(n_estimators=100, random_state=0).fit(X_train, y_train)

# Predict FAR outside the training range, at x = 100 (true answer: 200).
far = np.array([[100.0]])
print(f"True value at x=100: 200")
print(
    f"Linear model predicts:        {lin.predict(far)[0]:.1f}  (extrapolates the line)"
)
print(
    f"Random forest predicts:       {forest.predict(far)[0]:.1f}  (flat -- cannot exceed training range)"
)
`,
    },
  ]}
...
The forest cannot predict anything near 200 — it never saw a target that
large and physically cannot output one. The linear model nails *this*
particular case only because the true relationship really is a line, which
you would not know in advance. **The fix:** know your training range, be
...
# Add 200 columns of pure noise to the 13 real features.
...
print(f"CV accuracy with 13 real features:        {score_clean:.3f}")
...
a mere symptom. Every one was preventable by a habit on this page. Learning
to *expect* these traps is a large part of what separates a practitioner who
can be trusted from one who merely gets good-looking numbers.
...
<ChallengeCard
  ...
  estimatedTime="~7 min"
  instructions={`A dataset is wildly imbalanced: 950 of class 0 and 50 of
class 1 (the rare class you care about). A \`DummyClassifier\` that always
predicts the majority class is already fitted for you as \`model\`, and its
predictions are in \`pred\`.

1. Compute the plain accuracy of \`pred\` against \`y\` and store it in
   \`acc\` (use \`accuracy_score\`).
2. Compute the **recall on the rare class** (class 1) and store it in
   \`rare_recall\` (use \`recall_score(y, pred, pos_label=1)\`).
...
# 2. Recall on the rare class (class 1)
rare_recall = None

print("accuracy:", acc)
print("recall on rare class:", rare_recall)
`,
      solutionCode: `# 1. Plain accuracy of the majority-class predictor
acc = accuracy_score(y, pred)

# 2. Recall on the rare class (class 1)
rare_recall = recall_score(y, pred, pos_label=1)
  ...
  tests={[
    {
      ...
      description: "Majority-class predictor scores above 0.9 accuracy",
      code: `assert acc is not None and acc > 0.9, f"Expected accuracy above 0.9, got {acc}"`,
    },
    {
      id: "zero_recall",
...
<MultipleChoice
  markdown={`A random forest is trained on inputs \`x\` in the range 0 to 10. You ask it to predict at \`x = 100\`. What should you expect?
