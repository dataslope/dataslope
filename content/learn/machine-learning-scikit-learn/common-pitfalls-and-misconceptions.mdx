...
leaky_cv = cross_val_score(
    LogisticRegression(max_iter=5000), X_leaked, y, cv=5
).mean()
...
print(f"Training accuracy: {tree.score(X_train, y_train):.3f}  <- meaningless as a performance estimate")
...
print(f"Recall on the rare class: {recall_score(y, pred, pos_label=1):.3f}   <- it caught ZERO of them")
...
print(f"Linear model predicts:        {lin.predict(far)[0]:.1f}  (extrapolates the line)")
print(f"Random forest predicts:       {forest.predict(far)[0]:.1f}  (flat -- cannot exceed training range)")
`}
/>
...
score_clean = cross_val_score(make_pipeline(StandardScaler(), KNeighborsClassifier()), X, y, cv=5).mean()
score_noisy = cross_val_score(make_pipeline(StandardScaler(), KNeighborsClassifier()), X_noisy, y, cv=5).mean()
...
X, y = load_breast_cancer(return_X_y=True)`}
  initialCode={`# LEAKY starter (for reference): scales the whole dataset before CV.
# X_leaked = StandardScaler().fit_transform(X)
...
model = DummyClassifier(strategy="most_frequent").fit(X, y)
pred = model.predict(X)`}
  initialCode={`# 1. Plain accuracy of the majority-class predictor
acc = None
