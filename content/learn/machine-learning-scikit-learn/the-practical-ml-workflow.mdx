...
<svg
  viewBox="0 0 640 220"
  role="img"
  aria-label="Six stations sit on a dotted circular loop with an arrow showing the direction of travel, and one dotted ramp exits to a large green dot — the workflow is an ordered loop you repeat until it is honest to ship"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g fill="currentColor" opacity="0.3">
    <circle cx="317" cy="35" r="2.5" />
    <circle cx="341" cy="49" r="2.5" />
    <circle cx="369" cy="96" r="2.5" />
    <circle cx="369" cy="124" r="2.5" />
    <circle cx="341" cy="171" r="2.5" />
    <circle cx="317" cy="185" r="2.5" />
    <circle cx="263" cy="185" r="2.5" />
    <circle cx="239" cy="171" r="2.5" />
    <circle cx="211" cy="124" r="2.5" />
    <circle cx="211" cy="96" r="2.5" />
    <circle cx="239" cy="49" r="2.5" />
    <circle cx="263" cy="35" r="2.5" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="290" cy="30" r="9" />
    <circle cx="359" cy="150" r="9" />
    <circle cx="290" cy="190" r="9" />
    <circle cx="221" cy="70" r="9" />
  </g>
  <g style={{ color: "var(--ds-yellow-500)" }} fill="currentColor">
    <circle cx="359" cy="70" r="9" />
    <circle cx="221" cy="150" r="9" />
    <polygon points="332,38 348,46 330,56" fillOpacity="0.9" />
  </g>
  <g fill="currentColor" opacity="0.3">
    <circle cx="397" cy="158" r="3" />
    <circle cx="427" cy="164" r="3" />
    <circle cx="457" cy="168" r="3" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <circle cx="510" cy="172" r="16" />
  </g>
</svg>
...
just clearing a bar a coin flip could clear?" If your dataset is 90% one
class, a baseline scores 90% accuracy by doing nothing. A "great" 91% model
suddenly looks far less great. Without a baseline, you cannot tell skill
from the appearance of skill.
...
For the breast-cancer data, the model is a `LogisticRegression`, which
benefits from scaled features, so the pipeline is a `StandardScaler`
...
Here is the entire workflow in one runnable block — frame (binary tumor
classification, scored by accuracy against a baseline), inspect, split,
baseline, pipeline, cross-validate, tune, and the single final test
evaluation. This is the shape of nearly every honest scikit-learn project.
...
print(f"Class balance: {int((y == 1).sum())} benign, {int((y == 0).sum())} malignant")
...
Read the output as a story: the data is mildly imbalanced, the baseline sits
in the low-sixties, the pipeline leaps far above it under cross-validation,
a tiny grid search nudges it a touch further, and a single test-set
evaluation confirms the gain is real. Every number was earned honestly,
because the test set sat untouched until the final line. That is the
workflow.
...
<ChallengeCard
  ...
  estimatedTime="~9 min"
  instructions={`Walk the core of the workflow on the wine dataset, in order.

1. The data is loaded as \`X\`, \`y\`. **Split first**: 25% test,
  ...
  tests={[
    {
      ...
      description: "133 train rows and 45 test rows for 178 wine samples",
      code: `assert X_train.shape[0] == 133, f"Expected 133 train rows, got {X_train.shape[0]}"
assert X_test.shape[0] == 45, f"Expected 45 test rows, got {X_test.shape[0]}"`,
    },
    {
      id: "pipeline_shape",
...
assert step_names == ["scaler", "model"], f"Expected steps ['scaler', 'model'], got {step_names}"`,
    },
    {
      ...
      code: `assert cv_mean is not None and 0.5 <= cv_mean <= 1.0, f"cv_mean {cv_mean} is not a believable accuracy"`,
    },
    {
      id: "test_valid",
      name: "test_score is a valid accuracy",
      description: "Final test accuracy is in [0, 1]",
