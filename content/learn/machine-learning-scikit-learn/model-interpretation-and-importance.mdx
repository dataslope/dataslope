...
result = permutation_importance(
    forest, X_test, y_test, n_repeats=20, random_state=0
)

order = np.argsort(result.importances_mean)[::-1]
print("Permutation importances (mean drop in accuracy on held-out data):")
for i in order[:6]:
    print(f"  {names[i]:<28} {result.importances_mean[i]:.3f}  +/- {result.importances_std[i]:.3f}")
`}
/>
...
ax.barh([data.feature_names[i] for i in order][::-1],
        importances[order][::-1])
...
feature_names = list(data.feature_names)`}
  initialCode={`# 1. Fit the random forest
forest = None
