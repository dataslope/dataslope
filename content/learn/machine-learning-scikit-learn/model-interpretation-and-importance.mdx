...
breast tumors are malignant. Wonderful — but a doctor about to act on that
prediction will immediately ask a question your accuracy score cannot
answer: **why?** Which measurements made the model say "malignant"? Would
it have said the same thing if one value were a little different? A number
on a held-out set tells you *that* the model is good. It does not tell you
*how* the model thinks, and for most real decisions, that second question
...
<svg
  viewBox="0 0 640 215"
  role="img"
  aria-label="A translucent model box pours out horizontal bars of sharply decreasing length, the longest glowing green — feature importance asks a trained model which inputs it actually leaned on"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <rect x="80" y="62" width="120" height="96" rx="18" fillOpacity="0.15" />
    <circle cx="140" cy="110" r="16" />
  </g>
  <g fill="currentColor" opacity="0.3">
    <circle cx="216" cy="110" r="3" />
    <circle cx="234" cy="110" r="3" />
    <circle cx="246" cy="56" r="4" />
    <circle cx="246" cy="88" r="4" />
    <circle cx="246" cy="120" r="4" />
    <circle cx="246" cy="152" r="4" />
    <circle cx="246" cy="184" r="4" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <rect x="260" y="46" width="280" height="20" rx="10" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <rect x="260" y="78" width="200" height="20" rx="10" fillOpacity="0.6" />
    <rect x="260" y="110" width="130" height="20" rx="10" fillOpacity="0.45" />
    <rect x="260" y="142" width="76" height="20" rx="10" fillOpacity="0.3" />
    <rect x="260" y="174" width="38" height="20" rx="10" fillOpacity="0.2" />
  </g>
</svg>
...
breast-cancer data, which ships with named features.
...
for i in order[:5]:
    direction = "raises P(benign)" if coefs[i] > 0 else "lowers P(benign)"
    print(f"  {names[i]:<24} {coefs[i]:+.2f}  ({direction})")
`,
    },
  ]}
/>
...
suspicion. The breast-cancer data, for instance, has several near-duplicate
"mean / worst / error" measurements of the same quantity.
</Callout>
...
for i in order[:6]:
    print(f"  {names[i]:<28} {importances[i]:.3f}")
...
for i in order[:6]:
    print(
        f"  {names[i]:<28} {result.importances_mean[i]:.3f}  +/- {result.importances_std[i]:.3f}"
    )
`,
    },
  ]}
...
importances = forest.feature_importances_
order = np.argsort(importances)[::-1][:8]  # top 8

fig, ax = plt.subplots(figsize=(7, 4))
ax.barh([data.feature_names[i] for i in order][::-1], importances[order][::-1])
ax.set_xlabel("Importance (impurity-based)")
ax.set_title("Top features: wine random forest")
fig.tight_layout()
plt.show()
`,
    },
...
summarizes the model's behavior *across the whole dataset*. "Flavanoids and
color intensity drive this wine classifier" is a global statement.

There is a second flavor: **local** explanations, which explain *one
specific prediction*. "For *this particular* patient, the high worst-radius
value is what pushed the model toward 'malignant'" is a local statement.
...
action: "color intensity is the most important feature, so let us change
color intensity to change the outcome." Importance describes *prediction*,
...
<ChallengeCard
  ...
  estimatedTime="~7 min"
  instructions={`You will train a random forest on the wine dataset and pull
out its most important features.

1. The data is loaded for you: \`X\`, \`y\`, and \`feature_names\`.
...
`,
  ...
  tests={[
    ...
    },
    {
      ...
      description: "importances has 13 values that sum to about 1.0",
