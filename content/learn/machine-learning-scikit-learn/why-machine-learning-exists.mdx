...
<svg
  viewBox="0 0 640 200"
  role="img"
  aria-label="Yellow rule bars hold back a crowd of blue dots, but a red dot weaves through the misaligned gaps between the bars and escapes — hand-written rules always leave holes"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g style={{ color: "var(--ds-yellow-500)" }} fill="currentColor">
    <rect x="242" y="24" width="16" height="64" rx="8" fillOpacity="0.9" />
    <rect x="242" y="120" width="16" height="56" rx="8" fillOpacity="0.9" />
    <rect x="312" y="24" width="16" height="32" rx="8" fillOpacity="0.6" />
    <rect x="312" y="88" width="16" height="88" rx="8" fillOpacity="0.6" />
    <rect x="382" y="24" width="16" height="96" rx="8" fillOpacity="0.35" />
    <rect x="382" y="152" width="16" height="24" rx="8" fillOpacity="0.35" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="92" cy="68" r="8" />
    <circle cx="128" cy="96" r="8" />
    <circle cx="86" cy="128" r="8" />
    <circle cx="142" cy="148" r="8" />
    <circle cx="178" cy="78" r="8" fillOpacity="0.6" />
    <circle cx="206" cy="118" r="8" fillOpacity="0.6" />
  </g>
  <g style={{ color: "var(--ds-red-500)" }} fill="currentColor">
    <circle cx="196" cy="100" r="3" fillOpacity="0.35" />
    <circle cx="226" cy="103" r="3" fillOpacity="0.35" />
    <circle cx="250" cy="104" r="3" fillOpacity="0.4" />
    <circle cx="278" cy="92" r="3" fillOpacity="0.4" />
    <circle cx="302" cy="78" r="3" fillOpacity="0.45" />
    <circle cx="320" cy="72" r="3" fillOpacity="0.45" />
    <circle cx="344" cy="86" r="3" fillOpacity="0.5" />
    <circle cx="366" cy="112" r="3" fillOpacity="0.5" />
    <circle cx="390" cy="134" r="3" fillOpacity="0.55" />
    <circle cx="420" cy="140" r="3" fillOpacity="0.55" />
    <circle cx="452" cy="141" r="3" fillOpacity="0.6" />
    <circle cx="506" cy="141" r="9" />
    <polygon points="540,132 540,150 558,141" />
  </g>
</svg>
...
classify iris flowers, and we *try the rule-first approach*. With a quick
look at the data, a single threshold on petal length perfectly separates one
species (*setosa*) from the others. For that part of the problem, a one-line
rule is not just adequate — it is *better* than a model: simpler, exact, and
...
# Column 2 is petal length (cm). Class 0 is 'setosa'.

petal_length = X[:, 2]

# A hand-written rule: "short petals -> setosa".
rule_says_setosa = petal_length < 2.5
actually_setosa = y == 0

correct = (rule_says_setosa == actually_setosa).sum()
print(f"A one-line rule separates setosa perfectly: {correct}/{len(y)} correct.")
`,
    },
  ]}
/>
...
species, *versicolor* and *virginica*. Their measurements overlap heavily;
no single clean threshold divides them. This is where rule-writing stalls
and a learned model earns its place.
...
# Keep only the two hard-to-separate species (1 and 2).
mask = y != 0
X2, y2 = X[mask], y[mask]

# Try a hand-written threshold on petal length for these two.
petal_length = X2[:, 2]
threshold = 4.9  # a reasonable hand-picked guess
rule_pred = np.where(petal_length < threshold, 1, 2)
rule_acc = (rule_pred == y2).mean()

# Now let a model learn the boundary from all four features.
...
print(f"Learned model:              {model_acc:.0%} on held-out flowers.")
`,
    },
  ]}
/>
...
could. That is the whole argument of this page in one example: **rules for
the simple, models for the complex.** A mature solution often uses *both* —
cheap rules where they suffice and a model only where the problem genuinely
needs one.
...
regime and a new one (the relationship between input and output shifts). A
fixed rule, tuned to the old world, grows stale. A model simply retrained on
new data tracks the change.
