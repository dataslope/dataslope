...
<svg
  viewBox="0 0 640 220"
  role="img"
  aria-label="A huge blue circle, a tiny green square, and a small yellow dot pass through a narrow gate and come out the same size on the other side — scaling puts wildly different feature ranges on one common footing"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="120" cy="95" r="55" fillOpacity="0.8" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <rect x="188" y="153" width="14" height="14" rx="4" fillOpacity="0.8" />
  </g>
  <g style={{ color: "var(--ds-yellow-500)" }} fill="currentColor">
    <circle cx="212" cy="52" r="10" fillOpacity="0.8" />
  </g>
  <g fill="currentColor" opacity="0.25">
    <rect x="312" y="28" width="12" height="66" rx="6" />
    <rect x="312" y="126" width="12" height="66" rx="6" />
  </g>
  <g fill="currentColor" opacity="0.3">
    <circle cx="280" cy="110" r="3" />
    <circle cx="304" cy="110" r="3" />
    <circle cx="332" cy="110" r="3" />
    <circle cx="356" cy="110" r="3" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="425" cy="110" r="20" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <rect x="470" y="90" width="40" height="40" rx="10" />
  </g>
  <g style={{ color: "var(--ds-yellow-500)" }} fill="currentColor">
    <circle cx="555" cy="110" r="20" />
  </g>
</svg>
...
Let us see it on real data. The wine dataset has 13 chemical measurements
on wildly different scales — some near 0.1, others in the hundreds.
...
print("  column means (first 4):", np.round(X.mean(axis=0)[:4], 2))
print("  column stds  (first 4):", np.round(X.std(axis=0)[:4], 2))
...
print("\\nAFTER scaling")
print("  column means (first 4):", np.round(X_scaled.mean(axis=0)[:4], 2))
print("  column stds  (first 4):", np.round(X_scaled.std(axis=0)[:4], 2))
`,
    },
  ]}
/>
...
map. After scaling, every mean is essentially 0 and every standard
deviation is essentially 1. The columns are now directly comparable.
...
We will train a k-nearest-neighbors classifier on the wine data twice —
...
<ChallengeCard
  ...
  estimatedTime="~7 min"
  instructions={`The breast cancer dataset has 30 features on very different
scales. Scale them **without leaking the test set**.
...
assert abs(train_mean) < 1e-9, f"Training mean should be ~0 after scaling, got {train_mean}"`,
    },
    {
      id: "shapes",
