...
int main() {
    ...
    for (int u = 0; u < n; u++) {
        std::cout << u << ":";
        for (auto [v, w] : adj[u]) std::cout << " (" << v << "," << w << ")";
        std::cout << "\\n";
    }
    return 0;
}
...
int countEdges(const std::vector<std::vector<int>>& adj) {
    int degreeSum = 0;
    for (const auto& neighbors : adj) degreeSum += static_cast<int>(neighbors.size());
    return degreeSum / 2;
}
...
std::vector<std::vector<int>> toMatrix(const std::vector<std::vector<int>>& adj, int n) {
    // your code here
    return {};
}
