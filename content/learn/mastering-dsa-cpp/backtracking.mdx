...
<ChallengeCard adapter="cpp" title="Generate all subsets" instructions={`Implement \`std::vector<std::vector<int>> subsets(const std::vector<int>& nums)\`. Use deterministic bitmask order from 0 to 2^n-1 so stdout matches exactly.`} initialCode={`#include <iostream>
#include <vector>
...
<ChallengeCard adapter="cpp" title="N-Queens count" instructions={`Implement \`int countNQueens(int n)\`. The driver prints counts for n=1..6.`} initialCode={`#include <iostream>
#include <vector>
...
<ChallengeCard adapter="cpp" title="Word search" instructions={`Implement \`bool exist(std::vector<std::vector<char>>& board, const std::string& word)\`.`} initialCode={`#include <iostream>
#include <string>
#include <vector>
