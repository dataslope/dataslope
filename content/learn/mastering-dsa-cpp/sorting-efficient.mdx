...
int main() {
    ...
    for (const Record& r : rows) std::cout << r.group << ":" << r.name << "\\n";
    return 0;
}
`}
/>
...
int main() {
    ...
    for (const auto& item : scores) std::cout << item.first << "=" << item.second << "\\n";
    return 0;
}
`}
/>
...
#include <vector>
void mergeRange(std::vector<int>& v, std::vector<int>& temp, int left, int mid, int right) {
    ...
    for (int p = left; p <= right; p++) v[p] = temp[p];
}
void mergeSortRange(std::vector<int>& v, std::vector<int>& temp, int left, int right) {
    if (left >= right) return;
    int mid = left + (right - left) / 2;
