...
<ChallengeCard adapter="cpp" title="Climbing stairs" instructions={`Implement \`int climbStairs(int n)\` for n=1..45. The driver prints several values.`} starterCode={`#include <iostream>
...
<ChallengeCard adapter="cpp" title="Coin change min coins" instructions={`Implement \`int coinChange(std::vector<int> coins, int amount)\`. Return -1 if impossible.`} starterCode={`#include <algorithm>
#include <iostream>
#include <vector>
...
<ChallengeCard adapter="cpp" title="0/1 Knapsack max value" instructions={`Implement \`int knapsack(int W, const std::vector<int>& weights, const std::vector<int>& values)\`.`} starterCode={`#include <algorithm>
#include <iostream>
#include <vector>
