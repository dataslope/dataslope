...
int popMax(std::vector<int>& heap) {
    ...
    while (true) {
        ...
        int right = 2 * i + 2;
        int best = i;
        if (left < static_cast<int>(heap.size()) && heap[left] > heap[best]) best = left;
        if (right < static_cast<int>(heap.size()) && heap[right] > heap[best]) best = right;
        if (best == i) break;
...
void heapify(std::vector<int>& a) {
    for (int i = static_cast<int>(a.size()) / 2 - 1; i >= 0; i--) siftDown(a, i, static_cast<int>(a.size()));
}
...
int main() {
    ...
    for (int x : {4, 1, 7}) maxHeap.push(x);
    std::cout << maxHeap.top() << "\n";

    std::priority_queue<int, std::vector<int>, std::greater<int>> minHeap;
    for (int x : {4, 1, 7}) minHeap.push(x);
...
int main() {
    ...
    for (int x : mergeK(lists)) std::cout << x << "\n";
    return 0;
}
`}
  solutionCode={`#include <functional>
...
std::vector<int> mergeK(const std::vector<std::vector<int>>& lists) {
    ...
    while (!pq.empty()) {
        ...
        if (next < static_cast<int>(lists[listIndex].size())) pq.push({lists[listIndex][next], listIndex, next});
    }
    return ans;
}
