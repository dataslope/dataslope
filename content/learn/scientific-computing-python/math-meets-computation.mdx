...
plt.ylim(-2, 2); plt.legend(); plt.xlabel("x"); plt.ylabel("sin(x)")
...
- [o] It measures how much a relative input error gets amplified into a relative output error: a problem with $\\kappa = 10^k$ can lose up to $k$ digits of precision regardless of algorithm
  > Right. The condition number is a property of the *problem*, not of the *algorithm*. It puts a fundamental ceiling on accuracy — no clever code can beat it.
