...
<CodeBlock
  adapter="python"
  initialCode={`# A direct Python translation of a typical MATLAB session.
...
- [o] Because NumPy's heaviest routines (matrix multiply, linear solves, eigenvalues, FFTs) dispatch to BLAS, LAPACK, and related libraries, most of which are FORTRAN code that has been hand-tuned for decades
  > Right. NumPy is Python at the interface and FORTRAN/C at the core. Calling \`A @ B\` invokes a BLAS routine like DGEMM written in FORTRAN. This is why scientific Python is so fast despite Python itself being interpreted.
