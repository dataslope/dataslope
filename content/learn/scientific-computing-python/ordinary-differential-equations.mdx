...
sol = solve_ivp(pendulum, [0, 30], y0=[2.0, 0.0],
...
x, y = sol.sol(t)
plt.plot(t, x, label="prey"); plt.plot(t, y, label="predator")
...
for method in ["RK45", "BDF", "LSODA"]:
    ...
    try:
        ...
        print(f"{method:7s}: {sol.nfev:>6d} fevals, "
...
def time_to_apex(v0):
    def rhs(t, s): return [s[1], -9.81]
    def vel_zero(t, s): return s[1]
    vel_zero.terminal = True
    vel_zero.direction = -1
    sol = solve_ivp(rhs, [0, 1000], [0.0, v0],
                    events=vel_zero, rtol=1e-10, atol=1e-12)
...
- [o] Switch to an implicit stiff solver such as \`BDF\` or \`LSODA\`
  > Right. Wildly separated timescales are the textbook definition of stiffness. Explicit RK methods are forced to use tiny steps for *stability* — not accuracy. Implicit BDF/LSODA solvers handle stiff systems in dozens of steps where RK45 needs millions.
