...
<CodeBlock
  adapter="python"
  initialCode={`# How long would a human computer take to integrate
...
spline = CubicSpline(degrees, table)`}
  initialCode={`# Look up sin(37.4°) by interpolating between table entries
...
- [o] $1 - \\cos(x)$ subtracts two nearly equal numbers and loses almost all of its significant digits — a phenomenon called *catastrophic cancellation*
  > Exactly. For tiny $x$, $\\cos(x) \\approx 1$, so $1 - \\cos(x)$ is the difference of two floating-point numbers that agree in almost every digit. Subtracting them keeps only the few digits where they *disagree*, which is mostly round-off noise. The rewritten form $2\\sin^2(x/2)$ avoids the subtraction entirely.
