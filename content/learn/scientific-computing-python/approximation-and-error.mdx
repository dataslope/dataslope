...
- [o] Truncation error is shrinking but round-off error is growing; below the optimal $h$, round-off dominates because $f(x+h) - f(x)$ loses most of its significant digits to cancellation
  > Yes. This is the classic "V-shaped" total-error curve: truncation falls as $h \\to 0$, round-off grows as $h \\to 0$, and the minimum sits at the crossover.
...
- [o] About $32^4 \\approx 10^6$
  > Right. Method A's error is $O(h)$ and method B's is $O(h^4)$. Matching B's error of $\\sim (1/32)^4$ requires $h \\sim (1/32)^4$, i.e. about $32^4 \\approx 10^6$ steps for method A.
- About $32 \\cdot 4 = 128$
