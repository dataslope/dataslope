...
<ChallengeCard
  ...
  initialCode={`def safe_divide(a, b, default=None):
    # your code here
    return default
`}
...
For testing, assume a function that raises \`RateLimitError\` on the first \`n-1\` calls and succeeds on the \`n\`th call.`}
  initialCode={`class RateLimitError(Exception):
    pass
...
- [o] \`else\`
  > **Correct.** \`else\` is for code that should run on success (no exception).
- \`finally\`
...
- [o] It catches \`KeyboardInterrupt\` and \`SystemExit\`, making the program hard to kill.
  > **Correct.** Bare \`except:\` catches **everything**, including signals you want to let through.
...
- [o] When wrapping a lower-level exception in a higher-level one, to preserve the cause.
  > **Correct.** \`raise ConfigError(...) from e\` shows both the \`ConfigError\` and the underlying \`ValueError\`, with a causal link.
...
- [o] Runs no matter what: success, exception, return, or even sys.exit().
  > **Correct.** \`finally\` is guaranteed cleanup. Use it for closing files, releasing locks, etc.
...
- [o] Try the operation and catch the exception if it fails.
  > **Correct.** EAFP stands for "Easier to Ask for Forgiveness than Permission." It is idiomatic Python.
...
- [o] Python prints a traceback and exits with a non-zero exit code.
  > **Correct.** Unhandled exceptions terminate the program (or return 500 in a web context).
