...
<CodeBlock
  adapter="python"
  initialCode={`with open("data.txt", "w") as f:
    f.write("first\\nsecond\\nthird\\n")
...
<CodeBlock
  adapter="python"
  initialCode={`with open("poem.txt", "w") as f:
    f.write("alpha\\nbeta\\ngamma\\ndelta\\n")
...
<CodeBlock
  adapter="python"
  initialCode={`with open("output.txt", "w") as f:
    f.write("Hello, world!\\n")
...
<CodeBlock
  adapter="python"
  initialCode={`# Use print(..., file=f) for auto-newline behavior
...
\`main.py\` calls your function and prints the results. Do not edit \`main.py\` or \`sales.csv\`.`}
  ...
  files={[
    {
      filename: "main.py",
      initialContent: `from parser import parse_sales
...
\`main.py\` calls your function. Do not edit \`main.py\` or \`config.json\`.`}
  ...
  files={[
    {
      filename: "main.py",
      initialContent: `from config_loader import load_config
...
\`main.py\` calls your function multiple times. Do not edit \`main.py\`.`}
  ...
  files={[
    {
      filename: "main.py",
      initialContent: `from logger import log
...
`,
    ...
    {
      filename: "logger.py",
      initialContent: `def log(message):
    # your code here
    pass
`,
      solutionContent: `def log(message):
...
- [o] It automatically closes the file when the block exits, even if an exception is raised.
  > **Correct.** The context manager protocol guarantees cleanup.
...
- [o] When working with non-text files like images, executables, or pickle files.
  > **Correct.** Binary mode gives you raw bytes without text encoding.
...
- [o] The default encoding is platform-dependent, which can cause bugs on different systems.
  > **Correct.** macOS/Linux default to UTF-8; Windows often defaults to CP1252. Explicit is better than implicit.
...
- [o] \`json.dumps\` returns a JSON string; \`json.dump\` writes directly to a file object.
  > **Correct.** The \`s\` suffix conventionally means "string" in Python APIs.
...
- [o] On Windows, the default text mode translates \`\\n\` to \`\\r\\n\`, doubling line endings. \`newline=""\` disables this.
  > **Correct.** The CSV module handles line endings itself; you must disable the OS translation.
