...
<CodeBlock
  adapter="python"
  initialCode={`# Bad: where does 'sqrt' come from?
# from math import *
# print(sqrt(2))
...
<CodeBlock
  adapter="python"
  initialCode={`# inside myapp/db/queries.py
...
<ChallengeCard
  ...
  initialCode={`def filter_even(numbers):
    # your code here
    return []
`}
...
- [o] Python returns the cached module from \`sys.modules\`.
  > **Correct.** Modules are cached after the first import. Subsequent imports are nearly free.
...
- [o] \`import math\` (then call \`math.sqrt(...)\`)
  > **Correct.** This is the clearest: every call to \`math.sqrt\` tells the reader exactly where it comes from.
...
- [o] All directories in \`sys.path\`, which includes the current directory, \`PYTHONPATH\`, and \`site-packages\`.
  > **Correct.** \`sys.path\` is a list of directories Python searches in order.
