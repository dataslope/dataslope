...
<CodeBlock
  adapter="python"
  initialCode={`mixed = [1, "two", 3.0, [4, 5]]
print(mixed)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`from_range = list(range(5))
print(from_range)
...
Hint: slicing makes this one-liner.`}
  initialCode={`def rotate(items, n):
    # your code here
    return []
`}
...
You may assume the list is only one level deep.`}
  initialCode={`def flatten(nested):
    # your code here
    return []
`}
