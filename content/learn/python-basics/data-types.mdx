...
<CodeBlock
  adapter="python"
  initialCode={`print(isinstance(True, bool))   # True
...
<CodeBlock
  adapter="python"
  initialCode={`# float to int (truncates toward zero)
print(int(3.9))
print(int(-3.9))
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`# list / tuple / set are interchangeable in many cases
items = [1, 2, 2, 3]
print(tuple(items))
...
<CodeBlock
  adapter="python"
  initialCode={`# A function with no return statement returns None.
def shout(msg):
    print(msg.upper())
...
<ChallengeCard
  ...
  initialCode={`def safe_int(s, default=0):
    # Your code here
    return default
