...
\`main.py\` uses your classes. Do not edit \`main.py\`.`}
  ...
  files={[
    {
      filename: "main.py",
      initialContent: `from shapes import Rectangle, Circle
...
\`main.py\` uses your classes polymorphically. Do not edit \`main.py\`.`}
  ...
  files={[
    {
      filename: "main.py",
      initialContent: `from animals import Dog, Cat, Animal
...
\`main.py\` uses your classes. Do not edit \`main.py\`.`}
  ...
  files={[
    {
      filename: "main.py",
      initialContent: `from employees import Employee, Manager
...
class Manager(Employee):
    ...
    pass
`,
      solutionContent: `class Employee:
...
- [o] If two objects expose the same methods, they can be used interchangeably, regardless of inheritance.
  > **Correct.** "If it walks like a duck and quacks like a duck, it's a duck." You don't need a common base class.
...
- [o] When you want to allow subclasses (e.g., \`bool\` is a subclass of \`int\`).
  > **Correct.** \`isinstance\` returns \`True\` for the class and all subclasses.
...
- [o] The order Python searches for methods when a class has multiple inheritance.
  > **Correct.** The MRO is computed with the C3 linearization algorithm and stored in \`Class.__mro__\`.
...
- [o] A class that cannot be instantiated directly; subclasses must implement abstract methods.
  > **Correct.** Use \`abc.ABC\` and \`@abstractmethod\` to enforce contracts.
...
- [o] Composition is more flexible, easier to understand, and avoids fragile base class problems.
  > **Correct.** Composition ("has a") is clearer than inheritance ("is a") in many cases.
