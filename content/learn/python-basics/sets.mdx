...
<CodeBlock
  adapter="python"
  initialCode={`unique = set([1, 2, 2, 3, 3, 3])
print(unique)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`good = {(1, 2), "hi", 42, frozenset({1, 2})}
print(good)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`set_of_sets = {frozenset({1, 2}), frozenset({3, 4})}
print(set_of_sets)
`}
/>
