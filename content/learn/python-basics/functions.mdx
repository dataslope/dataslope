...
<CodeBlock
  adapter="python"
  initialCode={`# RIGHT: use None as the sentinel
...
<CodeBlock
  adapter="python"
  initialCode={`def apply_twice(func, x):
    return func(func(x))
...
<ChallengeCard
  ...
  initialCode={`def collect_events(event, log=[]):
    log.append(event)
    return log
`}
...
- [o] \`[1]\\n[1, 2]\`
  > **Correct.** The default list is evaluated once at \`def\` time and shared across calls. This is the classic mutable-default footgun.
- \`[1, 2]\\n[1, 2]\`
...
- [o] \`f(1, 2, 3, d=4)\`
  > **Correct.** \`a\` and \`b\` are positional-only, \`c\` can be positional or keyword, \`d\` must be keyword.
- \`f(a=1, b=2, c=3, d=4)\`
...
- [o] \`None\`
  > **Correct.** A function without an explicit \`return\` returns \`None\`.
- An empty string \`""\`
...
- [o] \`lambda\` functions are anonymous and show up as \`<lambda>\` in tracebacks.
  > **Correct.** Unlike \`def\`, a \`lambda\` has no name and appears as \`<lambda>\` in error messages.
...
- [o] It expands an iterable into separate positional arguments.
  > **Correct.** \`func(*[1, 2, 3])\` is equivalent to \`func(1, 2, 3)\`.
...
- [o] They help tools like \`mypy\` catch bugs and improve IDE autocompletion.
  > **Correct.** Type hints are for static analysis tools and IDEs, not the runtime.
