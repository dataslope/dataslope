...
<CodeBlock
  adapter="python"
  initialCode={`also_person = dict(name="Ada", year=1815)
print(also_person)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`from_pairs = dict([("a", 1), ("b", 2), ("c", 3)])
print(from_pairs)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`person = {"name": "Ada", "year": 1815, "role": "mathematician"}
...
<CodeBlock
  adapter="python"
  initialCode={`good = {("x", 1): "ok", 42: "also ok", "hi": "yep"}
print(good)
`}
/>
...
Assume all values are numbers.`}
  initialCode={`def merge_sum(d1, d2):
    # your code here
    return {}
`}
