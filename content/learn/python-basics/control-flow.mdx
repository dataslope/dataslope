...
<CodeBlock
  adapter="python"
  initialCode={`def http_status(code):
    match code:
        case 200:
            return "OK"
        case 404:
...
Assume \`score\` is an integer between 0 and 100.`}
  initialCode={`def letter_grade(score):
    # your code here
    return "F"
`}
...
<MultipleChoice
  markdown={`Which of the following is a valid Python 3.10+ \`match\` pattern?
...
- \`case x and y:\`
  > \`and\` is not valid pattern syntax; use \`case (x, y):\` for a tuple.`}
/>
