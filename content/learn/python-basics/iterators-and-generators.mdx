...
<CodeBlock
  adapter="python"
  initialCode={`data = ["alpha", "beta", "gamma"]
it = iter(data)
...
<CodeBlock
  adapter="python"
  initialCode={`# These are equivalent:
...
<CodeBlock
  adapter="python"
  initialCode={`from itertools import (
    count, cycle, repeat, chain, islice,
    takewhile, accumulate, combinations, product
)
...
Example: \`list(take(3, [1, 2, 3, 4, 5]))\` returns \`[1, 2, 3]\`.`}
  initialCode={`def take(n, iterable):
    # your code here
    if False:
        yield None
`}
...
Example: \`list(batched([1, 2, 3, 4, 5], 2))\` returns \`[(1, 2), (3, 4), (5,)]\`.`}
  initialCode={`def batched(iterable, n):
    # your code here
    if False:
        yield ()
`}
