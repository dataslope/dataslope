...
<CodeBlock
  adapter="python"
  initialCode={`evens = [x for x in range(20) if x % 2 == 0]
print(evens)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`upper = [w.upper() for w in ["a", "b", "c"]]
print(upper)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`words = ["apple", "banana", "cherry", "date"]
...
<CodeBlock
  adapter="python"
  initialCode={`pairs = [(x, y) for x in range(3) for y in range(3)]
print(pairs)
`}
/>
...
<CodeBlock
  adapter="python"
  initialCode={`# Normalize keys to lowercase
...
<CodeBlock
  adapter="python"
  initialCode={`# Generator expression streams one square at a time.
...
<CodeBlock
  adapter="python"
  initialCode={`total = sum(x * x for x in range(10))
print(total)
`}
/>
