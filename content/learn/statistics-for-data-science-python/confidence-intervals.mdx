...
estimate** — your single best guess for the population mean $\mu$. But
you already know $\bar{x}$ is a random variable: it has a *standard
...
by the confidence level). Combine them: $\text{estimate} \pm
(\text{critical value} \times \text{standard error})$. Memorize the
...
particular interval. Your specific interval either contains $\mu$ or it
doesn't — you just don't know which, because $\mu$ is fixed and
...
The diagram is the whole idea: $\mu$ never moves, but each sample
...
a known $\mu$, then draw 100 separate samples, build a 95% CI from
each, and color the ones that miss $\mu$. About 5 of the 100 should
...
for _ in range(n_intervals):
    ...
    lows.append(lo); highs.append(hi); centers.append(s.mean())
...
from $\mu$. You can't look at one interval and know its color; that's
...
- **"There's a 95% probability $\mu$ is in this interval."** Covered
...
"normal-approximation" (Wald) interval is $\hat{p} \pm z\sqrt{\hat{p}(1-\hat{p})/n}$.
...
away from the extremes, but it behaves badly when $\hat{p}$ is close to
...
- Width grows with the **confidence level** and the **variability**, and shrinks like **$1/\sqrt{n}$**. Wider is *less* precise, not "safer."
