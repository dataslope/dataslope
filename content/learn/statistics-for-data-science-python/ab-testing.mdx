...
| Smaller $\alpha$ (stricter) | needs **more** users |
...
n_per_arm = ((z_alpha * np.sqrt(2 * p_bar * (1 - p_bar))
              + z_beta * np.sqrt(p1*(1-p1) + p2*(1-p2)))**2) / (p2 - p1)**2

print(f"Detecting {p1:.0%} -> {p2:.0%} at {power:.0%} power, alpha={alpha}:")
print(f"  Need ~{np.ceil(n_per_arm):,.0f} users PER ARM "
...
the false-positive rate at the $\alpha$ you intended.
</Callout>
...
fig = px.bar(
    df, x="group", y="rate", error_y="err", color="group",
