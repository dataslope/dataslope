...
t_low, t_high = stats.t.interval(0.95, df=len(data) - 1,
                                 loc=data.mean(), scale=se_formula)
...
fig = px.histogram(boot_medians, nbins=50,
                   labels={"value": "bootstrap median"},
                   title="Bootstrap distribution of the median")
fig.add_vline(x=np.median(data), line_dash="dash",
