...
for name, x in [("symmetric", symmetric), ("right-skew", right), ("left-skew", left)]:
    print(f"{name:>11}: skew={stats.skew(x):+.2f}  "
...
fig = px.histogram(mixed, nbins=50,
...
for name, x in [("normal", normal_data), ("heavy-tailed", heavy_tail)]:
    ...
    print(f"{name:>13}: excess kurtosis={stats.kurtosis(x):+.2f}  "
...
fig = px.box(data, y="income", points="outliers",
...
readings = pd.Series(np.concatenate([
  ...
  initialCode={`# TODO: apply the 1.5xIQR rule
q1 = None
q3 = None
iqr = None
lower_fence = None
