...
Every test has an unknowable **truth** ($H_0$ is really true, or it's
really false) and a **decision** you make (reject $H_0$, or fail to
...
| | **You fail to reject $H_0$** | **You reject $H_0$** |
| --- | --- | --- |
| **$H_0$ is true** | Correct (specificity) | **Type I error** — false positive, rate $\alpha$ |
| **$H_0$ is false** | **Type II error** — false negative, rate $\beta$ | Correct detection — **power** $= 1 - \beta$ |

Two definitions to memorize, because everything else builds on them:

- **Type I error** ($\alpha$): rejecting a *true* null. A **false
  positive** — you "discovered" an effect that isn't real. Its rate is
  exactly the significance level $\alpha$ you chose.
- **Type II error** ($\beta$): failing to reject a *false* null. A **false
...
Here's the tension. $\alpha$ is the bar for "convincing." Lower the bar
(smaller $\alpha$) and you make fewer false alarms — but you also reject
*less often overall*, so you miss more real effects ($\beta$ goes up).
...
This is why the "right" $\alpha$ depends on which mistake is costlier. A
...
There is nothing sacred about 0.05. Pick $\alpha$ by asking: *in my
...
$\alpha$. When missing a real effect is the disaster (early disease
screening), loosen $\alpha$ or — better — buy more power with a bigger
sample.
</Callout>
...
you trade $\alpha$ against $\beta$. But collect more data and you can lower
*both* — a bigger sample sharpens the test's ability to tell signal from
noise, so the same $\alpha$ buys you more power. Power has four levers.
...
nature made it, $\alpha$ is usually pinned by convention, and variance is
...
2. Draw a sample and run the test. Did it reject $H_0$? Yes or no.
...
real, you'll fail to reject $H_0$ 60% of the time. "We found no
...
fig = px.line(x=sizes, y=power_by_n, markers=True,
              title="Power rises with sample size (true effect = 0.5)")
fig.add_hline(y=0.8, line_dash="dash", line_color="green")
fig.update_layout(xaxis_title="n per group",
...
fig = px.line(x=effects, y=power_by_effect, markers=True,
              title="Power rises with effect size (n = 40 per group)")
fig.add_hline(y=0.8, line_dash="dash", line_color="green")
fig.update_layout(xaxis_title="true effect size (difference in means)",
                  yaxis_title="estimated power",
                  yaxis_range=[0, 1])
fig.show()
...
*zero*, $H_0$ is true, so "rejecting" is a Type I error. That point sits
near 0.05, which is exactly $\alpha$. Let's verify that directly.

## The Type I error rate really is α

A well-behaved test, run when $H_0$ is *true*, should reject about
$\alpha$ of the time — no more, no less. Let's confirm by simulating a
...
meaning of $\alpha$: *the rate at which you'll cry wolf when there's no
wolf.* It is **not** "the probability $H_0$ is true" — it's a property of
your decision rule, fixed in advance, that holds whenever $H_0$ happens to
be true.

<Callout type="danger" title="α is NOT the probability that H0 is true">
$\alpha = 0.05$ means: *if $H_0$ is true, I will wrongly reject it 5% of
the time.* It is a long-run error rate of your procedure, not a statement
about any particular hypothesis. The probability that $H_0$ is true isn't
...
print(f"Average of only SIGNIFICANT estimates: {significant_estimates.mean():.3f}  (inflated!)")
...
alpha = 0.05
n_experiments = 3000`}
  initialCode={`# TODO: simulate n_experiments t-tests and set power = fraction that reject H0
power = None
`}
...
alpha = 0.05
n_experiments = 8000`}
  initialCode={`# TODO: simulate under H0 (no effect) and set type_I_rate = fraction that reject
type_I_rate = None
`}
...
- A test can err two ways: **Type I** (false positive, reject a true
  $H_0$, rate $\alpha$) and **Type II** (false negative, miss a real
  effect, rate $\beta$).
- **Power $= 1 - \beta$** is the chance of detecting a real effect. Design
  for at least **0.80**.
- With a fixed sample, $\alpha$ and $\beta$ **trade off**; the way to
  shrink both is a **larger $n$**. Power grows with **effect size**,
  **$n$**, **larger $\alpha$**, and **smaller variance**.
- $\alpha$ is a false-positive *rate*, **not** the probability $H_0$ is
