...
The notation $P(A \mid B)$ reads "the probability of $A$ **given** $B$" —
...
A quick concrete version: in a deck of 52 cards, $P(\text{King}) = 4/52$.
But if someone tells you the card is a face card, you've zoomed into the
12 face cards, and $P(\text{King} \mid \text{face card}) = 4/12 = 1/3$.
Same card, different probability, because you knew more.

## Independence vs dependence

Sometimes learning $B$ changes nothing. If $P(A \mid B) = P(A)$ — knowing
...
When $A$ and $B$ happen to be independent, $P(A \mid B) = P(A)$ and this
...
print(f"P(churn | free) = {table.loc['free','churned']}/{free_total} = {p_churn_given_free:.3f}")
...
Here's the trap that gives this whole topic its danger. **$P(A \mid B)$
and $P(B \mid A)$ are different numbers**, and confusing them is so common
it has a name: the **confusion of the inverse**.

- $P(\text{positive test} \mid \text{disease})$ — among *sick* people,
  how often does the test fire? (This is the test's sensitivity. Usually
  high.)
- $P(\text{disease} \mid \text{positive test})$ — among people who
...
overestimate $P(A \mid B)$. That single oversight is behind most
...
- **Specificity** = $P(\text{negative} \mid \text{healthy}) = 0.99$, so
...
<CodeBlock
  adapter="python"
  initialCode={`# Bayes' rule by hand: P(disease | positive).
...
informative the evidence is when $A$ is true $P(B \mid A)$, and how often
...
<CodeBlock
  adapter="python"
  initialCode={`# Spam filter framing of the SAME math.
...
counts = pd.DataFrame(
  ...
  initialCode={`# TODO: compute the three conditional / overall probabilities as floats
...
p_flag_given_legit = 0.03`}
  initialCode={`# TODO: compute p_flag (total probability of a flag) and ppv = P(fraud | flagged)
p_flag = None
ppv = None
`}
...
- $P(A \mid B)$ means "**among the B cases, how often is A also true**" — conditioning **zooms into a slice** of the sample space.
- **Independent** means $P(A \mid B) = P(A)$ (the shortcut multiply); otherwise events are **dependent** and you need the conditional form.
- $P(A \mid B) \ne P(B \mid A)$. Swapping them is the **confusion of the inverse** — the error behind most probability disasters.
