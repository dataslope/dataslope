...
> extreme** as what you saw, **assuming the null hypothesis $H_0$ is
> true**.

Read it slowly. Three pieces carry all the meaning:

- **"assuming $H_0$ is true"** — the whole calculation lives in a
...
The thing to the right of the bar — "given $H_0$" — is *assumed*, not
concluded. This is exactly why a p-value can never be the probability that
$H_0$ is true: you cannot end up with a probability *about* the thing you
*assumed* at the start.
</Callout>

## Build it yourself: the p-value as a tail area

Here is the idea that makes p-values click. Suppose $H_0$ is true. Then
...
their coin is fair. You flip it 100 times and get 62 heads. Under $H_0$
...
p_exact = stats.binomtest(observed_heads, n_flips, 0.5,
...
fig = px.histogram(null_diffs, nbins=60,
...
fig.update_layout(xaxis_title="difference in group means under H0",
...
Look at the histogram. It is centered on **zero** — because under $H_0$
...
This is the big one. A p-value is computed *assuming* $H_0$. You cannot
feed "$H_0$ is true" into a calculation and have its probability fall out
...
<CodeBlock
  adapter="python"
  initialCode={`# A rare disease: 1% of people have it.
...
print("Here a positive test still leaves a", f"{(1-p_disease_given_positive)*100:.0f}% chance you are healthy.")
...
**not** a 5% chance that $H_0$ is true. To get $P(H_0 \mid \text{data})$
you'd also need the prior plausibility of $H_0$ — which the p-value never
uses.
...
A p-value says nothing about how probable $H_1$ is, and nothing about how
...
was unremarkable under $H_0$ — but as the example just showed, a real
...
Here is the practical reason all of this matters. If $H_0$ is true, a
...
of them to come up "significant" at $\alpha = 0.05$ — by pure luck. Hunt
...
observed_stat = 1.9`}
  initialCode={`# TODO: right-tailed p-value = fraction of null_stats >= observed_stat
p_value = None
`}
...
null_stats = rng.normal(0, 1, size=50_000)
observed_stat = 1.9`}
  initialCode={`# TODO: two-sided p-value = fraction of |null_stats| >= |observed_stat|
p_two_sided = None
`}
...
- A p-value is $P(\text{data this extreme or more} \mid H_0)$ — a **tail
  ...
  by formula.
- It is **not** the probability $H_0$ is true, **not** the probability the
  result is chance, **not** the probability you're wrong, **not**
  $1 - P(H_1)$, **not** the effect size, and **not** the probability of
  replication.
- A small p-value says the data is surprising under $H_0$; it says nothing
