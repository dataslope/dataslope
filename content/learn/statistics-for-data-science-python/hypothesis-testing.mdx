...
$H_0$. It is the skeptic's default — "nothing is going on, the difference
is just chance, the coin is fair, the drug does nothing." Against it
stands the **alternative hypothesis**, $H_1$ (sometimes $H_a$) — "there
*is* an effect."

| Hypothesis | What it says | Examples |
| --- | --- | --- |
| $H_0$ (null) | No effect, no difference, status quo | "The two flows convert equally." "$\mu = 100$." |
| $H_1$ (alternative) | There is an effect or difference | "The new flow converts better." "$\mu \neq 100$." |

The whole procedure is a *proof by contradiction under uncertainty*. We
provisionally believe $H_0$, then ask: **if $H_0$ were true, would data
like mine be surprising?** If it would be very surprising, we conclude
$H_0$ is probably wrong and **reject** it. If the data is perfectly
ordinary under $H_0$, we have no grounds to abandon it — we **fail to
reject** it.
...
one ("something is going on"). $H_0$ is specific enough to compute with —
...
- **Presumption of innocence** = we assume $H_0$ until the data forces us
  off it.
- **Beyond reasonable doubt** = the significance level $\alpha$, the bar
  the evidence must clear.
- **"Guilty"** = reject $H_0$.
- **"Not guilty"** = fail to reject $H_0$ — and crucially, a "not guilty"
...
**Failing to reject $H_0$ does not prove $H_0$ is true.** It means your
data was consistent with $H_0$ — but it would often *also* be consistent
...
sits from what $H_0$ predicts, in units of "ordinary noise." For
comparing two means it's the *t*-statistic, roughly:

$$ t = \frac{\text{observed difference}}{\text{noise in that difference}} $$

A big $t$ (far from zero) means the observed gap dwarfs the typical
random wiggle — surprising under $H_0$. A small $t$ means the gap is the
...
$\alpha$ is the threshold for "too surprising to be coincidence." By
convention it's often **0.05**, meaning: *if $H_0$ were true, I am willing
to wrongly reject it 5% of the time.* You choose $\alpha$ **before**
...
Choosing $\alpha$ (or your hypotheses) *after* seeing the data is a form
...
- $H_0: \mu = 8000$ (the claim holds)
- $H_1: \mu \neq 8000$ (the average is different — two-sided, because
  "different" could be higher or lower)

**Step 3 — Significance level.** $\alpha = 0.05$, fixed now, before we
compute anything.
...
question — "is the data surprising under $H_0$?" — and nothing more.
...
*fails to reject* $H_0$ most of the time, because the gap is the size of
ordinary noise.
...
$H_0$ is true. But run it enough and a "reject" sneaks through. That
occasional false alarm *is* the $\alpha = 0.05$ risk made visible, and
...
- **Two-sided** ($H_1: \mu \neq 8000$): you care about a difference in
  *either* direction. Surprise lives in both tails. This is the safe
  default.
- **One-sided** ($H_1: \mu > 8000$, or $\mu < 8000$): you care about only
...
- You can state $H_0$ and $H_1$ *before* collecting data.
...
irrelevant to the business. Rejecting $H_0$ says "probably not exactly
...
Remember the convention: when the p-value is on the boundary (equal to alpha), we reject.`}
  initCode={`alpha = 0.05
p_value = 0.032`}
  initialCode={`# TODO: set decision to "reject H0" or "fail to reject H0"
decision = None
`}
...
alpha = 0.05`}
  initialCode={`# TODO: run a two-sample t-test; set t_stat, p_value (floats) and significant (bool)
t_stat = None
p_value = None
significant = None
`}
...
1. **It does not prove $H_0$.** "Fail to reject" means insufficient
   evidence, not "no effect." (The not-guilty verdict.)
2. **It does not give you the probability that $H_0$ is true.** The p-value
   assumes $H_0$ and measures the data — it is not $P(H_0 \mid \text{data})$.
...
5. **You can't choose $\alpha$ or the hypotheses after peeking.** The rules
...
- A hypothesis test asks one question: **would data like mine be
  surprising if the skeptical null $H_0$ were true?**
- Assume $H_0$, compute a **test statistic**, compare its **p-value** to a
  pre-chosen **$\alpha$**, then **reject** or **fail to reject**.
- **Fail to reject is not "accept."** Absence of evidence is not evidence
  of absence (the not-guilty verdict).
- Choose **$\alpha$ and your hypotheses before** seeing the data; pick
