...
$$ t = \frac{\text{difference you observed}}{\text{noise in that difference}} $$
...
print(decision, "- the program changed blood pressure." if paired.pvalue <= alpha else "")
`}
/>
...
alpha = 0.05`}
  initialCode={`# TODO: run Welch's two-sample t-test; set p_value (float) and reject (bool)
p_value = None
reject = None
`}
...
alpha = 0.05`}
  initialCode={`# TODO: paired t-test; set p_value (float), mean_improvement (float), verdict (str)
p_value = None
