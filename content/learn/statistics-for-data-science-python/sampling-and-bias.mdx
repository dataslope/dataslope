...
fig = px.line(df, x="n", y="estimate", color="method", markers=True, log_x=True,
...
for n in [200, 5000]:
    ...
    print(f"n = {n}")
    print(f"  random:      center={r.mean():.2f}  spread={r.std():.2f}  "
          f"bias={r.mean()-true_mean:+.2f}")
    print(f"  convenience: center={c.mean():.2f}  spread={c.std():.2f}  "
...
print(f"Survivorship inflation:                     {survivor_avg - true_avg:+.2f} points")
print()
...
conv_weights /= conv_weights.sum()`}
  initialCode={`# TODO: draw each method 300 times at n=800, record sample means,
...
total_n = 600`}
  initialCode={`# TODO: proportional stratified sampling, pooled estimate, and error.
...
print(f"stratified estimate = {strat_estimate:.2f}, true = {true_mean:.2f}, "
