...
for week in range(5):
    ...
    print(f"Week {week+1}: mean = {times.mean():6.2f} ms   "
...
print(f"\\nObserved gap of {observed_gap:+.2f} -> "
      f"plausibly just noise? {plausibly_noise}")
`}
/>
...
$$ \text{what you observe} = \text{signal} + \text{noise} $$
...
many_means = np.array([rng.normal(200, 25, size=300).mean()
                       for _ in range(2000)])

fig = px.histogram(many_means, nbins=40,
...
fig.add_vline(x=point_estimate, line_dash="dash",
              annotation_text="your one sample's mean")
fig.show()

print(f"Point answer:        {point_estimate:.2f} ms")
print(f"Distribution answer: most plausible means fall in "
...
def draw_sample():
  ...
  initialCode={`# TODO: draw 2000 samples, take each one's mean, collect into 'means'
