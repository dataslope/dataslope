...
*statistic* (like $\bar{x}$) computed over *many repeated samples*.
...
1. **The population distribution** is the spread of the raw values across
   ...
   diameter. It has a mean $\mu$ and standard deviation $\sigma$. It is
...
3. **The sampling distribution of a statistic** is something else
   ...
   the sample mean $\bar{x}$ — computed over **many different samples**,
...
fig = px.histogram(means, nbins=50,
...
print(f"SD of the sample means          = {means.std():.2f}   (MUCH smaller than sigma)")
`}
/>

Two facts jump out, and they are the two pillars of inference:

- **The sampling distribution is centered on $\mu$.** The mean of all the
...
- **The sampling distribution is much narrower than the population.** Its
  spread (here ~3) is far smaller than $\sigma$ (here 18). Averaging $n$
...
fig = px.histogram(df, x="value", facet_row="what", nbins=50, height=650,
...
print(f"(b) one sample raw values   -> sd = {one_sample.std():5.2f}  (~ matches population)")
...
hugging $\mu$. The raw data spreads over tens of units; the *mean*
...
on $\mu$ regardless. What changes is the **spread**: bigger samples
produce sample means that cluster more tightly around $\mu$. Let's vary
...
fig = px.histogram(df, x="x_bar", color="n", barmode="overlay", nbins=70,
                   opacity=0.6, title="Bigger n -> narrower sampling distribution (same center)",
...
for stat_name, fn in [("mean", np.mean), ("median", np.median), ("max", np.max)]:
    vals = np.array([
        fn(rng.choice(population, size=n, replace=False))
        for _ in range(4000)
    ])
    for v in vals:
...
df = pd.DataFrame(rows)
fig = px.histogram(df, x="value", facet_col="statistic", nbins=50,
...
n = 50
n_samples = 3000`}
  initialCode={`# TODO: build 'means' (array of 3000 sample means) and 'summary' dict.
means = None
...
n_samples = 2000`}
  initialCode={`# TODO: compute sd of sample means at each n, and their ratio.
result = {
    "sd_small": None,
    "sd_big": None,
    "ratio": None,
