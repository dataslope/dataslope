...
print("Mean of zip_code:    ", df["zip_code"].mean(),
      "  <- NONSENSE (zips are labels)")
print("Mean of rating_1to5: ", df["rating_1to5"].mean(),
      "      <- shaky (ordinal: gaps not equal)")
print("Mean of price_usd:   ", round(df["price_usd"].mean(), 2),
      "  <- valid (ratio scale)")
print()
...
print()
print("region_code dtype is now:", df["region_code"].dtype,
...
df = pd.DataFrame({
  ...
  initialCode={`# TODO: classify each column by its STATISTICAL type (not its dtype)
types = {
    "customer_id":     None,
    "country":         None,
    "satisfaction":    None,
...
df = pd.DataFrame({
  ...
  initialCode={`# TODO: pick the summary that MATCHES each column's type
summary = {
    "country_mode":  None,
    "rating_median": None,
    "price_mean":    None,
