...
For a **continuous** variable, asking $P(X = 3.14159\ldots)$ exactly is
...
For continuous variables, $P(X = \text{exact value}) = 0$, always.
...
deviation** $\text{SD}(X) = \sqrt{\text{Var}(X)}$, which is back in the
...
\\$40? It's just $1000 \times 40 = \$40,000$, regardless of how
...
probs  = np.array([0.40, 0.25, 0.20, 0.10, 0.05])`}
  initialCode={`# TODO: compute ex (E[X]) and varx (Var(X)) as floats
ex = None
varx = None
`}
...
p = [0.05, 0.20, 0.75]
net_payoff = [45, 5, -5]`}
  initialCode={`# TODO: compute ev (expected net payoff) and favorable (bool: ev > 0)
ev = None
favorable = None
`}
