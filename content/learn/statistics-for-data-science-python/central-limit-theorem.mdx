...
1. **It's about the *mean's* distribution**, not the data. The raw data
   ...
   distribution of $\bar{x}$* turns normal.
...
for n in [1, 5, 30, 100]:
    ...
    print(f"n = {n:3d}:  mean of x-bars = {means.mean():5.2f}  "
          f"sd = {means.std():5.2f}  skewness = {skew:+.2f}")

df = pd.DataFrame(rows)
fig = px.histogram(df, x="x_bar", facet_col="panel", facet_col_wrap=2,
                   nbins=50, height=620,
...
populations = {
    ...
    "bimodal (two humps)":        np.concatenate([
                                      rng.normal(5, 1.5, N // 2),
                                      rng.normal(15, 1.5, N // 2)]),
}
...
for name, pop in populations.items():
    ...
    fig.add_histogram(x=z, nbinsx=60, opacity=0.55, name=name,
                      histnorm="probability density")
    print(f"{name:30s} -> skewness of x-bar = {stats.skew(means):+.2f}")

# overlay the standard normal curve
xs = np.linspace(-4, 4, 200)
fig.add_scatter(x=xs, y=stats.norm.pdf(xs), mode="lines",
                name="standard normal", line=dict(color="black", width=3))
...
You'll hear "$n \ge 30$ and you're fine." Treat it as a *loose* guideline,
...
- Nearly symmetric population → the mean is approximately normal at very
  small $n$ (even $n = 5$).
- Mildly skewed → $n \approx 30$ is genuinely fine.
...
fig = px.line(x=ns, y=theory, markers=True,
              labels={"x": "sample size n", "y": "SD of x-bar"},
              title="SD of x-bar follows sigma / sqrt(n)")
fig.add_scatter(x=ns, y=empirical, mode="markers", name="simulated",
...
ns = [2, 10, 50, 200]
n_samples = 3000`}
  initialCode={`# TODO: for each n, build the sampling distribution of the mean and
...
n = 64
n_samples = 5000`}
  initialCode={`# TODO: build sample means, compute empirical sd, theory sd = SIGMA/sqrt(n),
