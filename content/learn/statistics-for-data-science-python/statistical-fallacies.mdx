...
**What it is.** If you run *one* test at $\alpha = 0.05$, there's a 5%
...
- **Correct for multiple comparisons** (e.g., Bonferroni: use
  $\alpha / m$ for $m$ tests; or control the false discovery rate).
...
<CodeBlock
  adapter="python"
  initialCode={`# Disease prevalence and test accuracy.
...
mean — **with no intervention required**.
