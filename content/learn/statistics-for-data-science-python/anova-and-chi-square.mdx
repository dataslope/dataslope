...
print(f"Means -> east {east.mean():.1f}, west {west.mean():.1f}, south {south.mean():.1f}")
...
- $H_1$: **at least one** group mean differs from the rest.

A large F with a small p-value says the group means are spread out more
than within-group noise can explain — so you reject $H_0$ and conclude
...
yes/no churn, survey response. Chi-square ("$\chi^2$") tests are built
...
- $H_1$: they are **associated** (not independent).
...
The hypotheses: $H_0$ says the counts follow the expected proportions;
$H_1$ says they don't. Same logic as before — observed vs expected — just
...
alpha = 0.05`}
  initialCode={`# TODO: one-way ANOVA; set F_stat, p_value (floats) and conclusion (str)
F_stat = None
p_value = None
conclusion = None
`}
...
alpha = 0.05`}
  initialCode={`# TODO: build 'table' with crosstab, run chi-square, set p_value (float) and associated (bool)
table = None
p_value = None
associated = None
`}
