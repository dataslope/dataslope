...
So $P(\text{even}) = 3/6 = 0.5$. That counting shortcut only works under
...
**1. The range rule.** Every probability satisfies $0 \le P(A) \le 1$,
...
$$
P(A \text{ or } B) = P(A) + P(B) \quad\text{(when A, B can't co-occur)}
$$
...
$$
P(A \text{ and } B) = P(A) \times P(B) \quad\text{(when A, B are independent)}
$$

This is why a run of outcomes gets unlikely fast: two fair-coin heads is
$0.5 \times 0.5 = 0.25$; ten in a row is $0.5^{10} \approx 0.001$.
...
You could multiply ($0.8^3 = 0.512$) — but watch how simulation gets the
...
<CodeBlock
  adapter="python"
  initialCode={`def prob_to_odds(p):
...
n = 200_000`}
  initialCode={`# TODO: simulate n rolls of two dice and estimate P(sum == 7) as a float p7
p7 = None
`}
...
n_attempts = 4
n_trials = 100_000`}
  initialCode={`# TODO: compute p_exact via the complement rule, and p_sim via simulation
p_exact = None
p_sim = None
`}
