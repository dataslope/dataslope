...
*estimator*; the parameter is the *estimand*. You use $\bar{x}$ to
estimate $\mu$, $\hat{p}$ to estimate $p$, and so on.
...
$\bar{x}=48.20$ from a sample, and the population $\mu$ is *near* it but
uncertain. The honest sentence names the estimate and its error:
"$\bar{x}=\\$48.20$, and $\mu$ is plausibly within \\$45–\\$51." Collapsing
...
population $\sigma$, use `ddof=1` (dividing by $n-1$). Pandas' `.std()`
...
sample_means = np.array([rng.normal(MU_TRUE, SIGMA_TRUE, 50).mean()
                         for _ in range(1000)])

fig = px.histogram(sample_means, nbins=40,
...
First, the sample means **pile up around the true $\mu$** — the
...
fig = px.line(x=sizes, y=errors, markers=True, log_x=True,
              labels={"x": "Sample size n (log scale)",
                      "y": "|x-bar - mu| (estimation error)"},
...
detail: precision improves with $\sqrt{n}$, **not** $n$. To halve your
...
with the standard error formula $\sigma/\sqrt{n}$ in *Standard Error*.
...
$\mu$, $\sigma$, and $p$ are properties of the population and don't
care how much you sampled. What grows with $n$ is the **precision of
your estimate**: $\bar{x}$ clusters more tightly around the unchanged
$\mu$. The target stays put; your aim gets steadier.
</Callout>
...
sample = rng.normal(MU_TRUE, SIGMA_TRUE, size=120)`}
  initialCode={`# TODO: build the 'estimates' dict from the sample only
estimates = {
    "x_bar": None,
    "s": None,
    "mean_error": None,
...
You computed statistics ($\bar{x}$, $s$, $\hat{p}$) from a sample and
compared them to parameters ($\mu$, $\sigma$, $p$) you'd normally never
...
- A **parameter** ($\mu$, $\sigma$, $p$) describes the population — fixed but unknown. A **statistic** ($\bar{x}$, $s$, $\hat{p}$) is computed from the sample — known but varies.
...
- Bigger $n$ **sharpens the estimate** (precision $\propto \sqrt{n}$); it does **not** change the population or its parameters.
