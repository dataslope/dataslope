...
$P(X = \text{exact value}) = 0$ for continuous variables. You always ask
about ranges.
...
The **cumulative distribution function (CDF)** gives $P(X \le x)$ — the
...
curve, defined by a mean $\mu$ (center) and standard deviation $\sigma$
...
`ppf(0.9)` returns the value $x$ such that $P(X \le x) = 0.9$ — the 90th
...
mu, sigma = 70, 8`}
  initialCode={`# TODO: compute P(60 < X < 80)
p_between = None
...
- Store the answer as a plain Python \`float\` in **\`cutoff\`**.`}
  initCode={`from scipy import stats
mean_wait = 4`}
  initialCode={`# TODO: find the 90th-percentile wait time
cutoff = None
...
- The **PDF height is a density, not a probability** — it can exceed 1.
  $P(X = \text{exact value}) = 0$; always ask about intervals.
