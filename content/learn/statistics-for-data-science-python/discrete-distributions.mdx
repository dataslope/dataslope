...
n = 500
p = 0.04`}
  initialCode={`# TODO: build the distribution and compute the two values
expected = None
p_at_least_25 = None
...
independently at a constant average **rate** $\lambda$ (lambda). There's
...
both equal $\lambda$. If your event counts have a variance much larger
...
- **The rate isn't constant**: traffic at noon ≠ traffic at 3 a.m.
  Pooling them into one $\lambda$ smears two different processes.
...
- \`P(X > 9)\` is exactly \`d.sf(9)\` (the survival function).`}
  initCode={`from scipy import stats
rate = 4.5`}
  initialCode={`# TODO: build the Poisson model and compute the two values
mean_errors = None
p_more_than_9 = None
...
p = 0.2
d = stats.geom(p)        # scipy's geom counts trials 1, 2, 3, ... (the trial of the first success)
