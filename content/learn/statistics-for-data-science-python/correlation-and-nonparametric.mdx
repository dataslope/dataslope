...
The p-value tests $H_0$: *the true correlation is 0*. A small p says the
...
strong **curved** one. The textbook example is a U-shape (like $y = x^2$
...
print(f"R-squared : {fit.rvalue**2:.3f}  ({fit.rvalue**2:.0%} of score variance explained)")
...
fig = px.scatter(x=hours, y=score, labels={"x": "hours studied", "y": "exam score"},
...
print(f"Wilcoxon signed-rank: statistic = {w_res.statistic:.1f}, p = {w_res.pvalue:.4f}")
print()
...
y = x**3 + rng.normal(0, 30, size=70)`}
  initialCode={`# TODO: set pearson_r, spearman_r, pearson_p (floats) and spearman_stronger (bool)
pearson_r = None
spearman_r = None
pearson_p = None
...
alpha = 0.05`}
  initialCode={`# TODO: run the right nonparametric test for INDEPENDENT groups
