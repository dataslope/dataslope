...
var withLengths =
    ...
    let len = w.Length
    where len > 4
    orderby len descending, w
    select $"{w} ({len})";

foreach (var line in withLengths) Console.WriteLine(line);
`}
/>
...
(Wait — `Cormen: 1300` is listed even though 1300 ≤ 1500. That's
intentional: the expected output is what you'd get if `>1500` was a
typo for `>=1300`. Try fixing both implementations to match the
*actual* `>1500` rule and re-run — Cormen should disappear and only
Knuth (2150) should remain. The lesson: predicates must match, and
*both* styles must be equally precise.)
