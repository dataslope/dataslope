...
foreach (var s in summary1) Console.WriteLine($"{s.Name} adult? {s.IsAdult}");
...
var orders = new[]
{
    new Order("Widget",  9.99m, 3),
    new Order("Gadget", 14.50m, 1),
    new Order("Widget", 12.00m, 2),
    new Order("Gizmo",  19.99m, 0),
    new Order("Widget",  8.50m, 4),
};

// All widget orders with at least one unit, projected to (price, total).
var widgetTotals = orders
