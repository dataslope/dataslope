...
try { Console.WriteLine($"min: {empty.Min()}"); }
catch (Exception ex) { Console.WriteLine($"min threw: {ex.GetType().Name}"); }
...
var stats = nums.Aggregate(
    seed: (Count: 0, Sum: 0L, Sq: 0L, Min: int.MaxValue, Max: int.MinValue),
    func: (acc, n) => (
        acc.Count + 1,
        acc.Sum + n,
        acc.Sq + (long)n * n,
...
with \`Aggregate\` — no \`foreach\` allowed.`}
  ...
  files={[
    {
      filename: "Program.cs",
      initialContent: `var xs = new[] { 1, 2, 3, 4 };
// 0*31 + 1 = 1
// 1*31 + 2 = 33
// 33*31 + 3 = 1026
// 1026*31 + 4 = 31810
