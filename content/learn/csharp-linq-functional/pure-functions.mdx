...
static List<int> Doubled(IEnumerable<int> xs)
{
    ...
    foreach (var x in xs) result.Add(x * 2);
    return result;
}
...
static Func<TArg, TRes> Memoize<TArg, TRes>(Func<TArg, TRes> f) where TArg : notnull
{
...
int callCount = 0;
int Slow(int x) { callCount++; return x * x; }
...
\`Program.cs\` shell calls them and prints the results.`}
  ...
  files={[
    {
      filename: "Program.cs",
      initialContent: `var xs = new[] { 1, 2, 3, 4, 5 };
