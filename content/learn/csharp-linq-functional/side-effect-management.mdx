...
public static class SalesParser
{
    public static IEnumerable<decimal> Parse(IEnumerable<string> lines) =>
        lines
            .Select(line => decimal.TryParse(line, out var n) ? (decimal?)n : null)
            .Where(n => n is > 0)
            .Select(n => n!.Value);
}
...
foreach (var p in positives) Console.WriteLine($"keeping {p}");
...
public static class Subs
{
    ...
    public static IEnumerable<Subscription> ExpiringSoon(IEnumerable<Subscription> subs, DateTime now) =>
        subs.Where(s => s.ExpiresOn <= now.AddDays(7));
}
