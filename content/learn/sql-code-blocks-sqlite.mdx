...
INSERT INTO books (title, author, year) VALUES
  ...
  starterCode={`SELECT title, author, year
FROM books
ORDER BY year;`}
/>
...
<SqlCodeBlock
  ...
  starterCode={`WITH RECURSIVE seq(n) AS (
  SELECT 1
  UNION ALL
...
INSERT INTO books (id, author_id, title, year) VALUES
  ...
  starterCode={`SELECT a.name AS author, b.title, b.year
FROM books b
...
SELECT n, 'row-' || n, ROUND(n * 1.5, 2) FROM seq;`}
  starterCode={`SELECT id, label, value
FROM metrics
ORDER BY id;`}
/>
