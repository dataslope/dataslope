...
<CodeBlock
  adapter="typescript"
  initialCode={`type Result<E, A> = { kind: "ok"; value: A } | { kind: "err"; errors: ReadonlyArray<E> };
...
function map2<E, A, B, C>(
  ...
  if (ra.kind === "err" && rb.kind === "err") return err(...ra.errors, ...rb.errors);
...
(async () => {
  ...
  console.log(await flatMapP(Promise.resolve(2), (n) => Promise.resolve(n + 1)));
})();
`}
/>
