...
<CodeBlock
  adapter="typescript"
  initialCode={`type User = { id: string; name: string; age: number };
...
<CodeBlock
  adapter="typescript"
  initialCode={`type User = { readonly id: string; readonly name: string; readonly age: number };
...
<CodeBlock
  adapter="typescript"
  initialCode={`// readonly on a single field
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Address = { readonly city: string; readonly country: string };
type User = {
  readonly id: string;
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Todo = { readonly id: number; readonly text: string; readonly done: boolean };
...
const withNew: readonly Todo[] = [...remaining, { id: 4, text: "Refactor", done: false }];
...
<CodeBlock
  adapter="typescript"
  initialCode={`type State = { readonly counter: number; readonly note: string };

// Pure transformations
const inc       = (s: State): State => ({ ...s, counter: s.counter + 1 });
const dec       = (s: State): State => ({ ...s, counter: s.counter - 1 });
const setNote   = (note: string) => (s: State): State => ({ ...s, note });
...
<CodeBlock
  adapter="typescript"
  initialCode={`// 1. Methods that mutate AND return a value (lying about purity)
const xs = [3, 1, 2];
...
<CodeBlock
  adapter="typescript"
  initialCode={`// 2. Date objects are mutable
...
`,
    ...
    {
      filename: "cart.ts",
      initialContent: `export type Item = { readonly id: string; readonly qty: number };
...
`,
      solutionContent: `export type Item = { readonly id: string; readonly qty: number };
export type Cart = readonly Item[];
