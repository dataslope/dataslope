...
<CodeBlock
  adapter="typescript"
  initialCode={`// PURE: depends only on x and y, returns a value, touches nothing else.
...
const appendOne = (xs: number[]): void => { xs.push(1); };
...
<CodeBlock
  adapter="typescript"
  initialCode={`const add = (x: number, y: number): number => x + y;
...
<CodeBlock
  adapter="typescript"
  initialCode={`function memoize<A, B>(f: (a: A) => B): (a: A) => B {
...
let calls = 0;
const slow = (x: number): number => { calls++; return x * x; };
...
[Functional Error Handling](./functional-error-handling) how to model
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Impure: logs, reads the clock, returns void
...
recordPurchase("u-2", 49.00);
`}
/>

<CodeBlock
  adapter="typescript"
  initialCode={`// Pure: builds a value
type Purchase = {
  readonly id: string;
...
const p2 = makePurchase("u-2", 49.00, 1_700_000_000_001, "def");
...
\`\`\``}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { sumOfSquares, meanOfSquares } from "./stats";
...
`,
    ...
    {
      filename: "stats.ts",
      initialContent: `export function sumOfSquares(xs: readonly number[]): number {
  // your code here
  return 0;
}
...
`,
      solutionContent: `export function sumOfSquares(xs: readonly number[]): number {
