...
<CodeBlock
  adapter="typescript"
  initialCode={`type State = { readonly count: number };
type Action =
  | { kind: "inc" }
  | { kind: "dec" }
  | { kind: "add"; by: number }
...
const history: Action[] = [
  { kind: "inc" }, { kind: "inc" }, { kind: "add", by: 10 },
  { kind: "reset" }, { kind: "inc" },
];
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Doc = { readonly text: string };
...
const actions: Action[] = [
  { kind: "type", ch: "h" }, { kind: "type", ch: "i" },
  { kind: "undo" }, { kind: "undo" },
  { kind: "redo" }, { kind: "type", ch: "!" },
];
...
<CodeBlock
  adapter="typescript"
  initialCode={`// State<S, A> = a function from state to [value, newState]
type State<S, A> = (s: S) => readonly [A, S];

const of  = <S, A>(a: A): State<S, A> => (s) => [a, s];

const map = <S, A, B>(sa: State<S, A>, f: (a: A) => B): State<S, B> =>
  (s) => { const [a, s2] = sa(s); return [f(a), s2]; };

const chain = <S, A, B>(sa: State<S, A>, f: (a: A) => State<S, B>): State<S, B> =>
  (s) => { const [a, s2] = sa(s); return f(a)(s2); };
