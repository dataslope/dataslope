...
<CodeBlock
  adapter="typescript"
  initialCode={`const id = <A,>(a: A): A => a;
...
<CodeBlock
  adapter="typescript"
  initialCode={`// All of these are fully parametric: they make no assumptions
...
const reverse = <A,>(xs: ReadonlyArray<A>): ReadonlyArray<A> => [...xs].reverse();
...
<CodeBlock
  adapter="typescript"
  initialCode={`type Ordering = -1 | 0 | 1;
...
const people: ReadonlyArray<readonly [string, number]> = [
  ["Ada", 36], ["Linus", 36], ["Grace", 85], ["Ada", 30],
];
