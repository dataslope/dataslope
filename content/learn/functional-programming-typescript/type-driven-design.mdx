...
<CodeBlock
  adapter="typescript"
  initialCode={`// ✗ WEAK MODEL — many illegal combinations
type ProfileWeak = {
  loading: boolean;
...
<CodeBlock
  adapter="typescript"
  initialCode={`// ✓ TIGHT MODEL — only valid states can be written
type Profile<A> =
  | { kind: "idle" }
  | { kind: "loading" }
  | { kind: "loaded";  data: A }
...
<CodeBlock
  adapter="typescript"
  initialCode={`// ✗ VALIDATE — the constraint is checked, then immediately forgotten
