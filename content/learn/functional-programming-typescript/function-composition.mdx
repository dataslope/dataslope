...
<CodeBlock
  adapter="typescript"
  initialCode={`function compose2<A, B, C>(g: (b: B) => C, f: (a: A) => B): (a: A) => C {
  return (a) => g(f(a));
}
...
<CodeBlock
  adapter="typescript"
  initialCode={`function pipe<A>(a: A): A;
...
<CodeBlock
  adapter="typescript"
  initialCode={`function compose<A>(): (a: A) => A;
function compose<A, B>(f1: (a: A) => B): (a: A) => B;
function compose<A, B, C>(f2: (b: B) => C, f1: (a: A) => B): (a: A) => C;
function compose<A, B, C, D>(f3: (c: C) => D, f2: (b: B) => C, f1: (a: A) => B): (a: A) => D;
...
<CodeBlock
  adapter="typescript"
  initialCode={`function pipe<T>(x: T, ...fns: ((x: T) => T)[]): T {
...
<CodeBlock
  adapter="typescript"
  initialCode={`const xs = ["  Hello  ", "  Functional  ", "  Programming  "];
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Uncurried (the usual way)
...
<CodeBlock
  adapter="typescript"
  initialCode={`// A small "partial" helper for ordinary functions
...
type Order = { readonly id: string; readonly status: "paid" | "draft" | "cancelled"; readonly total: number; readonly customer: string };
...
const onlyPaid    = <T extends { status: string }>(xs: readonly T[]) => xs.filter((x) => x.status === "paid");
const overAmount  = (n: number) => <T extends { total: number }>(xs: readonly T[]) => xs.filter((x) => x.total > n);
const customers   = <T extends { customer: string }>(xs: readonly T[]) => xs.map((x) => x.customer);
...
\`\`\``}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `import { slug } from "./text";
...
`,
    ...
    {
      filename: "text.ts",
      initialContent: `export function pipe<T>(x: T, ...fns: ((x: T) => T)[]): T {
  return fns.reduce((acc, f) => f(acc), x);
}
...
`,
      solutionContent: `export function pipe<T>(x: T, ...fns: ((x: T) => T)[]): T {
  return fns.reduce((acc, f) => f(acc), x);
}
