...
<CodeBlock
  adapter="typescript"
  initialCode={`// --- CORE: pure types and functions ---
...
type Summary = {
  ...
  readonly invoices: ReadonlyArray<{ id: string; status: "paid" | "due" | "overdue" }>;
};
...
\`\`\``}
  ...
  files={[
    {
      filename: "main.ts",
      initialContent: `// SHELL: read inputs, call the core, log the result
...
`,
    ...
    {
      filename: "core.ts",
      initialContent: `// CORE: pure, no I/O
...
`,
      solutionContent: `export type Order = { readonly customerId: string; readonly amount: number };
