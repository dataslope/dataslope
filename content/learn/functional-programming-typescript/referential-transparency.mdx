...
<CodeBlock
  adapter="typescript"
  initialCode={`// Pure: referentially transparent
...
<CodeBlock
  adapter="typescript"
  initialCode={`const double = (x: number): number => x * 2;
...
const fastFib = memoize<number, number>((n) => n < 2 ? n : fastFib(n - 1) + fastFib(n - 2));
...
<CodeBlock
  adapter="typescript"
  initialCode={`function distance(a: { x: number; y: number }, b: { x: number; y: number }): number {
  // dx and dy are computed twice in the naive version
  return Math.sqrt((a.x - b.x) ** 2 + (a.y - b.y) ** 2);
}

function distanceCSE(a: { x: number; y: number }, b: { x: number; y: number }): number {
  const dx = a.x - b.x;
  const dy = a.y - b.y;
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Innocent-looking but NOT referentially transparent
...
<CodeBlock
  adapter="typescript"
  initialCode={`// Before: not referentially transparent — output depends on the RNG
...
<CodeBlock
  adapter="typescript"
  initialCode={`// After: pure. The randomness is now an explicit input.
...
console.log(pickWinner(["A", "B", "C"], 0.10));  // "A"
console.log(pickWinner(["A", "B", "C"], 0.50));  // "B"
