...
<CodeBlock
  adapter="javascript"
  initialCode={`const user = { name: "Ada", age: 28, role: "admin" };
...
<CodeBlock
  adapter="javascript"
  initialCode={`// Find the first number bigger than 50.
...
<CodeBlock
  adapter="javascript"
  initialCode={`const numbers = [10, 22, 3, 47, 9];
...
<CodeBlock
  adapter="javascript"
  initialCode={`// A small multiplication table.
