...
<CodeBlock
  adapter="javascript"
  initialCode={`// TypeError: Cannot read properties of undefined (reading 'name')
const user = undefined;
try { console.log(user.name); } catch (e) { console.log("1:", e.message); }

// ReferenceError: x is not defined
try { console.log(notDeclared); } catch (e) { console.log("2:", e.message); }
...
try { recurse(); } catch (e) { console.log("3:", e.message); }
`}
/>
...
<CodeBlock
  adapter="javascript"
  initialCode={`function withdraw(account, amount) {
...
try { withdraw(acct, 999); } catch (e) { console.log(e.message); }
try { withdraw(acct, -5);  } catch (e) { console.log(e.message); }
`}
/>
...
`,
    ...
    {
      filename: "pricing.js",
      initialContent: `// Apply a percentage discount to every item, and return the total.
...
- It must not mutate the input array.`}
  ...
  let total = 0;
  for (let i = 0; i <= nums.length; i++) {  // bug-ish?
