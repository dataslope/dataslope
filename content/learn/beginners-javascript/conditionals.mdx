...
<CodeBlock
  adapter="javascript"
  initialCode={`function describeScore(score) {
  if (score >= 90) {
    return "A";
  } else if (score >= 80) {
    return "B";
...
<CodeBlock
  adapter="javascript"
  initialCode={`// Nested style — hard to read past a few levels.
...
<CodeBlock
  adapter="javascript"
  initialCode={`function describeDay(day) {
  switch (day) {
    case "Mon":
    case "Tue":
    case "Wed":
...
<CodeBlock
  adapter="javascript"
  initialCode={`const ROLES_TO_GREETING = {
...
<CodeBlock
  adapter="javascript"
  initialCode={`function canRent(age, hasLicence, hasCard) {
...
<CodeBlock
  adapter="javascript"
  initialCode={`for (let n = 1; n <= 30; n++) {
  let out;
  if (n % 15 === 0) {
    out = "FizzBuzz";
  } else if (n % 3 === 0) {
...
Use comparisons and conditionals, not a lookup table — practise the syntax.`}
  initialCode={`function action(color) {
  // your code here
  return "";
}
