...
<CodeBlock
  adapter="javascript"
  initialCode={`function later(value, ms) {
...
<CodeBlock
  adapter="javascript"
  initialCode={`function fakeFetch(url, ms) {
  return new Promise((resolve) =>
...
<CodeBlock
  adapter="javascript"
  initialCode={`const p = (v, ms, fail = false) =>
...
<CodeBlock
  adapter="javascript"
  initialCode={`async function fetchValue() {
  return 42;
}
...
<CodeBlock
  adapter="javascript"
  initialCode={`function wait(ms) { return new Promise(r => setTimeout(r, ms)); }
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      initialContent: `const { getUserAndPosts } = require("./client");
...
`,
    ...
    {
      filename: "client.js",
      initialContent: `// Simulated network helpers — both return Promises.
...
- Keep track of the last error so you can throw it at the end.`}
  initialCode={`async function retry(fn, attempts) {
  // your code here
}
