...
<CodeBlock
  adapter="javascript"
  initialCode={`// HARD: every name is generic. The reader has to decode the math.
function f(a, b) {
  let t = 0;
...
<CodeBlock
  adapter="javascript"
  initialCode={`// HARD: one function juggling four jobs.
...
<CodeBlock
  adapter="javascript"
  initialCode={`// HARD: rightward drift.
...
<CodeBlock
  adapter="javascript"
  initialCode={`// HARD: what is 0.0825? why 86400000?
...
<CodeBlock
  adapter="javascript"
  initialCode={`// BAD: swallow the error silently. You'll never know it happened.
...
<CodeBlock
  adapter="javascript"
  initialCode={`// At the boundary: validate.
...
try { isAdult(parseAge("oops")); } catch (e) { console.log(e.message); }
`}
/>
...
- Otherwise → \`"OK: <name>"\``}
  initialCode={`function enrollStudent(student) {
  // your refactor here
}
