...
<CodeBlock
  adapter="javascript"
  initialCode={`console.log("1: requesting");
...
<CodeBlock
  adapter="javascript"
  initialCode={`function fakeFetchUser(id, callback) {
...
<CodeBlock
  adapter="javascript"
  initialCode={`function later(value, ms, cb) {
...
<CodeBlock
  adapter="javascript"
  initialCode={`function loadAll(urls, done) {
...
This is a small but realistic exercise in "callback orchestration".`}
  initialCode={`function runSequentially(tasks, done) {
  // your code here
}
