...
<CodeBlock
  adapter="javascript"
  initialCode={`// Without functions — the formula is repeated and unnamed.
const bill1 = 80;
...
<CodeBlock
  adapter="javascript"
  initialCode={`// With a function — the formula has a name and lives in one place.
...
<CodeBlock
  adapter="javascript"
  initialCode={`// Three equivalent ways to define the same function:
...
<CodeBlock
  adapter="javascript"
  initialCode={`// A function in a variable.
...
<CodeBlock
  adapter="javascript"
  initialCode={`// Pure: depends only on its inputs, returns a value, no side effects.
function add(a, b) {
  return a + b;
}
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      initialContent: `const { totalWithTip, perPerson } = require("./bill");
...
`,
    ...
    {
      filename: "bill.js",
      initialContent: `function totalWithTip(bill, tipRate) {
  return bill + bill * tipRate;
}
...
- \`"obese"\` if bmi ≥ 30`}
  initialCode={`function bmi(weightKg, heightM) {
  // your code here
  return 0;
}
