...
<CodeBlock
  adapter="javascript"
  initialCode={`// PURE: depends only on its arguments, returns a value, changes nothing.
function add(a, b) {
  return a + b;
}
...
<CodeBlock
  adapter="javascript"
  initialCode={`// MUTATING style: change the array in place.
const nums = [1, 2, 3];
...
<CodeBlock
  adapter="javascript"
  initialCode={`// Pass a function as an argument.
const nums = [1, 2, 3, 4];
...
<CodeBlock
  adapter="javascript"
  initialCode={`const trim       = (s) => s.trim();
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      initialContent: `// IMPURE shell: reads input, calls the pure core, writes output.
...
`,
    ...
    {
      filename: "summary.js",
      initialContent: `// PURE core: data in, data out. No console.log, no I/O.
...
This is a classic immutable update.`}
  initialCode={`function incrementScores(students, name) {
