...
<CodeBlock
  adapter="javascript"
  initialCode={`function busyWait(ms) {
...
<CodeBlock
  adapter="javascript"
  initialCode={`console.log("1: script start");
...
<CodeBlock
  adapter="javascript"
  initialCode={`async function demo() {
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      initialContent: `const { runDemo } = require("./scheduling");
...
`,
    },
    {
      ...
      initialContent: `function runDemo() {
...
- Use a promise that resolves once everything has been pushed, and return it. The tests will \`await\` it.`}
  initialCode={`function schedule(log) {
  // your code here
