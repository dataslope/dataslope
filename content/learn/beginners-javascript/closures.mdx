...
<CodeBlock
  adapter="javascript"
  initialCode={`// Build an array of functions, one per number 0..4.
const fs = [];
for (let i = 0; i < 5; i++) {
  fs.push(function () {
    return i;
...
<CodeBlock
  adapter="javascript"
  initialCode={`function makeTaxCalculator(rate) {
...
The starting value of the counter is the \`start\` argument (default \`0\`). Each call to \`makeCounter\` should produce an independent counter.`}
  initialCode={`function makeCounter(start = 0) {
