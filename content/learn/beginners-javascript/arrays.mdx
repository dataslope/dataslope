...
<CodeBlock
  adapter="javascript"
  initialCode={`const xs = [10, 20, 30, 40];
...
<CodeBlock
  adapter="javascript"
  initialCode={`const xs = ["a", "b", "c", "d", "e"];
...
<CodeBlock
  adapter="javascript"
  initialCode={`const xs = [3, 1, 4, 1, 5, 9, 2, 6];
...
<CodeBlock
  adapter="javascript"
  initialCode={`const xs = [10, 20, 30, 40, 50];
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      initialContent: `const { stats } = require("./stats");
...
`,
    ...
    {
      filename: "stats.js",
      initialContent: `function stats(xs) {
  if (xs.length === 0) {
...
Hint: use the modulo operator (\`%\`) to handle \`k\` larger than the array length, then combine \`slice\` with the spread operator.`}
  initialCode={`function rotate(xs, k) {
  // your code here
  return xs;
}
