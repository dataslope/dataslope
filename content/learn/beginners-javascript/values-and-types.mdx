...
<CodeBlock
  adapter="javascript"
  initialCode={`console.log(typeof 42);            // "number"
...
<CodeBlock
  adapter="javascript"
  initialCode={`console.log("5" + 3);    // "53"  — number 3 was converted to the string "3", then strings were joined.
...
<CodeBlock
  adapter="javascript"
  initialCode={`let x;                         // declared but not assigned
...
<CodeBlock
  adapter="javascript"
  initialCode={`function describe(value) {
  if (value) {
