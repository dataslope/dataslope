...
try { /* important work */ }
catch (FormatException ex) { Console.WriteLine($"bad input: {ex.Message}"); }
`}
/>
...
try { Risky(); }
catch (Exception ex) { Console.WriteLine($"top level got: {ex.Message}"); }
`}
/>
...
`,
    },
    {
      ...
      initialContent: `public static class SafeMath
{
...
`,
      solutionContent: `public static class SafeMath
{
    public static int SumNumbers(string[] inputs)
    {
        int total = 0;
