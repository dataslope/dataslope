...
void DescribeNumbers(int[] nums)
{
    ...
    int min = nums[0];
    foreach (var n in nums) if (n < min) min = n;
...
int Sum(int[] xs)  { int s = 0; foreach (var n in xs) s += n; return s; }
int Min(int[] xs)  { int m = xs[0]; foreach (var n in xs) if (n < m) m = n; return m; }
int Max(int[] xs)  { int m = xs[0]; foreach (var n in xs) if (n > m) m = n; return m; }
