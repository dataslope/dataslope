...
INSERT INTO orders_flat VALUES
  ...
  initialCode={`-- A rushed update changes only one of Ada's rows.
UPDATE orders_flat
SET customer_email = 'ada@new.test'
WHERE order_id = 1;

-- The same customer now appears with two different emails.
SELECT customer_name, customer_email, COUNT(*) AS rows_with_that_email
FROM orders_flat
