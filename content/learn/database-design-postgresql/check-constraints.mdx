...
VALUES ('Notebook', 4.50, 'active');`}
  initialCode={`-- Negative prices are not valid products.
INSERT INTO products (name, price, status)
VALUES ('Impossible discount', -2.00, 'active');

-- Try commenting the insert above, then run this bad status instead.
INSERT INTO products (name, price, status)
VALUES ('Mystery item', 5.00, 'banana');
...
<SqlCodeBlock
  ...
  initialCode={`-- The end date is before the start date, so the row is rejected.
INSERT INTO events (name, start_date, end_date)
VALUES ('Backwards conference', '2026-06-10', '2026-06-08');
