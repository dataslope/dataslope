...
INSERT INTO employees (name, dept) VALUES
  ...
  ('Finn','Marketing');`}
  starterCode={`SELECT dept, COUNT(*) AS headcount
FROM employees
GROUP BY dept
HAVING COUNT(*) > 2
...
FROM employees
WHERE is_intern = false            -- step 1: drop interns (rows)
GROUP BY dept                      -- step 2: form department groups
HAVING AVG(salary) > 80000         -- step 3: keep high-paying groups
