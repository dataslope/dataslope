...
INSERT INTO sales (region, year, amount) VALUES
  ...
  starterCode={`SELECT region, year, SUM(amount) AS total
FROM sales
...
INSERT INTO employees (name, dept, salary, is_intern) VALUES
  ...
  starterCode={`SELECT dept, ROUND(AVG(salary)) AS avg_salary
FROM employees
...
;
`}
  solutionSql={`SELECT customer, SUM(amount) AS total
FROM orders
GROUP BY customer
