...
INSERT INTO orders (id, customer_id, order_date, total) VALUES
  ...
  initialCode={`SELECT c.name, o.order_date, o.total
FROM customers c
...
<MultipleChoice
  markdown={`How does an ER diagram typically show a **many-to-many** relationship between two entities?

- With a single direct line and a "many" symbol on both ends only.
  > While the cardinality is many-to-many, in a relational model it is realized through a third box.
- [o] As two one-to-many relationships pointing into a junction (middle) entity.
  > A junction entity resolves many-to-many into two one-to-many links, which is how the schema implements it.
- By merging the two entities into one box.
  > Merging would lose the separate entities and their independent rows.
- By removing the relationship line entirely.
  > The relationship still exists; it is expressed through the junction entity.

Many-to-many is drawn (and built) as two one-to-many links through a junction entity.`}
/>
