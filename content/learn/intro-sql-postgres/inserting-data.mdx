...
<SqlCodeBlock
  ...
  initialCode={`CREATE TABLE tasks (
...
INSERT INTO tasks (title, done) VALUES
  ...
  ('Walk the dog', false),
  ('Write report', false);`}
  initialCode={`-- Step 1: SELECT to preview which rows match.
SELECT * FROM tasks WHERE title LIKE 'W%';
