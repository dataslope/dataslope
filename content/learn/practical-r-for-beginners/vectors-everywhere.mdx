...
<CodeBlock
  adapter="r"
  initialCode={`temperatures <- c(68, 71, 70, 74, 72, 69, 75)
print(temperatures)
length(temperatures)
`}
/>
...
<CodeBlock
  adapter="r"
  initialCode={`# Mixing logical + numeric -> numeric
c(TRUE, FALSE, 3)
...
<CodeBlock
  adapter="r"
  initialCode={`days <- c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")
...
<CodeBlock
  adapter="r"
  initialCode={`ages <- c(alice = 30, bob = 25, carol = 41)
print(ages)
...
<CodeBlock
  adapter="r"
  initialCode={`summary(c(1, 2, 3, 4, 5, 100))
`}
/>
...
`}
  initialCode={`# days is already defined: c("Mon", ..., "Sun")
