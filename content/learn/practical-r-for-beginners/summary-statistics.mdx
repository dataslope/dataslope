...
<CodeBlock
  adapter="r"
  initialCode={`incomes <- c(32, 38, 42, 45, 48, 51, 55, 58, 62, 250)
...
<CodeBlock
  adapter="r"
  initialCode={`a <- c(48, 49, 50, 51, 52)        # tight
b <- c(10, 30, 50, 70, 90)        # spread

mean(a); mean(b)     # same mean!
sd(a);   sd(b)       # very different spread
var(a);  var(b)      # variance = sd^2
range(a); range(b)
IQR(a);  IQR(b)      # interquartile range: Q3 - Q1
`}
/>
...
<CodeBlock
  adapter="r"
  initialCode={`scores <- c(60, 65, 67, 70, 72, 75, 78, 80, 82, 85, 88, 90, 92, 95, 100)
...
<CodeBlock
  adapter="r"
  initialCode={`data(anscombe)

# Compute means and sds of all four (x, y) pairs
sapply(1:4, function(i) {
  c(mean_x = mean(anscombe[[paste0("x", i)]]),
...
<ChallengeCard
  ...
  initialCode={`describe <- function(x) {
