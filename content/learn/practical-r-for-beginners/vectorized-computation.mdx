...
<CodeBlock
  adapter="r"
  initialCode={`prices <- c(9.99, 14.50, 24.00, 5.75)
...
<CodeBlock
  adapter="r"
  initialCode={`quantities <- c(2, 1, 3, 4)
...
<CodeBlock
  adapter="r"
  initialCode={`# Length 6 + length 1 — the scalar gets recycled
...
<CodeBlock
  adapter="r"
  initialCode={`# Length 5 + length 2: doesn't divide evenly, R warns
c(1, 2, 3, 4, 5) + c(10, 20)
`}
/>
...
<CodeBlock
  adapter="r"
  initialCode={`daily_sales <- c(120, 150, 80, 200, 175, 90, 110)
...
<CodeBlock
  adapter="r"
  initialCode={`heights <- c(160, 172, 168, 181, 155, 175, 169, 178)
...
```mermaid
flowchart TB
  loop["for (i in 1:n) result[i] <- xs[i] + ys[i]"] -->|interpreted per element| slow["Slow"]
  vec["result <- xs + ys"] -->|single C call| fast["Fast"]
...
`}
  initialCode={`# celsius is already defined
