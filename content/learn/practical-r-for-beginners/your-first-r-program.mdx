...
<CodeBlock
  adapter="r"
  initialCode={`# Auto-printing: the result of an unassigned expression is shown
1 + 1
...
<CodeBlock
  adapter="r"
  initialCode={`# print() gives you R-style display
print("Hello, R!")
print(c(1, 2, 3))
...
<CodeBlock
  adapter="r"
  initialCode={`# This whole line is a comment
...
<CodeBlock
  adapter="r"
  initialCode={`# Misspelled function name
prnt("hello")
`}
/>
...
<CodeBlock
  adapter="r"
  initialCode={`# Tiny health checkup program
...
for (i in seq_along(names)) {
  status <- if (bmi[i] < 18.5) "underweight"
            else if (bmi[i] < 25) "healthy"
            else if (bmi[i] < 30) "overweight"
            else "obese"
