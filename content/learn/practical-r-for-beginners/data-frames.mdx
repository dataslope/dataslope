...
<CodeBlock
  adapter="r"
  initialCode={`employees <- data.frame(
...
<CodeBlock
  adapter="r"
  initialCode={`# A built-in data frame
data(mtcars)
...
<CodeBlock
  adapter="r"
  initialCode={`# mtcars: 32 cars (1974 Motor Trend magazine), 11 variables
head(mtcars)
...
Then assign \`avg_score\` to the mean of the \`score\` column.`}
  initialCode={`students  <- NULL  # build a data.frame with name, grade, score
