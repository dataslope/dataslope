...
<CodeBlock
  adapter="r"
  initialCode={`greet <- function(name, greeting = "Hello") {
...
<CodeBlock
  adapter="r"
  initialCode={`describe <- function(x, na.rm = TRUE) {
  list(
    n      = length(x),
    n_na   = sum(is.na(x)),
...
<CodeBlock
  adapter="r"
  initialCode={`# Compact: one function call per column you care about
...
<CodeBlock
  adapter="r"
  initialCode={`standardize <- function(x) {
...
<CodeBlock
  adapter="r"
  initialCode={`standardize <- function(x) (x - mean(x, na.rm = TRUE)) / sd(x, na.rm = TRUE)
...
<CodeBlock
  adapter="r"
  initialCode={`# Pure: just computes and returns
...
<CodeBlock
  adapter="r"
  initialCode={`temp_var_in_global <- "I exist out here"
...
<ChallengeCard
  ...
  initialCode={`summarize_group <- function(x) {
