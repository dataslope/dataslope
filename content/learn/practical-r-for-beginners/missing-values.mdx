...
<CodeBlock
  adapter="r"
  initialCode={`5 + NA            # unknown plus 5 = unknown
...
<CodeBlock
  adapter="r"
  initialCode={`temps <- c(68, 70, NA, 72, 75, NA, 69)
...
<CodeBlock
  adapter="r"
  initialCode={`x <- c(10, 20, NA, 40, NA, 60)
...
`}
  initialCode={`# week is already defined: 7 daily temps with NAs
