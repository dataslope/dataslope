...
<CodeBlock
  adapter="r"
  initialCode={`# A pretend "population" of 100,000 adult heights (cm)
set.seed(1)
...
hist(population, breaks = 60, col = "tomato",
...
cat(sprintf("Approx 95%% interval: [%.2f, %.2f]\\n",
            xbar - 2*se, xbar + 2*se))
`}
/>
