...
<Figure
  slug="jcol-equality-and-hashing-hashcode-inline-cutout"
  alt="A block dropped into a mixing machine whose chute swings to one of several bins"
  caption="`hashCode()` decides the bucket; `equals()` settles who is already in it."
/>

Two objects can land in the same bucket, and that is not the rare
accident it sounds like. With a thousand buckets it is already even money
that two keys collide by the fortieth key, at four percent full:
...
<Figure
  ...
  caption="Equal objects must hash alike, or the set will hold both copies."
/>
...
<Figure
  slug="jcol-equality-and-hashing-collections-actually-do-inline-cutout"
  alt="Two identical-looking blocks held up to a gauge that reads them as the same before either is filed"
  caption="A collection asks whether two elements are equal before it stores either."
/>
