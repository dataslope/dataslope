...
description: Why List<Dog> is not a List<Animal>, what ? extends T and ? super T really mean, and the producer-extends, consumer-super rule that makes it all click
---
...
<Figure
  slug="jcol-bounded-types-cutout"
  alt="Bounded types and wildcards, an isometric illustration"
/>
...
designed it that way, everything else, `? extends T`, `? super T`, and
the rule that ties them together, falls out naturally.
...
<Figure
  slug="jcol-bounded-types-and-wildcards-list-lt-dog-inline-cutout"
  alt="A crate of small parts refused at a slot cut for a crate of general parts, the two crates visibly different"
  caption="A `List<Dog>` is not a `List<Animal>`, because writing into it would break."
/>
...
<Figure
  slug="jcol-bounded-types-and-wildcards-inline-cutout"
  alt="A slot cut to accept a family of related shapes, wide enough for three of them and clearly too narrow for a fourth waiting beside it"
  caption="A wildcard widens the slot to a family of types, but not to all of them."
/>
...
<Figure
  slug="jcol-bounded-types-and-wildcards-setup-inline-cutout"
  alt="A slot cut to accept a family of related shapes, a clearly larger shape waiting beside it"
  caption="The bound is what decides which types the slot will actually take."
/>
