...
most-used container in almost any program, the Collections Framework
interface
...
<Figure
  slug="jcol-lists-list-interface-inline-cutout"
  alt="A rail of identical unmarked slots, blocks kept in the order they were set down"
  caption="A list keeps insertion order and lets duplicates through."
/>
...
<Figure
  slug="jcol-lists-arraylist-list-backed-inline-cutout"
  alt="A growing rail with a fixed strip visible underneath it, the strip replaced by a longer one when full"
  caption="An `ArrayList` is an array underneath, replaced by a bigger one when full."
/>
...
<Figure
  slug="jcol-lists-inline-cutout"
  alt="A rail of slots holding blocks with one being inserted in the middle and the blocks to its right sliding along to make room"
  caption="Inserting in the middle shifts everything after it along."
/>
...
`List.of()` is one of the Java 9 factory methods and the playground on these
pages runs Java 8, so `javac` here cannot resolve it. The Java 8 spelling is
`Collections.unmodifiableList(Arrays.asList("red", "green", "blue"))`, which
throws the same `UnsupportedOperationException`, but the point of this section
is the factory method itself, so it is shown rather than run.
</Callout>
...
public class Main {
  public static void main(String[] args) {
    // The Java 8 equivalent, which this playground can run.
    List<String> primary =
