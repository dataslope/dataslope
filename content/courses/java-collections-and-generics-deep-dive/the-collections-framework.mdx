...
<Figure
  slug="jcol-the-collections-framework-interface-hierarchy-inline-cutout"
  alt="A family of sockets that all take the same plug, and one socket set apart from them shaped to take a pair"
  caption="Collections share one shape, and `Map` sits apart because it holds pairs, not elements."
/>
...
<Figure
  slug="jcol-the-collections-framework-first-multi-file-example-inline-cutout"
  alt="One bench that takes any container with the right collar, the containers behind it swapped without the bench being touched"
  caption="Take a `List` rather than an `ArrayList` and the caller keeps the choice."
/>
...
<Figure
  slug="jcol-the-collections-framework-default-implementations-inline-cutout"
  alt="One shelf holding a single well-made container for each shape of socket on the wall behind it"
  caption="One production-grade implementation per interface is what you will reach for by default."
/>
...
<Figure
  slug="jcol-the-collections-framework-programming-interface-inline-cutout"
  alt="One socket taking three differently built plugs because all three carry the same collar at the tip"
  caption="Accept the interface and callers may hand you any implementation of it."
/>
...
<Figure
  slug="jcol-the-collections-framework-collections-utility-class-inline-cutout"
  alt="A rack of differently shaped containers beside one shelf of shared tools, each tool reaching into any of them"
  caption="Sorting and shuffling are written once and work on any collection that fits the interface."
/>
...
<Figure
  slug="jcol-the-collections-framework-iterator-real-walking-protocol-inline-cutout"
  alt="A hooked arm walking a row of items one at a time, able to lift one out as it passes"
  caption="One item at a time, with the option to remove the one just handed to you."
/>
