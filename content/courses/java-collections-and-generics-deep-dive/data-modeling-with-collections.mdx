...
<Callout type="info" title="Why the runnable version spells `Track` out">
Records arrived in Java 16 and the playground on these pages runs Java 8, so
`javac` here reports `cannot find symbol: class record`. The sketch above uses
the modern one-line spelling; the runnable blocks below write the same class
out by hand, which is exactly what a `record` declaration generates for you.
</Callout>
