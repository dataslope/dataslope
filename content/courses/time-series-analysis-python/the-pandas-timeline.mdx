...
<Figure
  slug="tsa-pandas-timeline-cutout"
  alt="A calendar rail running along the edge of a grid platform, date plates slotted into it as the index rather than as a column"
/>
...
<Figure
  slug="tsa-the-pandas-timeline-from-strings-timestamps-inline-cutout"
  alt="Written slips fed into a machine and leaving as tidy dated cards that clip onto a rail"
  caption="Date text goes in, uniform timestamps come out, ready to clip onto the timeline."
/>
...
<Figure
  slug="tsa-the-pandas-timeline-inline-cutout"
  alt="A rail whose sleepers are evenly spaced dates, with chips resting on the sleepers and two sleepers left bare"
  caption="With dates in the index, every value rests on the timestamp it belongs to."
/>
...
This is not a tidiness matter. Here is one series plotted against row order
and against real dates:

<Chart slug="gaps-in-a-time-axis" />

Row order closes the gaps, which changes the shape, changes every slope (a
slope is change per unit *time*, and the units are no longer time), and hides
the outage, which is usually the interesting part.

Losing the gap is usually the expensive part, because missing data is rarely
missing at random and something happened there. The right panel also shows the
second decision worth making deliberately: the line is broken rather than
bridged, because joining the two ends with a straight segment draws nine days
of readings nobody took. The fix in pandas is `pd.to_datetime`, then
`set_index`, then usually `asfreq("D")`, which materialises the absent days as
NaN so they are visibly missing instead of quietly gone.
...
<Figure
  slug="tsa-the-pandas-timeline-quiet-hero-automatic-alignment-inline-cutout"
  alt="Two runs of dated cards laid together by their dates rather than by their positions, gaps left where a date is missing on one side"
  caption="Pandas pairs values by index, not by position, which is why misaligned dates show as gaps."
/>
