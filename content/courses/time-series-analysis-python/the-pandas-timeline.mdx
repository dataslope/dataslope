...
<Figure
  ...
  caption="Putting dates in the index is laying this rail. Each value rests on the timestamp it belongs to, so the spacing between values becomes part of the data, and a date with nothing on it shows up as a bare sleeper rather than silently vanishing."
/>
...
<Figure
  ...
  caption="Parsing is this machine. Hand-written date text goes in and uniform dated cards come out, and only once they are uniform can they clip onto the rail in order, which is what every slice and every resample later depends on."
/>
