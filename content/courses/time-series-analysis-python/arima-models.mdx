---
title: "Autoregressive, Moving-Average, and ARIMA Models"
description: Building autoregressive, moving-average and ARIMA models with statsmodels, what each part means, why a moving-average model is not a moving average, choosing (p,d,q) from the autocorrelation plots, fitting and forecasting with widening uncertainty, and reading residual diagnostics.
---

Everything so far has been preparation. We learned to make a series
stationary (differencing) and to read its echoes (the autocorrelation
plots). Now we assemble those skills into the workhorses of classical
forecasting: the **autoregressive** and **moving-average** models, the
**ARMA** that combines them, and the **ARIMA** that adds differencing on
top. By the end you'll fit one, read its output, and produce a forecast
...
## The two ingredients: autoregressive and moving-average

An ARIMA model is built from two simple ideas about where the next value
comes from.

**Autoregressive, written AR(p).** "Regress the series on its own recent
*values*."
Each value is a weighted sum of the last $p$ values plus a random shock:

$$
y(t) = c + \varphi_1 y(t-1) + \varphi_2 y(t-2) + \cdots + \varphi_p y(t-p) + \varepsilon(t)
$$

This captures **momentum / persistence**: a high value tends to be followed
by another high value. The $\varphi$ weights say how strongly the past pulls the
present.

**Moving-average, written MA(q).** "Regress the series on its own recent
*shocks*."
Each value is the current shock plus a weighted sum of the last $q$ shocks
(the past forecast errors):

$$
y(t) = c + \varepsilon(t) + \theta_1 \varepsilon(t-1) + \cdots + \theta_q \varepsilon(t-q)
$$
...
<Figure
  slug="tsa-arima-models-inline-cutout"
  alt="Three small machines feeding one output, one looking back at past values, one at past errors, one at the trend"
  caption="The three machines are the three letters. One reads the recent values (the autoregressive part), one reads the recent errors (the moving-average part), and one takes care of the trend by differencing (the integrated part). What they feed jointly is the single number the model predicts."
/>
...
| --- | --- | --- |
| **`p`** | autoregressive order (past values) | the **partial autocorrelation** cut-off lag |
| **`d`** | differences for stationarity | how many differences make the stationarity test pass |
| **`q`** | moving-average order (past shocks) | the **autocorrelation** cut-off lag |
...
2. **Identify**, read the autocorrelation and partial autocorrelation of
   the stationary series → `p`, `q`.
...
```mermaid
flowchart TD
    A["Stationarize<br/>(log, difference) → d"] --> B["Identify p and q<br/>from the two autocorrelation plots"]
    B --> C["Fit ARIMA(p, d, q)"]
...
And what a forecast actually claims. The line is one number per step;
the cone is the interval, and it widens as the square root of the
horizon.

<Figure
  slug="tsa-arima-models-fitting-forecasting-inline-cutout"
  alt="A wave running along a rail and continuing past its last sleeper as a lighter dotted extension"
  caption="Fitting stops where the data stops. The lighter dotted stretch past the last sleeper is the forecast: the same wave continued under the rules the model just learned, drawn differently because nothing out there has been observed."
/>

<Chart slug="forecast-uncertainty-cone" />
...
## Choosing orders by information criterion (and its limits)

The autocorrelation plots suggest candidate orders, but often several are
plausible. The **Akaike information criterion** (AIC) scores a fitted model by its fit
...
<Figure
  slug="tsa-arima-models-two-ingredients-ar-inline-cutout"
  alt="Two machines feeding one output, one looking back at past blocks and one at past mistakes"
  caption="A fitted model is these two machines working together, one on past values and one on past mistakes. Diagnosing the residuals asks whether anything is left over that a third machine could still have used, and the answer you want is no."
/>
...
The residual autocorrelation still has a spike near lag 12, the yearly
seasonality a plain ARIMA can't capture. That's the residual diagnostic
doing its job: telling you the model is incomplete and pointing at
*seasonality* as the culprit.
The fix is a **seasonal** ARIMA, usually written SARIMA, which repeats each
of the three parts at the seasonal period, conceptually just "do the same
trick at lag 12 too."
...
- \`d\` from the autocorrelation plot and \`p\`, \`q\` from the information criterion only
  > \`d\` comes from stationarity testing (differencing), not the autocorrelation shape directly; and \`p\`/\`q\` are identified from the two autocorrelation plots, with the information criterion only as a tie-breaker.
...
- **ARIMA(p, d, q)** = AR + `d` differences + MA, integrating forecasts back
  to the original scale. Read **`p` from the partial autocorrelation**,
  **`q` from the autocorrelation**, **`d` from differencing**.
...
- The **Akaike information criterion** (lower is better) shortlists orders
  but is **in-sample**, never a substitute for out-of-sample testing.
