...
<Figure
  slug="tsa-missing-data-cutout"
  alt="An evenly spaced rail of tiles with two empty sockets, one being filled by a copy from the left and one by a bridging ramp"
/>
...
<Figure
  slug="tsa-handling-missing-temporal-data-inline-cutout"
  alt="A rail of evenly spaced date sleepers with two sleepers bare and a small bridging piece being fitted over them"
  caption="Laying the full calendar down first turns a missing row into a visible gap."
/>
...
<Figure
  slug="tsa-handling-missing-temporal-data-three-core-fill-inline-cutout"
  alt="A rail with a bare sleeper and three different bridging pieces held above it"
  caption="Three bridges for one gap: picking one is picking an assumption."
/>
...
fourth option gets included here because people reach for it constantly:
filling with the series mean, which quietly reads *every* value in the
series, including all the ones that hadn't happened yet.

<Chart slug="fill-strategies-that-peek" />

Notice which question the picture is *not* answering. It says nothing about
which fill is closest to the truth, and on data like this interpolation
would win that contest comfortably. That is exactly what makes it dangerous:
it is the accurate answer that a model in production could never have
computed, so it flatters your validation score and vanishes when the model
goes live.
