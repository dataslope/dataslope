...
report, so the PACF drops into the noise band while the ACF keeps decaying on
chain effects alone: today leans on yesterday, yesterday leaned on the day
before, and the correlation propagates down the line while shrinking.

That gives the rule people memorise. A PACF cutting off at lag p
with a decaying ACF means AR(p). An ACF that cuts off at lag q with a decaying
PACF means MA(q), the mirror image, because a moving-average process has a
finite memory of shocks. Both decaying means both parts are present and
neither order is readable by eye.
