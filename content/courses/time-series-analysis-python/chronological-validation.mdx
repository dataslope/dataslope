...
The model gets to see the future, and it does not need to be clever to use it:
any autocorrelated series lets a model interpolate a held-out month from its
neighbours, which is a far easier problem than forecasting and produces a
score that will not survive deployment. A rolling window of fixed length is
the third option, dropping the oldest data rather than accumulating it, and is
right when the process itself changes.
