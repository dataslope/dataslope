...
<Figure
  ...
  caption="Shuffling is the one move a time series forbids."
/>
...
<Figure
  ...
  caption="Same gaps, three metrics: each is a different rule for summarizing these sticks."
/>
...
<Chart slug="timeseries-cv-splits" />

In the top block, most of each fold's training data lies *after* its own test
months. The model gets to see the future, and it does not need to be clever to
use it: any autocorrelated series lets a model interpolate a held-out month
from its neighbours, which is a far easier problem than forecasting and
produces a score that will not survive deployment.

The lower block is the expanding window from the table above. A rolling window
of fixed length is the third option, dropping the oldest data rather than
accumulating it, and is right when the process itself changes.

The backtest costs two things worth naming: the earliest folds train on very
little, and the training size varies across folds, so the fold scores are not
identically distributed and averaging them is a rougher summary than usual.
...
<Figure
  ...
  caption="Cut the timeline once: fit on the left, and leave the right sealed until you score."
/>
