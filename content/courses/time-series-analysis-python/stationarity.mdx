...
every model on the pages ahead, the autoregressive and moving-average
families and the ARIMA that unites them, *assumes* it, and if
...
<Figure
  slug="tsa-stationarity-stationary-actually-means-inline-cutout"
  alt="Two tracks, one drifting steadily upward and one wobbling around a level line"
  caption="A stationary series wobbles around a level line instead of drifting away from it."
/>
...
Each of the three conditions fails with its own silhouette, and learning to
read them off a line plot is the check you run before you run any test:

<Chart slug="nonstationary-shapes" />
...
<Figure
  slug="tsa-stationarity-inline-cutout"
  alt="Two tracks side by side, the near one drifting steadily upward off its rail and the far one wobbling around a fixed centre line"
  caption="The wobbling track keeps returning to a centre; the drifting one has none to return to."
/>
...
<Figure
  slug="tsa-stationarity-testing-augmented-dickey-fuller-inline-cutout"
  alt="One gauge held against two tracks in turn, reading low for the level track and high for the drifting one"
  caption="The test puts a number on what the chart already suggests about drift."
/>
...
- There are companion tests (the Kwiatkowski-Phillips-Schmidt-Shin test
  flips the hypotheses; running both and cross-checking is common
  practice). For this course, Augmented Dickey-Fuller plus a careful
  look at the plot and rolling mean/variance is enough.

That first bullet is worth seeing rather than being told. Here are the three
failure modes again, each with the p-value the test actually returns for it:

<Chart slug="stationarity-three-failures" />

Two things in that picture should change how you read a p-value from this
test. The middle panel is called stationary and obviously isn't, which is
the blind spot: no unit root exists there for the test to find, so it
answers the question it was asked and stays silent about the one you cared
about. And the first and third panels get the *same* verdict for completely
different reasons, which means the verdict alone can't tell you what to do
next. Differencing once fixes the random walk and leaves the seasonal series
seasonal.

Those p-values are simulated rather than looked up: four thousand random walks
of the same length were generated and the same statistic computed on each, so
each number is the share of pure random walks that looked at least this
mean-reverting.
