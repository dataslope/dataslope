...
<Figure
  slug="tsa-resampling-cutout"
  alt="A fine-toothed rail of many small tiles on one platform condensing into a coarse rail of fewer larger tiles"
/>
...
There are exactly two kinds of quantity here, and they take different
aggregations. A flow is measured *over* an interval: orders, rainfall,
kilowatt-hours, revenue. A stock is measured *at* an instant: inventory,
balance, headcount, temperature.

<Chart slug="resample-sum-or-mean" />

The rule that covers nearly everything: if the units contain "per", it is a
flow and you sum; if you could measure it with a snapshot, it is a stock and
you do not.

Flows add across time, so the month is the sum of the days, and taking a mean
answers a duller question while making a 28-day month incomparable with a
31-day one for reasons that have nothing to do with the business. Stocks do
not add, and a month's value for one is a mean, a last or a max depending on
what the month is meant to stand for. The mistake survives because both
operations succeed and both return a plausible series.
...
<Figure
  slug="tsa-resampling-and-aggregation-inline-cutout"
  alt="A dense row of daily chips being swept into seven-chip trays, each tray leaving one summary block behind"
  caption="Regroup along the time axis, then summarize each group."
/>
