...
>
  ...
  <g style={{ color: "var(--ds-red-500)" }} fill="currentColor">
    <rect x="334" y="100" width="10" height="10" rx="3" />
  </g>
...
visits = pd.Series(
    ...
    index=days, name="visits",
      ...
      starterCode: `print("Daily visits (first 10 days):")
...
<svg
  viewBox="0 0 640 220"
  role="img"
  aria-label="On the left, four green rain squares combine into one tall green column; on the right, four blue readings at different heights settle into a single short blue bar at their middle level — flows add up to a sum, while levels summarize as a mean"
...
>
  <g
    ...
    strokeLinecap="round"
  >
    <path d="M180 74 V 100" />
    <path d="M460 82 V 108" />
  </g>
  <g style={{ color: "var(--ds-gray-400)" }} fill="currentColor">
    <polygon points="180,112 173,100 187,100" />
    <polygon points="460,120 453,108 467,108" />
  </g>
  <g style={{ color: "var(--ds-green-500)" }} fill="currentColor">
    <rect x="120" y="40" width="20" height="20" rx="5" />
    <rect x="146" y="40" width="20" height="20" rx="5" />
    <rect x="172" y="40" width="20" height="20" rx="5" />
    <rect x="198" y="40" width="20" height="20" rx="5" />
    <rect x="158" y="122" width="44" height="80" rx="6" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="404" cy="46" r="5.5" />
    <circle cx="436" cy="64" r="5.5" />
    <circle cx="468" cy="40" r="5.5" />
    <circle cx="500" cy="58" r="5.5" />
    <rect x="410" y="152" width="100" height="9" rx="4.5" />
  </g>
</svg>
...
temp_c     = pd.Series((22 + 4*np.sin(np.arange(30)/3) + rng.normal(0,1,30)).round(1),
      ...
      starterCode: `# Roll both up to a single monthly number, the RIGHT way for each.
...
print(
    "  WRONG 'June temp' as a sum:",
    temp_c.resample("ME").sum().round(1).iloc[0],
    "C  <- nonsense",
)
`,
    },
...
visits = pd.Series(
    ...
    index=days, name="visits",
      ...
      starterCode: `weekly = visits.resample("W").agg(["sum", "mean", "min", "max"]).round(1)
...
And monthly data downsamples further still. Here is the real Keeling Curve:
roll its monthly CO₂ readings up to a yearly **average** and the seasonal
sawtooth folds away, leaving the bare upward **trend**.
...
co2 = df.set_index("date")["average"]   # monthly atmospheric CO2, ppm`,
      starterCode: `# CO2 is a LEVEL (a concentration), not a flow -> summarize with MEAN.
annual = co2.resample("YE").mean().round(2)

print("Yearly average CO2 (ppm), last 6 years:")
print(annual.tail(6))
print()
print(f"1959 average: {co2.loc['1959'].mean():.2f} ppm")
print(f"Latest year : {annual.iloc[-1]:.2f} ppm")
print()
print("Averaging over a full year folds away the 12-month seasonal wiggle,")
print("leaving the steady rise - the long-run trend, laid bare.")
`,
    },
  ]}
/>
...
The yearly resample above has *no* sawtooth — averaging a full year folds
the 12-month seasonal cycle into a single number. Downsampling to the
seasonal period (or a multiple of it) is a quick, crude way to *see the
trend* without the seasonality shouting over it. We'll do this far more
carefully with decomposition soon. (Note we used `mean`, not `sum`: a CO₂
concentration is a *level*, so summing twelve monthly readings would be
meaningless.)
</Callout>
...
air3 = pd.Series(_passengers, index=idx, name="passengers")  # first 3 years`,
      starterCode: `# Upsample monthly -> daily. asfreq() lays down the finer grid and leaves
...
air3 = pd.Series(_passengers, index=idx, name="passengers")`,
      starterCode: `daily = air3.resample("D").asfreq()
...
<svg
  viewBox="0 0 640 200"
  role="img"
  aria-label="Three solid blue monthly dots on a coarse timeline drop down to a fine daily row where only their three slots are solid and every in-between slot is a dashed outline — upsampling lays down a finer grid, but the new values are assumptions, not measurements"
  style={{ display: "block", width: "100%", maxWidth: "640px", height: "auto", margin: "2.5rem auto" }}
>
  <g style={{ color: "var(--ds-gray-300)" }} fill="currentColor">
    <rect x="60" y="60" width="520" height="4" rx="2" />
    <rect x="60" y="164" width="520" height="4" rx="2" />
  </g>
  ...
  <g style={{ color: "var(--ds-gray-400)" }} fill="currentColor">
    <polygon points="320,118 313,106 327,106" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
    <circle cx="100" cy="62" r="7" />
    <circle cx="320" cy="62" r="7" />
    <circle cx="540" cy="62" r="7" />
    <rect x="92" y="132" width="16" height="16" rx="4" />
    <rect x="312" y="132" width="16" height="16" rx="4" />
    <rect x="532" y="132" width="16" height="16" rx="4" />
  </g>
  <g
    style={{ color: "var(--ds-gray-400)" }}
    fill="none"
    ...
    strokeDasharray="3 4"
  >
    <rect x="129" y="132" width="16" height="16" rx="4" />
    <rect x="166" y="132" width="16" height="16" rx="4" />
    <rect x="202" y="132" width="16" height="16" rx="4" />
    <rect x="239" y="132" width="16" height="16" rx="4" />
    <rect x="275" y="132" width="16" height="16" rx="4" />
    <rect x="349" y="132" width="16" height="16" rx="4" />
    <rect x="385" y="132" width="16" height="16" rx="4" />
    <rect x="422" y="132" width="16" height="16" rx="4" />
    <rect x="459" y="132" width="16" height="16" rx="4" />
    <rect x="495" y="132" width="16" height="16" rx="4" />
  </g>
</svg>
...
visits = pd.Series(rng.integers(80, 200, 90), index=days, name="visits")`,
      starterCode: `weekly_total = None  # total visits per week (sum)
...
<ChallengeCard
  adapter="python"
  title="Roll the Keeling Curve up to annual means"
  instructions={`The monthly CO₂ series \`co2\` (parts per million, a *level*) is loaded. Downsample it to **yearly averages** the correct way for a level, and find how fast it has been rising.

Produce:

- \`annual\` — a Series of each year's **mean** CO₂ (\`co2.resample("YE").mean()\`)
- \`rise_per_year\` — the average ppm increase per year across the record, computed as \`(annual.iloc[-1] - annual.iloc[0]) / (len(annual) - 1)\` (a float)

CO₂ is a concentration, so the right aggregation is \`mean\`, not \`sum\`. The series rises by a couple of ppm per year, so \`rise_per_year\` should be positive (and comfortably above 1).`}
  files={[
    {
      filename: "main.py",
      initCode: `import pandas as pd
...
co2 = df.set_index("date")["average"]`,
      starterCode: `annual = None        # yearly mean CO2
rise_per_year = None # average ppm increase per year
print(annual.tail() if annual is not None else None)
print("rise per year:", rise_per_year)
`,
      solutionCode: `annual = co2.resample("YE").mean()
rise_per_year = float((annual.iloc[-1] - annual.iloc[0]) / (len(annual) - 1))
print(annual.tail().round(2))
print("rise per year:", round(rise_per_year, 3), "ppm")
`,
    },
  ]}
  datasets={[{ path: "NOAA_mauna_loa_co2_monthly.csv", stageAs: "co2.csv" }]}
  tests={[
    {
      id: "is_series",
      name: "annual is a yearly Series",
      description: "One value per year",
      code: `import pandas as pd
assert isinstance(annual, pd.Series)
assert 50 <= len(annual) <= 80, f"Expected ~60+ yearly points, got {len(annual)}"`,
    },
    {
      id: "is_mean",
      name: "annual holds yearly MEANS, not sums",
      description: "Each value is in the ppm range, not the thousands",
      code: `assert 300 <= float(annual.iloc[0]) <= 360, "First-year mean should be a plausible CO2 level (~315 ppm), not a sum"`,
    },
    {
      id: "rising",
      name: "CO2 rises by more than 1 ppm/year on average",
      description: "A clear upward trend",
      code: `assert rise_per_year > 1.0, f"Expected a clear positive rise, got {rise_per_year}"`,
    },
  ]}
/>
