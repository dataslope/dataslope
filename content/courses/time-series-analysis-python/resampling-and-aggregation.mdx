...
<Figure
  slug="tsa-resampling-and-aggregation-downsampling-summarizing-coarser-inline-cutout"
  alt="A dense row of chips swept into seven-chip trays, each tray leaving one block"
  caption="Downsampling sweeps many fine-grained readings into one coarse bucket. Seven daily chips go into the tray and a single weekly block comes out, and which block you get is decided entirely by the aggregation you pick."
/>
...
<Figure
  ...
  caption="The whole lesson in one motion: regroup along the time axis, then summarize each group. The trays set the new resolution and the aggregation decides what each surviving block actually means."
/>
