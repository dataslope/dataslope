...
nothing did happen to any of them. The information was in the order, and
`train_test_split` and `KFold` both remove it by default, without a warning.

Two places this arrives as a default rather than a decision: `train_test_split`
shuffles unless you pass `shuffle=False`, and `KFold` shuffles unless you
reach for `TimeSeriesSplit`. Both produce a model that scores well and cannot
forecast, because the score was earned on interpolating a hole rather than on
extending the end.
