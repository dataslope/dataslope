...
Before a single forecast, before any resampling or any model, you need one idea
...
<Figure
  slug="tsa-what-makes-time-series-unique-defining-property-temporal-dependence-inline-cutout"
  alt="A run of marks where each one leans on the mark just before it, the lean visible the whole way along"
  caption="The value now is related to the values just before it, which is the whole subject."
/>
...
<Figure
  ...
  caption="Shuffle a time series and the shape it was carrying disappears."
/>
...
<Figure
  ...
  caption="Each value is pinned to a timestamp, and the timestamp cannot move."
/>
...
split does exactly that, and it's the default in almost every
machine-learning tutorial.
Here's the trap, made concrete.

<Figure
  slug="tsa-what-makes-time-series-unique-shuffling-sabotage-where-sneaks-inline-cutout"
  alt="A strip of dated marks cut into pieces and dealt out in random order, the run that went through them gone"
  caption="Shuffle a time series and you destroy the one variable that carried the structure."
/>
...
What makes this hard to catch is that shuffling leaves no fingerprint on any
of the numbers you would normally check. Below, one series and a shuffle of
it are profiled side by side:

<Chart slug="shuffling-destroys-order" />

Every summary a pipeline computes agrees that nothing happened, because
nothing did happen to any of them: same mean, same standard deviation, same
minimum, maximum, median and histogram. Only the autocorrelation plot
underneath notices, and it is the one plot a generic pipeline never draws.

And shuffling arrives as a default rather than a decision in two places:
`train_test_split` shuffles unless you pass `shuffle=False`, and `KFold`
shuffles unless you reach for `TimeSeriesSplit`. Both produce a model that
scores well and cannot forecast, because the score was earned on
interpolating a hole rather than on extending the end.
...
<Figure
  slug="tsa-what-makes-time-series-unique-real-world-stakes-inline-cutout"
  alt="A run of dated marks with the dates stripped away, the pattern that mattered gone with them"
  caption="In every one of these failures, the order of the data was thrown away."
/>
...
<Figure
  slug="tsa-what-makes-time-series-unique-first-honest-forecast-baseline-inline-cutout"
  alt="A forecast that simply repeats what happened one full cycle ago, drawn over the run it is predicting"
  caption="Repeating last season is the bar every model has to clear before it is worth anything."
/>
...
Now put a real model up against it, on a proper time-based holdout, and see
what the extra machinery actually buys:

<Chart slug="forecast-baseline-naive" />

The margin is the interesting part, in both directions. It is small enough
to be believable, and that is a feature: a series whose level drifts contains
a chunk of future that nothing could have forecast from the training window,
so the gap between any two honest methods on it is bounded by how much of the
future was knowable at all. Run the same model against the same baseline on a
series whose level holds still and the same code posts a much larger number,
without either model having improved. Report the pair, not the single
figure, and you have said something a reader can check.

The seasonal-naive baseline is not a straw man. It has no parameters, nothing
to fit and nothing to overfit, and on a series with a strong weekly or yearly
cycle a startling number of production models never beat it.
