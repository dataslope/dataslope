...
<Figure
  ...
  caption="Order is the information. Shuffle the rows of a customer table and you lose nothing; shuffle a time series and the shape it was carrying disappears, which is why the usual statistical instincts have to be retrained here."
/>
...
<Figure
  ...
  caption="Each observation is pinned to a timestamp, and the timestamp is not free to move. Lifting one chip shows its sleeper staying put, and that immovable spacing is what separates a time series from an ordinary table of numbers."
/>
