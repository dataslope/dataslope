...
<Figure
  slug="tsa-decomposition-cutout"
  alt="One combined wavy ribbon separating into three stacked plates, a rising slope, a repeating wave and a haze of specks"
/>
...
<Figure
  slug="tsa-decomposition-trend-seasonality-log-trick-turn-inline-cutout"
  alt="A wave whose swings widen to the right passing through a press and leaving with swings of even width"
  caption="The press is the logarithm. Swings that widen as the series rises go in, swings of even width come out, and even width is exactly what an additive decomposition needs before it can separate anything cleanly."
/>

Those four panels each have their own vertical scale, which hides how the
pieces relate. Drawn on one axis in the order they are added, "additive" stops
being a keyword argument:

<Chart slug="decomposition-buildup" />
...
"Is there seasonality here" is usually better asked as "how much of the
...
<Figure
  slug="tsa-decomposition-trend-seasonality-additive-or-multiplicative-inline-cutout"
  alt="Two waves along a rail, one whose swings stay the same width and one whose swings widen as it rises"
  caption="Two seasonal patterns, two models. Swings that keep the same width whatever the level are additive; swings that widen as the series rises are multiplicative, and telling which one you have is the choice this section is about."
/>
...
- Because of a bug in statsmodels
  > It's expected behavior, not a bug: a centered moving average mathematically cannot reach the edges. (seasonal-trend decomposition using loess, and model-based methods, handle ends differently.)
...
<Figure
  slug="tsa-decomposition-trend-seasonality-inline-cutout"
  alt="One wavy line on the left separating into three stacked lines on the right, a rising trend, a repeating wave and a rough remainder"
  caption="Decomposition is this separation: one observed line goes in, and a trend, a repeating seasonal wave and a rough remainder come out. Nothing is invented and nothing is lost, the three parts add back up to the line you started with."
/>
