...
<Figure
  slug="tsa-rolling-and-expanding-windows-moving-average-rolling-inline-cutout"
  alt="A run of marks with a twelve-notch measure laid along it, the sawtooth flattening out under the measure"
  caption="A window spanning exactly one seasonal cycle averages the season away and leaves the trend."
/>
...
A centred window is the right default for *description*, because it removes
noise without shifting the curve sideways, so a peak in the smoothed series
lands where the peak in the raw one is. Here is what it costs you the moment
the smoothed series becomes a feature:

<Chart slug="centered-window-leaks" />

The trailing curve's lag is not a defect to correct. It is the price of only
knowing the past, and a smoother that appears to avoid it has stopped being
causal. Use `center=True` for a chart and `center=False` for a feature, and
give them different column names when a notebook does both.

Fit a model on it and the model learns from tomorrow. Backtest that model and
the backtest agrees, because the leak is in the feature rather than in the
split, and no amount of care about train and test boundaries will catch it.
...
Worse, the final point of a trailing mean is the *least* current thing on
the chart. Both smoothed lines below peak after the raw series does, and the
wider window peaks later, so the tip of the curve is already reporting an
average of the recent past rather than the present:

<Chart slug="rolling-window-lag" />

Extending that tip forward would be projecting a number that was out of date
