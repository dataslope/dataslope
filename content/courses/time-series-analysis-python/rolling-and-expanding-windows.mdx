...
<Figure
  ...
  caption="The difference between the two window types is one edge. A rolling window keeps both edges moving, so it always covers the same span; an expanding window pins its left edge at the start, so it grows as it travels and never forgets anything."
/>
...
<Figure
  ...
  caption="Shifting is the primitive underneath all of this: move every value one sleeper along so that today can see yesterday. The bare first sleeper is the missing value that necessarily appears at the start, because nothing precedes the first observation."
/>
