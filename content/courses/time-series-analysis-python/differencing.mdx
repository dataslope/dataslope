...
removed and rises the moment noise is, so difference until it stops falling
and then stop. And a large negative correlation at lag one is not a property
of the process; it is the artefact of having subtracted each value from its
neighbour twice.
