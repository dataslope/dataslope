...
<Figure
  slug="tsa-differencing-cutout"
  alt="A rising staircase ribbon on a platform passing through a subtracting mechanism and emerging as a flat level band"
/>
...
<Chart slug="differencing-detrends" />
...
The trend is gone after one difference, and the stationarity test's p-value
collapses. But
...
<Callout type="info" title="This is exactly what a seasonal ARIMA automates">
The recipe "log, then a regular difference, then a seasonal difference" is
what a *seasonal* ARIMA, usually written SARIMA, encodes in its orders. We'll keep our hands
...
<Figure
  slug="tsa-differencing-differencing-inline-cutout"
  alt="A rising row of blocks with the gaps between neighbours drawn beneath as short bars around a flat centre"
  caption="One difference already flattens this row; a second would only add noise."
/>
...
<Chart slug="over-differencing" />

Two tells, and they agree. The variance falls while real structure is being
removed and rises the moment noise is, because subtracting two draws of the
same noise gives a quantity with twice the variance of one, so difference
until it stops falling and then stop. And a large negative correlation at lag
one is not a property of the process; it is the artefact of having subtracted
each value from its neighbour twice.

Both cost something. A model fitted to the over-differenced series will add a
moving-average term to cancel the artefact, which is a parameter spent undoing
self-inflicted damage, and every forecast interval built from it is wider by
twice over, which is not uncertainty about the world.
...
display(pd.DataFrame({"original": air, "reconstructed": reconstructed}).head(6))
print()
...
<ChallengeCard
  adapter="python"
  title="One difference makes the Keeling Curve stationary"
  instructions={`The monthly CO₂ series \`co2\` is non-stationary, its trend dominates everything. Show that a single ordinary difference fixes it.

Compute:

- \`co2_diff\`, the first difference \`co2.diff()\` (the month-to-month *change* in CO₂)
- \`p_raw\`, the ADF p-value of the raw \`co2\` (a float)
- \`p_diff\`, the ADF p-value of \`co2_diff\` after dropping NaNs (a float)

The raw series should be non-stationary (\`p_raw\` is very high, near 1.0) and the differenced series should be stationary (\`p_diff\` < 0.05). The first value of \`co2_diff\` must be \`NaN\` (there's no earlier month to subtract).`}
  files={[
    {
      filename: "main.py",
      initCode: `import numpy as np
...
co2 = df.set_index("date")["average"]`,
      starterCode: `co2_diff = None  # first difference of co2
p_raw = None     # ADF p-value of raw co2
p_diff = None    # ADF p-value of the differenced series
print("p_raw:", p_raw, " p_diff:", p_diff)
`,
      solutionCode: `co2_diff = co2.diff()
p_raw = float(adfuller(co2.dropna())[1])
p_diff = float(adfuller(co2_diff.dropna())[1])
print("p_raw:", round(p_raw, 4), " p_diff:", round(p_diff, 4))
`,
    },
  ]}
  ...
  tests={[
    {
      id: "first_nan",
      name: "first difference value is NaN",
      description: "No earlier month to subtract",
      code: `import pandas as pd
assert pd.isna(co2_diff.iloc[0]), "The first differenced value must be NaN"`,
    },
    {
      id: "raw_nonstationary",
      name: "raw CO2 is non-stationary",
      description: "The trend dominates",
      code: `assert p_raw > 0.5, f"Raw CO2 should be clearly non-stationary, got p={p_raw}"`,
    },
    {
      id: "diff_stationary",
      name: "one difference reaches stationarity",
      description: "Differenced series passes ADF",
      code: `assert p_diff < 0.05, f"Differenced CO2 should be stationary, got p={p_diff}"`,
    },
  ]}
/>
