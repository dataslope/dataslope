...
stationarity testing, differencing, reading the autocorrelation plots,
ARIMA, and honest validation. Here we run the *entire* pipeline, start to finish, on the
...
flowchart TD
    ...
    B --> C["3. Difference to stationarity<br/>(regular + seasonal) → the test passes"]
    C --> D["4. Identify orders<br/>(autocorrelation plots)"]
...
Apply regular and seasonal differencing until an Augmented Dickey-Fuller
test says the series is stationary.
...
Read the autocorrelation and partial autocorrelation of the stationary
series to choose the ARIMA terms.
...
We don't guess; we test. The raw series should fail the Augmented
Dickey-Fuller test, and the
...
With the series made stationary, both autocorrelation plots become
readable. We're looking
...
<Figure
  slug="tsa-capstone-forecasting-pipeline-step-5-7-split-inline-cutout"
  alt="A strip of past marks cut across by a line, a curve fitted to the left of it and drawn on past the cut over marks kept covered"
  caption="Fit on the past, predict the part you covered up, and only then compare the two."
/>
...
(from the autocorrelation plots), evaluated on data it *never saw*
(chronological split), that *beats a baseline* on honest error metrics, with
*uncertainty bands*
...
- That its information criterion was the lowest
  > The Akaike information criterion is an in-sample criterion and wasn't even computed here. What makes the out-of-sample number meaningful is beating a baseline on that same held-out data.
...
- test and engineer **stationarity** with the Augmented Dickey-Fuller test
  and **differencing**;
- read the **autocorrelation plots** to propose **ARIMA** orders;
- **fit** a model and forecast with honest **uncertainty**;
- and, most importantly, **validate chronologically**, measure with
  **absolute, squared and percentage error**, and prove your model **beats a
  baseline** without leaking
  the future.
