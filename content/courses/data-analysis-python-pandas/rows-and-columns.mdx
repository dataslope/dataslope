...
<Figure
  slug="pandas-rows-and-columns-cutout"
  alt="Rows and columns, an isometric illustration"
/>
...
<Figure
  slug="pandas-rows-and-columns-inline-cutout"
  alt="A board where one row is drawn out sideways as a strip and one column is lifted straight up as a stack"
  caption="A row is one record read across; a column is one measure read down."
/>
...
<Figure
  slug="pandas-rows-and-columns-basic-mental-model-inline-cutout"
  alt="One ruled board with a single row drawn out as a strip and a single column lifted out as a stack"
  caption="Most pandas operations work down a column rather than across a row."
/>
...
assert list(students.columns) == expected, f"Got {list(students.columns)}, expected {expected}"`,
    },
    {
      ...
      code: `import pandas as pd
assert pd.api.types.is_string_dtype(students["name"]), "name should be text"
