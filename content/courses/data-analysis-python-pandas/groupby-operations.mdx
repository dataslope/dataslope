...
<Figure
  slug="pandas-groupby-operations-basic-shape-inline-cutout"
  alt="A scatter of chips raked into three trays and each tray pressed into one summary block"
  caption="The number of groups decides the number of rows in the result."
/>
...
df = pd.DataFrame({
    ...
    "salary": [120, 95, 140, 80, 128, 110, 92],
      ...
      starterCode: `display(df.groupby("dept")["salary"].agg(["mean", "median", "min", "max", "count"]))
`,
    },
  ]}
/>
...
<Figure
  slug="pandas-groupby-operations-multiple-aggregations-at-inline-cutout"
  alt="One tray of chips feeding four gauges at once, each gauge reading a different single value"
  caption="One pass over a group can report several different measurements at once."
/>
...
<Figure
  slug="pandas-groupby-operations-inline-cutout"
  alt="A mixed pile of coloured blocks on the left being sorted into three bins by colour, and each bin producing one summary block on the right"
  caption="Each group reduces to one number, and `transform()` hands that number back to every row."
/>
...
display(df.groupby("dept").apply(top_two, include_groups=False))
`,
    },
  ]}
/>
