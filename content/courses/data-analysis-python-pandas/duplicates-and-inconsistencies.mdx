...
display(df[df.duplicated(subset=["customer_id"])])
`,
    },
  ]}
/>
...
<Figure
  ...
  caption="Exact copies are easy to spot; the row spelled differently is the hard one."
/>
...
display(df.drop_duplicates(subset=["customer_id"]))
`,
    },
  ]}
/>
...
<Figure
  slug="pandas-duplicates-and-inconsistencies-counts-as-duplicate-inline-cutout"
  alt="Two cards held side by side that match in every field but one, a magnifier over the field that differs"
  caption="What counts as a duplicate depends on which columns you require to match."
/>
...
<Figure
  slug="pandas-duplicates-and-inconsistencies-removing-duplicates-inline-cutout"
  alt="A line of cards with three identical ones, two lifted out by a gloved hand and dropped in a tray"
  caption="Dropping duplicates keeps one copy and sets the rest aside."
/>
