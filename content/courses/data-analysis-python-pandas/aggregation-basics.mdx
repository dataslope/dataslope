...
<Figure
  slug="pandas-aggregation-basics-aggregations-handle-missing-inline-cutout"
  alt="A column of chips with two covers among them tipped into a funnel, the covers falling aside and only chips reaching the gauge"
  caption="Aggregations drop the missing cells and average only the values that are there."
/>
...
<Figure
  slug="pandas-aggregation-basics-reduction-methods-inline-cutout"
  alt="One tall column of chips tipped into a funnel and leaving as a single block on the tray below"
  caption="An aggregation reduces a whole column to a single number."
/>
...
<Figure
  ...
  caption="Filter first, then aggregate, and the answer describes only the rows you kept."
/>
