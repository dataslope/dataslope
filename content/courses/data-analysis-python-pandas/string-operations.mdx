...
<Figure
  slug="pandas-string-operations-cutout"
  alt="String operations, an isometric illustration"
/>
...
<Figure
  slug="pandas-string-operations-basic-shape-inline-cutout"
  alt="A column of ribbons passing under one wide blade that trims every ribbon in the same stroke"
  caption="The `.str` accessor applies one text operation to every row in the column."
/>
...
display(df[df["title"].str.contains("Data", case=False)])
print()

# Anyone with "Lead" OR "Senior"
display(df[df["title"].str.contains("Lead|Senior", regex=True)])
`,
    },
  ]}
/>
...
<Figure
  slug="pandas-string-operations-inline-cutout"
  alt="A strip of letter tiles passing through three small hand tools in turn, one trimming its ends, one splitting it in two, one swapping a tile in the middle"
  caption="Text methods run element by element, and a missing cell stays missing."
/>
