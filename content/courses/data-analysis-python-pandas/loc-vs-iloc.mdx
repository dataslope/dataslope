...
<Figure
  slug="pandas-loc-vs-iloc-side-by-side-inline-cutout"
  alt="One board approached from two sides, a hand on the left holding a name tag and a hand on the right counting rows"
  caption="`.loc` reaches by label and `.iloc` by position; here both land on the same row."
/>
...
<Figure
  slug="pandas-loc-vs-iloc-slicing-comparison-inline-cutout"
  alt="Two brackets clamped over the same row of blocks, one ending on the marked block and one stopping just before it"
  caption="Label slices include the last item; position slices stop just before it."
/>
  ...
  files={[
    {
      filename: "main.py",
      initCode: `import pandas as pd
# Salaries as floats: a 10% raise produces fractional values, and pandas
# will not write those into an integer column (see the note below).
df = pd.DataFrame({
    "salary": [120.0, 95.0, 140.0, 80.0, 128.0],
    "dept":   ["Eng","Sales","Eng","Ops","Eng"],
}, index=["Aiko","Bilal","Chen","Diego","Eve"])`,
...
<Callout type="warn" title="Assignment cannot change a column's dtype">
Note that `salary` starts as a float here. Multiplying an **integer**
column by 1.10 produces fractional values, and pandas will not quietly
widen the column to hold them, it raises `TypeError: Invalid value ... for
dtype 'int64'`. That is deliberate: silently changing a column's type
under an assignment used to be a common source of surprise. If your
column really is integer, convert it first with
`df["salary"] = df["salary"].astype(float)`, or round the result back
with `.round().astype(int)`.
</Callout>
...
<Figure
  slug="pandas-loc-vs-iloc-inline-cutout"
  alt="One grid approached from two sides, a hand on the left picking a cell by the name tag on its row, a hand on the right counting rows to reach the same cell"
  caption="When the labels are integers, label and position stop agreeing with each other."
/>
