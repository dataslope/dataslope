...
<Figure
  slug="pandas-reproducible-analysis-reproducibility-ladder-inline-cutout"
  alt="Four steps rising, each step a sturdier bench than the one below it, the same result standing on every one"
  caption="Each rung makes the same result easier for someone else to reproduce."
/>
...
<Figure
  slug="pandas-reproducible-analysis-core-habits-inline-cutout"
  alt="A bench cleared back to its starting state and the whole run repeated from the first station to the last"
  caption="A notebook that only works in the order you happened to click is not reproducible."
/>
...
<Figure
  slug="pandas-reproducible-analysis-inline-cutout"
  alt="One sealed instruction strip run on two separate benches, both producing the same result block"
  caption="Reproducible means another machine runs your steps and gets your numbers."
/>
