...
<Chart
  ...
  caption="`how=` picks which *unmatched* keys survive; the matching itself is identical in all four, and pandas spells the last one `how='outer'`. Read the circles as sets of keys rather than of rows: `merge` still multiplies, so a key appearing three times on the left and twice on the right contributes six rows to that overlap, not one."
/>
...
<Figure
  slug="pandas-merging-and-joining-setting-stage-inline-cutout"
  alt="Two separate boards set on a bench facing each other, one shared column of notches between them"
  caption="A join needs one column that the two tables have in common."
/>
...
<Figure
  slug="pandas-merging-and-joining-inner-join-inline-cutout"
  alt="Two boards pushed together with only the rows whose notches align lifted onto a tray between them"
  caption="An inner join keeps only the rows that matched on both sides."
/>
...
display(a.merge(b, on="k", how="inner"))
`,
    },
  ]}
/>
...
display(a.merge(b, on="id"))  # default suffixes _x / _y
print()
display(a.merge(b, on="id", suffixes=("_yr1", "_yr2")))
`,
    },
  ]}
/>
...
display(all_check[all_check["_merge"] == "right_only"])
`,
    },
  ]}
/>
...
<Figure
  slug="pandas-merging-and-joining-inline-cutout"
  alt="Two grids with a matching column of keys sliding together like a zip, the joined rows emerging as one wider grid on the right"
  caption="Matching rows zip together, so the result grows wider rather than taller."
/>
