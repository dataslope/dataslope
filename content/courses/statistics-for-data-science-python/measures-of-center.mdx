...
<Figure
  ...
  alt="A skewed mound of blocks on a platform with three differently colored marker pins standing at three different positions"
/>
...
<Figure
  slug="stat-measures-of-center-three-centers-each-inline-cutout"
  alt="A plank on a fulcrum, a marker at the middle block, and a marker at the tallest pile"
  caption="The mean balances the plank; the median just marks the middle block."
/>
...
That robustness is worth watching happen. Hold nineteen values still, drag the
twentieth out to the right, and only one of the two summaries follows it:

<Chart slug="mean-median-drag" />
...
One trap belongs here rather than anywhere else, because it is the commonest
arithmetic error in any spreadsheet:

<Chart slug="average-of-averages" />

The quickest way to remember which calculation you want: if grouping first and
not grouping at all give different answers, you have made this mistake.

The right calculation is the weighted mean, with the group sizes as weights,
and it is identical to ignoring the groups and averaging the raw rows. It
generalizes past means, too: averaging five conversion rates, five percentage
changes, five medians or five R-squareds does not commute with grouping
either. A rate is a ratio, and ratios combine by adding numerators and adding
...
<Figure
  slug="stat-measures-of-center-inline-cutout"
  alt="One plank balanced on a fulcrum with blocks along it, and two other markers standing at the middle block and the tallest pile"
  caption="Mean, median and mode answer different questions and often disagree."
/>
...
One last variation, worth knowing exists even if you never fit one by hand.
Everything above summarizes a single column. As soon as the center depends on
something else, "typical pay *for a given amount of experience*", the same
mean-versus-median choice comes back, and the tool that keeps the median is
**quantile regression**. Fit it at several quantiles at once and you get a
picture no single line can give you:

<Chart slug="quantile-vs-mean-regression" />

Two familiar ideas are visible there at the same time. The dashed mean line
sits above the median line for exactly the reason it did in the income example
above, a long right tail drags the average and leaves the middle alone. And
the fan tells you something a center never can: the spread itself changes
across the range, so "the typical value" and "how much values vary around it"
are two different questions about every slice of the data.

## Putting it together: a robust center summary

In real exploratory work you rarely report a single number, you report a small
