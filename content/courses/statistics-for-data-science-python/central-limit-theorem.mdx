...
<Figure
  slug="stat-clt-cutout"
  alt="Three lopsided source mounds on a platform each feeding a chute that produces the same symmetric bell mound below"
/>
...
approximately normal**, centered on $\mu$, with spread $\sigma / \sqrt{n}$,
...
<Figure
  slug="stat-central-limit-theorem-theorem-plain-words-inline-cutout"
  alt="Lopsided piles of chips each reduced to one average block, the blocks building a smooth mound"
  caption="Average enough samples and their averages form a bell, whatever the source shape."
/>
...
<Figure
  slug="stat-central-limit-theorem-watch-happen-from-inline-cutout"
  alt="A lopsided pile scooped repeatedly, the scoop averages building a smooth symmetric mound on the rail beside it"
  caption="Even a heavily skewed population produces a symmetric distribution of means."
/>
...
of the mean has standard deviation $\sigma / \sqrt{n}$. That $1/\sqrt{n}$ is
the same diminishing-returns law we keep meeting, and it's worth
verifying directly.

<Figure
  slug="stat-central-limit-theorem-inline-cutout"
  alt="Three lopsided piles of blocks on the left, each producing one average block, and those averages stacking on the right into a smooth symmetric mound"
  caption="Each pile contributes one average, and the averages are what pile up normally."
/>
...
The simulated spread (the x markers) lands right on the $\sigma / \sqrt{n}$
...
$\text{Normal}(\mu,\ \sigma / \sqrt{n})$ for large $n$, *regardless of the
...
- "$n \ge 30$" is a **rough guideline**. Symmetric data normalizes at small $n$; heavily skewed/heavy-tailed data may need hundreds.
- The mean's bell has spread $\sigma / \sqrt{n}$, quadrupling $n$ halves it.
...
$\sigma / \sqrt{n}$. That spread, the standard deviation of an *estimate*,
