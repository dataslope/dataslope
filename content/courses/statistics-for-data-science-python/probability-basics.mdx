...
<Figure
  slug="stat-probability-basics-cutout"
  alt="A sample-space tray on a platform holding all possible outcome tokens, a smaller ring lassoing a subset of them"
/>
...
<Figure
  slug="stat-probability-basics-inline-cutout"
  alt="A spinner divided into unequal wedges with a pointer resting in the largest, and a tray of outcomes beneath it"
  caption="Probability is the share of the whole that an outcome takes up."
/>
...
$$
P(A \text{ or } B) = P(A) + P(B) \quad \text{(when } A, B \text{ can't co-occur)}
$$
...
$$
P(A \text{ and } B) = P(A) \times P(B) \quad \text{(when } A, B \text{ are independent)}
$$
...
Five independent runs of the same fair coin, so you can see that the
convergence is a property of the law rather than of one lucky sequence:
...
- **Probability** is $\text{favorable} / \text{total}$. If 1 of 5 equally likely
  outcomes is a win, P = 1/5 = 0.2.
- **Odds** are $\text{favorable} : \text{unfavorable}$, a ratio of the two *ways it
...
The two scales carry identical information, and the conversion is worth
seeing because they agree at low probabilities and diverge without limit
near certainty:
...
<Figure
  slug="stat-probability-basics-sample-spaces-events-inline-cutout"
  alt="A tray holding every possible outcome with a loop of cord drawn around some of them"
  caption="An event is a group of outcomes drawn from everything that could happen."
/>
...
- **Complement**: $P(\text{not } A) = 1 - P(A)$, the go-to move for "at least one" problems.
