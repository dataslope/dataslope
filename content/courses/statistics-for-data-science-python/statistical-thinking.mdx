...
<Figure
  slug="stat-statistical-thinking-cutout"
  alt="A hidden machine under a platform emitting one visible sample token, several ghosted alternative tokens fanned behind it"
/>
...
<Figure
  slug="stat-statistical-thinking-deterministic-thinking-vs-inline-cutout"
  alt="One lever that always moves a block the same distance beside one that moves it a different distance each pull"
  caption="Some processes repeat exactly; the ones worth measuring vary every time."
/>
...
<Figure
  slug="stat-statistical-thinking-signal-vs-noise-inline-cutout"
  alt="A jagged track with a level line drawn through it, the bumps on either side cancelling each other out"
  caption="Every wiggle looks like a story until you ask what the process itself is doing."
/>

Watch a fake trend appear from data where, by construction, there is no
trend whatsoever.

Four series with nothing in them but coin flips. One climbs, one drifts,
one rises and gives it back, one slides. Every story you can read into
them is one you brought yourself:
...
<Figure
  slug="stat-statistical-thinking-data-generating-process-inline-cutout"
  alt="A steady machine turning out parts, with a light shake applied to each as it leaves"
  caption="Signal is what the process does; noise is what the sampling adds on the way out."
/>
...
<Figure
  slug="stat-statistical-thinking-inline-cutout"
  alt="One chip held up alone, and the same chip shown as part of a mound where its position finally means something"
  caption="A single number means little until you see the distribution it came from."
/>
