...
<Figure
  slug="stat-p-values-cutout"
  alt="A bell curve on a platform with only the sliver of area beyond an observed marker shaded in a distinct colour"
/>
...
<Figure
  slug="stat-p-values-one-sentence-definition-inline-cutout"
  alt="A mound with a shaded tail and one marker standing out in that tail"
  caption="The p-value is how much of the tail lies beyond what you actually observed."
/>
...
In symbols, a p-value is $P(\text{data this extreme or more} \mid H_0)$.
...
<Figure
  slug="stat-p-values-p-value-not-inline-cutout"
  alt="One gauge with three different scales held up against its face, only one of the three lining up with the needle"
  caption="A p-value is one reading on one scale, and most of its famous meanings are a different scale."
/>
...
A p-value is $P(\text{data this extreme or more} \mid H_0 \text{ true})$, and nothing else.
Each misreading below, with why it fails:

| A p-value is NOT… | Why |
| --- | --- |
| $P(H_0 \text{ is true})$ | It *assumes* H₀; it cannot output a probability about H₀. |
| $P(\text{result was "due to chance"})$ | It already assumes chance-only; it can't also measure it. |
| the probability you are wrong | Being wrong depends on whether H₀ is actually true, which `p` never uses. |
| $1 - P(H_1)$ | It says nothing about how likely the alternative is. |
...
the other end. $P(\text{data} \mid H_0)$ and $P(H_0 \mid \text{data})$ are
...
**not** a 5% chance that H₀ is true. To get $P(H_0 \mid \text{data})$
...
**false**. The p-value is $P(\text{data} \mid H_0)$, not
$P(H_1 \mid \text{data})$. Swapping the two is the most expensive mistake
...
convenient cutoff, calling it a reasonable line for deciding a result
"deserves a second look," and openly noting the choice was somewhat
arbitrary. The field then ossified it into a pass/fail gate, with
enormous downstream consequences for what gets published and believed. In
2016 the **American Statistical Association** took the rare step of
issuing a formal statement warning against exactly this misuse,
treating `p < 0.05` as a bright line for truth, ignoring effect sizes,
and the p-hacking we're about to cover. Treat 0.05 as a default worth
questioning, not a verdict.
</Callout>
...
<Figure
  slug="stat-p-values-inline-cutout"
  alt="A mound of blocks with a small shaded tail at its right edge, and one observed marker standing out in that tail"
  caption="Test twenty true nulls and one lands in the tail by luck alone."
/>
...
"Significant." Scientists test whether jelly beans cause acne and find
nothing (p > 0.05). A bystander insists it must be a *specific color*, so
they test all 20 colors one by one. Nineteen come back null; the green
ones hit p < 0.05. The next panel is a newspaper headline: "GREEN JELLY
BEANS LINKED TO ACNE!", with the "only 5% chance of coincidence!"
caption that ignores the 19 other tests. One significant result out of 20
null tests is exactly what chance predicts. This is the multiple-comparisons
problem in a single comic, and you will see real analyses make the same
mistake without the punchline.
</Callout>
...
There are two standard corrections, and they are not interchangeable.
...
- A p-value is $P(\text{data this extreme or more} \mid H_0)$, a **tail
...
- It is **not** the probability H₀ is true, **not** the probability the
  ...
  $1 - P(H_1)$, **not** the effect size, and **not** the probability of
  replication.
