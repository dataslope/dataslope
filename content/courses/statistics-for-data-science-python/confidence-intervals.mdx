...
<Figure
  slug="stat-confidence-intervals-point-estimate-alone-inline-cutout"
  alt="A single mark on a rail beside the same mark with a soft band drawn either side"
  caption="A point estimate is one mark; an interval carries its own uncertainty with it."
/>
...
Every confidence interval you will ever build has three ingredients:
...
<Figure
  slug="stat-confidence-intervals-building-ci-mean-inline-cutout"
  alt="One mark on a rail with a band drawn either side of it, the band's width set by a caliper"
  caption="The interval's width comes from the standard error and the confidence you ask for."
/>
...
<Figure
  ...
  caption="Ninety-five per cent of such intervals cover the truth; this one either does or does not."
/>
...
Each of those misreadings describes a real interval, just not this one.
That makes them easy to settle on a picture: draw what each belief would
have to look like, on the same axis, next to what the interval actually
claims.

<Chart slug="ci-four-misreadings" />

The last row is the one worth staring at. The interval's endpoints are not
a boundary between "possible" and "impossible", they are a slice through a
curve that goes on smoothly in both directions, cut at the place a
convention chose. Nudge the convention and the endpoints move with it.
That is why "the CI excludes 0" and "there is no effect" are different
sentences.

This interval's right-hand edge sits at 104.7 at 95%, at 103.1 if the
convention were 80%, and at 106.2 if it were 99%, so whether a value just
outside is "ruled out" is a question about the standard of evidence rather
...
That same care applies to comparing two intervals against each other.
Below, two group intervals overlap, and yet the interval for their
*difference* clears zero comfortably:

<Chart slug="overlapping-cis-are-not-a-test" />

Comparing two intervals by eye is not the 5% test; it is a stricter one, at
roughly the 1% level. When the question is about a difference, compute an
interval for the difference.

The arithmetic is short. Two intervals stop touching once the means are more
than 1.96 times the *sum* of the standard errors apart, which is 12.2 units
here. But the standard error of a difference is not the sum, it is the square
root of the sum of squares, because variances add and standard errors do not.
With similar groups that is a factor of about √2, so there is a whole band of
separations where the intervals still overlap and the difference is already
significant, and this is one of them.
