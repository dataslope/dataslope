...
<Figure
  slug="stat-random-variables-cutout"
  alt="A mapping bridge on a platform carrying labelled outcome tokens across and dropping numbered blocks on the far side"
/>
...
<Figure
  slug="stat-random-variables-inline-cutout"
  alt="A machine that turns each outcome of a spinner into a block of a matching size dropping onto a rail"
  caption="Once outcomes carry numbers, averages and spread become possible."
/>
...
<Figure
  slug="stat-random-variables-random-variable-maps-inline-cutout"
  alt="A spinner whose each wedge feeds a machine that drops a block of a matching size onto a rail"
  caption="A random variable puts a number on each outcome the process can produce."
/>
...
<Figure
  slug="stat-random-variables-pmf-vs-pdf-intuition-inline-cutout"
  alt="A row of separate chips each with its own height, beside a smooth band where only an area between two marks can be read"
  caption="Discrete values carry probability each; a continuous one carries it only over an interval."
/>
...
The pmf with its balance point marked. E[X] is where the weights would
balance on a beam, which is a place, not an outcome:
...
deviation** $\operatorname{SD}(X) = \sqrt{\operatorname{Var}(X)}$, which is back in the
