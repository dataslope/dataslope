...
<Figure
  slug="stat-effect-sizes-cutout"
  alt="Two overlapping mounds on a platform with a caliper measuring the gap between their peaks in spread units"
/>
...
<Figure
  slug="stat-effect-sizes-cohen-s-d-inline-cutout"
  alt="The gap between two mounds measured by a caliper whose scale is the mounds' own width"
  caption="Cohen's d measures the gap between two means in units of their own spread."
/>
...
The overlap column is worth seeing rather than reading. Each panel below
is the same two groups at a different `d`, and even the "large" one leaves
them sharing most of their range:
...
<Figure
  slug="stat-effect-sizes-problem-significance-not-inline-cutout"
  alt="Two mounds barely apart with a huge gauge beside them reading high, and a tiny caliper measuring the actual gap"
  caption="A big enough sample makes a trivial gap significant; the effect size stays small."
/>
...
A p-value answers "could this be nothing", which stops being interesting at
large n because at large n almost nothing is exactly nothing. An effect size
answers "how big is it", which is the question that decides whether anybody
should act.

Nothing about the world moved between the left of that chart and the right of
it. What moved is how confidently "exactly zero" can be ruled out, and exactly
zero was never a serious hypothesis about two real groups. The confusion runs
one way, which is why it is worth naming: a significant result from a large
sample gets reported as an important one.
...
<Figure
  slug="stat-effect-sizes-inline-cutout"
  alt="Two mounds barely apart with a large marker showing the gap, beside two mounds far apart with the same marker looking tiny"
  caption="The same absolute gap is large or negligible depending on the spread around it."
/>

- **Risk difference** (absolute): $p_{\text{treat}} - p_{\text{control}}$. "The
  conversion rate went up by 2 percentage points."
- **Relative risk / risk ratio** (relative): $p_{\text{treat}} / p_{\text{control}}$.
