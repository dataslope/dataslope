...
description: Range, variance, standard deviation, the interquartile range, the median absolute deviation, and the coefficient of variation, why how spread out the data is matters as much as where its center sits.
---
...
<Figure
  slug="stat-measures-of-spread-cutout"
  alt="Two mounds on a platform sharing one centre pin, one narrow and tall, the other wide and low"
/>
...
The **range** is just $\text{max} - \text{min}$. It's the easiest spread to compute
...
print(f"Same process, n=20    -> range = {np.ptp(small):.1f}")
print(f"Same process, n=20000 -> range = {np.ptp(large):.1f}")
...
Measured rather than argued: 4,000 samples at each size, both divisors on
every one, averaged against the truth.
...
data: the 75th percentile minus the 25th percentile ($Q_3 - Q_1$).
...
$1.5 \times \text{IQR}$ outlier rule we'll meet in *Shape and Outliers*.
...
A statistic's **breakdown point** is the fraction of the data somebody has to
corrupt before they can move it wherever they like, and it is the cleanest
single number for robustness.

<Chart slug="breakdown-point-mad-sd" />

Those three thresholds are not arbitrary. A mean has a breakdown point of
zero, because one value taken to infinity takes the answer with it, and the
standard deviation is a mean of squared deviations. A quarter is where
corruption reaches the upper quartile. And a half is the highest breakdown
point anything can have, because past half the corrupted points are the
majority and no statistic can tell which half is the data.

That is also why "check for outliers first" is not a substitute. Outlier
detection means comparing points to a spread estimate, so a rule built on the
standard deviation is already broken by the outliers it is hunting. Robust
estimates are used to *find* outliers, not the other way round.

## The coefficient of variation: comparing spread across scales
...
Here is the comparison drawn, with the standard deviations and the ratios side
by side:

<Chart slug="coefficient-of-variation" />

Three conditions have to hold before that number means anything, and all three
are about the mean: a ratio scale with a real zero, a mean far from zero, and
positive values. Where they hold, the CV is the right way to compare
variability across quantities that share no units at all.

A standard deviation carries the units of the thing it measures, so comparing
two of them across different scales is comparing a length in millimetres with
a length in miles by counting digits. The coefficient of variation is
dimensionless, which is exactly what a cross-scale comparison needs. The
caveats bite where the mean gets small: a variable without a real zero changes
its CV when you change units, which is how you know the number was never a
property of the thing, and a quantity averaging 0.02 with a spread of 0.05 has
a CV of 2.5, which is arithmetic rather than information.
...
<Figure
  slug="stat-measures-of-spread-inline-cutout"
  alt="Two rows of chips of the same width, one clustered tightly at the centre and one spread evenly end to end"
  caption="A gap between the standard deviation and the interquartile range signals skew."
/>
