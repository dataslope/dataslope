...
- **One-way ANOVA** (analysis of variance), compares the means of **3+
  groups** at once.
...
<Figure
  slug="stat-anova-chi-square-cutout"
  alt="Four group mounds compared at once on one platform beside a grid of observed and expected count cells"
/>
...
<Figure
  slug="stat-anova-and-chi-square-common-misconceptions-gathered-inline-cutout"
  alt="Three covered pits along a bench, each with a plank laid over it"
  caption="Run enough pairwise tests and one fires by luck alone."
/>
...
<Figure
  slug="stat-anova-and-chi-square-inline-cutout"
  alt="Four mounds standing in a row with one gauge measuring the spread between their peaks against the spread within each"
  caption="ANOVA weighs the spread between group means against the spread inside each group."
/>

Both panels below carry the identical three group means. Only the noise
inside the groups changes, and that alone decides whether the gaps are
worth anything:
...
<Figure
  slug="stat-anova-and-chi-square-part-b-chi-inline-cutout"
  alt="A grid of trays holding counted chips, with a shorter solid marker block standing beside each tray showing the count that was expected"
  caption="Chi-square compares the counts you saw against the counts you expected."
/>

The chi-square statistic is compared against a family of distributions
rather than a single one. Its shape moves with the degrees of freedom,
which is why a chi-square critical value comes from a table instead of
being one number:
