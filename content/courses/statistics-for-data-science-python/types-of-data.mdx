...
<Figure
  slug="stat-types-of-data-big-split-categorical-inline-cutout"
  alt="A bench split by a divider, labelled tags on one side and a measuring rule on the other"
  caption="Labels or measurements: the split that decides every summary you can use."
/>
...
<Chart slug="measurement-scales-ladder" />

The value is all in the empty half of that table. Nearly every statistical
mistake made with a categorical variable is an operation from a higher rung
applied to a lower one, and not one of them raises an error.

The rungs are worth reading as questions rather than names. Ordinal adds
"which is more", so a median and a rank become available but not subtraction,
because the distance from agree to strongly agree is not a known quantity.
Interval adds "how much more", which makes a mean and a standard deviation
meaningful and stops short of ratios, because zero is a convention and 20 °C
is not twice 10 °C. Ratio adds "how many times more", because zero means none
of the thing, and that is what makes a geometric mean and a coefficient of
variation defined at all.
...
The arithmetic is valid in the sense that it finishes. Only the column's
meaning says whether the answer is nonsense, which is why a type system cannot
catch this and a data dictionary can.

<Chart slug="code-is-not-a-quantity" />

The habit that catches all four: for every numeric column, ask whether doubling
it means anything. If it does not, the column is a label with digits in it and
belongs in the dataframe as a category.

The four fail differently. A postcode is nominal, so 90210 is not more than
10001, it is a different place, and the mean of a set of them is usually the
sea. A jersey number is a label with a tradition attached, and 23.4 describes
nobody. A year is interval, so differences work and the mean is meaningful
while ratios are not, and "2024 is 1.005 times 2014" is a sentence no one
should write.
...
<Figure
  slug="stat-types-of-data-inline-cutout"
  alt="Four trays holding respectively labelled tags, ranked tags, counted blocks and a smooth measuring rule"
  caption="Nominal, ordinal, count and continuous each admit different operations."
/>
...
print()
# The median of an ordinal is the middle *category*, which is what the codes
# give you: pandas will not compute it for you (see the note below).
median_plan = df["plan"].cat.categories[int(df["plan"].cat.codes.median())]
print("Median plan (valid because ordinal+ordered):", median_plan)
print()
print(
...
<Callout type="info" title="Why the median is computed by hand here">
Pandas will not call `.median()` on a categorical column, even an ordered
one: it raises `TypeError: 'Categorical' with dtype category does not
support operation 'median'`. That is a deliberate refusal rather than a
gap. Pandas knows the *order* of your categories but not the *distances*
between them, and the median it would have to return is a category rather
than a number, so it declines to guess. Taking the median of
`.cat.codes` and indexing back into `.cat.categories` does exactly what an
ordinal median means: find the middle position, report the label there.
</Callout>
