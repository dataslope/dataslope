...
description: Why you should see a distribution before you summarize or test it, histograms, box plots, violins, cumulative distribution plots, and smoothed density curves, and how a single summary statistic can lie.
---
...
<Figure
  slug="stat-visualizing-cutout"
  alt="Four small panels on a platform showing a bar silhouette, a box marker, a violin shape and a staircase curve"
/>
...
<Figure
  slug="stat-visualizing-distributions-summaries-lie-always-inline-cutout"
  alt="One gauge with a single pointer lifted away to reveal four very different scatters beneath it"
  caption="Very different shapes can hide behind identical summary numbers."
/>
...
That's the one-dimensional version of the trap. The two-dimensional
version is Anscombe's quartet itself, and it's worth seeing on the real
data. The four datasets share the same mean of `x`, mean of `y`,
standard deviations, and correlation, yet a scatter plot of each tells
a different story. Run the summary first:
...
aq = pd.read_csv("anscombe.csv")`,
      starterCode: `display(aq.groupby("dataset")[["x", "y"]].agg(["mean", "std"]).round(2))
print("\\nFour datasets, indistinguishable summaries.")
`,
    },
  ]}
/>

Now plot them. The numbers said "identical"; the pictures say anything
but. This is the canonical argument for plotting before you summarize.
...
The same four datasets, drawn with the fitted line all of them share:
...
The trap in one picture. Same data in all three panels, only the bin width
changes, and nothing in the data tells you which panel to believe:
...
A **box plot** draws the distribution's skeleton: the box spans $Q_1$ to
$Q_3$ (the IQR), a line marks the median, and the whiskers reach to the
last points within the $1.5 \times IQR$ fences from *Measures of Spread* and
...
<Figure
  slug="stat-visualizing-distributions-inline-cutout"
  alt="The same stack of chips shown as a mound, as a box with a thin rod at each end and as a rising staircase"
  caption="Histogram, box plot and cumulative curve show the same data three ways."
/>
...
The knob is the point: a KDE is a modelling choice wearing the costume of a
summary, and the bandwidth decides how many peaks you see.

<Chart slug="kde-bandwidth" />

## ECDF: the plot with no knobs

The **empirical cumulative distribution function (ECDF)** answers, for
every value $x$, "what fraction of the data is $\le x$?" You sort the
