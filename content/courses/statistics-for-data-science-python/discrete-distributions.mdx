...
<Figure
  slug="stat-discrete-distributions-cutout"
  alt="A row of separate integer bins on a platform with spheres stacked to differing heights in each"
/>
...
<Figure
  slug="stat-discrete-distributions-bernoulli-single-yes-inline-cutout"
  alt="One two-position switch thrown once, its lamp either lit or dark"
  caption="A Bernoulli trial is one switch thrown once: the atom of the others."
/>
...
<Figure
  slug="stat-discrete-distributions-binomial-counting-successes-inline-cutout"
  alt="Ten two-position switches thrown in a row with a tally rack counting how many came up lit"
  caption="The binomial counts how many of n independent trials came up a success."
/>
...
$P(X \ge k+1)$. So $P(X \ge 10)$ is `.sf(9)`, and $P(X \le 4)$ is
`.cdf(4)`. Mixing up $>$ and $\ge$ here is the single most common
...
Before writing any code, here is what the two parameters do. `n` sets how
many trials there are to count, so it fixes the range; `p` sets where the
mass piles up inside that range:
...
Three of the binomial's four assumptions are usually obvious: a fixed number
of trials, two outcomes, a constant probability. The fourth, independence, is
the one that quietly fails, because real trials are usually people and people
come in households, classes, sessions and regions.

<Chart slug="binomial-assumption-breaks" />

The multiplier has a name, the design effect, and it is 1 + (m − 1)ρ for
clusters of size m. It is worth knowing because it is brutal: a ρ of 0.05 with
clusters of fifty gives a design effect near 3.5. The fix is to analyse at the
level randomization happened, and what does not work is ignoring it, which is
easy to do because the point estimate is fine.

Those 400 rows in 40 clusters carry about as much information as 93
independent trials, and the consequence lands on every interval. Computed with
n = 400, a binomial confidence interval is roughly 0.48 of the width it should
be, so an A/B test on clustered data finds significance about as often as
running the test several times and keeping the best.

## Poisson: counting rare events in a fixed interval


How the shape moves with the rate, and the property that identifies it:
the mean and the variance are the same number.
...
$\lambda = 3 \times 8 = 24$, not 3. Rates add: an interval twice as long
...
That identity is also the Poisson's whole personality, and its weak point. Fit
one to any counts and it will match the average exactly, then be wrong about
the spread by however much the data disagrees.

<Chart slug="poisson-overdispersion" />

The check is one line: the variance-to-mean ratio, called the dispersion. One
means Poisson; above one is overdispersion, and that is what nearly all real
count data does, because Poisson assumes events arrive independently at a
constant rate and real events cluster. The standard replacement is the
negative binomial, which changes nothing about the mean and everything about
the intervals.

The tail is where that costs money, because every capacity plan, alerting
threshold and staffing model built on the fit is calibrated for a world with
less variance than the one it runs in. Outages come in storms, sales come in
campaigns, arrivals come in buses.
...
The shape is the same for every p, and it is worth seeing once, because the
mean is a poor summary of it. The most likely number of attempts is always
one, since the highest-probability outcome is that it worked straight away,
and the curve only decreases from there.

<Chart slug="geometric-waiting-time" />

The property that makes this distribution strange is memorylessness: given
that the first seven attempts failed, the distribution of how many more you
need is the same one you started with. Seven failures are not evidence that a
success is due, and not evidence that it is not.

A retry budget planned around the mean therefore fails often enough to matter.
Memorylessness is also the assumption worth checking, because real retries are
usually not independent: whatever made the first attempt fail is often still
happening, which makes the tail heavier than this and a fixed budget less safe
still.
...
<Figure
  slug="stat-discrete-distributions-inline-cutout"
  alt="A row of separate bins holding whole blocks, with clear gaps between the bins and no partial blocks anywhere"
  caption="Discrete values come in whole steps, with nothing in the gaps between them."
/>
...
<MultipleChoice
  markdown={`For \`stats.binom(n=100, p=0.2)\`, which expression gives $P(X \\ge 30)$ (30 or more successes)?

- \`d.sf(30)\`
  > \`sf(30)\` is $P(X > 30) = P(X \\ge 31)$, which excludes the value 30 and is off by one.
- \`1 - d.sf(30)\`
  > This equals \`cdf(30)\` = $P(X \\le 30)$, again the lower tail rather than $P(X \\ge 30)$.
- [o] \`d.sf(29)\`
  > Since \`sf(k)\` is $P(X > k)$, evaluating it at 29 gives $P(X > 29) = P(X \\ge 30)$, the inclusive upper tail.
- \`d.cdf(30)\`
  > \`cdf(30)\` is $P(X \\le 30)$, the lower tail, the opposite of what's asked.

For discrete distributions, translate the inequality first: $P(X \\ge k)$ is \`sf(k-1)\`.`}
/>
...
- The scipy recipe is universal: freeze the distribution, then call
  `.pmf()`, `.cdf()`, `.sf()`, `.mean()`, `.var()`, `.rvs()`. Remember $P(X \ge k)$
  is `.sf(k - 1)`.
</Callout>
