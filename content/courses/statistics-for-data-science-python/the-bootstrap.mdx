...
<Figure
  slug="stat-the-bootstrap-core-idea-sample-inline-cutout"
  alt="A small tray of chips being scooped from repeatedly, each scoop poured back before the next"
  caption="Each chip goes back before the next is drawn, which is what makes it a bootstrap."
/>
...
<Figure
  ...
  caption="Resampling your own sample, with replacement, stands in for sampling the world."
/>
...
<Figure
  slug="stat-the-bootstrap-payoff-ci-median-which-inline-cutout"
  alt="One sample drawn from over and over with replacement, the middle reading of each redraw piling up into a distribution"
  caption="Resample the sample and the spread of the redrawn medians is the interval you wanted."
/>
...
There is also a class of statistic the method cannot handle at all, and it is
worth seeing beside one it handles beautifully:

<Chart slug="bootstrap-fails-for-max" />

The shape of the failure is worth remembering. Statistics that depend on the
*interior* of a distribution bootstrap beautifully, because a resample is a
fair sample of the interior. Statistics that depend on the *edge* do not,
because the edge of a resample is bounded by the edge of the sample and the
method can never look past what it has already seen.

A resample can only contain values the original sample contained, so the
bootstrapped maximum is always one of the few largest observations. The
practical test: if drawing one more observation from the population would move
the statistic more often than resampling does, the bootstrap is understating
its uncertainty. Extremes are the clear case, and the right tool for them is
extreme value theory, which models the tail rather than resampling it.
