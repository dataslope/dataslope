...
Notice the $\text{mean} - \text{median}$ column tracks the skew sign exactly. That's
...
Put the grouping back and the valley explains itself:

<Chart slug="modality-is-a-mixture" />

Bimodality is not a property a variable has, it is a symptom, and the
diagnosis is almost always that two things were pooled. The fix is not a
fancier distribution: it is to split and report both, because everything
downstream assumed one population.

The standard deviation is no better, since it is now measuring the distance
between two groups rather than the spread of anything, and the grouping was in
the data all along, in a column nobody plotted. Drivers with cyclists, cache
hits with cache misses, two cohorts, two currencies. The awkward case is when
the grouping was never recorded: a mixture model can then recover the
components from the shape alone, which is a real technique and much weaker
evidence, because a two-component fit will happily split a distribution that
is merely skewed.
...
<Figure
  slug="stat-shape-and-outliers-skewness-which-way-inline-cutout"
  alt="Two mounds side by side, one with a long tail trailing right and one trailing left"
  caption="Skew is which side the long tail falls on, and it moves the mean toward it."
/>
...
Mean and standard deviation pin a normal distribution down completely, and for
anything else they leave the part that matters unspecified:

<Chart slug="kurtosis-same-sd" />

Every rule built on "three sigma" is quietly assuming a shape. Finance,
insurance, network traffic and queueing all live in distributions that do not
have it.

The heavy-tailed one is *narrower* in the middle and much fatter at the edges:
more values very close to the mean, more very far from it, fewer in between.
The uniform has no tail past its edges at all. Kurtosis is the number that
separates them, and it is worth knowing what it is not: despite decades of
textbooks it is not peakedness. It is dominated by the fourth power of the
distance from the mean, so it is almost entirely a statement about how much
probability lives far out.
...
<Figure
  slug="stat-shape-and-outliers-inline-cutout"
  alt="A lopsided mound of chips with a long tail to one side and two chips sitting far out beyond the tail"
  caption="An outlier is a point far past the tail, and it may be real or an error."
/>
...
**The 1.5×IQR rule (box-plot fences).** Compute $Q_1$, $Q_3$, and
$IQR = Q_3 - Q_1$. Anything below $Q_1 - 1.5 \times IQR$ or above
$Q_3 + 1.5 \times IQR$ is flagged. Because it's built on quartiles, it's
**robust**, the outliers themselves don't move the fences, so it
works on skewed data.

**The z-score rule.** Compute each point's $z = (x - \text{mean}) / \text{std}$ and
...
A reusable flagger built on the interquartile range is something you'll
write constantly while exploring. The pattern: compute the fences, return which values fall outside,
and how many.
