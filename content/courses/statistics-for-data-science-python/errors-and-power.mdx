...
<Figure
  slug="stat-errors-power-cutout"
  alt="A two-by-two grid of four cells on a platform, two cells holding a correct tick and two holding a distinct error marker"
/>
...
<Figure
  slug="stat-errors-and-power-2x2-decision-matrix-inline-cutout"
  alt="A two-by-two board of trays, one holding a false alarm and another a missed catch"
  caption="Two ways to be wrong: a false alarm, and a real effect you missed."
/>
...
<Figure
  slug="stat-errors-and-power-tradeoff-can-t-inline-cutout"
  alt="A beam on a pivot with a false alarm on one end and a missed catch on the other, pressing one down raising the other"
  caption="With the sample fixed, pushing one error rate down pushes the other up."
/>
...
The two error rates are traded against each other by where the threshold
sits, so tightening one loosens the other. Sweeping the threshold across its
whole useful range turns that direction into numbers you can read off:

<Chart slug="error-rates-vs-threshold" />
...
<Figure
  slug="stat-errors-and-power-inline-cutout"
  alt="A two-by-two board of trays, one tray holding a false alarm and another a missed catch, with a dial beside it turning to shrink the miss"
  caption="More data is the only lever that shrinks both kinds of error at once."
/>
...
def estimate_power(effect, n, reps=800):
    rej = 0
    for _ in range(reps):
...
The size of the distortion depends on how underpowered the study was:

<Chart slug="winners-curse" />

The filter is not neutral about size. A study can only reach significance if
its estimate is large enough, so among the studies that pass, the estimates
are the ones where the noise happened to point the right way, and the
published figure is biased upward by construction without anybody having done
anything wrong. The fixes are structural rather than statistical: power the
study, and pre-register so the non-significant runs stay visible.

Everyone knows an underpowered study usually misses real effects, which is
merely wasteful. Fewer people notice that when it does find something, the
number it reports is wrong in a predictable direction, and that number is what
feeds the meta-analysis, the press release and the next study's power
calculation.
