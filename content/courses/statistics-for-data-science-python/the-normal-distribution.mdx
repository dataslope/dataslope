...
<svg
  viewBox="0 0 640 220"
  role="img"
  aria-label="A bell curve built from three nested translucent layers, a wide base, a middle shell, and a tight core around the yellow center post, with a lone red dot stranded beyond the widest layer, the 68, 95, and 99.7 percent zones of the normal"
...
The normal has exactly two parameters: the mean μ sets the
**center**, and the standard deviation σ sets the **spread**.
Change μ and the curve slides; change σ and it gets wider or
...
### A real measurement that's close to normal

Theory says measurements that accumulate many small effects should look
bell-shaped. Does that hold on real data? The **Palmer penguins**
dataset gives us a clean test: the flipper lengths of a *single* species
(Adelie) are exactly the kind of "sum of many small biological nudges"
that the CLT predicts. Plot a histogram with a fitted normal overlaid.
...
penguins = pd.read_csv("penguins.csv")`,
      starterCode: `flippers = penguins.loc[
    penguins["species"] == "Adelie", "flipper_length_mm"
].dropna()

mu, sigma = flippers.mean(), flippers.std()
grid = np.linspace(flippers.min(), flippers.max(), 200)
...
print(f"mean = {mu:.1f} mm,  std = {sigma:.1f} mm")
`,
    },
  ]}
/>

It's not a textbook-perfect bell, real data never is, but the fitted
normal tracks the histogram closely. That "close enough" is what lets us
use normal-based tools on measurements like this. Mixing all three
species together would *break* it: you'd get the lumpy, multi-peaked
shape of three different bells stacked on top of each other, which is the
warning the last section of this page makes precise.
...
If exam scores are normal with μ = 100, σ = 15, then ~95% of
...
**standard normal**: μ = 0, σ = 1, `stats.norm()` with no
arguments.
...
<svg
  viewBox="0 0 640 200"
  role="img"
  aria-label="A blue test score and a green height each slide down a thin line from their own differently scaled rulers onto one shared z ruler below, landing side by side at the same tick, standardizing strips the units so unlike things can be compared on a common scale"
  style={{ display: "block", width: "100%", maxWidth: "600px", height: "auto", margin: "2.5rem auto" }}
>
  <g fill="var(--ds-gray-300)">
    <rect x="70" y="46" width="220" height="3" rx="1.5" />
    <rect x="350" y="46" width="220" height="3" rx="1.5" />
  </g>
  <g style={{ color: "var(--ds-blue-500)" }} fill="currentColor">
  ...
  <g stroke="var(--ds-gray-300)" strokeWidth="2">
    <line x1="254" y1="48" x2="446" y2="124" />
    <line x1="528" y1="48" x2="460" y2="124" />
  </g>
  <rect x="80" y="140" width="480" height="4" rx="2" fill="var(--ds-gray-400)" />
...
scores = [900, 1050, 1300]`,
      starterCode: `# TODO: compute z_scores and percentiles (both lists of 3 floats, same order)
z_scores = None
percentiles = None
  ...
  files={[
    {
      ...
      initCode: `from scipy import stats
mu, sigma = 1050, 200`,
      starterCode: `# TODO: find the top-5% cutoff score
cutoff = None
...
fig.add_trace(
    go.Histogram(
        x=income, histnorm="probability density", nbinsx=60, name="actual income"
    )
)
...
- A **z-score** `z = (x − μ) / σ` is a *position* (SDs from the mean),
