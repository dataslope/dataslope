...
<Figure
  slug="stat-working-with-distributions-four-questions-four-inline-cutout"
  alt="Four small tools hanging above one smooth ramp, each shown taking a different reading from it"
  caption="Four methods cover every distribution: density, cumulative, quantile and draw."
/>
...
One more question comes up often enough to name: *how many are still
going after time t?* That is the survival function, which is just
`1 - cdf` given a name of its own.

<Chart slug="survival-curve" />
...
<Figure
  slug="stat-working-with-distributions-inline-cutout"
  alt="A smooth ramp over a rail with a shaded slice cut from it and a gauge reading that slice's share"
  caption="Fixing the shift and the scale freezes a distribution you can then read off."
/>
...
A good fit and a bad one, side by side, so the second check has something
to be compared against:

<Chart slug="qq-plot-fit" />
