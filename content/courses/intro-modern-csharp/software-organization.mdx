...
<Figure
  slug="mcs-software-organization-cutout"
...
<Figure
  slug="mcs-software-organization-files-inline-cutout"
  alt="A set of labelled folders in a rack, each holding one kind of card"
  caption="One public class per file, named after the class, so you open rather than search."
/>
...
<Figure
  slug="mcs-software-organization-namespaces-inline-cutout"
  alt="Two identically shaped parts kept apart by the room each hangs in rather than by their shape"
  caption="Two libraries may each have a `Logger`, because the namespace keeps them apart."
/>
...
The C# runtime on these pages evaluates your code as a *script*, the way
`dotnet script` or the C# Interactive window does, and a script is a single
implicit compilation unit. A file-scoped `namespace Banking;` has to precede
every other member of its file, so once the two files above are combined the
compiler reports `CS8956: File-scoped namespace must precede all other members
in a file`. Namespaces across separate files are the whole point of this
lesson, so the example is shown rather than run.
</Callout>
...
<Figure
  slug="mcs-software-organization-projects-solutions-inline-cutout"
  alt="Several small workshops listed on one plan sheet, each with its own bench and shared doors between them"
  caption="A project is one unit that builds; a solution is the set of them that ships together."
/>
...
<Callout type="info" title="Not runnable in the browser">
Same reason as the namespace example above: three files with file-scoped
namespaces cannot be combined into the single script the in-browser C# runtime
compiles. Splitting a program across `Bank.Domain` and `Bank.App` is the
lesson, so it is shown rather than run.
</Callout>
