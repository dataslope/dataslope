...
<Figure
  slug="mcs-managed-runtimes-cutout"
  alt="Managed runtimes, an isometric illustration"
/>

We'll stay conceptual, no deep internals. The goal is intuition,
not a reading list of runtime source code.
...
<Figure
  slug="mcs-managed-runtimes-runtime-one-paragraph-inline-cutout"
  alt="A program block resting on a platform with a small tidy-up cart running underneath it"
  caption="You trade a little control and speed for a whole class of bugs you never write."
/>
...
<Figure
  slug="mcs-managed-runtimes-inline-cutout"
  alt="A program block resting on a wide platform, with a small tidy-up cart under the platform sweeping discarded blocks away on its own"
  caption="The runtime handles the memory, so nothing has to be freed by hand."
/>
...
The collector can afford to run constantly because of one lopsided fact
about how programs allocate: nearly everything dies immediately, so a
sweep of the youngest objects costs almost nothing.

<Chart slug="object-lifetime-survival" />

That shape is why .NET's collector is *generational*: it checks the
newest objects often and cheaply, and only rarely bothers walking the
survivors. The practical consequence for you is that short-lived
allocations are far less expensive than they sound.
