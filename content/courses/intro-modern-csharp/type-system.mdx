...
<Figure
  slug="mcs-type-system-static-typing-one-inline-cutout"
  alt="A shaped gate that stops a mismatched block before the belt starts moving at all"
  caption="Static typing checks the fit before the program is ever run."
/>
...
The whole-number widths are not a style choice. Each has a ceiling, and
the question is always whether the thing you are counting fits under it:

<Chart slug="integer-type-ranges" />

For everyday code the rule that falls out of this is short: use `int`,
and switch to `long` the moment you are counting something the world
might have more than two billion of. Money stays the exception to
both, `decimal`, for the rounding reasons the last chapter showed.
...
<Figure
  slug="mcs-type-system-inline-cutout"
  alt="A gate with shaped slots, blocks of the wrong shape stopping at the gate and blocks of the right shape passing through onto a rail"
  caption="A conversion is how a value of one type is made to fit another."
/>
