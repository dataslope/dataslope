...
<Figure
  slug="mcs-error-handling-cutout"
  alt="Error handling, an isometric illustration"
/>
...
<Figure
  slug="mcs-error-handling-flow-exception-inline-cutout"
  alt="A block leaving a belt, falling past two open baskets, and being caught by the third"
  caption="An exception travels up the stack until some handler catches it."
/>
...
<Figure
  slug="mcs-error-handling-inline-cutout"
  alt="A cracked block dropping off a bench into a padded catch basket, while a small sweeping arm clears the bench behind it whether or not anything fell"
  caption="`finally` runs whether or not anything was thrown, which is what it is for."
/>
...
This is a really common beginner trap, and it has a measurable cost. A
throw is not a jump: the runtime captures a stack trace and walks frames
looking for a handler, which is microseconds against nanoseconds for a
returned status.

<Figure
  slug="mcs-error-handling-exceptions-mechanism-inline-cutout"
  alt="A block falling off a belt into a padded basket held out beneath it"
  caption="An exception is for the case you cannot handle here, not for ordinary flow."
/>

<Chart slug="exception-vs-return-cost" />

Note that this only matters at volume. One exception on a genuinely
exceptional path costs nothing you will ever measure. A thousand
exceptions a second on a path that is really just "the user typed
something that isn't a number" is a different story.
