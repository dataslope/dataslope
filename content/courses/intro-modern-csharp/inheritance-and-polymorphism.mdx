...
<Figure
  slug="mcs-inheritance-and-polymorphism-inline-cutout"
  alt="Three machines of different shapes all wearing the same standard front panel, one handle turned on each producing a different result"
  caption="One call, different behaviour, decided by the actual type behind the handle."
/>
...
<Figure
  slug="mcs-inheritance-and-polymorphism-inheritance-basic-mechanics-inline-cutout"
  alt="A base cabinet with one cabinet behind it built on the same frame with one extra drawer"
  caption="A derived class keeps the base frame and adds what it needs."
/>
