...
<Figure
  slug="mcs-methods-modularity-cutout"
...
<Figure
  slug="mcs-methods-and-modularity-first-method-inline-cutout"
  alt="A small boxed machine with one hopper and one chute, standing alone on a clear bench"
  caption="A method takes its inputs, does one job, and hands back a result."
/>
...
Console.WriteLine(Maths.Add(1.5, 2.5));
Console.WriteLine(Maths.Add("hello, ", "world"));

// Overloads have to live on a type. Top-level statements compile to
// *local* functions, and C# does not allow two local functions to share
// a name, so this is the shape overloading actually takes.
