...
`AsParallel()` lives in `System.Linq.Parallel`, which the .NET WebAssembly
bundle these pages load does not ship, so the compiler reports `CS1061:
'IEnumerable<int>' does not contain a definition for 'AsParallel'`. There would
be nothing to distribute across in any case: the runtime here is
single-threaded. Run this one in a desktop .NET project, where PLINQ has real
cores to use.
</Callout>
