...
<Figure
  slug="cs-filtering-and-projection-cutout"
  alt="Filtering and projection, an isometric illustration"
/>
...
<Figure
  slug="cs-filtering-and-projection-mental-model-inline-cutout"
  alt="A row of parcels passing a grate that drops some, then a press that reduces each survivor to one small tag"
  caption="A rejected element never reaches the later operators at all."
/>
...
<Figure
  slug="cs-filtering-and-projection-inline-cutout"
  alt="A row of mixed parcels passing a grate that drops half of them, and the survivors passing a stamp that reduces each to one small tag"
  caption="`Where()` drops the elements that fail; `Select()` reshapes the ones that pass."
/>
