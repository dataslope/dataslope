...
<Figure
  slug="cs-immutability-bug-immutability-prevents-inline-cutout"
  alt="Two benches sharing one tray, a hand at each bench changing it, and the block on it ending up wrong"
  caption="Two places holding one mutable object is where the value goes wrong."
/>
...
And when it does matter, the immutable collections are not doing the naive
thing. `ImmutableList<T>` is a tree, so an update rewrites one path through
it and shares the rest with the list you started from:
...
<Figure
  slug="cs-immutability-inline-cutout"
  alt="A sealed block set aside untouched while a fresh copy with one changed face is placed beside it"
  caption="An immutable value is never changed; a modified copy is made instead."
/>
