...
<Figure
  slug="cs-selectmany-and-flattening-problem-select-alone-inline-cutout"
  alt="Three small boxes each holding three blocks, lifted out still boxed, beside the same boxes tipped onto one rail"
  caption="`Select()` leaves you a sequence of sequences; `SelectMany()` flattens it."
/>
...
It carries one hazard worth knowing. Flattening multiplies rows, which is
fine until something downstream sums a value from the parent side:
...
<Figure
  ...
  caption="Each inner collection is tipped out onto one line in order."
/>
