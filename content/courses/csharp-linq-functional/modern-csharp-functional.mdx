...
<Figure
  slug="cs-modern-functional-cutout"
  alt="Modern C# as a functional language, an isometric illustration"
/>
...
<Figure
  slug="cs-modern-csharp-functional-lambda-expressions-c-inline-cutout"
  alt="A small tool shaped on a card and clipped straight into a waiting bracket on a machine, never given a name plate of its own"
  caption="A lambda is a function written where it is used, with no name of its own."
/>
...
<Figure
  slug="cs-modern-csharp-functional-pattern-matching-switch-expressions-inline-cutout"
  alt="A set of shaped gates in a row, exactly one letting the piece through and handing back a value"
  caption="A switch expression returns a value, so it composes wherever a value is wanted."
/>
...
static string Describe(object o) => o switch { null => "nothing",
                                               int n when n <
                                                   0 => $"negative int {n}",
                                               int n => $"int {n}",
...
static (int min, int max, double avg) Stats(int[] xs) => (xs.Min(), xs.Max(), xs.Average());
...
var sales = new List<Sale> {
    new("EU", "Widget", 12.50m),   new("EU", "Gadget", 44.00m),   new("EU", "Widget", 9.99m),
    new("APAC", "Widget", 31.00m), new("APAC", "Gadget", 21.50m), new("US", "Gizmo", 77.00m),
};

var report = sales.GroupBy(s => s.Region)
                 .Select(g => new {
                     Region = g.Key,
                     Products = g.GroupBy(s => s.Product).Select(p => $"{p.Key}={p.Sum(s => s.Amount):C}").ToArray(),
