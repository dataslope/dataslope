...
<Figure
  slug="cs-composing-pipelines-cutout"
  alt="Composing pipelines, an isometric illustration"
/>
...
<Figure
  slug="cs-composing-pipelines-inline-cutout"
  alt="Four short pipe sections being clicked together into one continuous line, shapes entering at the left and leaving sorted at the right"
  caption="Operators clip together, and the sequence flows through all of them in one pass."
/>
...
var report = orders.Active().Recent(DateTime.UtcNow.AddDays(-30)).HighValue().Sum(o => o.Total);
...
{
    public static IEnumerable<Order> Active(this IEnumerable<Order> source) => source.Where(o => o.Status == "active");

    public static IEnumerable<Order> Recent(this IEnumerable<Order> source,
                                            DateTime since) => source.Where(o => o.PlacedAt >= since);

    public static IEnumerable<Order> HighValue(this IEnumerable<Order> source,
                                               decimal min = 1000m) => source.Where(o => o.Total > min);
}
`,
    },
  ]}
...
<Figure
  slug="cs-composing-pipelines-building-custom-operators-inline-cutout"
  alt="A hand-built section of pipe being clipped into a run of standard sections, indistinguishable once in place"
  caption="An operator you write with `yield` chains exactly like a built-in one."
/>
...
var changes = prices.Pairwise().Select(p => p.curr - p.prev);

foreach (var d in changes)
    Console.WriteLine($"change: {d:+0.0;-0.0}");
...
<Figure
  slug="cs-composing-pipelines-every-pipeline-value-inline-cutout"
  alt="A whole chain of joined machines lifted as one piece and set into a larger machine's slot"
  caption="A whole pipeline is itself a value you can pass on and reuse."
/>
...
Func<Func<int, bool>, Func<int, bool>, Func<int, bool>> And = (p, q) => n => p(n) && q(n);
