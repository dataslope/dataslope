...
<Figure
  slug="cs-set-operations-cutout"
  alt="Set operations and joins, an isometric illustration"
/>
...
<Figure
  slug="cs-set-operations-distinct-removing-duplicates-inline-cutout"
  alt="A row of blocks where three are identical, two of them lifted out and set aside"
  caption="`Distinct()` keeps the first of each value and sets the repeats aside."
/>
...
<Figure
  slug="cs-set-operations-inline-cutout"
  alt="Two overlapping rings of blocks on a bench, the blocks in the overlap lifted onto a small stand between them"
  caption="Union, intersect and except work on the overlap between two sequences."
/>
...
Use \`Except()\` and \`OrderBy()\`. Don't write a \`foreach\`.`}
  ...
  files={[
    {
      filename: "Program.cs",
      starterCode: `var last = new[] { "Ada", "Bilal", "Chiara", "Bilal" };
