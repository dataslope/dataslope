...
<Figure
  slug="cs-higher-order-functions-cutout"
...
<Figure
  slug="cs-higher-order-functions-pattern-before-name-inline-cutout"
  alt="A machine with an empty bracket on its side, three different small tools each able to clip into that bracket"
  caption="The behaviour is a value you clip in, so one machine does many jobs."
/>
...
<Figure
  slug="cs-higher-order-functions-inline-cutout"
  alt="A machine on the left with an open slot in its side, a small tool being slid into that slot, and the machine stamping a different shape because of it"
  caption="A higher-order function takes a function as its argument or returns one."
/>
