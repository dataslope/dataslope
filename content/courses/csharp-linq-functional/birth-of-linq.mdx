...
Before we touch LINQ, language-integrated query, we have to understand
the *problem* it solves. LINQ did not appear because Microsoft thought it would be
...
<Figure
  slug="cs-birth-of-linq-cutout"
  alt="Why LINQ exists, an isometric illustration"
/>
...
<Figure
  slug="cs-birth-of-linq-shape-problem-inline-cutout"
  alt="The same awkward shape of hand-built machinery repeated on four benches, each built slightly differently"
  caption="The same query, hand-rolled four times, comes out four different ways."
/>
...
<Figure
  slug="cs-birth-of-linq-bad-gets-one-inline-cutout"
  alt="One small job buried inside a tall stack of scaffolding, the scaffolding far larger than the job"
  caption="Written by hand, the plumbing dwarfs the question you were asking."
/>
...
section](/courses/csharp-linq-functional/why-declarative-won) at the
end of the course.
...
Section 2 needs `System.Xml.Linq`, which the .NET WebAssembly bundle these
pages load does not ship, so the block fails to compile as a whole (`CS0234:
The type or namespace name 'Xml' does not exist in the namespace 'System'`).
Sections 1 and 3 would run fine on their own, but splitting them out would
lose the very thing the example is here to show, three sources answering to the
same operators, so it is shown intact rather than run.
</Callout>

The two runnable sources, on their own:
...
// LINQ to Objects, over an in-memory collection.
var nums = new[] { 1, 2, 3, 4, 5 };
var doubled = nums.Where(n => n % 2 == 1).Select(n => n * 2);
Console.WriteLine("Objects: " + string.Join(", ", doubled));

// The same operators, now grouping.
var ages = new[] { 36, 41, 29, 52, 33, 27 };
var bucketed = ages.GroupBy(a => a / 10 * 10).OrderBy(g => g.Key).Select(g => $"{g.Key}s: {g.Count()}");
...
<Figure
  slug="cs-birth-of-linq-loops-cutout"
  alt="A knot of tangled cord on one side, and on the other a clean pipe carrying three shapes through a filter into a tray"
  caption="C# 3.0, 2007: the bugs were in the shape of the code, so the shape is what LINQ changed."
/>
