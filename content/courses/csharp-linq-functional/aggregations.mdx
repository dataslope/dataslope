...
<Figure
  slug="cs-aggregations-cutout"
  alt="Aggregations, an isometric illustration"
/>
...
<Figure
  slug="cs-aggregations-friendly-built-ins-inline-cutout"
  alt="One tray of blocks feeding four small gauges in a row, each gauge reading a different single value"
  caption="Each built-in aggregate reads the same sequence and reports one number."
/>

One of them is worth a warning. `Average()` is a single number that a
skewed distribution moves a long way from the typical value, so it answers
"what is the mean?" and not "what is a normal one like?":
...
<Figure
  slug="cs-aggregations-empty-sequence-behavior-inline-cutout"
  alt="An empty tray feeding a gauge whose needle sits hard against its stop rather than at zero"
  caption="On an empty sequence some aggregates return zero and others throw."
/>
...
<Figure
  slug="cs-aggregations-inline-cutout"
  alt="A row of blocks folding into one another left to right until a single block remains at the end of the rail"
  caption="`Aggregate()` folds the sequence into an accumulator, one element at a time."
/>
