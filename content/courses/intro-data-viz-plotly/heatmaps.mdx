...
There is a fifth that outranks all of them, and it is the row order. A matrix
has no natural sequence unless the labels are a time or a size, so the default
order hands the reader an accident:
