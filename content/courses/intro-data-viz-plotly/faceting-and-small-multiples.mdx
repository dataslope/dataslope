...
<Figure
  slug="viz-faceting-and-small-multiples-pitch-inline-cutout"
  alt="One crowded board of overlapping ribbons beside four small boards each carrying a single ribbon"
  caption="Shared axes are what let small multiples be compared with each other."
/>
...
The classic use of facets is breaking up a too-busy line chart. The grey
behind each panel below is the spaghetti, kept so the comparison to the group
survives the split:

<Chart slug="spaghetti-vs-facets" />

<Figure
  slug="viz-faceting-and-small-multiples-inline-cutout"
  alt="One crowded chart panel on the left splitting into a row of four small panels on the right, each holding a single clean line"
  caption="Splitting one crowded chart into small panels makes every line readable again."
/>
...
There is a quieter reason too. Faceting divides the evidence, and a panel
built from a sixth of the data draws a slope that is mostly noise:

<Chart slug="facet-splits-the-data" />
