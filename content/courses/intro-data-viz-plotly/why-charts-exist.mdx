...
<Figure
  slug="viz-why-charts-exist-cutout"
  alt="Why charts exist, an isometric illustration"
/>
...
The difference is not aesthetic, and it is measurable. Some visual differences
are found without searching at all, and the giveaway is that adding more items
does not slow you down:

<Chart slug="popout-vs-serial-search" />
...
## Two charts that mattered

Two examples of what all that buys you, one where it worked and one where
it did not.

In 1911 two astronomers plotted two quantities that had been sitting in
tables for decades: how hot a star's surface is, and how much light it
gives off. Nobody expected the points to do anything in particular.

<Chart slug="hertzsprung-russell-scatter" />

The finding is really the *empty space*. Stars do not fill the plane, they
fall on one diagonal with two detached islands. A gap between rows of a table
is invisible; a gap in a scatter is the first thing you see.

Each of those regions turned out to be a stage of one life story: the diagonal
is where a star spends nearly all of its life, the clump above it is what
happens when the hydrogen runs out and the star swells, and the scatter at the
bottom is the exposed core left behind. The named stars carry their real
published values; the background population is simulated from the standard
relations, so its shape is right and its individual dots are not real stars.

The other example is the clearest case anyone has of a chart that would have
changed history. On the night before the Challenger launch, engineers who
believed the cold would destroy the O-ring seals faxed NASA the flights that
had suffered O-ring damage.

<Chart slug="challenger-o-ring-1986" />

Sending only the damaged flights is the natural thing to do when the subject
is damage, and it is exactly the wrong thing. Selecting rows on the outcome
throws away every cold flight that came back clean and every warm one that
did, which is where the signal lives, so seven damaged flights spread from
53°F to 75°F look like noise. Nothing in the left panel is mislabelled,
distorted, or badly drawn. It is wrong about *which rows are in it*, and that
was enough.
...
Five numbers that are all about the same, drawn and then tabulated, make
the case in one picture:

<Chart slug="table-beats-chart" />
...
Twenty-four numbers, twice. Same information, different amount of work:

<Chart slug="cognitive-offloading" />

<Figure
  slug="viz-why-charts-exist-charts-as-cognitive-offloading-inline-cutout"
  alt="A heavy load lifted off one figure's shoulders onto a board that they and a second figure both read from"
  caption="A chart moves the work of holding numbers in mind onto the page, where two people can share it."
/>
...
<Figure
  slug="viz-why-charts-exist-tables-make-read-inline-cutout"
  alt="A dense board of filled cells being read cell by cell beside a chart of solid bars taken in at a glance"
  caption="A table is read value by value; a chart is taken in all at once."
/>
