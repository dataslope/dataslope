...
fall on one diagonal with two detached islands, and those islands turned out
to be stages of a single life story. A gap between rows of a table is
invisible; a gap in a scatter is the first thing you see.
...
Selecting rows on the outcome throws away every cold flight that came back
clean, which is where the signal lives. Nothing in the left panel is
mislabelled, distorted, or badly drawn. It is wrong about *which rows are in
