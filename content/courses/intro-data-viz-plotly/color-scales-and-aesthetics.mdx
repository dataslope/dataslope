...
<Figure
  slug="viz-color-scales-cutout"
  alt="Color scales and aesthetics, an isometric illustration"
/>
...
<Figure
  slug="viz-color-scales-and-aesthetics-inline-cutout"
  alt="A strip running from pale to deep in one hue on the left, a strip running between two opposite hues in the middle, and separate distinct patches on the right"
  caption="Sequential, diverging and qualitative scales answer three different questions."
/>

Each family is a claim about the structure of the variable, so the wrong
one makes a claim the data will not support:
...
<Figure
  slug="viz-color-scales-and-aesthetics-three-families-color-inline-cutout"
  alt="Three swatch strips, one running pale to deep in one ink, one running between two inks, one of separate distinct colors"
  caption="Pick the family from what the numbers mean, not from what looks lively."
/>

## The Jet / rainbow trap, revisited

Lightness is what makes a color scale readable, and it is exactly what the
rainbow gets wrong. Watch what each of these does with it:
...
<Figure
  slug="viz-color-scales-and-aesthetics-jet-rainbow-trap-inline-cutout"
  alt="A strip cycling through many hues with false bands appearing at its joins, beside a smooth single-ink strip"
  caption="A rainbow scale invents bands the data never had; a uniform one does not."
/>

## Accessibility: colorblind-safe palettes

Roughly **8% of men and 0.5% of women** have some form of color
vision deficiency, most commonly red-green confusion. Here is what a
red-green pairing and a colorblind-safe one each look like to a reader with
the commonest form of it:

<Chart slug="colorblind-safe" />

Several Plotly palettes are colorblind-safe by design:
...
A palette spent on categories the axis already names emphasises
everything, which is the same as emphasising nothing:

<Chart slug="emphasis-one-color" />
...
One more, which only bites once a site has a dark mode: a palette is tied to
the background it was chosen for. The charts on this page are drawn from
seven color roles, and each role stores two values, one for white and one
for near-black. Here is why that is not redundant work:

<Chart slug="palette-on-dark-and-light" />

Contrast against white is limited by how *dark* a color is, and contrast
against black by how *light* it is, so a hue bright enough to glow on black
is by construction too pale to sit on white. Brightening a palette for dark
mode is not a reversible operation.
