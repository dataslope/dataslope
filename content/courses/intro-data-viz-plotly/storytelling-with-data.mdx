...
<Figure
  ...
  caption="Write the claim in one sentence first; the chart is then built to support it."
/>
...
It is worth seeing how much of the work the title alone does, with no
annotation, no highlight and no color change:

<Chart slug="title-carries-the-finding" />

A descriptive title leaves the analysis to the reader, so most people glance,
see bars going roughly sideways and move on, which is a reasonable response to
being handed a dataset instead of a claim. Stating the finding also makes the
chart falsifiable: somebody who disagrees now has something specific to
disagree with, which is the reason for showing the data underneath at all. If
you can say what a chart shows in one sentence, that sentence is the title. If
you cannot, the chart is not finished.

The usual objection is that this is editorialising, and it is. The
alternative is not neutrality: a descriptive title still carries a point of
view, in what was measured, what was left out and how it was grouped.
Stating the claim makes that view arguable instead of hiding it in the
defaults.
...
Nothing in the data changed between the panels. What changed is which part of
it the reader was told to look at, and that is enough. Here is the same move
in code, so you can swap the framing yourself:
