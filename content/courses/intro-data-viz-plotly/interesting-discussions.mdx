...
<Figure
  slug="viz-interesting-discussions-cutout"
  alt="Interesting discussions, an isometric illustration"
/>
...
<Figure
  slug="viz-interesting-discussions-charts-move-into-inline-cutout"
  alt="A framed chart and a framed painting propped side by side on a wide executive desk, the painting turned face down and the chart standing upright"
  caption="The spreadsheet put chart-making on every desk, for better and for worse."
/>
...
The last value is sixty-eight times the first. An ordinary bar chart draws
1875 as a hairline; a log axis stops the growth from looking like growth. Du
Bois kept the bar linear so that every year is still a length, and wound it up
so the whole run fits the sheet. That is a design decision with a stated
trade-off, made on a plate that spent most of a century filed under folk art.

The cost is real and worth saying, because he clearly knew it: two coiled bars
cannot be compared at a glance the way two upright ones can. He traded that
away to keep the encoding honest at both ends of a sixty-eight-fold range.

Statistical graphics also spread out of public health and into commerce.
...
<Figure
  slug="viz-interesting-discussions-decade-spreadsheet-inline-cutout"
  alt="A grid of ruled cells on a small desk machine, one cell changed by hand and a whole column re-reading itself"
  caption="VisiCalc, 1979: change one cell and the whole sheet answers, which sold the machine."
/>
...
By the late 1990s, business-intelligence tools like **Cognos**,
**Business Objects**,
...
<Figure
  slug="viz-interesting-discussions-dashboard-era-good-idea-inline-cutout"
  alt="A wall of small panels each carrying a different mark, a figure standing well back from it reading none of them"
  caption="Tableau, 2003: dragging columns onto a canvas made charts cheap, and dashboards multiplied."
/>
...
<Figure
  slug="viz-interesting-discussions-d3-svg-engine-under-inline-cutout"
  alt="Shapes on a sheet that answer to a pointer, each redrawn as the figures behind them change"
  caption="Shapes bound to data, redrawn when the data changes: the machinery under every interactive chart."
/>
...
Before any of this could react to a mouse, there was one other way to let a
reader ask questions of a picture: encode so much in the geometry that every
question becomes something you do with your eyes. The purest example ever
drawn is a railway timetable, published by **Étienne-Jules Marey** in 1878 and
credited by him to the engineer Charles Ibry.

<Chart slug="marey-train-graph" />

Time across, distance down, one line per train. Slope is speed, so the
fastest train is the steepest line. A flat stretch is a train standing at a
platform. A crossing is two trains in the same place at the same moment. The
vertical gap between two lines going the same way is how much track is between
them, which is the number the entire signalling system exists to keep above
zero. A printed timetable holds identical information and answers none of
those questions without arithmetic. Railways still plan track this way.

Every property of a line is a fact you would otherwise have to compute. The up
and down expresses pass near Dijon at about the moment their lines cross, which
no printed timetable states anywhere.
