...
<Figure
  slug="viz-simple-white-template-cutout"
  alt="The simple_white template, an isometric illustration"
/>
...
<Figure
  slug="viz-the-simple-white-template-template-inline-cutout"
  alt="A solid inked panel with its margins and type sizes marked out in a contrasting ink, no data drawn on it yet"
  caption="A template sets the type, margins and grid before any data is drawn."
/>
...
The gridline decision is the one worth looking at directly, because a
gridline is a measuring aid and should be exactly as visible as the measuring
needs:

<Chart slug="gridline-weight-ladder" />

Nothing changes across those five panels except the stroke opacity of the
horizontal rules. That is a useful thing to notice about chart quality in
general: a lot of it is decided by choices nobody would call design
decisions.

With no grid you can see the shape and cannot put a number on any point
without walking your eye to the axis and losing your place. One step in, the
rules are there and you do not notice them, which is where a grid belongs. By
the fourth panel the grid is as dark as the data and the chart has two
subjects; by the fifth the series is something drawn on graph paper. The
template this course uses sits at about the second panel, which is why its
charts look empty at first and stop looking empty about a week later.
...
in *any* template still communicates the story. Here are the same
misleading bars before and after the cleanup, and then with the one edit
that actually mattered:

<Chart slug="template-doesnt-fix-encoding" />
