...
<Figure
  ...
  caption="Unpivot folds many columns into one, and pivot unfolds them again."
/>
...
display(long.group_by("month").agg(pl.col("sales").sum()).sort("month"))
display(long.filter(pl.col("sales") > 150))
display(long.group_by("region").agg(pl.col("sales").mean().round(1)).sort("region"))
`,
    },
  ]}
/>
...
<Figure
  slug="pl-pivot-and-unpivot-pivot-long-wide-inline-cutout"
  alt="A narrow tall board unfolding sideways into a wide flat one, the same squares visible in both"
  caption="Work in long form and pivot only for the table a human will read."
/>
...
display(df.transpose(include_header=True, header_name="column", column_names="metric"))
`,
    },
  ]}
/>
