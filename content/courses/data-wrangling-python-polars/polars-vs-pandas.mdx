...
<Figure
  ...
  caption="There is no index in Polars, so a row is identified by its values alone."
/>
...
<Figure
  slug="pl-polars-vs-pandas-2-write-expressions-not-inline-cutout"
  alt="A written instruction card that can be filed, passed on and reused, held up beside the tray it has not touched yet"
  caption="An expression is a value describing a computation, so it can be built before any data exists."
/>
...
display(nordics.with_columns(recipe))
display(iberia.with_columns(recipe))
`,
    },
  ]}
/>
...
<Figure
  slug="pl-polars-vs-pandas-3-null-null-nan-inline-cutout"
  alt="An empty slot and a slot holding a mark meaning the measurement failed, kept in two clearly different racks"
  caption="`null` means missing and `NaN` means a failed float, and Polars keeps the two apart."
/>
...
<Figure
  slug="pl-polars-vs-pandas-4-types-strict-feature-inline-cutout"
  alt="One column of a rack holding a single shape of item, beside a column stuffed with mismatched shapes jammed together"
  caption="A column has one type, and refusing the mixed column is what makes the rest fast."
/>
...
<Figure
  slug="pl-polars-vs-pandas-5-nothing-mutates-place-inline-cutout"
  alt="Each station handing on a new tray while the tray it was given stays behind on the bench"
  caption="Every operation returns a new frame, and the buffers underneath are shared rather than copied."
/>
