...
<Figure
  slug="pl-group-by-and-agg-basic-shape-inline-cutout"
  alt="Chips swept into three trays by a comb, each tray tipped into a small press leaving one block on top"
  caption="Group the rows, then let each group's expressions press it down to one row."
/>
...
print(penguins.group_by("island").agg(pl.len()))

# Either sort afterwards...
print(penguins.group_by("island").agg(pl.len()).sort("island"))

# ...or ask for the original appearance order
print(penguins.group_by("island", maintain_order=True).agg(pl.len()))
`,
    },
  ]}
/>
