...
<Figure
  ...
  alt="A friendly polar bear with warm cream fur and soft grey shading laying the last of four long horizontal bars in blue, green, red and yellow flat along a wide platform, beside a second platform where the same four colors stand as four thick upright columns"
/>
...
3. **Single instruction, multiple data (SIMD).** One instruction can add
   eight `Float64` values at once,
   but only if they are adjacent. A column is adjacent by
   construction.

<Figure
  slug="pl-columnar-memory-and-arrow-cpu-cares-so-inline-cutout"
  alt="A machine reaching along one continuous column of blocks in a single sweep rather than hopping across rows"
  caption="A column is contiguous in memory, so the processor reads it in one sweep."
/>
...
<Figure
  slug="pl-columnar-memory-and-arrow-apache-arrow-adds-inline-cutout"
  alt="Two different machines drawing from one shared tray without either repacking it first"
  caption="Arrow is one agreed layout, so two tools can share memory without repacking."
/>
...
<Figure
  slug="pl-arrow-shared-columns-cutout"
  alt="Three different machine cabinets all reading from one long shared column of stacked cells running between them"
  caption="Apache Arrow, 2016: one agreed memory layout, so a column can cross a language boundary without being copied."
/>
