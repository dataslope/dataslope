...
<Figure
  ...
  alt="A friendly polar bear with warm cream fur and soft grey shading standing at the centre of a wide platform divided into four raised bays in blue, green, red and yellow, each bay holding one thick solid block of a different shape"
/>
...
<Figure
  slug="pl-the-four-contexts-select-result-whole-inline-cutout"
  alt="An expression card slotted into a machine that hands back a whole new board rather than a single column"
  caption="In `select()` the expressions are the whole result; anything unnamed is gone."
/>
...
display(df.with_columns(pl.col("name").str.strip_chars().str.to_titlecase()))
`,
    },
  ]}
/>
...
<Figure
  slug="pl-the-four-contexts-inline-cutout"
  alt="One expression card being slotted into four different machines in turn, each machine using it in its own way"
  caption="One expression means four different things depending on where you put it."
/>
...
display(penguins.select(above_average.alias("above")).head(3))

# with_columns: attach it to the existing frame
display(penguins.with_columns(above_average.alias("above")).head(3))
