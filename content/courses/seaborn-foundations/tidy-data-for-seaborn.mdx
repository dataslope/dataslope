...
<Figure
  slug="sns-tidy-data-for-seaborn-inline-cutout"
  alt="A jumbled sheet on the left rebuilt into a clean board on the right, one row per record and one column per measure"
  caption="Seaborn wants tidy data: one row per observation, one column per variable."
/>
...
<Figure
  slug="sns-tidy-data-for-seaborn-tidy-means-inline-cutout"
  alt="A board where every row is one record and every column one measure"
  caption="Once the table is tidy, every Seaborn function takes column names directly."
/>
