...
<Figure
  slug="sns-color-palettes-cutout"
  alt="Three palette strips standing on a platform, one of distinct chips, one a single-hue ramp, one running from red through neutral to blue"
/>
...
<Figure
  slug="sns-color-palettes-three-palette-types-inline-cutout"
  alt="Three swatch strips, one grading in one ink, one grading between two, one of distinct colors"
  caption="Sequential grades one hue, diverging runs through a pale middle, qualitative jumps."
/>
...
<Figure
  slug="sns-color-palettes-qualitative-distinct-colors-inline-cutout"
  alt="A strip of clearly separate swatches with no two shading into one another"
  caption="A qualitative palette gives every category equal visual weight."
/>
...
The failure this prevents is worth seeing, because the wrong palette does not
just look odd, it makes a claim:

<Chart slug="sequential-scale-on-categories" />

The effect is not subtle, because the eye reads light-to-dark as low-to-high,
that being what lightness is for, so the chart asserts a ranking that is not
in the data, in a channel readers trust without checking. Worse, the ranking
it asserts is the alphabetical order the groupby happened to return. The rule
is a short table: nominal data takes a qualitative palette, ordinal data takes
a sequential one, and data with a meaningful middle takes a diverging one.

Picking the palette *family* is the decision. Picking which pretty colors go in
it is not.
...
<Figure
  slug="sns-color-palettes-inline-cutout"
  alt="Three strips of swatches, one running pale to deep in one hue, one running between two hues, one of separate distinct colors"
  caption="Pick the palette family from what the numbers mean, not from taste."
/>
...
The same five-category chart in a default palette and a colorblind-safe
one, each shown as a red-green viewer sees it:
