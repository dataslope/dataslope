...
<Figure
  slug="penguins"
  alt="A family of penguins — echoing the Palmer Penguins dataset used throughout this course"
  priority
/>
...
Enough preamble — here is a real Seaborn figure you can run this second.
**This whole course runs in your browser:** there is nothing to install,
and pandas, Seaborn, and the example data are already loaded. Press
**Run** and watch a few hundred penguins sort themselves into three
species.

Do not worry about the details yet — by the end of this course every line
will feel *obvious*, and you will be able to say what each one is *for*.
...
penguins = pd.read_csv("penguins.csv")`,
      starterCode: `# "Flipper length vs. body mass, colored by species."
sns.relplot(
    data=penguins,
    x="flipper_length_mm",
    y="body_mass_g",
...
color assignment, no legend code. You named the columns and the *roles*
they should play — `x`, `y`, `hue` — and Seaborn did the rest. That is the
whole game.

<Callout type="tip" title="Now change one thing">
The fastest way to build intuition is to *poke*. In the block above, try
**one** edit and re-run:

- swap the y-axis: change `y="body_mass_g"` to `y="bill_length_mm"`;
- recolor by a different grouping: change `hue="species"` to `hue="island"`;
- split it into panels: add `col="sex"` on its own line inside the call.

Each tiny change asks Seaborn a different question, and the picture answers
instantly. That loop — tweak, run, look — is the whole rhythm of this
course.
</Callout>
