...
<Figure
  slug="sns-continuous-categorical-cutout"
  alt="A smooth measuring ramp on one side of a platform and a row of separate labelled bins on the other"
/>
...
Not every channel in that table is equally good at carrying a number.
Position is read most accurately, then length, then angle, then area, and
color last, which is why a value you want compared precisely belongs on an
axis and not in a hue:
...
<Figure
  slug="sns-continuous-vs-categorical-inline-cutout"
  alt="A smooth rule with a slider beside a set of separate labelled trays, a chip belonging to one or the other"
  caption="A value either sits somewhere on a scale or belongs to one of a few bins."
/>
...
<Figure
  slug="sns-continuous-vs-categorical-gotcha-numbers-are-inline-cutout"
  alt="Blocks of rising height standing on labelled trays, a hand lifting them off the rising order into separate bins"
  caption="Seaborn reads the dtype, so a category stored as numbers is treated as continuous."
/>
...
<Figure
  slug="sns-continuous-vs-categorical-two-kinds-variables-inline-cutout"
  alt="A smooth rule with a slider beside a set of separate labelled trays"
  caption="Which kind a column is decides every chart choice that follows."
/>
