...
<Figure
  slug="sns-kde-cutout"
  alt="A stepped bar silhouette on a platform with a smooth continuous mound curve laid over it, a bandwidth dial beside it"
/>
...
<Figure
  slug="sns-distributions-kde-drawing-one-inline-cutout"
  alt="A smooth arched cap laid over a row of solid inked blocks of different heights, touching their tops but not their edges"
  caption="Two humps in the curve mean two groups hiding in one column."
/>
...
<Figure
  slug="sns-distributions-kde-bandwidth-kde-s-inline-cutout"
  alt="The same chips under three curves, one jagged, one smooth, one so smooth it has lost the shape"
  caption="Too little smoothing invents peaks; too much erases the real ones."
/>

One sample, three bandwidths. The rug underneath is the same in all three,
so the only thing changing is the smoothing:
...
<Figure
  slug="sns-distributions-kde-inline-cutout"
  alt="A row of stacked blocks forming a rough histogram, and a smooth curve laid over them following the same shape"
  caption="A density curve smooths the bin edges away and keeps the shape."
/>
