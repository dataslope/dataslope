...
<Figure
  slug="sysc-data-types-and-memory-fundamental-types-inline-cutout"
  alt="Four blocks of visibly different widths resting in recesses cut to each"
  caption="Each type reserves a fixed number of bytes, and the count is platform dependent."
/>
...
<Figure
  slug="sysc-data-types-and-memory-fixed-size-turns-inline-cutout"
  alt="A block too large for its recess, its edge overhanging into the next recess along"
  caption="A value too large for its type wraps, quietly, with no error at all."
/>
...
<Callout type="warn" title="Signed overflow is undefined behavior, unsigned overflow is not">
...
<Figure
  ...
  caption="Fixed-width types say exactly how many bytes, on every platform."
/>
...
<Figure
  slug="sysc-data-types-and-memory-endianness-which-byte-comes-inline-cutout"
  alt="The same four-part value loaded into a rail from the low end on one machine and from the high end on another"
  caption="Little-endian stores the low byte first, which matters the moment bytes leave the machine."
/>
