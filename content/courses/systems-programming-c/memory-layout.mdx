...
<Figure
  slug="sysc-memory-layout-cutout"
  alt="A tall cutaway column on a platform divided into four labelled bands, one growing upward and one downward toward the middle"
/>
...
<Figure
  slug="sysc-memory-layout-text-read-only-inline-cutout"
  alt="One band of a long bar sealed under a clamped cover while the other bands stay open"
  caption="The code segment is mapped read-only, so writing to it faults."
/>
...
<Figure
  slug="sysc-memory-layout-inline-cutout"
  alt="One long horizontal bar running the full width of the frame, divided end to end into four bands of different colours, a small arrow above the third band pointing right and a small arrow above the fourth pointing left so the two grow toward each other, and three loose crates resting inside the third band"
  caption="The stack grows down and the heap grows up, toward each other."
/>
...
Worth having in view before either: what it costs to reach each level of the
hierarchy the program is laid out across.

<Figure
  slug="sysc-memory-layout-seeing-regions-inline-cutout"
  alt="One long horizontal bar divided end to end into four bands of different colours"
  caption="Text, data, heap and stack, each with its own rules and direction of growth."
/>
