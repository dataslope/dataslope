...
<Figure
  slug="sysc-heap-cutout"
  alt="A wide open field of free blocks on a platform with a requesting hand receiving one block and a return chute beside it"
/>
...
<Figure
  slug="sysc-heap-and-dynamic-allocation-inline-cutout"
  alt="An open yard of crates of different sizes with gaps between them, a figure fitting one new crate into a gap that just fits"
  caption="The heap hands out blocks of the size you ask for, wherever one fits."
/>
...
The "not a syscall" part is worth a number. Crossing into the kernel costs
a privilege switch and a copy across the boundary, per call, which is why
the library batches: `malloc()` asks the kernel for a large region rarely,
and `FILE*` buffers writes for exactly the same reason.

<Chart slug="syscall-vs-userspace-cost" />

So an allocator is ordinary code sitting between you and those rare kernel
requests. Anyone can write one, and one person famously did. In 1987,
...
<Figure
  slug="sysc-heap-and-dynamic-allocation-first-malloc-inline-cutout"
  alt="A crate handed out over a counter with a tag clipped to the hand that took it"
  caption="`malloc()` gives you a block and the responsibility for returning it."
/>
...
<Figure
  slug="sysc-heap-and-dynamic-allocation-growable-array-n-inline-cutout"
  alt="A rail being replaced by a longer rail with its blocks carried across, the short one set aside"
  caption="Growing an array means a bigger block, a copy across, and the old one freed."
/>
...
<Figure
  slug="sysc-heap-and-dynamic-allocation-building-dynamic-array-mini-inline-cutout"
  alt="A tray that copies itself into a larger tray the moment it fills, the old tray tipped out behind it"
  caption="Double the capacity when it fills, and the copy is rare enough to amortize away."
/>
...
- Assume it succeeded and keep using the same pointer.
  > A failed \`realloc()\` returns \`NULL\`; using \`NULL\` as if it were the old block is undefined behavior.
