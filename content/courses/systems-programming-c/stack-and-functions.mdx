...
<Figure
  slug="sysc-stack-and-functions-anatomy-stack-frame-inline-cutout"
  alt="One tray in cross-section with separate compartments for its arguments, its locals and its return cord"
  caption="A frame holds the arguments, the locals, and where to return to."
/>
...
<Figure
  slug="sysc-stack-and-functions-see-stack-addresses-inline-cutout"
  alt="A column of trays with a small plate on the edge of each, the plates stepping evenly down the column"
  caption="Each nested call sits at a lower address than the one before it."
/>
...
<Figure
  slug="sysc-stack-and-functions-inline-cutout"
  alt="One tray shown open in cross-section with three separate compartments inside it for arguments, local blocks and a return arrow, resting on a column of identical closed trays"
  caption="Every recursive call takes another frame, so depth is a memory limit."
/>
...
Where exactly that happens is the stack budget divided by the frame size,
and the frame size is set by the locals rather than by the algorithm, so
one fat array in the recursive function moves the cliff a long way in:
