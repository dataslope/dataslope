...
C has no string type. It has arrays of `char`, and a convention: the
string ends at the first zero byte. The five-letter word `Hello`
therefore occupies six bytes:
...
<Figure
  ...
  caption="A string literal lives in read-only memory and cannot be modified."
/>
...
<Figure
  ...
  caption="Without the terminating null byte, nothing tells a function where to stop."
/>
...
<Figure
  ...
  caption="`strncpy()` may leave the copy with no terminator at all."
/>
