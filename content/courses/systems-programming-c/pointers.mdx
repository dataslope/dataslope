...
<Figure
  slug="sysc-pointers-variables-live-at-inline-cutout"
  alt="A row of lockers each with its own small plate, one block resting inside one of them"
  caption="Every variable has an address, and that address is what a pointer holds."
/>
...
<Figure
  slug="sysc-pointers-declaring-dereferencing-pointers-inline-cutout"
  alt="A card with an arrow to a locker, and a hand following that arrow to lift out what the locker holds"
  caption="Dereferencing follows the address and reads what is stored there."
/>
...
<Figure
  slug="sysc-pointers-null-points-nothing-pointer-inline-cutout"
  alt="A signpost pointing at an empty patch of ground, with a figure walking confidently toward it"
  caption="`NULL` is the pointer that goes nowhere, and following it is the classic C crash."
/>
...
<Figure
  ...
  caption="`const` can protect the pointer, the thing it points at, or both."
/>
