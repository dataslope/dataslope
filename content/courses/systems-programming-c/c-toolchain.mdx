...
What looks like one command is really four programs handing work to each
other. Each stage takes a file and produces a different one, and you
can stop the chain at any point to see what came out:
...
<Figure
  slug="sysc-toolchain-cutout"
  alt="Four sequential machine stations on a platform, a source sheet entering the first and a solid binary block leaving the last"
/>
...
<Figure
  slug="sysc-c-toolchain-header-files-include-inline-cutout"
  alt="A strip pasted onto a card with a small clasp that refuses to paste it a second time"
  caption="An include guard is what stops the same header being pasted in twice."
/>
...
<Figure
  slug="sysc-c-toolchain-stage-1-preprocessor-inline-cutout"
  alt="A card passing a first machine that pastes extra strips onto it before it continues"
  caption="The preprocessor pastes headers in before the compiler sees anything."
/>
...
<Figure
  slug="sysc-c-toolchain-stage-4-linking-inline-cutout"
  alt="Several finished parts drawn together into one assembly, with a stock fitting pulled off the shelf to fill the last gap"
  caption="The linker joins the object files and pulls the standard library in behind them."
/>
...
<Figure
  slug="sysc-c-toolchain-inline-cutout"
  alt="A source card passing through four machines in a row, emerging as a card, an object block, a linked block, and finally a running machine"
  caption="Source, assembly, object, executable: each stage hands on a different artefact."
/>
