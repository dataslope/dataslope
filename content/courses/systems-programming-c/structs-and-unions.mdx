...
<Figure
  slug="sysc-structs-and-unions-defining-using-struct-inline-cutout"
  alt="An empty fitted tray whose compartments are cut to different shapes before anything is placed in them"
  caption="A struct lays out named compartments, each with its own offset."
/>
...
<Figure
  slug="sysc-structs-and-unions-memory-layout-padding-inline-cutout"
  alt="Parts seated along a row of recesses with small blank spacers slipped in so each part starts on a ruled line"
  caption="The compiler inserts blank bytes so each member starts on an address it can be read from."
/>
...
<Figure
  ...
  caption="A struct gives every member its own compartment; a union gives them all the same one."
/>
...
<Figure
  slug="sysc-structs-and-unions-unions-inline-cutout"
  alt="One recess cut into a bench that holds either of two differently shaped parts, and never both at the same time"
  caption="A union gives its members one shared space, so only one of them is valid at a time."
/>
...
<Figure
  slug="sysc-structs-and-unions-flexible-array-members-inline-cutout"
  alt="One continuous tray holding a header and its contents in a single piece, beside a header with a cord running off to a tray elsewhere"
  caption="Header and contents in one allocation: one `malloc()`, one `free()`, one cache-friendly block."
/>
