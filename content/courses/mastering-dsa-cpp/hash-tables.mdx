...
<Figure
  slug="dsa-hash-tables-core-idea-inline-cutout"
  alt="A key dropped into a mixing machine whose chute swings to one of several bins"
  caption="The hash turns a key straight into a bucket number, with no searching."
/>
...
<Figure
  slug="dsa-hash-tables-collisions-chaining-inline-cutout"
  alt="Two keys whose chutes swing to the same bin, the bin holding a short cord of blocks rather than one"
  caption="Two keys can land in one bucket, and chaining keeps both there."
/>
...
<Figure
  ...
  caption="Average constant time assumes the keys spread out; adversarial keys do not."
/>
...
**Two Sum** is the canonical "trade memory for time" move. The $O(n^2)$ approach checks every pair; the hash version walks the array once, asking at each element "have I already seen `target - x`?", turning a pair search into n single lookups. This is the trick the complexity chapter promised when you wrote `countPairs` the slow way.
