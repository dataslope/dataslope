...
<Figure
  slug="dsa-searching-cutout"
  alt="A sorted row of ascending blocks with a bracket spanning the whole row, a second bracket spanning only its right half"
/>
...
<Figure
  slug="dsa-searching-linear-search-inline-cutout"
  alt="A hand walking a row of blocks one at a time from the left"
  caption="Linear search checks every element; binary search halves the field each time."
/>

$O(n)$ against $O(\log n)$ is a pair of symbols until you put numbers on it. On a
million-element array the difference is half a million comparisons against
twenty:

<Chart slug="binary-vs-linear-search" />
...
<Figure
  slug="dsa-searching-bug-hid-decades-inline-cutout"
  alt="Two large marks added together on a narrow rail that cannot hold the total, the sum wrapping round below zero"
  caption="`(low + high) / 2` overflows on a large array, which took years to surface."
/>
...
<Figure
  slug="dsa-searching-overflow-cutout"
  alt="A long measuring beam with a marker at its midpoint, its ends pressed inward by heavy blocks until a crack opens at the marker"
  caption="2006: low plus high overflowed, and the bug sat in textbooks and the JDK until arrays got big enough to reach it."
/>
