...
<Figure
  slug="dsa-doubly-linked-lists-cutout"
  alt="Three cube nodes in a row joined by paired arrows running both directions, the middle node lifted slightly out of the chain"
/>
...
<Figure
  slug="dsa-doubly-linked-lists-inline-cutout"
  alt="A row of blocks joined by cords running both ways between neighbours, a hand able to walk the row from either end"
  caption="Links both ways mean you can walk the list from either end."
/>
...
<Figure
  slug="dsa-doubly-linked-lists-tiny-implementation-inline-cutout"
  alt="Three blocks joined by cords running both ways, a hand walking the row from the right-hand end"
  caption="Every insert and erase has two pointers to fix, not one."
/>
...
`std::list<T>` is the standard library doubly linked list. It has stable iterators: inserting or erasing elsewhere does not invalidate iterators to other elements. It does **not** support random access with `list[i]`.

<Figure
  slug="dsa-doubly-linked-lists-std-list-inline-cutout"
  alt="A ready-made run of blocks joined both ways, lifted whole off a shelf"
  caption="`std::list` is the ready-made version, with iterators that stay valid."
/>
