...
Why does this matter for algorithms specifically? Because a careless `std::vector<int> v` parameter copies *every element* on *every call*. Pass a million-element vector by value inside a loop and your "$O(n)$" function quietly becomes $O(n^2)$. `const&` makes the copy disappear.
