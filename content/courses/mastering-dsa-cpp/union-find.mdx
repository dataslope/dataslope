...
description: Learn the disjoint-set find and unite operations, path compression, union by rank, connected components, cycle detection, and Kruskal's minimum spanning tree in C++
---
...
<Figure
  ...
  caption="Each set is a tree, and union pins one root under the other."
/>
...
<Figure
  slug="dsa-union-find-union-by-rank-size-inline-cutout"
  alt="Two trees joined by hanging the shorter one beneath the taller, keeping the whole thing flat"
  caption="Hang the shorter tree under the taller and the structure never grows deep."
/>
