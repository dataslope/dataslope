...
description: Learn the binary search tree invariant, search, insertion, deletion, validation, and ordered C++ containers
---
...
<Figure
  slug="dsa-bst-cutout"
  alt="A branching node structure where every left branch holds a smaller block and every right branch a larger one, a descending path highlighted"
/>
...
<Figure
  slug="dsa-binary-search-trees-search-inline-cutout"
  alt="A hand following one path down a branching frame, taking the left branch for smaller and the right for larger"
  caption="Each comparison chooses a branch and discards the whole other side."
/>
...
<Figure
  slug="dsa-binary-search-trees-insert-inline-cutout"
  alt="A new block walked down a branching frame until it reaches an empty socket and clips in"
  caption="Insertion follows the search path and clips in at the first empty slot."
/>
...
<Figure
  slug="dsa-binary-search-trees-inline-cutout"
  alt="A branching frame where every left branch carries a smaller block and every right branch a larger one, a hand following one path down"
  caption="Smaller left, larger right, at every node, is the whole invariant."
/>
