...
`+` creates a brand-new string each time; `+=` appends in place, reusing spare capacity exactly like `push_back()` on a vector. Inside a loop the difference compounds: building an n-character string with repeated `s = s + part` copies the whole prefix every iteration, $O(n^2)$, while `s += part` stays amortized $O(n)$. Same output, very different bill.
