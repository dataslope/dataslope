...
<Figure
  slug="dsa-trees-cutout"
  alt="A branching structure of connected spheres rising from a single root node, each node splitting into two children"
/>
...
<Figure
  slug="dsa-trees-tree-vocabulary-inline-cutout"
  alt="A branching frame with small blank plates hanging at its root, a fork and a leaf"
  caption="Root, node, leaf: the three words the rest of the vocabulary hangs off."
/>
...
<Figure
  slug="dsa-trees-inline-cutout"
  alt="A frame branching from one root block into two, then four, with a hand following a single branch downward"
  caption="A binary tree node has at most two children, which fixes the shape."
/>
...
<Figure
  slug="dsa-trees-calculating-tree-properties-inline-cutout"
  alt="A branching frame where the same measuring tool is applied to each limb and the readings added at the join"
  caption="Solve the left subtree, solve the right, combine: most tree properties are that shape."
/>
