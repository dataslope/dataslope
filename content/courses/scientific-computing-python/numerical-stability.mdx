...
<Figure
  slug="scipy-numerical-stability-stability-intuitively-inline-cutout"
  alt="Two chains of steps from the same start, one ending on its mark and one visibly off"
  caption="Two algorithms for the same answer can end in very different places."
/>
...
<Figure
  slug="scipy-numerical-stability-conditioning-intuitively-inline-cutout"
  alt="A lever where a tiny nudge at one end swings the far end wildly"
  caption="An ill-conditioned problem amplifies a tiny change into a large one."
/>
...
<Figure
  ...
  caption="A stable algorithm keeps the small errors small all the way through."
/>
