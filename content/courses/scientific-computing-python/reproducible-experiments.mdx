...
<Figure
  slug="scipy-reproducible-cutout"
  alt="A sealed environment capsule on a platform with a fixed seed dial on its side stamping three identical output sheets"
/>
...
<Figure
  slug="scipy-reproducible-experiments-can-go-wrong-inline-cutout"
  alt="Two identical worktables laid out with the same tools, each holding a differently shaped finished piece"
  caption="Same tools, different result, is the failure reproducibility is meant to prevent."
/>
...
<Figure
  slug="scipy-reproducible-experiments-pillar-1-pin-environment-inline-cutout"
  alt="A set of tools tagged with their exact sizes and locked into a case, so the same case can be opened elsewhere"
  caption="Record the exact versions, or the same code meets different libraries next month."
/>
...
<Figure
  slug="scipy-reproducible-experiments-pillar-2-seed-everything-inline-cutout"
  alt="One stamped starting token copied to every bench in a row, each bench then producing the same run every time"
  caption="Seed every source of randomness and the same run produces the same numbers tomorrow."
/>
...
<Figure
  slug="scipy-reproducible-experiments-pillar-3-record-inputs-inline-cutout"
  alt="A finished run leaving a small written manifest tied to its output tray, naming everything that went into it"
  caption="A run that does not record what went into it cannot be repeated, only re-attempted."
/>
...
<Figure
  slug="scipy-reproducible-experiments-parameter-sweeps-disciplined-way-inline-cutout"
  alt="A rack of runs each with one dial set differently, their readings collected onto a single sheet"
  caption="One dial changed per run, every reading recorded, and nothing copy-pasted between them."
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `import itertools, json
import numpy as np
...
<Figure
  slug="scipy-reproducible-experiments-notebooks-vs-scripts-inline-cutout"
  alt="A bench of cards run in whatever order a hand reached for them, beside the same cards fixed into a rail in one settled order"
  caption="A notebook remembers the order you clicked; a script has only the order it is written in."
/>
