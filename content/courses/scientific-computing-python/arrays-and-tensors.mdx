...
<Figure
  slug="scipy-arrays-tensors-cutout"
  alt="A contiguous memory strip on a platform reinterpreted above it as a flat grid and then as a stacked cube of cells"
/>
...
<Figure
  slug="scipy-arrays-and-tensors-creating-arrays-inline-cutout"
  alt="A solid grid of cells filling in one by one from a hopper, each filled cell a block of ink"
  caption="An array is one flat buffer plus a shape that says how to read it."
/>
...
<Figure
  slug="scipy-arrays-and-tensors-indexing-views-fancy-inline-cutout"
  alt="A frame laid over a grid showing part of it, a scoop lifting scattered cells, and a solid cover plate hiding the rest"
  caption="A slice is a view of the same memory; fancy indexing copies."
/>
...
Basic slicing gives you a **view**: no data is copied, and writing
through it changes the original. Fancy indexing is the opposite, and the
distinction is one of the most common sources of NumPy surprises:
...
$\times$ channels), time-series ensembles, and partial differential
equation solution states
live.

<Figure
  slug="scipy-arrays-and-tensors-inline-cutout"
  alt="A single row of cells, a flat grid of them, and a stack of those grids, standing side by side in increasing dimension"
  caption="The same buffer is a vector, a matrix or a stack, depending on its shape."
/>
