...
<Figure
  slug="scipy-vectorized-cutout"
  alt="A wide press descending on an entire grid of cells in one stroke beside a slow single-cell stamping arm"
/>
...
<Figure
  slug="scipy-vectorized-thinking-single-most-important-inline-cutout"
  alt="A stopwatch beside two benches, one stamping blocks one at a time and one stamping a whole row"
  caption="The loop and the vectorised version do the same arithmetic at very different speeds."
/>
...
<Figure
  slug="scipy-vectorized-thinking-inline-cutout"
  alt="One hand stamping a single block at a time on the left, and a full-width press stamping a whole row in one stroke on the right"
  caption="One operation over a whole array beats one operation per element."
/>
