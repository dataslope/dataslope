...
<Figure
  slug="db-many-to-many-inline-cutout"
  alt="Two rows of blocks facing each other with a small junction board between them, every cord passing through a slot on that board rather than joining directly"
  caption="The third table is what turns an impossible shape into two ordinary ones."
/>
...
<Figure
  slug="db-many-to-many-seeing-both-sides-inline-cutout"
  alt="Two rows of crates where each crate on the left has cords to several on the right and each on the right has cords back to several on the left"
  caption="Many on the left and many on the right is a shape one foreign key cannot hold."
/>
