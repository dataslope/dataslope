...
<Figure
  slug="db-foreign-keys-relationship-needs-reference-inline-cutout"
  alt="A row in one board carrying a notch that matches a row in another, a cord running between the two"
  caption="A foreign key is a rule the database defends, not a note about intent."
/>
...
<Figure
  slug="db-foreign-keys-inline-cutout"
  alt="Two filing drawers side by side, a cord running from a tab in the first into a matching slot in the second, and a small lock where the cord enters"
  caption="The lock is on the child: it cannot point at a parent that is not there."
/>
