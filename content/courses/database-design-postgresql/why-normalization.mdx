...
<Figure
  slug="db-why-normalization-cutout"
  alt="Why normalization exists, an isometric illustration"
/>
...
<Figure
  slug="db-why-normalization-disease-duplicated-facts-inline-cutout"
  alt="The same block appearing in four different drawers, one of them visibly a different colour from the rest"
  caption="Copy a fact into four places and one of the copies will eventually disagree."
/>
...
<Figure
  slug="db-why-normalization-three-anomalies-inline-cutout"
  alt="A shelf where pulling one card out takes an unrelated fact away with it, leaving a gap nothing else on the shelf records"
  caption="Delete the last order and the customer disappears with it, because the fact lived in that row."
/>
...
<Figure
  slug="db-why-normalization-inline-cutout"
  alt="One bloated table on the left leaking duplicate blocks, and three tidy narrow tables on the right holding each block exactly once"
  caption="Normalising stores each fact exactly once, so there is nothing to fall out of step."
/>
...
Storage is the least interesting benefit, but it is the easiest to see:
repeating a fact on every row costs more the more rows there are.

<Chart slug="normalization-storage" />
