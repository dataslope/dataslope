...
<Figure
  ...
  caption="An orphan is a row still pointing at something that is no longer there."
/>
...
may lose the order. A report may count revenue without a customer. A screen
...
<Figure
  slug="db-referential-integrity-delete-behavior-design-choice-inline-cutout"
  alt="A gate that refuses to take a crate while cords still run from it, beside a gate that takes the crate and everything on those cords with it"
  caption="`RESTRICT` refuses while children remain; `CASCADE` takes them along. Both are decisions."
/>
...
<Figure
  ...
  caption="The database refuses the delete while another row still depends on it."
/>
