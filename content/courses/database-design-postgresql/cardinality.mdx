...
<Figure
  slug="db-cardinality-cutout"
  alt="Cardinality, an isometric illustration"
/>
...
<Figure
  slug="db-cardinality-cardinality-about-counts-inline-cutout"
  alt="One crate joined by a cord that fans out into six strands, each strand reaching its own smaller crate"
  caption="Cardinality is a count: how many rows on one side may reach how many on the other."
/>
...
<Figure
  slug="db-cardinality-one-many-inline-cutout"
  alt="One crate on the left with a cord that splits into four prongs, each prong reaching a smaller crate"
  caption="One row can fan out to many, but each of the many points back to just one."
/>
...
<Figure
  slug="db-cardinality-inline-cutout"
  alt="Three pairs of crates joined by cords, one cord single at both ends, one splitting at one end, one splitting at both"
  caption="Which end the cord splits at is what decides where the key has to live."
/>
