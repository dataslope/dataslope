...
An object is never only its fields. Every one carries a header the JVM
uses for its identity hash, lock state and garbage-collection bookkeeping,
which is why a million boxed integers cost several times what a million
primitive ones do:
