...
<Figure
  slug="oopj-objects-in-memory-two-regions-stack-inline-cutout"
  alt="A name plate in a column of trays joined by a cord to a crate in an open yard"
  caption="The variable lives on the stack; the object it names lives on the heap."
/>
...
<Figure
  slug="oopj-objects-in-memory-inline-cutout"
  alt="A small name plate on the left joined by a cord to a crate standing in an open yard on the right, and a second plate pointing at the same crate"
  caption="Copying a reference gives two names for one object, not two objects."
/>
...
Every object passes through the same few states between `new` and the
moment its memory comes back:
...
The bookkeeping that makes this possible is not free. An object is never
only its fields: each one carries a header the JVM uses for its identity
hash, lock state and collection bookkeeping, which is why a million boxed
integers cost several times what a million primitive ones do.

<Chart slug="object-header-overhead" />
