...
<Figure
  slug="rlang-grouped-analysis-cutout"
  alt="A grid of tiles splitting into three coloured piles, each pile compressing into one summary block, then combining into a small result tray"
/>
...
<Figure
  slug="rlang-grouped-analysis-basic-pattern-inline-cutout"
  alt="A board split into three stacks by a comb, each stack pressed into one summary block"
  caption="Group, then summarise: the rows collapse to one per group."
/>
...
<Figure
  slug="rlang-grouped-analysis-multiple-summaries-at-inline-cutout"
  alt="Three trays of chips each feeding four gauges at once"
  caption="One pass over each group can report several summaries at once."
/>
...
<Figure
  slug="rlang-grouped-analysis-inline-cutout"
  alt="A board split into three stacks by a comb, each stack pressed into a single summary block on its own tray"
  caption="Grouped `mutate()` computes within each group but keeps every row."
/>
