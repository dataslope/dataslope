...
<Figure
  slug="rlang-inference-cutout"
  alt="A horizontal range bar on a platform with a bracketed interval marked on it and a single point estimate pin inside"
/>

## A confidence interval, plainly

What "95% confident" means, drawn: a hundred intervals from a hundred
samples, about five of which miss.
...
The p-value is the shaded area: how much of the null's distribution sits at
least as far out as what you saw.

<Figure
  slug="rlang-intuition-for-inference-confidence-interval-plainly-inline-cutout"
  alt="One measured mark on a rail with a soft band drawn either side of it"
  caption="A confidence interval reports the estimate together with its own wobble."
/>
...
<Figure
  slug="rlang-intuition-for-inference-p-value-plainly-inline-cutout"
  alt="A mound with a shaded tail and one marker standing out in that tail"
  caption="The p-value is how much of the null's distribution lies past what you saw."
/>

## Effect size: significance ≠ importance

What the standard effect sizes actually look like as overlap between two
groups:
...
cat(
  "A: diff =", round(mean(A2) - mean(A1), 3),
...
cat(
  "B: diff =", round(mean(B2) - mean(B1), 3),
...
<Figure
  slug="rlang-intuition-for-inference-inline-cutout"
  alt="A small scoop of chips on a tray, with a large shadowed bin behind it that the scoop is being used to describe"
  caption="Inference uses the scoop in front of you to describe the bin you cannot see."
/>
...
- The mean is exactly 50.
  > A CI gives a range of plausible values; it does not pin the mean to the midpoint.
- There is a 95% chance the true mean is between 45 and 55.
  > A common misreading. The true mean is fixed, so it is either in this interval or not, the "95%" describes the long-run behavior of the procedure, not this one interval.
- 95% of the data falls between 45 and 55.
  > This describes the spread of individual data points, not the uncertainty about the mean. A CI is about the parameter, not the raw values.`}
/>
...
- The probability the finding is a fluke is 0.001.
  > "Chance the finding is a fluke" is a probability about the hypothesis, which a p-value does not give. It is a probability about the data under the null.
- The probability the null hypothesis is true is 0.001.
  > A p-value is computed *assuming* the null is true; it cannot also be the probability that the null is true.
- The difference is definitely large.
  > A small p-value speaks to evidence against the null, not to the size of the effect, that's what effect size measures.`}
/>
