...
<Figure
  slug="gg-thinking-in-layers-cutout"
  alt="Three transparent chart sheets floating one above another, each carrying different marks, aligning into a single combined image"
/>
...
When you add `+ geom_smooth()`, you add another. They render from the
bottom up:
...
Layers are drawn in the order you add them, so the trend line sits over
the points. Swap the two lines and the points win:
...
<Figure
  slug="gg-thinking-in-layers-inline-cutout"
  alt="Three transparent panels held apart in a stepped stack, one carrying points, one a line, one a band, and the finished plot below where they line up"
  caption="Layers are independent, and the order you add them is the order they stack."
/>
