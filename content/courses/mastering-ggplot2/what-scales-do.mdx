...
<Figure
  slug="gg-what-scales-do-cutout"
  alt="A translator mechanism with raw numeric tokens entering one side and pixel positions and color swatches leaving the other"
/>
...
There is no `scale_` line in that plot, and yet the x axis has a range,
breaks and labels. ggplot2 added a scale for you. Writing it out changes
nothing about the output, which is the point:
...
<Figure
  slug="gg-what-scales-do-inline-cutout"
  alt="A raw block of data on the left passing through a gauge in the middle that converts it into a position, a size and a color on the right"
  caption="A scale converts data values into the position, size or color drawn."
/>
