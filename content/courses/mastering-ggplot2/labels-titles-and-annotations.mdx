...
<Figure
  slug="gg-labels-annotations-cutout"
  alt="A chart plane with blank ribbon banners hovering above and beside it and a dashed reference line crossing the marks"
/>
...
<Figure
  slug="gg-labels-titles-and-annotations-labs-one-place-inline-cutout"
  alt="One panel on a bench with cords running from it to every blank plate around a finished plot"
  caption="`labs()` is the one place every text label on the plot is named."
/>
...
A note on where the reader's eye goes first. Colour used sparingly is
found without a search; used everywhere it is found by reading:
...
<Figure
  slug="gg-labels-titles-and-annotations-inline-cutout"
  alt="A finished plot with a title plate above it, a key at its side and one small flag planted on a single mark"
  caption="Titles, keys and annotations change legibility, never the analysis."
/>
