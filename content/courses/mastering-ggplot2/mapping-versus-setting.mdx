...
<Figure
  slug="gg-mapping-vs-setting-cutout"
  alt="Two side-by-side plot frames, one where every dot is a single flat colour and one where the dots take colours from a data column"
/>
...
<Figure
  slug="gg-mapping-versus-setting-inline-cutout"
  alt="One dial wired to a column of data on the left, and on the right a dial simply turned by hand to a fixed position"
  caption="Inside `aes()` a column drives the dial; outside it, you set the dial by hand."
/>
...
<Figure
  slug="gg-mapping-versus-setting-setting-constant-every-inline-cutout"
  alt="A dial turned by hand to a fixed position with no cord running to any data column"
  caption="A set value is the same for every mark, so no legend appears."
/>
