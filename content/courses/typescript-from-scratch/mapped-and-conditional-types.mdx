...
<Figure
  slug="tsx-mapped-conditional-cutout"
  alt="A template row of sockets on a platform passing through a transformer that returns every socket altered the same way"
/>
...
<Figure
  slug="tsx-mapped-and-conditional-types-inline-cutout"
  alt="A rack of sockets passing under a template that reshapes every socket in one stroke"
  caption="One template reshapes every property of a type in a single pass."
/>
...
<Figure
  slug="tsx-mapped-and-conditional-types-modifiers-readonly-inline-cutout"
  alt="A rack of sockets passing under a template that clamps a lid on each and makes each optional"
  caption="A mapped type can add or strip `readonly` and `?` across every property at once."
/>
...
<Figure
  slug="tsx-mapped-and-conditional-types-mapped-types-inline-cutout"
  alt="A tray of assorted fittings passing through a press that stamps the same new collar onto every one"
  caption="A conditional type picks one of two results by testing what it was given."
/>
