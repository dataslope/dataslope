...
<Figure
  slug="tsx-how-it-works-cutout"
  alt="A source sheet entering a four-stage machine on a platform, a checked report leaving one port and plain output the other"
/>
...
<Figure
  slug="tsx-how-typescript-works-compile-time-vs-inline-cutout"
  alt="A gate checking a card on a still bench, then lifting away as the belt starts to move"
  caption="The checking happens before the program runs, then the types are erased."
/>
...
<Figure
  slug="tsx-how-typescript-works-inline-cutout"
  alt="A card passing a checking gate that inspects it and then falls away, the card continuing unchanged"
  caption="The language service runs the same checks as you type, inside the editor."
/>
...
<Figure
  slug="tsx-how-typescript-works-javascript-typescript-relationship-inline-cutout"
  alt="A card with a shaped gate fitted over it, the gate lifting away and leaving the card untouched"
  caption="What ships is JavaScript; the types were scaffolding for the build."
/>
...
- [o] They are erased during compilation and do not exist in the emitted JavaScript
  > TypeScript performs type checking at *compile time*, then removes all type annotations before emitting JavaScript. The runtime behavior is identical to plain JavaScript, there's no runtime type enforcement unless you add it yourself.
