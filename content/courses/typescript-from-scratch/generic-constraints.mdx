...
<Figure
  slug="tsx-generic-constraints-cutout"
  alt="An adaptable socket on a platform fitted with a keyed guard that admits only tokens carrying a matching notch"
/>
...
<Figure
  slug="tsx-generic-constraints-inline-cutout"
  alt="An adjustable slot that widens to take many shapes but stops at a fixed limit, one oversized shape resting outside it"
  caption="A constraint widens the slot to many types while still ruling some out."
/>
...
<Figure
  ...
  caption="`keyof` lifts a type's property names out as a union you can constrain against."
/>
...
<Figure
  slug="tsx-generic-constraints-default-type-parameters-inline-cutout"
  alt="An adjustable socket resting at a marked default position until it is set otherwise"
  caption="A default type parameter is used until the caller names a different one."
/>
