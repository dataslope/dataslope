...
<Figure
  ...
  caption="One generic container works for any element type, with the type still checked."
/>

Here are both halves side by side, the `any` version that forgets and the
generic version that remembers:
...
<Figure
  slug="tsx-generics-basics-generic-functions-inline-cutout"
  alt="One machine shown working on three different shapes of block without any change to itself"
  caption="The same function serves every type because the type travels with the value."
/>
...
<Figure
  slug="tsx-generics-basics-generic-interfaces-type-inline-cutout"
  alt="One contract card held against three crates of different contents, matching all three"
  caption="One parameterised interface describes many concrete shapes at once."
/>
