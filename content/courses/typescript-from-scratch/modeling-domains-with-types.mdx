...
Four independent flags is sixteen representable users, and a real product
has about four kinds. The gap between those two numbers is where the bugs
live, because nothing in the type says which combinations are impossible:
...
<Figure
  slug="tsx-modeling-domains-with-types-solution-discriminated-unions-inline-cutout"
  alt="Three boxes with differently shaped tags, a sorter reading only the tag"
  caption="Each state carries only the data that state actually has."
/>
...
<Figure
  slug="tsx-modeling-domains-with-types-evolving-sloppy-type-inline-cutout"
  alt="A loose open tray being rebuilt into a fitted case whose compartments admit only one arrangement"
  caption="Tightening a loose type turns runtime checks into compile-time guarantees."
/>
...
<Figure
  slug="tsx-modeling-domains-with-types-inline-cutout"
  alt="A set of shaped sockets cut so that an impossible combination physically cannot be assembled"
  caption="Model the domain so that an impossible state cannot be constructed at all."
/>
