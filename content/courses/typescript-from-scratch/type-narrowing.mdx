...
<Figure
  slug="tsx-type-narrowing-cutout"
  alt="A mixed group of token shapes entering a branching gate that separates them into single-shape lanes"
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `function print(value: string | number): void {
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `function describe(value: string | number | boolean | undefined): string {
  if (typeof value === "string") {
    return \`String: "\${value}"\`;
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `function process(value: string | null): void {
  if (value === null) {
...
<Figure
  slug="tsx-type-narrowing-narrowing-problem-inline-cutout"
  alt="A wide chute carrying three different shapes with a single narrow opening at its end"
  caption="Narrowing is how a wide union becomes a single type you can act on."
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `function printLength(value: string | null | undefined): void {
  if (value) {
...
<Figure
  slug="tsx-type-narrowing-inline-cutout"
  alt="A wide chute carrying a cube, a cylinder and a cone, passing through two upright plates in turn, the cube left stopped against the first plate and the cylinder left stopped against the second, so only the cone continues on to the right"
  caption="Each check rules out possibilities, until one type is left."
/>
...
That caveat is why this is the guard most often used to mean something it
does not mean. Here are all the guards so far side by side, against one wide
union, with the two rows that cost people an afternoon marked:

<Chart slug="narrowing-guards-grid" />
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `function assertIsString(value: unknown): asserts value is string {
  if (typeof value !== "string") {
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `function assertNonNull<T>(value: T | null | undefined): asserts value is T {
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `const mixed: (string | number)[] = [1, "two", 3, "four"];
