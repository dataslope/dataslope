...
<Figure
  slug="tsx-api-contracts-cutout"
  alt="Two separate module platforms joined by a single shared contract plate that both must match at the seam"
/>
...
<Figure
  slug="tsx-api-modeling-and-contracts-problem-implicit-contracts-inline-cutout"
  alt="Two benches joined by a chute with no fitting at either end, blocks falling through the gap"
  caption="Without a written contract, nothing stops the wrong shape crossing the gap."
/>
...
<Figure
  slug="tsx-api-modeling-and-contracts-explicit-contracts-types-inline-cutout"
  alt="Two benches joined by a chute with a matching shaped fitting clamped at each end"
  caption="A typed contract puts a matching fitting on both ends of the call."
/>
...
3. **Transport layer**: HTTP handlers, WebSocket listeners, command-line parsers, anything that talks to the outside world.
