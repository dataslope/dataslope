...
<Figure
  slug="tsx-discriminated-unions-exhaustiveness-checking-inline-cutout"
  alt="A sorter with a chute for every tag shape and a bell that rings when a tag has no chute"
  caption="Add a variant with no branch and the compiler rings the bell."
/>
...
<Figure
  slug="tsx-discriminated-unions-inline-cutout"
  alt="Three sealed boxes each wearing a differently shaped tag, a sorter reading only the tag to send each box down its own chute"
...
<Figure
  slug="tsx-discriminated-unions-discriminant-field-inline-cutout"
  alt="Three sealed boxes each wearing a differently shaped tag on the same corner"
  caption="Every variant carries the same tag field in the same place."
/>
