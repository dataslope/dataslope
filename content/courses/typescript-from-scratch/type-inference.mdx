...
<Figure
  slug="tsx-type-inference-inline-cutout"
  alt="A block dropped into an empty socket, the socket closing around it to take exactly its shape without anyone measuring"
  caption="From mixed elements the compiler picks the narrowest type that fits them all."
/>
...
<Figure
  slug="tsx-type-inference-variable-initializer-inference-inline-cutout"
  alt="A block dropped into a soft empty socket that closes around it to take exactly its shape"
  caption="The initialiser is enough: the compiler takes the type from the value."
/>
