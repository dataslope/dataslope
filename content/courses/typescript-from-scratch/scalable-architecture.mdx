...
<Figure
  slug="tsx-scalable-architecture-cutout"
  alt="Three stacked layer platforms with every connector arrow pointing in one direction only, inward toward the core"
/>
...
<Figure
  slug="tsx-scalable-architecture-problem-tangled-dependencies-inline-cutout"
  alt="A bench where cords run between every pair of machines, crossing everywhere"
  caption="When everything depends on everything, nothing can change on its own."
/>
...
<Figure
  slug="tsx-scalable-architecture-monorepo-type-sharing-teaser-inline-cutout"
  alt="One shared drawer of shapes drawn on by three separate workshops in the same building"
  caption="Shared types are how separate packages keep speaking the same language across layers."
/>
...
<Figure
  slug="tsx-scalable-architecture-application-layer-use-cases-inline-cutout"
  alt="An inner ring holding the rules with every outside connection plugged into its rim rather than reaching through it"
  caption="The use cases hold the rules, and everything at the edge plugs into them."
/>
...
<Figure
  slug="tsx-scalable-architecture-dependency-inversion-typescript-inline-cutout"
  alt="An inner room describing the socket it needs, with the outer wall built to fit that socket"
  caption="The core declares the interface it needs, and the outside layer supplies something matching."
/>
...
function createOrder(orderId: string, total: number): Order | null {
  ...
  // The brand lives only in the type system: \`declare const\` emits no
  // JavaScript, so naming OrderBrand here would be a ReferenceError at run
  // time. The cast is where you assert the value has been validated.
  return { orderId, total } as unknown as Order;
}
