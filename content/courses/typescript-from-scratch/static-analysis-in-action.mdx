...
<Figure
  slug="tsx-static-analysis-cutout"
  alt="A checking lens sweeping a block assembly on a platform, flagging one mismatched socket before the machine runs"
/>
...
<Figure
  slug="tsx-static-analysis-in-action-compiler-proves-inline-cutout"
  alt="A shaped gate stopping a mismatched block before the belt has started moving"
  caption="Static analysis catches the mismatch while the belt is still stopped."
/>

What makes that worth the annotations is *when* the proofs happen. A bug is
never only the cost of its fix; it is the fix plus everything that happened
between writing it and finding it:
...
<Figure
  slug="tsx-static-analysis-in-action-exhaustiveness-checking-inline-cutout"
  alt="A rack with every slot filled and one narrow empty slot left at the end that nothing on the bench can fit"
  caption="Handle every case and the default branch has type `never`, which nothing can be assigned to."
/>
...
<Figure
  slug="tsx-static-analysis-in-action-refactor-safety-inline-cutout"
  alt="One part renamed at its source and every place that reached for it lighting up at the same moment"
  caption="Rename at the source and every stale reference becomes a compile error, not a runtime one."
/>
...
<Figure
  slug="tsx-static-analysis-in-action-pipeline-revisited-inline-cutout"
  alt="One gauge feeding two outputs, the left taken once at the end of a run and the right taken again after every touch"
  caption="The emitter reads the checker once per build; the editor reads it on every keystroke."
/>
...
<Figure
  slug="tsx-static-analysis-in-action-null-safety-strictnullchecks-inline-cutout"
  alt="A part of two possible shapes passing a gate that admits only one of them, and leaving as that single shape"
  caption="After the null check the compiler knows the type is narrower, and lets you use it."
/>
