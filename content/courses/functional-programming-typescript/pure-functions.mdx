...
<Figure
  slug="fp-pure-functions-definition-can-hold-inline-cutout"
  alt="A sealed machine with no hoses, wires or taps anywhere on it, one block in and one block out"
  caption="Same input, same output, and nothing observable changes on the way."
/>
...
remember it. This is called **memoization**. On a function that calls
itself, that one property is worth several orders of magnitude:

<Chart slug="memoization-call-tree" />
...
<Figure
  slug="fp-pure-functions-inline-cutout"
  alt="A sealed machine with no hoses or wires leaving it, giving the same output twice for the same input"
  caption="A pure function has no hidden inputs or outputs, so it repeats exactly."
/>
...
That function cannot be tested without stubbing the clock, the random
number generator and the console. Here is the same behaviour with the
