...
<Figure
  slug="fp-declarative-thinking-cutout"
  alt="Declarative thinking, an isometric illustration"
/>
...
<Figure
  slug="fp-declarative-thinking-same-task-both-inline-cutout"
  alt="A figure directing each step of a build on one bench, and on the next a card describing the finished stack with the machine building it"
  caption="Directing every step, or describing the outcome: two ways to the same result."
/>
...
<Figure
  slug="fp-declarative-thinking-inline-cutout"
  alt="A figure at the left holding up a card showing a finished stack, while the machine beside them builds that stack without being told each step"
  caption="Declarative code says what the result is, not the steps to build it."
/>
...
Pipelines work best when each stage does *one thing*.

A stage-per-job pipeline is also what makes laziness possible, and
laziness is what stops a pipeline that ends in "give me ten" from
reading a million rows:

<Chart slug="pipeline-laziness" />

Here is a realistic example, computing per-customer paid totals:
