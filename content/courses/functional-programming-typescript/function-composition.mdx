...
<Figure
  ...
  caption="Bolting functions end to end gives one function with one input and one output."
/>
...
<Figure
  slug="fp-function-composition-composition-means-mathematically-inline-cutout"
  alt="Two machines bolted mouth to chute so the pair has one inlet and one outlet, a single block passing through both"
  caption="Composing two functions means feeding the first one's output into the second."
/>
