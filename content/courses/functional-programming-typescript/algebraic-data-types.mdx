...
<Figure
  slug="fp-algebraic-data-types-cutout"
  alt="Algebraic data types, an isometric illustration"
/>
...
add. A `boolean` has 2 values; a pair of booleans has $2 \times 2 = 4$.
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `// Record / object: a product
type User = {
...
<Figure
  slug="fp-algebraic-data-types-inline-cutout"
  alt="A rack holding three differently shaped slots where exactly one slot is filled at a time, and a lid that only closes when one is"
  caption="A sum type holds exactly one of its alternatives at a time."
/>
...
<Figure
  slug="fp-algebraic-data-types-sums-typescript-discriminated-inline-cutout"
  alt="Three boxes with differently shaped tags, a sorter reading only the tag to send each down its own chute"
  caption="The tag is read first, and it decides which shape the rest must have."
/>
...
Products multiply and sums add, and that arithmetic is the whole argument
for reaching for a sum:

<Chart slug="illegal-states-explosion" />
...
function describe(u: User): string {
  switch (u.status.kind) {
    case "active":
      return \`\${u.id} is active\`;
    case "suspended":
      return \`\${u.id} suspended until \${u.status.until} (\${u.status.reason})\`;
    case "deleted":
      return \`\${u.id} deleted at \${u.status.deletedAt}\`;
  }
}
...
const s: User = {
  id: "u2",
  status: { kind: "suspended", reason: "spam", until: 2030 },
};
...
<Figure
  slug="fp-algebraic-data-types-two-operations-products-inline-cutout"
  alt="One tray holding three compartments all filled at once, beside another tray with three compartments where exactly one is filled"
  caption="A product holds every field at once; a sum holds exactly one."
/>
  ...
  switch (e.kind) {
    ...
    case "payment_received":
      switch (e.method.kind) {
        case "card":
          return \`paid \${e.amount} on \${e.orderId} via card *\${e.method.last4}\`;
        case "bank":
          return \`paid \${e.amount} on \${e.orderId} via bank \${e.method.iban}\`;
        case "crypto":
          return \`paid \${e.amount} on \${e.orderId} in \${e.method.currency}\`;
      }
...
\`\`\``}
  ...
  files={[
    {
      filename: "main.ts",
      starterCode: `import { area } from "./shape";
...
`,
    ...
    {
      filename: "shape.ts",
      starterCode: `export type Shape = { kind: "circle"; radius: number };
...
`,
      solutionCode: `export type Shape =
  | { kind: "circle"; radius: number }
  | { kind: "rectangle"; width: number; height: number }
  | { kind: "triangle"; base: number; height: number }
