...
<Figure
  slug="fp-pattern-matching-setup-discriminated-union-inline-cutout"
  alt="Three sealed boxes each wearing a differently shaped tag, waiting at the mouth of a sorter"
  caption="Every variant carries a tag, which is what the match reads first."
/>
...
<Figure
  slug="fp-pattern-matching-inline-cutout"
  alt="A shape sorter board with four differently cut holes, each shape dropping through the one hole it fits and landing in its own tray"
  caption="Each shape drops through the one branch it fits, and no other."
/>
``` `${current}:${emergency}` ```.
