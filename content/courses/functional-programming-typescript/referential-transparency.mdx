...
<Figure
  slug="fp-referential-transparency-concrete-demonstration-inline-cutout"
  alt="A block and the finished result of that block, either one dropping into the same slot and the machine behaving identically"
  caption="If an expression can be swapped for its value, the program is unchanged."
/>
