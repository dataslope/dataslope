...
Call it twice with the same argument and you can get different answers,
so the call cannot be replaced by its result. Moving the randomness into
a parameter fixes that without making the function less useful:
