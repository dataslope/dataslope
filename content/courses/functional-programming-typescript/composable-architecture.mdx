...
<Figure
  slug="fp-composable-architecture-picture-inline-cutout"
  alt="A wall assembled from interchangeable panels, one panel lifted out and a differently shaped one dropped into the gap"
  caption="Swap the panel at the edge and the wall behind it never has to change."
/>
...
<Figure
  slug="fp-composable-architecture-this-design-wins-inline-cutout"
  alt="One unit unclipped from the middle of a chain and replaced with another, the chain running on unchanged"
  caption="Replace one unit and the rest of the chain does not notice."
/>
...
<Figure
  slug="fp-composable-architecture-inline-cutout"
  alt="Small units with identical fittings clipping into long chains in several different orders"
  caption="Identical fittings are what let the same parts chain in any order."
/>

When you internalize that, all the functional techniques you learned
