...
<Figure
  slug="fp-immutability-inline-cutout"
  alt="A sealed block on the left, and beside it a fresh copy with one face changed, the original still sealed and untouched"
  caption="Nothing is changed in place; a modified copy is made instead."
/>
...
`sort()` is the famous one, but it is not alone. JavaScript's `Date` is
mutable too, and it catches people for the same reason:
...
<Figure
  slug="fp-immutability-cost-mutation-one-inline-cutout"
  alt="Two benches sharing one tray, a hand at each altering it, and the block on it ending up wrong for both"
  caption="Two holders of one mutable object is where the value quietly goes wrong."
/>
