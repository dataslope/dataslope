...
description: From lambda calculus and Lisp to ML, Haskell, Erlang, Scala, and the modern functional ecosystem.
---
...
<Figure
  slug="fp-roots-of-fp-cutout"
  alt="The roots of functional thinking, an isometric illustration"
/>
...
<Figure
  ...
  alt="Three identical machines chained in a row, each taking one block in and handing a different block out"
  caption="Alonzo Church, 1936: a formal system containing nothing but functions applied to functions."
/>
...
<Figure
  slug="fp-roots-lisp-cutout"
  alt="A figure at a 1950s console holding open a set of nested boxes, each holding a smaller box inside it"
  caption="John McCarthy, MIT, 1958: the lambda calculus turned into a language, with lists all the way down."
/>

```mermaid
timeline
    title FP languages and ideas (1958 to today)
...
1995, the pitch that drew him in was the chance to put **Scheme**,
a famously elegant Lisp dialect, in the browser. Management then
decided the new language had to *look* like Java, and Eich built the
first version in a now-legendary ten days in May 1995. The result
was a language with Java's curly braces but Scheme's soul:
first-class functions and closures survived the corporate
makeover. Every `.map` and every callback you've ever written runs
on machinery smuggled in from the functional tradition.

For years, though, the language was used mostly imperatively:
`for` loops, mutation, prototypes.
...
Three variables to track, and the *what* is buried in the *how*. Lisp's
answer, twenty years earlier, was to let the call stack do the counting:
...
No index, but now there is a base case and a recursive case to hold in
your head. The ML tradition went one step further and named the three
operations, so the shape of the computation is the code:
...
<Figure
  slug="fp-roots-of-functional-thinking-lambda-calculus-math-inline-cutout"
  alt="A plain sheet of hand-drawn marks on a bench with a working machine built directly on top of it"
  caption="The lambda calculus was mathematics first, and the machines came after."
/>
