...
<Figure
  slug="duck-why-windows-cutout"
  alt="A grid platform where every row stays in place while a translucent frame hovers over a group, projecting one summary value onto each row"
/>
...
<Figure
  slug="duck-why-window-functions-over-clause-makes-window-inline-cutout"
  alt="An aggregate that leaves every row standing and writes its total alongside, instead of collapsing them into one"
  caption="`OVER` is what stops an aggregate collapsing the rows it summarises."
/>
...
Notice all 24 rows survived, each carrying the same grand total,
`3225.00`. A plain `SUM(amount)` would have returned a single row with
that number and thrown the detail away. The `OVER ()` is the entire
difference: it says "summarise, but give the answer back to every row."
...
<Figure
  slug="duck-why-window-functions-problem-group-by-inline-cutout"
  alt="Chips collapsing into one summary block on one bench, and on the next the chips still standing each with its own tag"
  caption="A window computes a group summary and hands it back to every row in the group."
/>
...
<Figure
  slug="duck-why-window-functions-inline-cutout"
  alt="A pile of blocks collapsing into one summary block on the left, and on the right the same blocks still standing in their original row, each wearing its own small summary tag"
  caption="`GROUP BY` collapses the rows; a window function leaves them standing."
/>
