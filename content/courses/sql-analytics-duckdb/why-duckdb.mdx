...
<Figure
  slug="duck-why-duckdb-problem-duckdb-solves-inline-cutout"
  alt="A heavy remote strongroom with a long pipe beside a compact engine sitting on the workbench"
  caption="DuckDB runs in the same process as your analysis, with no server in between."
/>
...
<Figure
  slug="duck-why-duckdb-inline-cutout"
  alt="A heavy remote strongroom with a long pipe on one bench, and on the next a compact engine sitting right beside the workbench"
  caption="No connection, no cluster: the engine sits on the bench beside the work."
/>
...
SELECT
  ...
  island,
  COUNT(*) AS n,
  -- The CSV writes missing values as "NA", so the column infers as text;
  -- TRY_CAST turns what is numeric into a number and the rest into NULL.
  ROUND(AVG(TRY_CAST(body_mass_g AS DOUBLE)), 0) AS avg_mass
FROM
  read_csv_auto(
