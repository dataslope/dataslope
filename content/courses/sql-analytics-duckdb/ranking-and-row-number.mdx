...
<Figure
  slug="duck-ranking-cutout"
  alt="Three podium steps on a platform inside a partitioned lane, small numbered markers resting on each step"
/>
...
<Figure
  slug="duck-ranking-and-row-number-inline-cutout"
  alt="A row of blocks combed into height order with a pennant planted on each, the pennants shortening along the row and two equal blocks sharing one"
  caption="`PARTITION BY` restarts the numbering inside each group."
/>
...
<Figure
  slug="duck-ranking-and-row-number-numbering-rows-row-inline-cutout"
  alt="A row of blocks in height order with a pennant planted on each, the pennants shortening along the row"
  caption="Ranking answers where a row stands relative to the others."
/>
