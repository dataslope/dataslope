...
<Figure
  slug="ml-encoding-categorical-features-problem-models-need-inline-cutout"
  alt="A machine whose inlet accepts only plain blocks, three coloured tags waiting outside it"
  caption="A model only takes numbers, so categories have to be encoded first."
/>
...
<Figure
  slug="ml-encoding-categorical-features-tempting-trap-integer-inline-cutout"
  alt="Three coloured tags replaced by blocks of rising height, the heights implying an order the colours never had"
  caption="Numbering categories invents an order the model will take seriously."
/>
...
distance-based model like k-nearest neighbors will think red and green
are "close" while
...
<Figure
  slug="ml-encoding-categorical-features-inline-cutout"
  alt="Three coloured tags each replaced by a small row of on-off switches, one switch raised in each row"
  caption="One-hot gives each category its own column, with a single switch on."
/>
