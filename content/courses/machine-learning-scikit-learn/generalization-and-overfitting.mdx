...
<Figure
  slug="ml-generalization-cutout"
  alt="Generalization, overfitting, and underfitting, an isometric illustration"
/>
...
<Figure
  slug="ml-generalization-and-overfitting-generalization-really-means-inline-cutout"
  alt="A shape fitted to a set of chips and then tested against a fresh set it has never seen"
  caption="Generalisation is measured on data the model has never been shown."
/>
...
<Figure
  slug="ml-generalization-and-overfitting-seeing-both-failures-inline-cutout"
  alt="A line threading through every chip on the left and a straight rod ignoring them all on the right"
  caption="Too flexible chases the noise; too rigid misses the pattern altogether."
/>

We use *synthetic* data here on purpose: overfitting is defined as fitting
noise instead of the true pattern, and the only way to point at that
distinction is to know the true pattern exactly. With real data the truth is
hidden, so we generate it ourselves and then watch which models recover it.
...
Let us quantify what the eye just saw. Sweep the polynomial degree from
rigid to flexible and, at each step, measure error on the training data and
on a held-out test set, and the two errors part company:

<Chart slug="bias-variance" />

Run it yourself and watch the same two curves appear:
...
First work out which failure you have. A **learning curve** plots training
and validation *error* against the amount of training data, and its shape
tells you what to do: the gap between the curves is variance, and the level
they settle at is bias. A gap still closing at the right-hand edge means
more data is the cheapest fix; two curves that met early and levelled off
high mean the model is too simple, and no amount of data will help.

<Chart slug="learning-curve" />

<Figure
  slug="ml-generalization-and-overfitting-inline-cutout"
  alt="Two boards side by side, the left with a line threading through every single point, the right with a smooth line passing near them, and a fresh point landing far from the left line"
  caption="The curve that fits every training point is the one a new point defeats."
/>
...
A famous cautionary tale is **Google Flu Trends**, launched in 2008 to
predict flu outbreaks from search queries. It worked impressively at first,
then drifted badly, by some seasons it overshot real flu rates by roughly
double. A 2014 *Science* analysis ("The Parable of Google Flu") argued the
model had latched onto search patterns that did not generalize as behavior
and the world shifted under it. A model that does not keep generalizing is a
model that quietly stops working.
