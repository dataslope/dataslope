...
<Figure
  slug="ml-regression-classification-clustering-regression-predicting-number-inline-cutout"
  alt="A cloud of chips with a rod through it and a marker sliding along the rod to a new position"
  caption="Regression predicts where on a continuous scale the answer falls."
/>
...
<Figure
  slug="ml-regression-classification-clustering-clustering-discovering-groups-inline-cutout"
  alt="A scatter of plain untagged chips falling naturally into three clumps"
  caption="With no labels at all, the grouping has to come from the data's own shape."
/>
...
<Figure
  slug="ml-regression-classification-clustering-inline-cutout"
  alt="Three benches, one fitting a ramp to chips, one sorting chips into two bins, one grouping chips with no bins given"
  caption="Fitting a curve, sorting into classes, or finding groups: three different jobs."
/>
