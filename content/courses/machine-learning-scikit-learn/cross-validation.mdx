...
<Figure
  slug="ml-cross-validation-cutout"
  alt="Cross-validation, an isometric illustration"
/>
...
<Figure
  slug="ml-cross-validation-problem-one-split-inline-cutout"
  alt="A pile of blocks cut once by a single blade, a spinner beside it showing the cut could have fallen elsewhere"
  caption="A single split is one roll of the dice, and the score moves with the seed."
/>
...
Nothing changed but the luck of the draw, yet the accuracy swings by more
than ten percentage points from the unluckiest split to the luckiest. If you
had run *one* split and reported its number as "the" accuracy, you might
have been several points too optimistic or too pessimistic, and you would
never have known. Which split is the "true" one? None of them. The truth is
somewhere in the middle, and a single split cannot tell you where.

Here is the same wobble next to the fix. Each dot is one evaluation of the
same model, and the only difference between the rows is how much of the data
each estimate was allowed to see:
...
<Figure
  slug="ml-cross-validation-fix-k-fold-inline-cutout"
  alt="One pile divided into five trays, with five rows below where a different tray is sealed each time"
  caption="Each round seals a different fold, so every row is tested exactly once."
/>
...
## The leakage trap cross-validation makes obvious
...
and your cross-validation score is optimistic. The fix is to put preprocessing **inside**
...
hyperparameters. The moment you select the option with the best cross-validation score,
...
<Figure
  slug="ml-cross-validation-inline-cutout"
  alt="One pile of blocks divided into five trays, and five rows below where a different tray is sealed each time while the other four are used"
  caption="Averaging the rounds gives a steadier number than any single split."
/>
...
- **"The cross-validation mean is the exact production accuracy."** It is a better
...
machine learning project and every Kaggle competition. When a paper claims a model is
...
<ChallengeCard
  ...
  estimatedTime="~6 min"
  instructions={`We have filtered the penguins down to the two most separable species,
**Adelie** and **Gentoo** (\`X\`, \`y\` provided).
...
(above 0.9), which it should be for this very separable pair.`}
  datasets={[{ path: "palmer-penguins.csv", stageAs: "penguins.csv" }]}
  files={[
    {
      filename: "main.py",
...
features = ["bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"]
X = two[features].values
y = two["species"].values`,
...
assert abs(mean_score - cv_scores.mean()) < 1e-9`,
    },
    {
      ...
      description: "A scaled logistic model separates Adelie from Gentoo cleanly",
