...
<Figure
  slug="ml-hierarchical-clustering-cutout"
  alt="Hierarchical clustering, an isometric illustration"
/>
...
<Figure
  slug="ml-hierarchical-clustering-problem-solves-inline-cutout"
  alt="A scatter of chips with no bins given, a hand pairing the two closest and then pairing the pairs"
  caption="Merging the closest pair over and over builds groups without fixing k first."
/>
...
<Figure
  slug="ml-hierarchical-clustering-intuition-start-apart-inline-cutout"
  alt="Chips joined into pairs, pairs joined into groups, groups joined at the top into one branching frame"
  caption="Every merge is recorded, so you can cut the tree at any level you like."
/>
...
<Figure
  slug="ml-hierarchical-clustering-seeing-tree-dendrogram-on-inline-cutout"
  alt="A small set of marks joined into a branching frame, the height of each join showing how late the two came together"
  caption="The height of each join is how different the two groups were when they merged."
/>
...
## On real data: grouping California districts

Synthetic blobs make the dendrogram legible, but the method is the same on
real features. Here we cluster California census districts using only their
numeric attributes, no target, just structure. Two real-world details show
up immediately. First, the features live on wildly different scales
(`Population` is in the thousands, `AveRooms` is a single digit), so without
`StandardScaler` the largest-magnitude column would dominate every distance.
Second, agglomerative clustering is roughly *$O(n^2)$*, so we draw a **random
sample of 400 districts** rather than all ~20,000, large enough to find
structure, small enough to stay fast. We fix `random_state` on `.sample()` so
the draw is reproducible; the clustering itself needs no seed.
...
# Hierarchical clustering is O(n^2), so sample a manageable subset.
# (Clustering is deterministic; the seed here only fixes WHICH rows we draw.)
sample = housing.sample(n=400, random_state=0)

# Features only, clustering is unsupervised. Scale so no column dominates.
X_scaled = StandardScaler().fit_transform(sample)

# No random_state on the model itself: agglomerative clustering is deterministic.
agg = AgglomerativeClustering(n_clusters=3)
labels = agg.fit_predict(X_scaled)

values, counts = np.unique(labels, return_counts=True)
print("Districts sampled:   ", len(sample))
print("Cluster labels found:", values)
print("Districts per cluster:", counts)
`,
    },
  ]}
/>

You get three groups of districts with no labels to check against, that is
clustering. The clusters tend to track things like income and location, but
the point here is mechanical: the exact same `fit_predict()` you ran on blobs
works on real, unscaled, differently-sized features, *once you standardize*.
On a sample this size the tree builds instantly; push the sample toward tens
of thousands and you would feel the quadratic cost the next section warns
about.
...
<Figure
  slug="ml-hierarchical-clustering-inline-cutout"
  alt="Chips joined into pairs, pairs joined into groups, and groups joined at the top into one branching frame"
  caption="The whole nesting is the output, not one fixed set of labels."
/>
  ...
  tests={[
    {
      ...
      description: "Scaling preserves the shape of X",
...
assert len(labels) == len(X), f"Expected {len(X)} labels, got {len(labels)}"`,
    },
    {
      id: "largest_size",
...
assert 0 < largest_cluster <= len(X), f"Largest cluster out of range: {largest_cluster}"`,
    },
  ]}
/>
