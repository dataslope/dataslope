...
<Figure
  slug="ml-supervised-vs-unsupervised-cutout"
  alt="Supervised vs unsupervised learning, an isometric illustration"
/>
...
<Figure
  slug="ml-supervised-vs-unsupervised-deciding-question-there-inline-cutout"
  alt="One bench where every block arrives with a tag and another where blocks arrive plain"
  caption="One question decides it: is there a labelled target to predict?"
/>
...
is the setting of every example so far in the course, penguin species, house
prices, loan defaults, because supervised learning is the most common and
most directly useful kind.

<Figure
  slug="ml-supervised-vs-unsupervised-supervised-learning-learning-inline-cutout"
  alt="Tagged blocks entering a machine and the machine learning to place a tag on an untagged one"
  caption="Supervised learning copies a mapping from examples that carry answers."
/>
...
classifier on the Palmer penguins, a supervised task, because every training
penguin comes labeled with its true species, and then *score* it on held-out
penguins. The crucial detail: we pass both `X` and `y` to `fit()`, because
...
X = penguins[features]
y = penguins["species"]
print("We have labels. First few:", list(y[:5]))
...
print(f"Accuracy on held-out penguins: {accuracy:.0%}")
`,
    },
  ]}
/>
...
<Figure
  slug="ml-supervised-vs-unsupervised-inline-cutout"
  alt="One bench where every block arrives with a tag, and another where blocks arrive plain and are sorted into groups by shape alone"
  caption="With labels you learn a mapping; without them you look for structure."
/>

We will cluster synthetic data made of three natural blobs of points. Synthetic
blobs are the right choice here precisely *because* we control them: we know
there really are three groups, so we can judge whether the algorithm recovers
the structure we built in. Critically, although `make_blobs` *can* hand back
the true blob labels, we **throw them away** and never give them to the model.
`KMeans` sees only the point coordinates `X` and must discover the groupings on
its own.
...
features = ["bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"]
Xs, ys = penguins[features], penguins["species"]
sup = LogisticRegression(max_iter=2000)
