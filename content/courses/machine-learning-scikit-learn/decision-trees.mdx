...
<Figure
  slug="ml-decision-trees-core-idea-split-inline-cutout"
  alt="A mixed heap divided by one cut into two heaps that are each more uniform than the first"
  caption="A tree picks the split that leaves each side as pure as it can."
/>
...
<Figure
  slug="ml-decision-trees-problem-tree-solves-inline-cutout"
  alt="A heap of mixed chips that no single straight blade can separate, beside a branching chute that does"
  caption="A straight boundary cannot separate these; a sequence of questions can."
/>
...
genuine convenience that distance-based models like k-nearest neighbors
do not enjoy.
</Callout>
...
<Figure
  slug="ml-decision-trees-inline-cutout"
  alt="A branching chute where each fork carries a small gauge, a block dropping through three forks in turn and landing in one of eight trays"
  caption="For regression the leaf holds an average instead of a majority class."
/>
...
Both curves below come from the same tree fitted at increasing depths. The
