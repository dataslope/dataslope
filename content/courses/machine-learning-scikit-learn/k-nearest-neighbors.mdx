...
<Figure
  ...
  alt="One new chip on a board taking the color of the three chips nearest to it"
...
The same training data at three values of `k`, with the decision boundary
each one produces:
...
<Figure
  slug="ml-k-nearest-neighbors-make-break-issue-feature-inline-cutout"
  alt="A compass whose one long arm swamps the short one, the pair then trimmed until both arms match"
  caption="Nearest depends on the units, so an unscaled column decides every neighbour."
/>
...
<Figure
  ...
  alt="One new chip placed on a board of colored chips, with short cords running from it to the three closest chips and taking their color"
