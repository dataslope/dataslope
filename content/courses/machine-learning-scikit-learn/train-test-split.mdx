...
<Figure
  slug="ml-train-test-split-problem-solves-inline-cutout"
  alt="A machine graded on the very blocks it was built from, its gauge reading perfectly and meaning nothing"
  caption="Graded on its own training rows, any model looks better than it really is."
/>
...
<Figure
  slug="ml-train-test-split-fix-hold-some-inline-cutout"
  alt="A pile divided by a partition with the smaller share sealed under a lid"
  caption="Lock part of the data away and it stays an honest test to the very end."
/>
...
<Figure
  ...
  caption="`train_test_split()` shuffles the rows first, then cuts them in two."
/>
