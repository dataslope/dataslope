...
<Figure
  slug="ml-bias-variance-cutout"
  alt="The bias–variance tradeoff, an isometric illustration"
/>
...
<Figure
  slug="ml-the-bias-variance-tradeoff-error-has-parts-inline-cutout"
  alt="One gauge reading total error beside three smaller gauges whose readings add up to it"
  caption="Total error splits into bias, variance, and noise you can never remove."
/>
...
<Figure
  slug="ml-the-bias-variance-tradeoff-dartboard-picture-inline-cutout"
  alt="Two dartboards, one with a tight cluster off centre and one with a wide scatter around the centre"
  caption="Bias is aiming off centre; variance is a scatter that shifts with the data."
/>
...
<Figure
  slug="ml-the-bias-variance-tradeoff-inline-cutout"
  alt="Two dartboards on a wall, one with a tight cluster far off centre and the other with a wide scatter around the centre"
  caption="The tight cluster is consistent but wrong; the spread is right on average."
/>

This demonstration only works with synthetic data: measuring bias and
variance requires drawing *many* fresh training sets from a *known* true
function, and real datasets give you neither. We generate the truth so we
can resample it at will and compare each fit against it.
