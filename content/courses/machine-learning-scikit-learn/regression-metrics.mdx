---
title: "Regression Metrics: Absolute, Squared, and Explained Error"
...
<Figure
  slug="ml-regression-metrics-cutout"
  alt="Regression metrics, an isometric illustration"
/>
...
<Figure
  slug="ml-regression-metrics-inline-cutout"
  alt="A straight rod laid through a cloud of chips with short vertical sticks measuring each chip's distance to the rod"
  caption="Every regression metric is built from these gaps between actual and predicted."
/>
...
# Predict a diamond's price (in dollars) from its numeric measurements.
...
print("First five actuals:   ", np.round(y_test[:5].to_numpy(), 1))
...
<Figure
  slug="ml-regression-metrics-mae-mean-absolute-inline-cutout"
  alt="Short vertical sticks measuring each chip's distance to a rod, all sticks laid end to end and halved"
  caption="Mean absolute error averages the gaps, treating every mistake alike."
/>
...
print(f"MAE: {mae:.1f}")
print(
    "Interpretation: on average the predicted price is off by about",
    f"\${mae:.0f}.",
)
`,
    },
...
<Figure
  slug="ml-regression-metrics-mse-mean-squared-inline-cutout"
  alt="Sticks measuring chip distances to a rod, each stick redrawn as a square of its own length"
  caption="Squaring makes one large error count for more than several small ones."
/>
...
Forty-nine residuals held fixed, one swept from zero to forty:
...
<ChallengeCard
  ...
  estimatedTime="~7 min"
  instructions={`A \`LinearRegression\` is already fit on the diamonds training
set (predicting \`price\` from the numeric features), and \`y_test\` /
\`y_pred\` are available.
...
confirm \`r2\` lands in the range a numeric-feature linear model reaches on
diamonds (around 0.85).`}
  datasets={[{ path: "diamonds.csv", stageAs: "diamonds.csv" }]}
  files={[
    {
      filename: "main.py",
...
assert rmse >= mae, "RMSE should never be smaller than MAE"`,
    ...
    {
      id: "r2_ok",
      name: "R² is correct and in a sensible range for diamonds",
      description: "This numeric-feature model explains most of the price variance",
      code: `from sklearn.metrics import r2_score
assert r2 is not None and abs(r2 - r2_score(y_test, y_pred)) < 1e-6
assert 0.8 < r2 < 0.92, f"Expected 0.8 < R2 < 0.92 on diamonds, got {r2}"`,
    },
  ]}
/>
