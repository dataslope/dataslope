...
<Figure
  slug="ml-k-means-clustering-problem-solves-inline-cutout"
  alt="A scatter of chips with three posts being driven in among them, each chip roped to its nearest post"
  caption="K-means places k centres and gives every point to its nearest one."
/>
...
<Figure
  slug="ml-k-means-clustering-intuition-centers-points-inline-cutout"
  alt="Three posts among a scatter of chips, each chip roped to its nearest post and the posts shifting to the middle of their ropes"
  caption="Assign, move each centre to the middle of what it caught, and repeat."
/>
...
<Figure
  slug="ml-k-means-clustering-inertia-hard-question-choosing-inline-cutout"
  alt="A falling curve of readings with a clear bend, past which each further step buys almost nothing"
  caption="Inertia always falls as `k` grows, so the bend matters and the minimum does not."
/>
...
<Figure
  slug="ml-k-means-clustering-must-standardize-first-inline-cutout"
  alt="Two rulers of very different lengths laid against the same row of marks, then both rulers recut to one length"
  caption="K-means measures distance, so a column with a bigger range quietly decides every cluster."
/>
...
display(np.round(housing[["Population", "Latitude"]].describe().loc[["min", "max"]], 2))
...
display(np.round(housing_labeled.groupby("cluster")[["MedInc", "Latitude", "Longitude"]].mean(), 2))
`,
    },
  ]}
/>
...
<Figure
  slug="ml-k-means-clustering-inline-cutout"
  alt="A board of scattered chips with three marker posts, each chip joined by a short cord to its nearest post, the posts standing at the middle of their own group"
  caption="Nearest centre only works when the true groups are round-ish blobs."
/>
