...
<Figure
  slug="ml-evaluating-clusters-cutout"
  alt="Evaluating clusters, an isometric illustration"
/>
...
<Figure
  slug="ml-evaluating-clusters-inline-cutout"
  alt="Two boards of grouped chips, one with tight well-separated groups and one whose groups visibly bleed together"
  caption="With no labels, all you can judge is how tight and separate the groups are."
/>
...
<Figure
  slug="ml-evaluating-clusters-silhouette-score-inline-cutout"
  alt="One chip measured for its distance to its own group and to the nearest other group, two calipers on it at once"
  caption="The silhouette compares each point's own group against the nearest rival."
/>
...
<Figure
  slug="ml-evaluating-clusters-using-silhouette-score-inline-cutout"
  alt="Three boards of grouped chips with a gauge beside each, one gauge visibly higher than the others"
  caption="Score several values of k and let the highest silhouette pick one."
/>
...
Synthetic blobs make the peak obvious. Real data rarely cooperates. Below we
sweep `k` over the California housing districts (features only, no price,
this is pure clustering) after standardizing, since the silhouette compares
distances and unscaled columns like `Population` would otherwise dominate.
...
# silhouette_score is O(n^2), so sample before scoring on 20k+ districts.
sample = housing.sample(n=2000, random_state=0)
...
The scores hover in a modest band and the "peak" is shallow, there is no
crisp natural number of clusters, just a few defensible choices. That is the
normal experience on real data, and exactly why the silhouette is a guide to
weigh against domain sense, not an oracle that hands you the answer.

That is also what separates it from the elbow plot in *K-Means Clustering*.
Inertia only ever falls, so an elbow has to be judged; the silhouette is not
monotone in `k`, so it has a genuine maximum you can point at:

<Chart slug="silhouette-vs-k" />
