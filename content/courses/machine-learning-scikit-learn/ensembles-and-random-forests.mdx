...
<Figure
  slug="ml-ensembles-cutout"
  alt="Ensembles and random forests, an isometric illustration"
/>
...
<Figure
  slug="ml-ensembles-and-random-forests-intuition-wisdom-crowds-inline-cutout"
  alt="One gauge alone reading badly beside twenty gauges whose averaged needle sits on the mark"
  caption="Averaging many noisy trees cancels out their individual mistakes."
/>
...
Averaging cuts variance, but only to the extent the members disagree.
...
<Figure
  slug="ml-ensembles-and-random-forests-trick-one-bootstrap-inline-cutout"
  alt="Several scoops taken from one bin with replacement, each scoop feeding its own small machine"
  caption="Each tree grows on its own resample, so no two trees see the same data."
/>
...
<Figure
  slug="ml-random-forests-breiman-cutout"
  alt="A row of five small trees, each a slightly different shape, and one basket collecting a single token from each"
  caption="Leo Breiman, 2001: many trees, each grown on a different slice, and the vote is the answer."
/>
