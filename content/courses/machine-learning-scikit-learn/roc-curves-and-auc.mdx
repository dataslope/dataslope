---
title: ROC Curves and the Area Under Them
description: Precision and recall judge a classifier at one threshold. ROC curves judge it at every threshold at once, and the area under the curve boils that into a single, threshold-free score.
---
...
separate from any one operating point? The **ROC curve** (for receiver
operating characteristic, a name inherited from wartime radar) answers by
evaluating the model at *every* threshold simultaneously, and the **area
under that curve**, AUC, summarizes the whole picture in one number.
...
<Figure
  slug="ml-roc-curves-and-auc-two-rates-move-inline-cutout"
  alt="One dial turned through its range with two gauges beside it moving in opposite directions"
  caption="Loosen the threshold and you catch more positives but raise more false alarms."
/>
...
<Figure
  slug="ml-roc-curves-and-auc-roc-curve-inline-cutout"
  alt="A dial turned through its range with a curve tracing out beside it and the area beneath shaded"
  caption="The curve is every threshold at once, and the shaded area summarises it."
/>

One curve on its own is hard to judge, because every ROC curve bows the same
way. Three of them over the no-skill diagonal give you the scale:
...
<Figure
  slug="ml-roc-curves-and-auc-inline-cutout"
  alt="A dial being turned through its range on the left, and on the right a curve tracing out as it turns, with the area under that curve shaded"
  caption="A model that ranks well can still be thresholded badly for your problem."
/>
