...
<Figure
  slug="ml-pipelines-cutout"
  alt="Pipelines and ColumnTransformer, an isometric illustration"
/>
...
<Figure
  slug="ml-pipelines-and-columntransformer-problem-preprocessing-modeling-inline-cutout"
  alt="Two benches worked separately with a chip slipping between them, beside one bench where both are bolted together"
  caption="Two separate benches are where a step goes missing; one bolted unit cannot."
/>
...
<Figure
  slug="ml-pipelines-and-columntransformer-pipeline-one-estimator-inline-cutout"
  alt="Several small machines welded into one housing with a single handle on its front"
  caption="A pipeline is one estimator, so you fit it once and cannot skip a step."
/>
...
<Figure
  slug="ml-pipelines-and-columntransformer-inline-cutout"
  alt="Several columns each taking a different route through its own small machine before all routes join into one"
  caption="`ColumnTransformer` sends each group of columns down its own preparation."
/>
...
<Figure
  slug="ml-pipelines-and-columntransformer-columntransformer-different-transforms-different-inline-cutout"
  alt="One board's columns sent off to three different machines and returning to a single tray in the order they left"
  caption="Numeric columns scale, categorical columns encode, and one `fit()` call runs both."
/>
