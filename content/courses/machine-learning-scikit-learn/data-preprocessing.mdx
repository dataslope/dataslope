...
Two features that matter equally, in units that do not, and the share of
the distance between two rows that each one ends up contributing:
...
A related problem is worth meeting alongside it, because it also lives in the
feature column and also has no diagnostic:

<Chart slug="regression-dilution" />

Noise in y costs precision and biases nothing. Noise in *x* biases the slope
towards zero, systematically, and more rows do not help. Fixes exist and all
of them need extra information: repeated measurements, an instrumental
variable, or an errors-in-variables model.

The mechanism is one line: least squares divides the covariance of x and y by
the variance of x, and adding independent noise to x leaves the covariance
alone while inflating the denominator. The dangerous part is what the
diagnostics do. R-squared falls, which you would notice, and the residuals get
*more* evenly spread, because the added noise is homoscedastic by
construction, so the residual plot looks better rather than worse. This shows
up wherever a predictor is a proxy: self-reported intake, a survey score, a
single blood-pressure reading, a noisy sensor.
...
<Figure
  slug="ml-data-preprocessing-standardscaler-turning-features-inline-cutout"
  alt="Two rules of very different lengths passing through a press and leaving the same length"
  caption="Standardising puts every feature on one scale, so none dominates by units."
/>
...
<Figure
  slug="ml-data-preprocessing-problem-scaling-solves-inline-cutout"
  alt="Two measuring rules of wildly different lengths feeding one balance, the balance tipped hard by the longer"
  caption="A distance-based model is dominated by whichever feature has the widest range."
/>
...
- **K-Means** clustering and **principal component analysis** are built on
...
<Figure
  slug="ml-data-preprocessing-inline-cutout"
  alt="Raw blocks passing through a washing, trimming and gauging line before reaching the machine at the end of the bench"
  caption="Preprocessing is fitted on the training rows only, then applied to both."
/>
