...
<Figure
  slug="ml-hyperparameter-tuning-parameters-vs-hyperparameters-inline-cutout"
  alt="A machine with dials on its outside set by hand and gears inside that set themselves as it runs"
  caption="Parameters are learned during fitting; hyperparameters you set beforehand."
/>
...
| --- | --- | --- |
| `KNeighborsClassifier` | `n_neighbors` | *(none, k-nearest neighbors just stores the data)* |
...
<Figure
  slug="ml-hyperparameter-tuning-exists-models-do-inline-cutout"
  alt="A machine with three dials on its face and no marks on them, a hand turning each in turn"
  caption="The best setting of one dial depends on the others, so they are searched together."
/>
...
<Figure
  slug="ml-hyperparameter-tuning-gridsearchcv-try-every-combination-inline-cutout"
  alt="A rack of dials worked through every combination of settings in turn, with the best combination marked"
  caption="Every combination of settings tried under cross-validation, and the winner refitted."
/>
...
combination with the highest mean cross-validation score, and exposes it as
...
<Figure
  ...
  caption="A grid search scores every combination and reports the winner."
/>
