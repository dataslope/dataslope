...
<Figure
  slug="c-how-computers-execute-cutout"
  alt="How computers execute programs, an isometric illustration"
/>
...
<Figure
  slug="c-how-computers-execute-programs-computer-has-two-inline-cutout"
  alt="One small engine and one wall of slots standing side by side on a bench, a single cord joining them"
  caption="A processor and memory, joined by a bus, is the whole machine."
/>
...
<Figure
  slug="c-how-computers-execute-programs-memory-as-row-mailboxes-inline-cutout"
  alt="A wall of small identical doors in one long row, four neighbouring doors holding a single value between them"
  caption="Memory is one long row of addressed slots, and an `int` occupies four neighbours."
/>
...
<Figure
  slug="c-how-computers-execute-programs-fetch-decode-execute-cycle-inline-cutout"
  alt="A hand crank turning one notch to lift a single card, setting it down worked, and coming back for the next"
  caption="Fetch one instruction, decode it, execute it, repeat: the entire job of a processor."
/>
...
<Figure
  slug="c-how-computers-execute-programs-running-program-actually-means-inline-cutout"
  alt="A strip of instructions drawn steadily past a reading head, with three separate bins beside it holding what the work touches"
  caption="Instructions stream past the processor while code, stack and heap sit in memory beside it."
/>
...
<Figure
  slug="c-how-computers-execute-programs-worked-example-on-paper-inline-cutout"
  alt="One door in a wall of identical doors opened, its contents read, ten more added, and the result set back behind the same door"
  caption="`x = x + 10` is a read from an address, an addition, and a write back to it."
/>
...
<Figure
  slug="c-how-computers-execute-programs-programming-then-just-inline-cutout"
  alt="A plain arrangement of levers and rails set out on a card, the card being everything a language finally produces"
  caption="Every language exists to make arranging those instructions bearable for a human."
/>
