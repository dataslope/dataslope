...
<Figure
  slug="c-input-and-output-cutout"
  alt="Input and output, an isometric illustration"
/>
...
<Figure
  slug="c-input-and-output-printing-printf-inline-cutout"
  alt="A machine with a printer on its side pushing out one short strip of type"
  caption="`printf()` writes a formatted line to the output stream."
/>
...
<Figure
  slug="c-input-and-output-inline-cutout"
  alt="A small terminal with a slot at the left taking a punched slip and a printer at the right pushing out a finished strip"
  caption="Input comes in as bytes and `scanf()` needs the address of where to put them."
/>
...
There is no keyboard to type at inside a lesson, so the block below has
a **STDIN** panel under the editor holding the text the program reads.
Edit it and press Run again: it is standard input, exactly as if you had
typed it at a terminal.
...
uninitialized!). Put `hello` in the STDIN panel above and run it again
...
The two numbers are on one line here, but `%d` skips whitespace of every
kind, so putting them on two separate lines in the STDIN panel reads
exactly the same. That is worth knowing: to `scanf()`, a newline is
nothing more than another space.
...
`EOF` arrives when the input runs out. Here that is the end of the
STDIN panel; at a real terminal it is Ctrl-D (Ctrl-Z on Windows), and
for a program in a pipeline it is the moment the program upstream
closes its end.
