...
The row that surprises people is the speed one, because "slower" hides
how much slower. A stack allocation is one instruction that moves a
pointer. A heap allocation has to go and *find* a free block big
enough, which is real bookkeeping:

<Chart slug="heap-vs-stack-cost" />

This is not an argument against the heap. It is an argument against
reaching for it when a local would do, and it explains why a program
that allocates inside a hot loop is so often fixed by allocating once
outside it instead.
...
<Figure
  slug="c-dynamic-memory-malloc-free-inline-cutout"
  alt="A crate handed out over a counter, used at a bench, and handed back over the same counter"
  caption="`malloc()` hands you a block, and `free()` is how you give it back."
/>
...
<Figure
  slug="c-dynamic-memory-inline-cutout"
  alt="A counter on the left handing out a crate of the exact size asked for, a workbench using it in the middle, and the same crate being returned to the counter on the right"
  caption="Ask for exactly what you need, and return exactly what you asked for."
/>
...
<Figure
  slug="c-dynamic-memory-stack-vs-heap-inline-cutout"
  alt="A neat column of trays on the left and an open yard of crates on the right, cords running between them"
  caption="Stack memory dies with the function; heap memory lives until you free it."
/>
