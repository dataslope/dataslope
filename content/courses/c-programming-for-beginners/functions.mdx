...
<Figure
  slug="c-functions-anatomy-function-inline-cutout"
  alt="A machine shown in cross-section, its input opening, its inner works and its output chute all visible at once"
  caption="A function has a signature, a body and a return, and only the body is hidden."
/>
...
<Figure
  slug="c-functions-parameters-are-local-inline-cutout"
  alt="A block copied at a machine's mouth, the copy going in and being altered inside while the original sits untouched outside"
  caption="C copies each argument, so the caller's variable is never touched."
/>
...
<Figure
  ...
  caption="A function is a boundary: fixed inputs in, one value out."
/>
...
Even *with* a base case there is a ceiling, and it is not set by your
algorithm. Every call needs a frame on the stack, so the depth you can
reach is the stack's fixed budget divided by the size of one frame,
and a frame grows with whatever locals the function keeps:

<Chart slug="stack-depth-overflow" />

A function with a couple of `int` locals gets over 170,000 calls deep
before the stack runs out. Declare a `char buf[4096]` inside that same
function and it dies at around 2,000, roughly eighty times shallower,
without one line of the algorithm changing. Worth remembering the next
time a recursive function crashes on input that "isn't even that big".
