...
<Figure
  slug="c-conditionals-cutout"
  alt="Conditionals, an isometric illustration"
/>
...
<Figure
  slug="c-conditionals-if-statement-inline-cutout"
  alt="One hinged flap on a chute, blocks clearing a marked bar passing under it and the rest turned aside"
  caption="A bare `if` has one gate: the value passes the test or carries on past it."
/>
...
<Figure
  slug="c-conditionals-if-else-inline-cutout"
  alt="A chute with a two-way flap sending blocks down one of two paths, both ending in a tray"
  caption="The first branch whose condition holds runs, and the rest are skipped."
/>
...
<Figure
  slug="c-conditionals-inline-cutout"
  alt="A track meeting a set of points, a lever thrown one way sending the cart down the upper branch and the lower branch left empty"
  caption="Nested conditions branch again inside a branch, which is why they get hard to read."
/>
