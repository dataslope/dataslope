...
<Figure
  slug="conversation"
  alt="People in conversation with overlapping speech bubbles — the raw material of natural language processing"
  priority
/>
...
## Your first taste, right now

Enough preamble — let us actually *do* some NLP. The block below takes a
sentence, splits it into words, and counts how often each one appears. It uses
nothing but plain Python, so it runs the instant you press **Run** — no
downloads, no waiting. **This whole course runs inside your browser**, and this
is the first proof.
...
# Change this sentence to anything you like, then press Run again.
sentence = "the cat sat on the mat and the cat purred"

# Lowercase, split on spaces, and count each word.
...
Three lines of real work — split, count, rank — and you already have a tiny
text analysis. Try the one tweak the comment invites: **change the sentence**
to a line of your own and run it again. Notice that "the" almost certainly tops
the chart; that lopsidedness is a deep fact about language we will return to.
And notice the catch already lurking: `"mat"` and `"mat,"` would count as two
different words, because `.split()` knows nothing about punctuation. Fixing that
properly is the first real lesson of the course.
...
**Check Answer**, and see which tests pass. The blocks that use NLTK quietly
load the data they need in a collapsed, read-only setup section so you can
focus entirely on the NLP code.
</Callout>
...
## A taste of the full pipeline

The counting demo above was pure Python. Here is what the *real* toolkit looks
like: a complete, classic NLP pipeline that takes a short paragraph and runs it
through the whole journey of this course — split into words, normalize, drop
stopwords, reduce to dictionary roots, and count what is left. This one uses
NLTK, so the first run pauses briefly to load the data it needs; press **Run**,
and open the collapsed setup section above the editor if you are curious what is
