...
<Figure
  slug="nlp-tf-idf-intuition-before-formula-inline-cutout"
  alt="A balance with a very common tile on one pan barely moving it and a rare tile on the other tipping it"
  caption="A word in every document tells you nothing; a rare word tells you a lot."
/>
...
"data" appears twice in seven words, so its term frequency is $2/7 \approx 0.286$,
...
10000 and swamp everything else; the log compresses that to a sane $\approx 9.2$.
...
TF-IDF for a word in a document is simply $\text{TF} \times \text{IDF}$. A high score means the word
...
<Figure
  slug="nlp-tf-idf-inline-cutout"
  alt="A balance with a common word tile on one pan barely moving it, and a rare word tile on the other pan tipping the balance hard"
  caption="Term frequency times inverse document frequency weights the distinctive words."
/>
...
It is worth being clear about what this cannot do. TF-IDF compares
