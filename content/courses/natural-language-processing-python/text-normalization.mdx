...
Most normalizers reach for a regular expression here, so one warning
before you write one. A quantifier nested inside another quantifier gives
a backtracking engine exponentially many ways to fail, and a near-miss
input turns a fast pattern into a hang:
