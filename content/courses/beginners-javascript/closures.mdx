...
<Figure
  slug="js-closures-cutout"
  alt="Closures, an isometric illustration"
/>
...
<Figure
  slug="js-closures-closures-state-inline-cutout"
  alt="A sealed box with a counter dial on its face, the dial clicking up one notch each time a handle is pressed"
  caption="A closure keeps private state that survives between calls."
/>
...
<Figure
  slug="js-closures-simplest-closure-inline-cutout"
  alt="A figure carrying a small sealed box out through a doorway, one block from the room still visible inside the box, the room and its shelves filling the rest of the frame"
  caption="A function carries its birthplace with it, including the variables it used."
/>
...
const addUKTax = makeTaxCalculator(0.2);
...
<Figure
  slug="js-closures-inline-cutout"
  alt="A small machine leaving a workshop on the left carrying its own sealed toolbox, the workshop shuttered behind it, and the machine still working from that toolbox on the right"
  caption="The surrounding scope can close, and the function still reaches what it kept."
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      starterCode: `const { makeRateLimiter } = require("./rateLimiter");
...
`,
    },
    {
      ...
      starterCode: `// Returns a function that allows up to \`max\` calls, then denies the rest.
