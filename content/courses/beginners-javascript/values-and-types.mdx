...
<Figure
  slug="js-values-and-types-cutout"
  alt="Values and types, an isometric illustration"
/>
...
<Figure
  slug="js-values-and-types-value-piece-information-inline-cutout"
  alt="One single block resting alone on an otherwise empty bench, a caliper measuring it"
  caption="A type is the kind of value, and it decides what you may do with it."
/>
...
- **Object types**: arrays, plain objects, functions, dates,
  regexes, `Map`s, `Set`s, and more. These are *composite* values:
  they have parts, they live in the heap, and they are referred to
  by reference.
...
The full behaviour of `==` is a table rather than a rule, which is the
real argument for reaching for `===` by default.

<Chart slug="js-equality-grid" />
...
<Figure
  slug="js-values-and-types-inline-cutout"
  alt="Five blocks of clearly different kinds resting in their own moulded recesses on a tray"
  caption="Each type has its own shape, and mixing them is where surprises live."
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      starterCode: `const { square, isEven, describe } = require("./numbers");
...
`,
    ...
    {
      filename: "numbers.js",
      starterCode: `// Each function takes a value, returns a value.
