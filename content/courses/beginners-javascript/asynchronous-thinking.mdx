...
<Figure
  slug="js-asynchronous-thinking-cutout"
  alt="Asynchronous thinking, an isometric illustration"
/>
...
language like JavaScript, that's catastrophic, the interface freezes,
no other code runs, and your one CPU sits idle waiting.

<Figure
  slug="js-asynchronous-thinking-async-exists-at-inline-cutout"
  alt="A single counter where one slow order blocks a long queue behind it, and beside it the same counter with the slow order set aside to one side while the queue moves"
  caption="One slow job blocks everything behind it unless you set it aside."
/>
...
<Figure
  slug="js-asynchronous-thinking-non-blocking-everyday-inline-cutout"
  alt="A figure putting a kettle on, walking away to fold cloths, and returning when the kettle whistles"
  caption="Start the slow thing, get on with something else, come back when it signals."
/>
...
<Figure
  slug="js-asynchronous-thinking-inline-cutout"
  alt="A single worker at a counter starting a slow kettle, serving two other customers while it boils, and returning to the kettle when it whistles"
  caption="One worker can keep several jobs in flight without doing two at once."
/>
...
but only one bit of JavaScript is executing at any given moment.
...
One thread has a consequence you can measure: anything that takes longer
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      starterCode: `const { createBus } = require("./bus");
...
`,
    ...
    {
      filename: "bus.js",
      starterCode: `function createBus() {
...
pattern the browser document model uses (`element.addEventListener()`), Node uses
