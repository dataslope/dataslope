...
<Figure
  slug="js-maintainable-code-cutout"
  alt="Writing maintainable code, an isometric illustration"
/>
...
<Figure
  slug="js-maintainable-code-names-are-documentation-inline-cutout"
  alt="Two identical rows of unlabelled tins beside a row of the same tins carrying clear blank-shaped tags, a hand reaching straight to the right one"
  caption="A good name saves the reader the comment you would otherwise have to write."
/>
...
<Figure
  slug="js-maintainable-code-functions-should-do-inline-cutout"
  alt="One crowded machine doing four jobs at once beside four small machines each doing one, all four clipping together"
  caption="Four small machines that clip together beat one machine doing four jobs."
/>
...
that applies it automatically. The specific rules matter less than
the *consistency*, a lesson the industry learned the expensive
way. Programmers have burned astonishing amounts of energy on
style wars, none more famous than **tabs versus spaces**: a
decades-long holy war over which invisible character should indent
code. It escalated to the point that when Stack Overflow ran its
2017 Developer Survey, an analyst noticed that developers who
indent with spaces reported notably *higher salaries* than
tab-indenters, a correlation, of course, not a cause, but it says
something that anyone checked. The grown-up resolution is the one
modern teams use: pick either, configure the formatter, and never
discuss it again. Two strong personal habits the formatter can't
enforce for you:
...
<Figure
  slug="js-maintainable-code-consistent-style-inline-cutout"
  alt="A page of parts all cut to the same jig, with one part filed by hand and instantly noticeable among them"
  caption="Which style matters less than everyone using it, which is why a formatter decides."
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      starterCode: `// Before refactor: everything in one file, no clear shape.
...
<CodeBlock
  ...
  files={[
    {
      filename: "index.js",
      starterCode: `const { totalWithTaxForType } = require("./reporting");
