...
<Figure
  slug="js-arrays-creating-arrays-inline-cutout"
  alt="An empty tray being filled left to right with blocks, the tray's slots appearing only as each block is set down"
  caption="An array grows as you add to it; the slots appear when they are filled."
/>
...
<Figure
  slug="js-arrays-length-inline-cutout"
  alt="A filled rail of blocks with a measuring rod laid along it, the rod's end stopping exactly where the last block ends"
  caption="An array knows its own length, and it is one more than the last index."
/>
...
We met loops in the *Loops and Iteration* chapter. Recall:
...
<Figure
  slug="js-arrays-inline-cutout"
  alt="A long shelf of six equal slots holding blocks in a fixed order, a hand reaching straight past the first three to lift the fourth out"
  caption="Position is the whole idea: index 3 goes straight to the fourth slot."
/>
