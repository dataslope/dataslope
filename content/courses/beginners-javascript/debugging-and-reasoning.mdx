...
<Figure
  slug="js-debugging-and-reasoning-scientific-method-approach-inline-cutout"
  alt="A written guess pinned above a bench, one part changed, and the result checked against the guess"
  caption="Guess first, change one thing, then check the result against the guess."
/>
...
<Figure
  ...
  caption="Find the one link that is broken rather than rebuilding the whole chain."
/>
...
When something throws, JavaScript prints a **stack trace**, a snapshot
...
<Figure
  slug="js-debugging-and-reasoning-debugging-really-inline-cutout"
  alt="A figure holding a lamp to a machine that has stopped, the lamp lighting one jammed part rather than the whole machine"
  caption="Saying the code out loud is what makes the wrong assumption audible."
/>
...
2. **Pure when possible.** As you saw in the functional programming chapter, side
