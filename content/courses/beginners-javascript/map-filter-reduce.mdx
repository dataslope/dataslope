...
<Figure
  slug="js-map-filter-reduce-cutout"
  alt="Map, filter, and reduce, an isometric illustration"
/>
...
<Figure
  slug="js-map-filter-reduce-map-transform-every-inline-cutout"
  alt="A full row of blocks passing under one wide roller that repaints every block in the row in a single pass"
  caption="`map()` returns a new array of the same length, one result per element."
/>
...
<Figure
  slug="js-map-filter-reduce-filter-keep-only-inline-cutout"
  alt="A row of mixed blocks passing over a grate, the ones that fail a test dropping through and the rest carrying on unchanged"
  caption="`filter()` keeps the elements that pass the test and drops the rest."
/>
...
<Figure
  ...
  caption="Transform, keep, collapse: three shapes that cover most array work."
/>
...
and each one allocates a new array on the way through.

<Chart slug="chain-vs-single-pass" />
