...
<Figure
  slug="js-conditionals-cutout"
  alt="Conditionals and decisions, an isometric illustration"
/>
...
<Figure
  slug="js-conditionals-if-statement-inline-cutout"
  alt="A single hinged flap over a chute, open for blocks that clear a marked bar and shut for the rest"
  caption="A bare `if` has one path: the value either passes the test or carries on."
/>
...
<Figure
  slug="js-conditionals-if-else-inline-cutout"
  alt="A chute with a two-way flap, blocks that clear a marked bar going left and the rest going right, both paths ending in a tray"
  caption="A condition sends the value down one branch or the other, never both."
/>
...
<Figure
  slug="js-conditionals-inline-cutout"
  alt="A chute with a hinged flap, blocks that clear a marked height going one way and the rest going the other"
  caption="`switch` is the same decision written as a list of cases instead of a chain."
/>
...
The classic beginner exercise, adapted from a children's counting
game: print the numbers 1 to 30, but replace multiples of 3 with
`"Fizz"`, multiples of 5 with `"Buzz"`, and multiples of both with
`"FizzBuzz"`.

FizzBuzz carries a surprising amount of programming-culture
baggage. In 2007, the developer Imran Ghory suggested it as a
screening question for programming interviews, and Jeff Atwood's
follow-up post "Why Can't Programmers.. Program?" made it famous by
reporting something interviewers kept rediscovering: a remarkable
share of candidates with computer science degrees struggled to
write it on a whiteboard. The exercise tests nothing exotic, just
whether conditionals, ordering, and the remainder operator are
truly at your fingertips rather than vaguely familiar. Which makes
it the perfect end-of-page check for you, right now:
