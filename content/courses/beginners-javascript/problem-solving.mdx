...
<Figure
  slug="js-problem-solving-cutout"
  alt="Problem solving with code, an isometric illustration"
/>
...
<Figure
  slug="js-problem-solving-five-steps-inline-cutout"
  alt="Five footplates crossing a bench from a start pad to a finish pad, each laid down after the one before"
  caption="Understand, plan, write, test, refine: the steps in the order they help."
/>
...
<Figure
  slug="js-problem-solving-worked-example-shopping-cart-inline-cutout"
  alt="A row of price tags fed one at a time onto a running tally strip, then a single stamp pressing that total down to a smaller figure"
  caption="Decompose first, then loop: the total is built one line at a time, then discounted."
/>
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.js",
      starterCode: `function finalTotal(items, code) {
  let subtotal = 0;
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.js",
      starterCode: `function subtotal(items) {
  let s = 0;
...
function discountFor(code, subtotalAmount) {
  if (code === "SAVE10") return subtotalAmount * 0.1;
...
<Figure
  slug="js-problem-solving-refine-matters-inline-cutout"
  alt="A rough part filed down a second time until its edges come clean, the two versions set side by side"
  caption="Writing it again more clearly is how you come to write it clearly the first time."
/>
...
<Figure
  slug="js-problem-solving-stuck-inline-cutout"
  alt="A figure stepping back from a bench with their hands at their sides, the half-built work left exactly as it was"
  caption="Programming is mostly thinking, so when the typing stops working, stop typing."
/>
...
- Any other code (including empty string) → no discount.`}
  ...
  files={[
    {
      filename: "index.js",
      starterCode: `const { finalTotal } = require("./cart");
...
`,
      solutionCode: `function subtotal(items) {
  let s = 0;
  for (const item of items) s += item.price * item.quantity;
  return s;
}
