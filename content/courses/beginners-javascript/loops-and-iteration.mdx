...
<Figure
  slug="js-loops-and-iteration-cutout"
  alt="Loops and iteration, an isometric illustration"
/>
...
<Figure
  slug="js-loops-and-iteration-while-loop-inline-cutout"
  alt="A belt that keeps circling past a gauge, the gate at its end staying shut while the gauge still reads high"
  caption="A `while` loop keeps going as long as its test still reads true."
/>
...
<Figure
  slug="js-loops-and-iteration-loop-inline-cutout"
  alt="A closed belt with a counter dial beside it, the dial clicking once per pass and a gate lifting when it reaches its last notch"
  caption="Start with a blank result, walk the list, update it: the commonest loop there is."
/>
...
<Figure
  slug="js-loops-and-iteration-inline-cutout"
  alt="A closed circular track with one cart running around it, a counter wheel clicking up at each pass, and a gate at the right the cart finally leaves through"
  caption="The inner loop runs all the way through for each pass of the outer one."
/>
