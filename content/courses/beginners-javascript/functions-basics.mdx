...
<Figure
  slug="js-functions-basics-function-inline-cutout"
  alt="A plain box on a bench with one hopper above and one chute below, its workings hidden behind a closed panel"
  caption="A function hides its workings; the caller sees only what goes in and out."
/>
...
<Figure
  slug="js-functions-basics-inline-cutout"
  alt="A boxed machine on a bench with a funnel on top and a spout at the side, a block dropped into the funnel and a different block leaving the spout"
  caption="One call: arguments in, body runs, a value comes back out."
/>
...
Three tables, three copies of the same arithmetic. Change the tip rate
and you have to find every copy. Now watch the same work done once:
...
<Figure
  ...
  alt="A machine being bolted together on a bench but not yet switched on, its hopper still empty"
  caption="Declaring a function builds it; nothing runs until you call it."
/>
