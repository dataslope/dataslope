...
<Figure
  slug="python-basics-data-types-cutout"
  alt="An assortment of objects grouped by kind: a stack of coins, a ribbon, a toggle switch, and a bundle of boxes"
/>
...
<Figure
  slug="python-data-types-real-world-context-inline-cutout"
  alt="A gate that checks a block as it passes beside another that checks only when the block is used"
  caption="Python checks a value's type when you use it, not when you name it."
/>
...
<Figure
  slug="python-data-types-type-hierarchy-inline-cutout"
  alt="A branching frame of sockets, broad at the top and narrowing into specific shapes below"
  caption="Types form a hierarchy: object at the top, increasingly specific shapes below."
/>
...
<Figure
  slug="python-data-types-inline-cutout"
  alt="Five blocks of different kinds resting in shaped recesses, none able to sit in another's place"
  caption="Each type has its own shape, so moving between them means an explicit conversion."
/>

Converting between types is explicit in Python. The type name is the
conversion function:
...
Going the other way, `str()` gives the decimal form and the base
functions give the rest:
...
Watch what `int()` does to a float: it truncates toward zero rather
than rounding, so `3.9` becomes `3` and `-3.9` becomes `-3`:
...
The container types convert into each other the same way, which is how
you deduplicate a list in one step:
...
The method returned a new string and `s` never moved. To keep the
result you have to point the name at it:
...
A mutable object behaves the opposite way. `append` changes the list
itself, and every name bound to that list sees the change:
...
`None` also turns up where you did not put it. A function that never
returns anything returns `None`, which is why forgetting a `return` is
such a quiet bug:
