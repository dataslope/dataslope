...
<Figure
  slug="python-basics-loops-cutout"
  alt="The Dataslope marmot riding a cart around a looping isometric roller-coaster track"
/>
...
<Figure
  slug="python-loops-real-world-impact-inline-cutout"
  alt="A belt circling past a station with a counter clicking each pass"
  caption="Anything you would otherwise do by hand a thousand times is one loop with a counter."
/>
...
<Figure
  slug="python-loops-loop-inline-cutout"
  alt="A belt carrying blocks past a station one at a time with a gate at its end"
  caption="A `for` loop takes one item at a time and stops when the sequence runs out."
/>
...
Anything iterable works, including a dictionary, which yields its keys:
...
Give `range` two arguments and the first is where it starts. The stop
value is still excluded, which is why this prints 1 through 5:
...
A negative step counts down, which is the usual way to walk backwards:
...
`enumerate` counts from 0 by default. Pass `start=1` when the numbers
...
`zip` is not limited to two sequences, and it stops as soon as the
...
`continue` is the gentler sibling of `break`. It abandons the current
iteration and moves to the next one, leaving the loop running:
...
The two combine happily. Read the body top to bottom and each guard
fires in the order it is written:
...
<Figure
  slug="python-loops-inline-cutout"
  alt="A closed belt carrying blocks past a station one at a time, with a small counter clicking and a gate that opens on the last"
  caption="Every loop needs something that eventually opens the gate and lets it end."
/>

That reads oddly until you see what it is for. `for/else` expresses
"search, and do something if the search failed" without needing a flag
variable:
...
`break` and `continue` work in a `while` loop exactly as they do in a
`for` loop:
