...
<Figure
  ...
  alt="A drawer being pulled open with a small clasp that must be closed again after"
  caption="Every file you open holds a resource until it is closed again."
/>
...
<Figure
  ...
  caption="Reading a file is open, read, close, and `with` handles the closing for you."
/>
...
<Figure
  slug="python-files-file-i-o-inline-cutout"
  alt="A drawer opened, a strip of tape drawn out, and the drawer closed again"
  caption="Writing follows the same open and close cycle, with the tape running the other way."
/>
...
The default encoding is platform-dependent: UTF-8 on macOS/Linux, often CP1252 on Windows. **Explicitly pass `encoding="utf-8"`** to avoid cross-platform bugs, especially when your code runs in continuous integration or Docker containers.
</Callout>
