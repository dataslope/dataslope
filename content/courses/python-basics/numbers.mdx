...
<Figure
  slug="python-basics-numbers-cutout"
  alt="A wooden abacus beside a number line arrow, with counting beads sliding along a rod"
/>
...
<Figure
  slug="python-numbers-integers-inline-cutout"
  alt="A rule with evenly spaced marks and a slider resting exactly on one"
  caption="An int lands on an exact mark, however large the number gets."
/>
...
Long literals get hard to read at a glance, so Python lets you group
the digits with underscores. They are ignored entirely by the parser:
...
<Figure
  slug="python-numbers-floats-inline-cutout"
  alt="A rule whose marks are evenly spaced at one end and crowded at the other"
  caption="A float's precision is not uniform: the marks crowd together as values grow."
/>
...
For exact decimal arithmetic (money, above all), use `decimal.Decimal`:
...
It is worth seeing the problem `Decimal` exists to solve. Binary floats
cannot represent most decimal fractions exactly, and the error compounds:
...
<Figure
  slug="python-numbers-inline-cutout"
  alt="A rule with a slider moving in exact steps, and beside it a rule whose marks crowd together toward one end"
  caption="Exact steps or crowded marks is the difference between an int and a float."
/>

`Fraction` keeps a number as an exact ratio, so a third stays a third
rather than becoming 0.3333333333333333.

Python also has complex numbers built in, with `j` for the imaginary
unit:
...
They arithmetic the way you would hope:
...
Floor division rounds *down*, not toward zero, which is why `-7 // 2`
is `-4`. The remainder operator and `**` complete the set:
...
The trigonometric functions all take radians:
...
And the rounding family, which is more precise about what it does than
the built-in `round()`:
...
Seeding the generator with the same number gives back the same
sequence, which is what makes a random simulation reproducible: rerun it
and you can compare like with like. `sample` is the one to reach for
when you need several draws without repeats:
