...
<Figure
  slug="python-basics-tuples-cutout"
  alt="Two padlocked rings linked together as a fixed pair, with a sealed wax stamp beside them"
/>
...
<Figure
  slug="python-tuples-creating-tuples-inline-cutout"
  alt="A strip of three blocks sealed under a clamped bar"
  caption="Writing the commas is what makes a tuple, and it is fixed from then on."
/>
...
One or more items, separated by commas. The parentheses are usually
optional, but they make the intent obvious:
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `from_list = tuple([1, 2, 3])
...
Slicing works too, and returns a new tuple:
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `first, *middle, last = [1, 2, 3, 4, 5]
print("first:", first)
print("middle:", middle)
print("last:", last)
`,
...
The starred name does not have to be in the middle. Head-and-tail is
...
That function looks like it returns two values. It returns one tuple,
which unpacking then splits at the call site. Every multi-value return
in Python works this way:
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `def divmod_manual(a, b):
    return (a // b, a % b)
...
<Figure
  slug="python-tuples-inline-cutout"
  alt="A sealed strip holding three blocks that cannot be prised out, beside an open rail whose blocks lift freely"
  caption="A tuple is sealed once built, while a list stays open to changes."
/>

Positional tuples get hard to read once there are more than two or
three fields. `namedtuple` gives the positions names without giving up
the tuple behaviour:
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `from typing import NamedTuple
...
<ChallengeCard
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `def split_minmax(values):
    # your code here
    return (0, 0)
`,
...
<ChallengeCard
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `def stats_of(record):
    # your code here
    return ("", 0.0)
`,
...
- A \`ValueError\`
  > Starred unpacking accepts any number of leftover items, so no error is raised here.
- [o] \`[2, 3, 4]\`
