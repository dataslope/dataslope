...
<Figure
  slug="python-basics-next-steps-cutout"
  alt="The Dataslope marmot at a signpost on a hill, three arrows pointing toward distant platforms"
/>

<Callout type="success" title="You are ready">
After this course, you can read most Python codebases, contribute to open-source projects, build command-line tools, automate tasks, write tests, and start learning frameworks. The fundamentals you've learned here will serve you for years. Now it's time to specialize.
</Callout>
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `from typing import Iterable
...
<CodeBlock
  ...
  files={[
    {
      filename: "main.py",
      starterCode: `# Example: pandas DataFrame
import pandas as pd
...
If you work with data, analysis, visualization, modeling, extract-transform-load pipelines, Python is the lingua franca. pandas, NumPy, and scikit-learn are the most-used libraries on PyPI. Learning them opens doors to data science, machine learning engineering, and analytics roles.
</Callout>
...
5. **A library you publish to PyPI**: Write tests, set up `pyproject.toml`, push to GitHub with a continuous integration workflow (GitHub Actions), and publish with `twine` or `uv publish`.
