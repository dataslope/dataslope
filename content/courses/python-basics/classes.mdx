...
<Figure
  slug="python-classes-inline-cutout"
  alt="A template pinned above a bench and three objects stamped from it, each carrying the same handles but a different filling"
  caption="A class is the template; each instance is stamped from it with its own contents."
/>
...
<Figure
  slug="python-classes-minimal-class-inline-cutout"
  alt="A bare template pinned above a bench with nothing yet stamped from it"
  caption="An empty class body still defines a valid type, ready to stamp instances from."
/>
...
<Figure
  slug="python-classes-init-method-inline-cutout"
  alt="A crate having each compartment filled at one station the moment it leaves the press"
  caption="Every new instance stops at `__init__`, the station where its fields get filled in."
/>
...
| `__repr__` | Unambiguous string | `repr(obj)`, interactive prompt |
