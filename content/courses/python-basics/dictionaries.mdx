...
<Figure
  slug="python-basics-dictionaries-cutout"
  alt="A wall of small mailbox doors, each with a key in its lock, a few ajar to reveal a rolled scroll inside"
/>
...
<Figure
  slug="python-dictionaries-inline-cutout"
  alt="A wall of small labelled drawers, one key held up to the wall and exactly one drawer sliding open in response"
  caption="A key opens one drawer directly, with no searching along the wall."
/>
...
<Figure
  slug="python-dictionaries-creating-dicts-inline-cutout"
  alt="An empty wall of small drawers, each gaining a key and a blank tag as it is filled"
  caption="A dict starts empty and gains one labelled slot per key you add."
/>
...
More usefully, write the pairs straight into the braces, each key
...
`dict()` also accepts a sequence of key-value pairs, which is what you
reach for when the keys are computed rather than typed:
...
`get()` returned `None` for the missing key. Give it a second argument
and it returns that instead, which is usually what you actually want:
...
The same syntax updates an existing key. Assignment does not care
whether the key was already there, which is worth remembering the first
time you overwrite something by accident:
...
`del` discards the value. `pop()` removes the key and hands the value
back, so you can use it on the way out:
...
Hashing is what makes a lookup independent of the dictionary's size, and
it is why keys have to be immutable: a key that changes after insertion
hashes to a bucket nobody will look in.
...
A list cannot be a key, because a list can change and a dictionary
needs its keys to stay put. Python says so rather than letting you find
out later:
...
`defaultdict` solves the neighbouring problem: it creates the missing
value for you the first time you touch a key, so grouping items needs no
...
<Figure
  slug="python-dictionaries-dictionaries-work-inline-cutout"
  alt="A key dropped into a mixing machine whose chute swings to one of a wall of drawers"
  caption="Hashing turns a key into the address of exactly one drawer."
/>

Dictionaries are also the shape JSON arrives in, which is most of why
you will meet them constantly. `json.dumps()` turns one into text:
...
And `json.loads()` turns text back into a dictionary:
