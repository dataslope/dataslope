...
<Figure
  slug="python-basics-control-flow-cutout"
  alt="The Dataslope marmot at a railway switch lever, choosing between two diverging tracks under a signal light"
/>
...
<Figure
  slug="python-control-flow-real-world-impact-inline-cutout"
  alt="A rail with points and a loop, carts routed differently at each pass"
  caption="Control flow is points and loops: each value gets routed by the test it meets."
/>
...
<Figure
  slug="python-control-flow-just-if-inline-cutout"
  alt="A single hinged flap over a chute with one path beneath it"
  caption="A bare `if` is one flap: the value takes the branch or carries straight on."
/>
...
`elif` chains as many tests as you like, and `else` catches everything
that reached the bottom without matching:
...
Truthiness covers most cases, but not all of them. When `None` and an
empty value mean different things, test for `None` explicitly:
...
Because it is an expression rather than a statement, it can sit inside
another expression, such as an f-string:
...
Chained comparisons read exactly like the mathematics they came from,
which makes range checks unusually pleasant in Python:
...
<Figure
  slug="python-control-flow-inline-cutout"
  alt="A track with points and a loop, a cart taking the branch while the loop curls back behind it"
  caption="Branching sends a value one way; looping curls it back for another pass."
/>

Structural pattern matching, added in Python 3.10, goes further than a
chain of `elif`s: it matches on the *shape* of a value and pulls pieces
out of it in the same step.
