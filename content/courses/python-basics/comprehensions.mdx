...
Add an `if` at the end and the comprehension becomes a filter as well
as a transform. Nothing that fails the test reaches the output list:
...
The expression at the front does not have to be arithmetic. Anything
you can write about `w` can go there:
...
The same shape works on any iterable, not just ranges:
...
Two `for` clauses nest, outer first, exactly as they would if you wrote
them as indented loops. That produces every combination:
...
And an `if` after both of them filters the combined result:
...
Dict comprehensions take the same `if` filter, and iterating
`.items()` gives you the key and value to work with:
...
Because a set discards duplicates, a set comprehension is also the
shortest deduplication in the language:
...
That built a list of a million integers just to add them up and throw
them away. Drop the brackets and you get a **generator expression**,
which produces one value at a time and never holds the whole sequence:
