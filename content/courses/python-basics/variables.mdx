...
<Figure
  slug="python-basics-variables-cutout"
  alt="An isometric shelf of labelled storage boxes and jars, each holding a different geometric object"
/>
...
<Figure
  slug="python-variables-inline-cutout"
  alt="A labelled tag tied to a block, then untied and tied to a different block, the first block left where it was"
  caption="A variable is a tag you can move, not a box the value lives in."
/>
...
<Figure
  slug="python-variables-dynamic-typing-freedom-inline-cutout"
  alt="A tag moving freely between blocks of different shapes, with one slot where the wrong shape jams"
  caption="A name takes any type, so a mismatch only shows up where the value is used."
/>

Names in Python carry no type of their own. The **value** has the type,
and you can ask any value what it is:
...
Which means rebinding a name to a different kind of value is perfectly
legal:
...
It is legal for containers too. Legal is not the same as wise: code
where one name holds three different kinds of thing is hard to follow,
and discouraging exactly that is what the type annotations below are
for.
...
Assignment also comes in a multiple form, which is how a function that
returns several values gets read apart:
...
And the trick that unpacking makes possible, familiar to anyone who has
written a sort by hand in another language:
