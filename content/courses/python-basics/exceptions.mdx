...
<Figure
  slug="python-exceptions-inline-cutout"
  alt="A chute with a side door, a cracked block being routed through the door into a padded tray while the rest continue down"
  caption="An exception routes the failing case aside so the rest keeps flowing."
/>
...
<Figure
  slug="python-exceptions-exception-propagation-up-inline-cutout"
  alt="A block falling down a column of trays, each tray refusing it, until one catches"
  caption="An exception falls up the call stack until some `except` clause catches it."
/>
...
## Asking forgiveness vs looking before you leap
...
<Figure
  slug="python-exceptions-happens-exceptions-are-inline-cutout"
  alt="A block falling past every basket on a bench and off the end onto the floor"
  caption="With no matching `except` anywhere, the exception falls through and stops the program."
/>
