---
title: Scope
description: How Python looks up names, searching outward from local scope to the built-ins, and the global / nonlocal keywords
---
...
<Figure
  slug="python-basics-scope-cutout"
  alt="Nested open-topped glass boxes with a single glowing token resting inside the innermost one"
/>
...
<Figure
  slug="python-scope-inline-cutout"
  alt="Four frames standing nested one inside another with a block resting in each, and a hand reaching out from the innermost frame through each one in turn until it finds the block it wants"
  caption="Name lookup works outward: local, enclosing, global, then built-in."
/>
...
<Figure
  slug="python-scope-scope-matters-real-inline-cutout"
  alt="A lamp inside a walled booth lighting only that booth, the corridor outside dark"
  caption="A name defined inside a function lights up there and nowhere else."
/>
