...
<Figure
  slug="python-sets-inline-cutout"
  alt="A bin that takes each distinct block once, an identical second block bouncing off its rim onto the bench"
  caption="A set keeps one of each value and silently drops the duplicates."
/>
...
`set()` accepts any iterable, and a string is a sequence of characters:
...
Alongside the four operators there are the containment tests, which
ask about the relationship between two sets rather than combining them:
...
<Figure
  slug="python-sets-sets-work-inline-cutout"
  alt="A bin with a narrow mouth accepting a block once and rejecting an identical second"
  caption="The set checks membership at the rim, in constant time, however full it is."
/>
...
All of that is convenient. The reason sets matter in practice is
speed: membership testing is roughly constant time, where a list has to
walk itself. Over a million items the difference is not subtle:
...
Set members must be hashable for the same reason dictionary keys must
be, so a list cannot go in one:
...
Which is also what lets you build a set of sets:
