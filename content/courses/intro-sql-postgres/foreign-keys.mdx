...
<Figure
  slug="sql-foreign-keys-foreign-key-pointer-inline-cutout"
  alt="A row carrying an arrow that reaches across to a row in another board bearing a punched tab"
  caption="A foreign key holds another row's primary key, which is what links them."
/>
...
<Figure
  slug="sql-foreign-keys-inline-cutout"
  alt="A tag in one board's row carrying the same notch as a row in another board, a cord joining the two"
  caption="The database enforces the link, so a reference cannot point at nothing."
/>
