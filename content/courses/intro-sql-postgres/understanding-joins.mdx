...
<Figure
  slug="sql-understanding-joins-question-join-answers-inline-cutout"
  alt="Two separate boards on a bench with a single question card held between them"
  caption="A join answers a question that needs both tables at once."
/>
...
<Figure
  slug="sql-understanding-joins-inline-cutout"
  alt="Two overlapping grids on a light table, the overlapping region lit and the outer regions dimmed, with a small selector dial beside them"
  caption="Aliases keep a join readable once three or four tables are involved."
/>
...
Matching is also how joins surprise people. If a key repeats on both
