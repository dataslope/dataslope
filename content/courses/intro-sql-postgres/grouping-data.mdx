...
<Figure
  slug="sql-grouping-data-cutout"
  alt="Grouping data with GROUP BY, an isometric illustration"
/>
...
<Figure
  slug="sql-grouping-data-idea-split-then-inline-cutout"
  alt="A scatter of chips raked into three trays, each tray then pressed into a single block"
  caption="Group the rows, then compute the aggregate once per group."
/>
...
INSERT INTO sales (region, year, amount) VALUES
  ('West', 2025, 100), ('West', 2025, 50), ('West', 2025, 75),
  ('West', 2025, 120), ('West', 2026, 200), ('West', 2026, 90),
  ('West', 2026, 145), ('West', 2026, 60),
  ('East', 2025, 80), ('East', 2025, 110), ('East', 2025, 45),
  ('East', 2025, 95), ('East', 2026, 120), ('East', 2026, 60),
  ('East', 2026, 175), ('East', 2026, 85),
  ('North', 2025, 65), ('North', 2025, 130), ('North', 2025, 40),
  ('North', 2025, 88), ('North', 2026, 155), ('North', 2026, 70),
  ('North', 2026, 110), ('North', 2026, 95);`}
  starterCode={`SELECT
  region,
  year,
  SUM(amount) AS total
...
<Figure
  slug="sql-grouping-data-inline-cutout"
  alt="A scatter of mixed blocks being raked into three neat piles, and one tall summary block standing beside each pile"
  caption="Group by two columns and each unique pair becomes its own group."
/>
...
INSERT INTO employees (name, dept, salary, is_intern) VALUES
  ...
  ('Cara','Engineering',30000,true),('Frank','Engineering',110000,false),
  ('Grace','Engineering',132000,false),('Hana','Engineering',88000,false),
  ('Rosa','Engineering',105000,false),('Sam','Engineering',28000,true),
  ('Dave','Sales',75000,false),('Eve','Sales',72000,false),
  ('Ivan','Sales',76000,false),('Jade','Sales',91000,false),
  ('Kai','Sales',26000,true),('Tara','Sales',84000,false),
  ('Nina','Support',58000,false),('Omar','Support',61000,false),
  ('Pia','Support',24000,true),('Quinn','Support',63000,false),
  ('Finn','Marketing',68000,false),('Lena','Marketing',74000,false),
  ('Mark','Marketing',62000,false),('Uri','Finance',88000,false),
  ('Vera','Finance',92000,false),('Wes','Legal',115000,false);`}
  starterCode={`SELECT
  dept,
  ROUND(AVG(salary)) AS avg_salary
FROM
...
<SqlChallengeCard
  dialect="postgres"
  title="Big-spending customers"
  instructions={`The \`orders\` table has \`customer\` and \`amount\` columns. For each customer who has spent **more than 100** in total, return their \`customer\` name and total spend as \`total\`, sorted from the highest total to the lowest.`}
...
INSERT INTO orders (customer, amount) VALUES
  ('Ada', 120.00), ('Linus', 80.00), ('Ada', 30.00),
  ('Grace', 200.00), ('Linus', 15.00), ('Ada', 25.00),
  ('Mae', 42.00), ('Rosa', 18.50), ('Omar', 95.00), ('Priya', 12.75),
  ('Grace', 60.00), ('Mae', 33.25), ('Rosa', 27.00), ('Omar', 44.00),
  ('Priya', 8.90), ('Tariq', 76.40), ('Uma', 21.60), ('Tariq', 55.10),
  ('Wen', 14.30), ('Uma', 9.45), ('Wen', 6.20), ('Mae', 19.80),
  ('Rosa', 31.15), ('Omar', 7.65);`}
  starterCode={`-- Total per customer, then keep only the big spenders:
SELECT
  customer,
  SUM(amount) AS total
FROM
...
ORDER BY
  ...
  tests={[
    {
      id: "columns",
      name: "Returns customer and total",
      description: "The group key and its summed amount.",
      expectedColumns: ["customer", "total"],
    },
    {
      id: "row_count",
      name: "Returns four customers",
      description: "Only four of the ten customers clear the 100 threshold.",
      expectedRowCount: 4,
    },
    {
      id: "matches",
      name: "Matches the reference solution",
      description: "Grace (260.00) at the top, down to Tariq (131.50).",
...
Grouping is the point where SQL starts doing analysis rather than
lookup. The next page adds the missing piece: filtering the groups
themselves, after they have been summarised.
