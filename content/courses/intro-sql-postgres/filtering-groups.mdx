...
<Figure
  slug="sql-filtering-groups-where-can-t-inline-cutout"
  alt="A grate placed before a press stopping single chips, and the same grate placed after it stopping whole summary blocks"
  caption="`WHERE` runs before the groups exist; `HAVING` runs after they do."
/>
...
Four departments survive: Engineering (7 people), Sales (6),
Marketing (4) and Support (4). Finance (2) and Legal (1) are filtered
out *as groups*, after being counted. Notice `HAVING` uses an
aggregate, `COUNT(*)`, which is exactly what `WHERE` could not do.
...
<Figure
  slug="sql-filtering-groups-inline-cutout"
  alt="Three summary blocks standing on their trays, one tray removed entirely because its block is too short"
  caption="`HAVING` removes whole groups, judged by their own summary values."
/>
...
INSERT INTO orders (customer, amount) VALUES
  ...
  ('Mae', 27.50), ('Grace', 63.00), ('Rosa', 14.25), ('Omar', 88.00),
  ('Ada', 33.75), ('Linus', 19.00), ('Mae', 45.60), ('Priya', 71.20),
  ('Grace', 22.40), ('Rosa', 56.80), ('Tariq', 9.99), ('Ada', 38.15),
  ('Mae', 61.30), ('Linus', 24.70), ('Linus', 47.85), ('Uma', 30.00),
  ('Ada', 52.40), ('Grace', 16.60);`}
  starterCode={`SELECT
  customer,
...
ORDER BY
  ...
  tests={[
    ...
    {
      id: "row_count",
      name: "Returns five customers",
      description: "Four of the nine customers ordered exactly once and are filtered out.",
      expectedRowCount: 5,
    },
    {
      id: "matches",
      name: "Matches the reference solution",
      description: "Ada (6 orders) down to Rosa (2).",
...
`WHERE` before, `HAVING` after. If you can say which side of the
grouping a condition belongs on, you understand the execution order
well enough for almost anything you will meet in practice.
