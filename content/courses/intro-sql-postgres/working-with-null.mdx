...
<Figure
  slug="sql-working-with-null-null-escapes-into-real-inline-cutout"
  alt="A parcel carrying a blank tag waved through every gate on a line while tagged parcels are stopped and checked"
  caption="A missing value that every check waves through is how `NULL` reaches production."
/>
...
<Figure
  slug="sql-working-with-null-filling-nulls-coalesce-inline-cutout"
  alt="A chute where an empty slot is topped up from a standby hopper before the run moves on"
  caption="`COALESCE()` reads as use this, or that instead when the first one is missing."
/>
...
<Figure
  slug="sql-working-with-null-allow-null-at-all-inline-cutout"
  alt="One slot on a rack left honestly empty beside a slot filled with a stand-in token that reads exactly like the real thing"
  caption="Unknown is an honest state, and the fake stand-in value is the thing that actually lies."
/>
...
INSERT INTO customers (name, email) VALUES
  ('Ada', 'ada@example.com'), ('Grace', NULL),
  ('Linus', 'linus@example.com'), ('Mae', NULL),
  ('Rosa', 'rosa@example.com'), ('Ivan', NULL),
  ('Nadia', 'nadia@example.com'), ('Omar', 'omar@example.com'),
  ('Priya', NULL), ('Quinn', 'quinn@example.com'),
  ('Ruth', 'ruth@example.com'), ('Sam', NULL),
  ('Tariq', 'tariq@example.com'), ('Uma', 'uma@example.com'),
  ('Viktor', NULL), ('Wen', 'wen@example.com'),
  ('Xenia', 'xenia@example.com'), ('Yusuf', NULL),
  ('Zara', 'zara@example.com'), ('Bruno', 'bruno@example.com'),
  ('Chidi', NULL), ('Dara', 'dara@example.com'),
  ('Elif', 'elif@example.com'), ('Farid', 'farid@example.com');`}
  starterCode={`SELECT
  name,
...
ORDER BY
  ...
  tests={[
    ...
    },
    {
      ...
      description: "The eight customers with no email show 'no email'; the rest show their addresses.",
...
`NULL` is the part of SQL that most often surprises people months into
using it, so it is worth over-learning now. Whenever a result has fewer
rows than you expected, ask what happened to the unknown values.
