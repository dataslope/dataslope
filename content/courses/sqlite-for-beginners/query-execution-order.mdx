...
<Figure
  slug="lite-execution-order-cutout"
  alt="A numbered sequence of six stations on a curved track, the written order shown as a separate straight ribbon above it"
/>
...
<Figure
  slug="lite-query-execution-order-inline-cutout"
  alt="Six stations along a bench with the parcel visibly starting at the fourth station, not the first"
  caption="SQLite runs `FROM` first and `SELECT` near the end, whatever the writing order."
/>
...
<Figure
  slug="lite-query-execution-order-this-explains-real-inline-cutout"
  alt="A parcel stopped at the fourth station of six with a small warning flag raised there"
  caption="The execution order predicts exactly which alias errors you will hit."
/>
...
<Figure
  slug="lite-query-execution-order-tracing-full-query-inline-cutout"
  alt="A table passing through a line of stations, each one reshaping it and handing it on smaller than before"
  caption="Each clause reshapes the table and hands it on, in an order that is not the written one."
/>
...
*Hint: compare where each clause sits in the execution order relative to \`SELECT\`.*
...
*Hint: think about when the group's \`SUM()\` first exists in the execution order.*
