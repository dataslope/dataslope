...
<Figure
  slug="lite-filtering-groups-where-filters-rows-inline-cutout"
  alt="A grate placed before the press so single chips are stopped while whole trays are still forming"
  caption="`WHERE` runs before the groups exist, so it cannot test a group total."
/>
...
<Figure
  ...
  caption="`HAVING` runs after grouping and can drop a whole group by its summary."
/>
...
INSERT INTO employees (name, dept) VALUES
  ...
  ('Gina', 'Engineering'), ('Hugo', 'Engineering'), ('Mona', 'Engineering'),
  ('Theo', 'Engineering'), ('Yara', 'Engineering'),
  ('Dave', 'Sales'), ('Eve', 'Sales'), ('Iris', 'Sales'),
  ('Nils', 'Sales'), ('Rui', 'Sales'), ('Vik', 'Sales'),
  ('Jonas', 'Support'), ('Kira', 'Support'), ('Otto', 'Support'),
  ('Sara', 'Support'), ('Wanda', 'Support'),
  ('Finn', 'Marketing'), ('Leo', 'Marketing'),
  ('Pia', 'Finance'), ('Ulla', 'Finance'),
  ('Quinn', 'Legal');`}
  starterCode={`SELECT
  dept,
  COUNT(*) AS headcount
...
Three departments survive. Engineering has 8 employees, Sales has 6 and
Support has 5, so all three clear the bar. Marketing and Finance have 2
each and Legal has 1, so those groups are filtered out after being
counted. Note the order of events: every department was grouped and
counted first, and only then did `HAVING` throw three of the answers
away.

```mermaid
flowchart TB
    Groups["Department groups"] --> Eng["Engineering<br/><code>COUNT = 8</code>"]
    Groups --> Sales["Sales<br/><code>COUNT = 6</code>"]
    Groups --> Sup["Support<br/><code>COUNT = 5</code>"]
    Groups --> Mkt["Marketing<br/><code>COUNT = 2</code>"]
    Groups --> Leg["Legal<br/><code>COUNT = 1</code>"]
...
INSERT INTO sales (product, amount, channel) VALUES
  ('Notebook', 12.00, 'Online'), ('Notebook', 18.00, 'Store'),
  ('Notebook', 9.00, 'Online'), ('Backpack', 45.00, 'Store'),
  ('Backpack', 20.00, 'Online'), ('Pencil', 3.00, 'Online'),
  ('Notebook', 15.75, 'Online'), ('Notebook', 11.40, 'Store'),
  ('Backpack', 38.50, 'Online'), ('Backpack', 52.00, 'Store'),
  ('Mug', 7.25, 'Online'), ('Mug', 6.80, 'Store'),
  ('Pencil', 4.10, 'Store'), ('Pencil', 2.50, 'Online'),
  ('Eraser', 1.20, 'Online'), ('Eraser', 0.95, 'Store'),
  ('Notebook', 13.60, 'Online'), ('Mug', 8.90, 'Online'),
  ('Backpack', 41.25, 'Online'), ('Pencil', 3.75, 'Online'),
  ('Mug', 5.50, 'Store'), ('Eraser', 1.45, 'Online'),
  ('Notebook', 16.20, 'Store'), ('Backpack', 47.80, 'Store');`}
  starterCode={`SELECT
  product,
