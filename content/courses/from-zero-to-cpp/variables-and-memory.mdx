...
<Figure
  slug="cpp-variables-and-memory-cutout"
...
<Figure
  slug="cpp-variables-and-memory-declarations-vs-definitions-inline-cutout"
  alt="An empty locker with a blank plate, the same locker with a block placed in it, and the block being swapped for another"
  caption="Declaring names the slot, defining creates it, and assigning puts a value into it."
/>
...
<Figure
  slug="cpp-variables-and-memory-always-initialize-inline-cutout"
  alt="An empty locker opened and read straight away, handing out whatever the last user happened to leave in it"
  caption="An uninitialized local holds whatever was there before, and reading it is undefined behaviour."
/>
...
<Figure
  slug="cpp-variables-and-memory-types-promises-about-memory-inline-cutout"
  alt="A socket cut to one exact profile with a stamped promise fixed beside it, checked before anything is set into it"
  caption="A type fixes how many bytes, how the bits are read, and what may be done with them."
/>
...
<Figure
  slug="cpp-variables-and-memory-auto-let-compiler-infer-inline-cutout"
  alt="A slot left unmarked on a bench, its shape taken directly from the part being placed into it"
  caption="`auto` does not mean untyped; the compiler fills the type in from the initializer."
/>
...
<Figure
  slug="cpp-variables-and-memory-scope-where-variable-visible-inline-cutout"
  alt="A lamp lighting only the length of bench inside one bay, going dark the moment the bay ends"
  caption="A name is visible from its declaration to the end of its block, and nowhere else."
/>
...
<Figure
  slug="cpp-variables-and-memory-lifetime-variable-lives-dies-inline-cutout"
  alt="A locker that opens as a line is reached and empties itself the moment the closing mark of its block goes past"
  caption="A local variable is born at its declaration and dies at the closing brace of its block."
/>
...
<Figure
  slug="cpp-variables-and-memory-const-variables-don-t-inline-cutout"
  alt="Parts coming off the press already fixed, with only a few of them marked as adjustable"
  caption="Declare everything `const` and take it off only what genuinely has to change."
/>
...
<Figure
  slug="cpp-variables-and-memory-implicit-conversions-famous-footgun-inline-cutout"
  alt="A wide part quietly shaved to fit a narrower slot, the shaving falling away unnoticed"
  caption="A narrowing conversion happens quietly, and what falls off is not reported."
/>
