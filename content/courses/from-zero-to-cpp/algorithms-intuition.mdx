...
<Figure
  slug="cpp-algorithms-intuition-cutout"
  alt="Algorithms intuition, an isometric illustration"
/>
...
<Figure
  slug="cpp-algorithms-intuition-working-mental-model-inline-cutout"
  alt="Three ramps of different steepness rising from one start, a cart on each having travelled very different distances"
  caption="The steeper cost curve overtakes the gentler one as the input grows."
/>

Plotted for real, with the input size on one axis and the work on the
other, the families separate hard. Note where they cross: at small
inputs they are almost indistinguishable, which is exactly why a bad
choice survives testing and fails in production.

<Chart slug="big-o-growth" />

Those families have names, and the names are the vocabulary every
programmer uses to talk about cost:
...
| $O(n^2)$ | "n squared" | Doubling n quadruples the work. Fine for n=1000, painful for n=100,000. |
| $O(2^n)$ | "exponential" | Adding one input element doubles the time. Only tractable for tiny n. |
...
<Figure
  slug="cpp-algorithms-intuition-counting-code-inline-cutout"
  alt="A belt with a counter dial beside it clicking once for every block that passes"
  caption="Counting the operations is how you compare two approaches before running them."
/>
...
| Approach | Cost | Idea |
| --- | --- | --- |
| Try every pair | $O(n^2)$ | Nested loops |
...
Picking $O(n^2)$ when $O(n \log n)$ is one extra line of code is not
