...
<Figure
  slug="cpp-encapsulation-and-abstraction-invariants-truths-class-inline-cutout"
  alt="A sealed cabinet with a gauge on its face whose needle never leaves a marked band however the handles are turned"
  caption="An invariant is a promise the class keeps however its handles are used."
/>
...
<Figure
  slug="cpp-encapsulation-and-abstraction-abstraction-same-interface-different-inline-cutout"
  alt="One socket panel fitted to the front of three quite different machines, each doing the same job behind it its own way"
  caption="Keep the interface and the storage behind it can be replaced without callers noticing."
/>
...
<Figure
  slug="cpp-encapsulation-and-abstraction-interface-leaks-hyrum-s-inline-cutout"
  alt="A panel with one clear handle on its face and several bare edges, hands gripping the edges instead of the handle"
  caption="Expose anything and somebody depends on it, whether or not you meant it as an interface."
/>
