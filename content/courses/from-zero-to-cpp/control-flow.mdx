...
<Figure
  slug="cpp-control-flow-if-else-decisions-inline-cutout"
  alt="A chute with a two-way flap sending blocks down one of two paths, both ending in a tray"
  caption="A condition forks the path, and only one branch is taken."
/>
...
<Figure
  slug="cpp-control-flow-sequential-flow-inline-cutout"
  alt="A straight rail with four stations along it, a cart passing each in turn without branching"
  caption="Without a branch, statements run in order, one after another."
/>
...
<Figure
  ...
  caption="Branching goes one way; looping curls back for another pass."
/>
