...
Which means the cost of a move does not depend on the size of what is
being moved, and the cost of a copy is nothing but:

<Chart slug="copy-vs-move-cost" />
...
<Figure
  slug="cpp-move-semantics-intuition-inline-cutout"
  alt="A heavy crate being copied into a second identical crate on the left, and on the right the same crate simply having its handle passed to another pair of hands"
  caption="Copying duplicates the contents; moving just hands over the handle."
/>
...
<Figure
  slug="cpp-move-semantics-intuition-copy-vs-move-inline-cutout"
  alt="A heavy crate duplicated into a second identical crate on the left, and on the right the same crate simply having its handle passed across"
  caption="`std::move()` does not move anything; it only permits a move."
/>
