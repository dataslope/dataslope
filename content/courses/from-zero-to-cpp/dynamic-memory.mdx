...
<Figure
  slug="cpp-dynamic-memory-stack-vs-heap-inline-cutout"
  alt="A neat column of trays beside an open yard of crates, one cord running between them"
  caption="The stack cleans up after itself; the heap does exactly what you tell it."
/>

The two are not just different places, they have very different prices. A
...
Two things follow from that gap. Allocating in a tight loop is worth
avoiding, and allocating *once* outside the loop usually fixes it. And
a "slow" program is often not doing slow work at all, it is asking the
allocator for memory far more often than it needs to.
...
<Figure
  slug="cpp-dynamic-memory-inline-cutout"
  alt="A yard where a crate is requested from a counter, used at a bench, and returned to the counter when finished"
  caption="Every `new` needs its matching `delete`, on every path out of the function."
/>
