...
<Figure
  slug="java-the-type-system-inline-cutout"
  alt="A rack of shaped sockets with each block held above the only socket it fits, one block hovering with no socket to match it"
  caption="Every value has a declared type, and the compiler checks it before you run."
/>
...
The ceilings themselves are worth seeing once, because they explain
which conversions are safe. Each width has a largest value it can
hold, and the chart marks real quantities against those ceilings:

<Chart slug="integer-type-ranges" />

Read off where the marks fall and the rule of thumb writes itself. An
`int` comfortably counts the rows in a database table or the seconds
in a year, and just as comfortably fails to count the world's people
or the milliseconds in a century. Whenever a quantity might cross one
of those lines, reach for `long` and stop thinking about it.
...
<Figure
  slug="java-the-type-system-java-s-primitive-inline-cutout"
  alt="Eight small blocks of clearly different sizes resting in recesses cut exactly to each"
  caption="The eight primitives are values themselves, not references to anything."
/>
