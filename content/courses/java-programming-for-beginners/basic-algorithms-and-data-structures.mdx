...
<Figure
  slug="java-algorithms-data-structures-cutout"
  alt="Basic algorithms and data structures, an isometric illustration"
/>
...
<Figure
  slug="java-basic-algorithms-and-data-structures-linear-search-inline-cutout"
  alt="A hand walking a row of blocks one at a time from the left until it reaches the marked one"
  caption="Linear search checks every element until it finds the one you want."
/>
...
<Figure
  slug="java-basic-algorithms-and-data-structures-binary-search-inline-cutout"
  alt="A sorted row of blocks with a hand splitting it in half, discarding one half, and splitting again"
  caption="On sorted data, each comparison throws away half of what is left."
/>
...
Java standard library itself had carried this flaw for nine years: when `lo` and `hi`
...
slow, for that you'd use the built-in `Arrays.sort()`, which
is `O(n log n)`.
