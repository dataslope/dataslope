...
<Figure
  slug="java-arrays-and-collections-arrays-fixed-size-inline-cutout"
  alt="A rigid strip of six slots all filled, a seventh block waiting beside it"
  caption="An array's length is fixed the moment it is created."
/>

Two arrays below, built the two ways you will see in real code: one
sized up front and filled slot by slot, one written out as a literal.
Run it, then try asking for `scores[5]` and read the exception you
get, that off-by-one is the most common array mistake in Java and it
...
<Figure
  slug="java-arrays-and-collections-arraylist-growable-generic-inline-cutout"
  alt="A rail that grows a further slot as a seventh block is set on it"
  caption="An `ArrayList` grows a slot when it needs one; an array cannot."
/>

A short to-do list, grown one item at a time. Notice what is missing
compared to the array above: nothing anywhere declares a size, and
the `remove` call shrinks the list without any copying on your part:
...
<Figure
  slug="java-arrays-and-collections-inline-cutout"
  alt="A fixed rail with exactly six slots on the left and an expandable rail on the right that has grown a seventh slot to take one more block"
  caption="Fixed size or growable is the choice between an array and a list."
/>
