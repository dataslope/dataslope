...
Two practical readings. A method with a handful of locals recurses
tens of thousands deep before anything goes wrong, which is why
`factorial(4)` never worries you. And a recursion that walks a
*user-supplied* structure, a deeply nested file tree, a hostile JSON
document, can be pushed off the end of that budget by input alone.
That is why production code that recurses over untrusted data usually
gets rewritten as a loop with an explicit stack.
