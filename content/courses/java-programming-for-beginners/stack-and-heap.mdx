...
<Figure
  slug="java-stack-and-heap-cutout"
  alt="The stack and the heap, an isometric illustration"
/>
...
<Figure
  slug="java-stack-and-heap-two-regions-side-inline-cutout"
  alt="A neat column of trays on the left and an open yard of crates on the right, cords between them"
  caption="The stack holds neat frames, the heap holds objects, and references run between them."
/>
...
<Figure
  slug="java-stack-and-heap-inline-cutout"
  alt="A tidy vertical column of trays on the left and an open yard of scattered crates on the right, thin cords running from trays to the crates they point at"
  caption="Locals sit in stack frames; objects live on the heap and are pointed at."
/>
...
That limit is not mysterious, it is arithmetic. The stack is a fixed
allocation handed to each thread when it starts, so the depth a
program survives is simply that budget divided by the size of one
frame, and a frame grows with the number of locals the method keeps:

<Chart slug="stack-depth-overflow" />

Two practical readings. A method with a couple of small locals gets
well over a hundred thousand calls deep before the stack runs out,
which is why `factorial(4)` never worries you. But declare a sizeable
local array inside that same method and the survivable depth drops by
orders of magnitude, with the algorithm unchanged. And a recursion
that walks a *user-supplied* structure, a deeply nested file tree, a
hostile JSON document, can be pushed off the end of that budget by
input alone. That is why production code recursing over untrusted data
usually gets rewritten as a loop with an explicit stack.
