...
The path from a fuzzy idea to running code runs through all four,
in order:

```mermaid
flowchart LR
    V["Vague problem<br/>in English"] --> D[Decompose]
    D --> P["Find<br/>patterns"]
    P --> A[Abstract]
    A --> AL["Design an<br/>algorithm"]
    AL --> C["Code it"]
```
...
<Figure
  ...
  alt="One mould on a bench with four finished casts standing beside it, each cast a different color but the same shape"
