---
title: The Java Virtual Machine, Conceptually
...
<Figure
  slug="java-jvm-conceptually-inline-cutout"
  alt="One program block on the left feeding a single translating machine in the middle, which drives three different machine cabinets on the right"
  caption="One bytecode program, one virtual machine, many different machines beneath."
/>

Stacked up, the whole tower from your code down to the silicon looks
like this:
...
3. **An execution engine**, interprets bytecode, and (over time)
   *compiles hot methods* to native CPU instructions for speed
   (this is just-in-time compilation).
4. **A standard library and runtime**, the classes in `java.lang`,
   `java.util`, `java.io`, etc. that ship with every JVM.

<Figure
  slug="java-jvm-conceptually-virtual-machine-really-inline-cutout"
  alt="A machine drawn as a platform that sits on three different benches, the program on top unchanged"
  caption="Four services in one box, and your program only ever talks to the box."
/>

Drawn as a single unit, with your bytecode arriving on one side:
