...
<Figure
  slug="java-control-flow-if-else-inline-cutout"
  alt="A chute with a two-way flap, both paths ending in their own tray"
  caption="A condition sends execution down one path or the other, never both."
/>
...
<Figure
  slug="java-control-flow-switch-inline-cutout"
  alt="A single lever selecting one of five parallel tracks, the other four left idle"
  caption="`switch` picks one branch by value, leaving the others untouched."
/>
...
`i++` is shorthand for `i = i + 1`. The same shorthand exists for the
other operators: `i--`, `i += 2`, `i *= 3`.
