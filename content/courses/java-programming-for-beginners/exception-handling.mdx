...
locally, it **throws** an exception. The Java virtual machine walks back up the
...
<Figure
  slug="java-exception-handling-exception-inline-cutout"
  alt="A block leaving a belt mid-run and falling toward a padded basket held beneath"
  caption="An exception leaves the normal path and looks for something to catch it."
/>
...
<Figure
  slug="java-exception-handling-inline-cutout"
  alt="A parcel with a stamped warning label on its side halted at a closed gate, the gate staying latched until a matching catch basket has been bolted onto the line beyond it"
  caption="A checked exception will not pass until you have written the catch for it."
/>
...
Notice the **wrapping** technique: we catch the standard library's
