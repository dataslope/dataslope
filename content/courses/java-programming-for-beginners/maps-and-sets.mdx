...
<Figure
  slug="java-maps-and-sets-set-uniqueness-membership-inline-cutout"
  alt="A bin that takes each distinct block once, an identical second bouncing off its rim"
  caption="A set keeps one of each value and drops the duplicates silently."
/>
...
Set that against the two other ways of finding something. A plain
`List` has to walk its elements one at a time, so the work grows with
the collection. A *sorted* list can halve the candidates on every
comparison. Hashing does neither: it goes straight to the bucket.

<Chart slug="collection-lookup-growth" />

The lesson of this chart is not that hashing wins. It is *where*
hashing starts winning. At a handful of elements all three lines sit
on top of each other, which is exactly why a `List` feels fine in
testing and falls over in production: the gap only opens at a size no
small test ever reaches.
...
<Figure
  slug="java-maps-and-sets-inline-cutout"
  alt="A wall of keyed drawers on the left and a plain bin on the right that will hold only one of each block"
  caption="A map looks up by key; a set only answers whether something is present."
/>
