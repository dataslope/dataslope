...
description: Brand loading animations built from the Dataslope logo — diamond spinners in several styles and colour variants.
---
...
Every loader accepts a `color` prop, so the mark can be tinted to any CSS colour — use the Dataslope palette (`var(--ds-green-500)`, `var(--ds-red-500)`, …) or plain black/white for overlays. Shown on the "Diamond assemble + quarter-turn" loader:
...
// Tinted with a brand-palette colour
