---
title: "Challenge Cards: C++"
description: Sandbox for `<ChallengeCard>` C++ variations.
---

C++ challenges compile via **browsercc** (clang + lld with the C++20
standard) and run as WASI binaries in your browser.

## 1. Stream output

<ChallengeCard
  adapter="cpp"
  title="Print a greeting with iostream"
  instructions={`Make the program print exactly \`Hello, C++!\`.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <iostream>

int main() {
  // your code here
  return 0;
}
`,
      solutionCode: `#include <iostream>

int main() {
  std::cout << "Hello, C++!\\n";
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "hello",
      name: "Prints `Hello, C++!`",
      description: "Exact stdout match",
      expect: { stdoutEquals: "Hello, C++!" },
    },
  ]}
/>

```jsx
<ChallengeCard
  adapter="cpp"
  title="Print a greeting with iostream"
  instructions={`Make the program print exactly \`Hello, C++!\`.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <iostream>

int main() {
  // your code here
  return 0;
}
`,
      solutionCode: `#include <iostream>

int main() {
  std::cout << "Hello, C++!\\n";
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "hello",
      name: "Prints `Hello, C++!`",
      description: "Exact stdout match",
      expect: { stdoutEquals: "Hello, C++!" },
    },
  ]}
/>
```

## 2. Containers and algorithms

<ChallengeCard
  adapter="cpp"
  title="Sort and print"
  instructions={`Given the integer vector below, sort it in ascending order and print each element on its own line.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <algorithm>
#include <iostream>
#include <vector>

int main() {
  std::vector<int> xs = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3};
  // your code here
  return 0;
}
`,
      solutionCode: `#include <algorithm>
#include <iostream>
#include <vector>

int main() {
  std::vector<int> xs = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3};
  std::sort(xs.begin(), xs.end());
  for (int x : xs)
    std::cout << x << "\\n";
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "sorted",
      name: "Sorted ascending",
      description: "10 lines, sorted",
      expect: {
        stdoutLines: ["1", "1", "2", "3", "3", "4", "5", "5", "6", "9"],
        stdoutLinesExact: true,
      },
    },
  ]}
/>

```jsx
<ChallengeCard
  adapter="cpp"
  title="Sort and print"
  instructions={`Given the integer vector below, sort it in ascending order and print each element on its own line.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <algorithm>
#include <iostream>
#include <vector>

int main() {
  std::vector<int> xs = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3};
  // your code here
  return 0;
}
`,
      solutionCode: `#include <algorithm>
#include <iostream>
#include <vector>

int main() {
  std::vector<int> xs = {3, 1, 4, 1, 5, 9, 2, 6, 5, 3};
  std::sort(xs.begin(), xs.end());
  for (int x : xs)
    std::cout << x << "\\n";
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "sorted",
      name: "Sorted ascending",
      description: "10 lines, sorted",
      expect: {
        stdoutLines: ["1", "1", "2", "3", "3", "4", "5", "5", "6", "9"],
        stdoutLinesExact: true,
      },
    },
  ]}
/>
```

## 3. Class definition (stdout)

<ChallengeCard
  adapter="cpp"
  title="Implement a Counter class"
  instructions={`Define a class \`Counter\` with a member function \`tick()\` that increments an internal counter and a \`value()\` method that returns its current value. The driver code in \`main\` will then print the counter after each tick.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <iostream>

class Counter {
public:
  // your code here
};

int main() {
  Counter c;
  for (int i = 0; i < 3; i++) {
    c.tick();
    std::cout << c.value() << "\\n";
  }
  return 0;
}
`,
      solutionCode: `#include <iostream>

class Counter {
  int n = 0;

public:
  void tick() { ++n; }
  int value() const { return n; }
};

int main() {
  Counter c;
  for (int i = 0; i < 3; i++) {
    c.tick();
    std::cout << c.value() << "\\n";
  }
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "counts",
      name: "Counts 1, 2, 3",
      description: "Three ticks produce 1, 2, 3 on separate lines",
      expect: {
        stdoutLines: ["1", "2", "3"],
        stdoutLinesExact: true,
      },
    },
  ]}
/>

```jsx
<ChallengeCard
  adapter="cpp"
  title="Implement a Counter class"
  instructions={`Define a class \`Counter\` with a member function \`tick()\` that increments an internal counter and a \`value()\` method that returns its current value. The driver code in \`main\` will then print the counter after each tick.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <iostream>

class Counter {
public:
  // your code here
};

int main() {
  Counter c;
  for (int i = 0; i < 3; i++) {
    c.tick();
    std::cout << c.value() << "\\n";
  }
  return 0;
}
`,
      solutionCode: `#include <iostream>

class Counter {
  int n = 0;

public:
  void tick() { ++n; }
  int value() const { return n; }
};

int main() {
  Counter c;
  for (int i = 0; i < 3; i++) {
    c.tick();
    std::cout << c.value() << "\\n";
  }
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "counts",
      name: "Counts 1, 2, 3",
      description: "Three ticks produce 1, 2, 3 on separate lines",
      expect: {
        stdoutLines: ["1", "2", "3"],
        stdoutLinesExact: true,
      },
    },
  ]}
/>
```

## Multi-file challenge

Multi-file challenges open with a non-sortable, non-closeable tab bar
above the editor. Every file is staged into browsercc's virtual
filesystem before each run, so the entry translation unit can
`#include` sibling headers.

<ChallengeCard
  adapter="cpp"
  title="Implement Greeter::greet()"
  instructions={`Open \`greeter.cpp\` and implement \`Greeter::greet()\` so that it prints \`Hello, World!\` followed by a newline. \`main.cpp\` is provided, it constructs a \`Greeter\` and calls \`greet()\`.`}
  entryFilename="main.cpp"
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include "greeter.h"

int main() {
  Greeter g;
  g.greet();
  return 0;
}
`,
    },
    {
      filename: "greeter.h",
      starterCode: `#pragma once
class Greeter {
public:
  void greet();
};
`,
    },
    {
      filename: "greeter.cpp",
      starterCode: `#include "greeter.h"
#include <iostream>

void Greeter::greet() {
  // your code here
}
`,
      solutionCode: `#include "greeter.h"
#include <iostream>

void Greeter::greet() { std::cout << "Hello, World!\\n"; }
`,
    },
  ]}
  tests={[
    {
      id: "greets",
      name: "Prints `Hello, World!`",
      description: "Exact stdout match",
      expect: { stdoutEquals: "Hello, World!" },
    },
  ]}
/>

```jsx
<ChallengeCard
  adapter="cpp"
  title="Implement Greeter::greet()"
  instructions={`Open \`greeter.cpp\` and implement \`Greeter::greet()\` so that it prints \`Hello, World!\` followed by a newline. \`main.cpp\` is provided, it constructs a \`Greeter\` and calls \`greet()\`.`}
  entryFilename="main.cpp"
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include "greeter.h"

int main() {
  Greeter g;
  g.greet();
  return 0;
}
`,
    },
    {
      filename: "greeter.h",
      starterCode: `#pragma once
class Greeter {
public:
  void greet();
};
`,
    },
    {
      filename: "greeter.cpp",
      starterCode: `#include "greeter.h"
#include <iostream>

void Greeter::greet() {
  // your code here
}
`,
      solutionCode: `#include "greeter.h"
#include <iostream>

void Greeter::greet() { std::cout << "Hello, World!\\n"; }
`,
    },
  ]}
  tests={[
    {
      id: "greets",
      name: "Prints `Hello, World!`",
      description: "Exact stdout match",
      expect: { stdoutEquals: "Hello, World!" },
    },
  ]}
/>
```

## Reading standard input

A card takes the same `stdin` prop, and on a card the input is part of
the question: the tests grade output produced from it, so the reference
solution is written against it.

<ChallengeCard
  adapter="cpp"
  title="Longest word on stdin"
  stdin={`the quick brown fox
jumped over
a remarkably lazy dog`}
  instructions={`Read every whitespace-separated word from standard input and print the
longest one, with its length:

\`\`\`text
remarkably (10)
\`\`\`

If two words tie, print the first.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <iostream>
#include <string>

int main() {
  std::string word;
  // read words until end of input, keep the longest
  return 0;
}
`,
      solutionCode: `#include <iostream>
#include <string>

int main() {
  std::string word;
  std::string longest;
  while (std::cin >> word) {
    if (word.size() > longest.size()) longest = word;
  }
  std::cout << longest << " (" << longest.size() << ")\\n";
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "longest",
      name: "Prints the longest word and its length",
      description: "Exact stdout match (trimmed)",
      expect: { stdoutEquals: "remarkably (10)" },
    },
  ]}
/>

```jsx
<ChallengeCard
  adapter="cpp"
  title="Longest word on stdin"
  stdin={`the quick brown fox
jumped over
a remarkably lazy dog`}
  instructions={`Read every whitespace-separated word from standard input and print the
longest one, with its length:

\`\`\`text
remarkably (10)
\`\`\`

If two words tie, print the first.`}
  files={[
    {
      filename: "main.cpp",
      starterCode: `#include <iostream>
#include <string>

int main() {
  std::string word;
  // read words until end of input, keep the longest
  return 0;
}
`,
      solutionCode: `#include <iostream>
#include <string>

int main() {
  std::string word;
  std::string longest;
  while (std::cin >> word) {
    if (word.size() > longest.size()) longest = word;
  }
  std::cout << longest << " (" << longest.size() << ")\\n";
  return 0;
}
`,
    },
  ]}
  tests={[
    {
      id: "longest",
      name: "Prints the longest word and its length",
      description: "Exact stdout match (trimmed)",
      expect: { stdoutEquals: "remarkably (10)" },
    },
  ]}
/>
```
